#!/usr/bin/env python3
"""Install VAC native binaries (Rust CLI plus ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/vastar/vac/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. vac-<target>.zst)
...
BINARY_COMPONENTS = {
    ...
    "vac-responses-api-proxy": BinaryComponent(
        artifact_prefix="vac-responses-api-proxy",
        dest_dir="vac-responses-api-proxy",
        binary_basename="vac-responses-api-proxy",
    ),
    "vac-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="vac-windows-sandbox-setup",
        dest_dir="vac",
        binary_basename="vac-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "vac-command-runner": BinaryComponent(
        artifact_prefix="vac-command-runner",
        dest_dir="vac",
        binary_basename="vac-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native VAC binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to vac, vac-windows-sandbox-setup,"
            " vac-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    vac_cli_root = (args.root or VAC_CLI_ROOT).resolve()
    vendor_dir = vac_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or [
        "vac",
        "vac-windows-sandbox-setup",
        "vac-command-runner",
        "rg",
    ]
    ...
    with _gha_group(f"Download native artifacts from workflow {workflow_id}"):
        with tempfile.TemporaryDirectory(prefix="vac-native-artifacts-") as artifacts_dir_str:
