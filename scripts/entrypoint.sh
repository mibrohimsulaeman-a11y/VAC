...
# ---------------------------------------------------------------------------
# VAC Agent container entrypoint  (root → gosu drop pattern)
#
# When the sandbox spawns a container with --user 0:0 and sets
# VAC_TARGET_UID / VAC_TARGET_GID, this script:
...
IMAGE_UID=1000
IMAGE_GID=1000
HOME_DIR="${VAC_HOME_DIR:-/home/agent}"
AQUA_CACHE_DIR="${VAC_AQUA_CACHE_DIR:-${HOME_DIR}/.local/share/aquaproj-aqua}"
AQUA_OWNERSHIP_MARKER="${AQUA_CACHE_DIR}/.vac-owner"
export HOME="$HOME_DIR"

CURRENT_UID=$(id -u)

if [ "$CURRENT_UID" = "0" ] && [ -n "$VAC_TARGET_UID" ]; then
    # Running as root inside the sandbox with a target UID requested.
    TARGET_UID="$VAC_TARGET_UID"
    TARGET_GID="${VAC_TARGET_GID:-$TARGET_UID}"
    ...
    if [ "$TARGET_UID" != "$IMAGE_UID" ] || [ "$TARGET_GID" != "$IMAGE_GID" ]; then
        ...
        # boundaries, so :ro mounts like .vac/config.toml, .ssh/, etc.
        ...
        # These are VAC-managed caches, not user bind mounts. Cache the
    ...
    # root before this script runs. These VAC-owned SQLite parents must stay
    # writable after gosu drops to the target agent user, even when the target
    # UID/GID already matches the image defaults.
    for dir in "$HOME/.vac/data" "$HOME/.agent-board"; do
