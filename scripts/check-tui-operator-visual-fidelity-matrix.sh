...
ansi_dir="docs/validation/tui-operator-ansi-snapshots"
...
grep -q 'tool timeline (last 5;' "$snapshot_dir/agent-working-120x36.txt" || fail "agent small snapshot missing timeline cap"
...
grep -q 'right / Diagnostics' "$snapshot_dir/capability-dashboard-180x48.txt" || fail "dashboard large snapshot missing diagnostics"
