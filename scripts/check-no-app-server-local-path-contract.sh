...
tui_refs=$(grep -nE 'legacy-app-server-compat|vac-app-server|app_server_compat' vac-rs/crates/surfaces/tui/Cargo.toml | wc -l | tr -d ' ')
if [ "$tui_refs" != "0" ]; then
  echo "FAIL: TUI manifest still exposes app-server compatibility" >&2
  grep -nE 'legacy-app-server-compat|vac-app-server|app_server_compat' vac-rs/crates/surfaces/tui/Cargo.toml >&2
  fail=1
else
...
for legacy in \
  vac-rs/crates/surfaces/tui/src/legacy_app_server_compat.rs \
  vac-rs/crates/surfaces/tui/src/legacy_app_server_session.rs \
  vac-rs/crates/surfaces/tui/src/legacy_app_server_session \
  vac-rs/crates/surfaces/tui/src/app_server_session.rs; do
  if [ -e "$legacy" ]; then
...
for provider in 'vac-login = { workspace = true }' 'vac-model-provider = { workspace = true }' 'reqwest = { workspace = true'; do
  if grep -Fq "$provider" vac-rs/crates/surfaces/tui/Cargo.toml; then
...
if [ -f vac-rs/crates/surfaces/tui/src/runtime_owner_session.rs ] \
  && grep -q 'mod runtime_owner_session;' vac-rs/crates/surfaces/tui/src/lib.rs \
  && grep -q 'crate::runtime_owner_session::' vac-rs/crates/surfaces/tui/src/local_runtime_session.rs; then
...
active_import_refs=$(grep -RInE 'use vac_app_server|use vac_app_server_client|vac_app_server::|vac_app_server_client::|legacy-app-server-compat' \
  vac-rs/crates/surfaces/tui/src vac-rs/crates/capabilities/local-runtime-owner/src vac-rs/crates/surfaces/cli/src vac-rs/core/src \
