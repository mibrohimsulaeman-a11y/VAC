...
def read_proof(root: Path) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    if not path.is_file():
        return None, ["missing_ci_scoped_validation_proof"]
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        return None, [f"invalid_ci_scoped_validation_proof_json:{exc}"]
    if not isinstance(value, dict):
        return None, ["ci_scoped_validation_proof_root_not_object"]
    return value, []
