...
if find "$ROOT" \( -path '*/target/*' -o -path '*/.git/*' -o -path '*/node_modules/*' \) -print -quit | grep -q .; then
  fail "workspace contains excluded build/cache path (target/.git/node_modules)"
fi
