...
use vac_agent_identity::AgentIdentityKey;
use vac_agent_identity::AgentTaskAuthorizationTarget;
use vac_agent_identity::authorization_header_for_agent_task;
use vac_api::AuthProvider;
...
struct AgentIdentityAuthProvider {
    auth: vac_login::auth::AgentIdentityAuth,
}
...
// providers and custom test providers with `requires_vastar_auth = false`.
#[derive(Clone, Debug)]
...
pub(crate) fn resolve_provider_auth(
    auth: Option<&VACAuth>,
    provider: &ModelProviderInfo,
) -> vac_protocol::error::Result<SharedAuthProvider> {
...
fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> vac_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party VAC auth snapshot.
pub fn auth_provider_from_auth(auth: &VACAuth) -> SharedAuthProvider {
    match auth {
        VACAuth::AgentIdentity(auth) => Arc::new(AgentIdentityAuthProvider { auth: auth.clone() }),
        VACAuth::ApiKey(_) | VACAuth::Chatgpt(_) | VACAuth::ChatgptAuthTokens(_) => {
