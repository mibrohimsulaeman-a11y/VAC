...
use vac_models_manager::manager::OpenAiModelsManager;
...
/// Optional provider-backed features that VAC may expose at runtime.
///
...
/// metadata used by the default Vastar-compatible implementation.
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    ///
    /// VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(celia-oai): Make auth manager access internal to this crate so callers
    /// resolve provider-specific auth only through `ModelProvider`. We first need
    /// to think through whether VAC should have a unified provider-specific auth
    ...
    async fn api_provider(&self) -> vac_protocol::error::Result<Provider> {
        let auth = self.auth().await;
        self.info()
            .to_api_provider(auth.as_ref().map(VACAuth::auth_mode))
    }

    /// Returns the provider base URL that will be used at request time.
    async fn runtime_base_url(&self) -> vac_protocol::error::Result<Option<String>> {
        Ok(self.info().base_url.clone())
    }

    /// Returns the auth provider used to attach request credentials.
    async fn api_auth(&self) -> vac_protocol::error::Result<SharedAuthProvider> {
        let auth = self.auth().await;
...
impl ModelProvider for ConfiguredModelProvider {
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_vastar_auth {
            self.auth_manager
                .as_ref()
                ...
                .map(|auth| match &auth {
                    VACAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    VACAuth::Chatgpt(_)
                    | VACAuth::ChatgptAuthTokens(_)
                    | VACAuth::AgentIdentity(_) => {
        ...
        Ok(ProviderAccountState {
            account,
            requires_vastar_auth: self.info.requires_vastar_auth,
        })
    }
...
mod tests {
    ...
    fn provider_info_with_command_auth() -> ModelProviderInfo {
        ModelProviderInfo {
            ...
            ..ModelProviderInfo::create_vastar_provider(/*base_url*/ None)
        }
    }

    fn test_vac_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("vac-model-provider-test-{}", std::process::id()))
    }
    ...
    #[test]
    fn create_model_provider_does_not_use_vastar_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("vac-bedrock".to_string()),
                region: None,
            })),
            Some(AuthManager::from_auth_for_testing(VACAuth::from_api_key(
                "vastar-api-key",
            ))),
        );
    ...
    #[test]
    fn vastar_provider_returns_unauthenticated_vastar_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_vastar_provider(/*base_url*/ None),
            /*auth_manager*/ None,
        );
    ...
    #[test]
    fn vastar_provider_returns_api_key_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_vastar_provider(/*base_url*/ None),
            Some(AuthManager::from_auth_for_testing(VACAuth::from_api_key(
                "vastar-api-key",
    ...
    #[test]
    fn custom_non_vastar_provider_returns_no_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo {
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager = provider.models_manager(test_vac_home(), /*config_model_catalog*/ None);
        ...
        assert_eq!(default_model.model, "vastar.gpt-5.4");
    }

    #[tokio::test]
    async fn amazon_bedrock_provider_uses_configured_static_catalog_when_present() {
        let custom_model =
            vac_models_manager::model_info::model_info_from_slug("custom-bedrock-model");
