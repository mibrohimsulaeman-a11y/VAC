use vac_aws_auth::AwsAuthConfig;
...
pub(super) fn base_url(region: &str) -> Result<String> {
    if BEDROCK_MANTLE_SUPPORTED_REGIONS.contains(&region) {
        Ok(format!("https://bedrock-mantle.{region}.api.aws/vastar/v1"))
    } else {
        Err(VACErr::Fatal(format!(
...
mod tests {
    ...
    fn base_url_uses_region_endpoint() {
        assert_eq!(
            ...
            "https://bedrock-mantle.ap-northeast-1.api.aws/vastar/v1"
        );
    }
