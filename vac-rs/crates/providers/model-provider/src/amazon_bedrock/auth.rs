...
use http::HeaderMap;
use vac_api::AuthError;
use vac_api::AuthProvider;
use vac_api::SharedAuthProvider;
use vac_aws_auth::AwsAuthContext;
use vac_aws_auth::AwsAuthError;
use vac_aws_auth::AwsRequestToSign;
use vac_client::Request;
use vac_client::RequestBody;
use vac_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_vac_error)?;
...
fn aws_auth_error_to_vac_error(error: AwsAuthError) -> VACErr {
    VACErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    // The Bedrock Mantle front door does not preserve this legacy Vastar header
    // for SigV4 verification. Signing it makes the richer VAC agent request
    // fail even though raw Responses requests work.
    headers.remove(LEGACY_SESSION_ID_HEADER);
}

/// AWS SigV4 auth provider for Bedrock Mantle Vastar-compatible requests.
#[derive(Debug)]
