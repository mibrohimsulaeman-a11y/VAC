...
use sha2::Sha512;
use vac_protocol::auth::PlanType as AuthPlanType;
...
pub fn agent_identity_jwks_url(base_url: &str) -> String {
    if !legacy_chatgpt_agent_identity_jwks_enabled() {
        return "legacy-chatgpt-agent-identity-jwks-disabled".to_string();
    }
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "vac-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "vac-cli".to_string(),
        },
...
mod tests {
    ...
    use vac_protocol::auth::KnownPlan;
