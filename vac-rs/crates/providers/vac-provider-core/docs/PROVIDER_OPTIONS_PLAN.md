...
The VAC Provider Core SDK needs to support commonly-used provider options that developers expect when working with OpenAI, Anthropic, and Google Gemini APIs. This plan focuses on the most frequently used features, not full feature parity.
...
| File | Changes |
|------|---------|
| `vac-provider-core_adapter.rs` | Update option conversion (set new fields to None initially) |
...
7. **Update vac-provider-core_adapter** - Handle new options in conversion
...
```rust
use vac_provider_core::{ProviderOptions, OpenAIOptions, ServiceTier};
