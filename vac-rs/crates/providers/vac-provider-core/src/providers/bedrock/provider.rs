...
/// Map Bedrock InvokeModel errors to vac-provider-core Error types
///
/// Converts the SDK error to the crate-level `aws_sdk_bedrockruntime::Error` enum
/// (which has `From<SdkError<InvokeModelError>>` impl), then maps to vac-provider-core errors.
///
/// Mapping:
...
/// Map Bedrock InvokeModelWithResponseStream errors to vac-provider-core Error types
...
///
/// Maps the crate-level `aws_sdk_bedrockruntime::Error` enum to vac-provider-core error types.
