...
use super::convert::to_vac_request;
use super::stream::create_stream;
use super::types::{VACModelsResponse, VACResponse, VacProviderConfig};
...
/// VAC provider
///
/// Routes inference requests through VAC's OpenAI-compatible API.
pub struct VacProvider {
    config: VacProviderConfig,
    client: Client,
...
impl VacProvider {
    ...
    pub fn new(config: VacProviderConfig) -> Result<Self> {
        if config.api_key.is_empty() {
            return Err(Error::MissingApiKey("vac".to_string()));
        }
    ...
    pub fn from_env() -> Result<Self> {
        Self::new(VacProviderConfig::default())
    }

    /// Fetch models from the VAC `/v1/models` API endpoint
    async fn fetch_models(&self) -> Result<Vec<Model>> {
        ...
        if !response.status().is_success() {
            ...
            let friendly_error = parse_vac_error(&error_text, status.as_u16());
            return Err(Error::provider_error(friendly_error));
        }

        let resp: VACModelsResponse = response.json().await?;

        // Tag all models with the vac provider and prefix IDs with the
...
impl Provider for VacProvider {
    ...
    async fn generate(&self, request: GenerateRequest) -> Result<GenerateResponse> {
        ...
        let openai_req = to_vac_request(&request, false);
        ...
        let resp: VACResponse = response.json().await?;
        from_vac_response(resp)
    }

    async fn stream(&self, request: GenerateRequest) -> Result<GenerateStream> {
        let url = format!("{}/v1/chat/completions", self.config.base_url);

        let openai_req = to_vac_request(&request, true);
        ...
        // Reuse OpenAI stream parsing since VAC uses OpenAI-compatible streaming
    ...
    async fn list_models(&self) -> Result<Vec<Model>> {
        ...
        // Fetch from the VAC API and cache the result
...
/// Convert VAC response to SDK response
fn from_vac_response(resp: VACResponse) -> Result<GenerateResponse> {
    let choice = resp
        .choices
        .first()
        .ok_or_else(|| Error::invalid_response("No choices in response"))?;

    let content = parse_vac_message(&choice.message)?;
    ...
    // Extract cache tokens from VAC's response format
...
/// Parse VAC message content
fn parse_vac_message(msg: &super::types::VACMessage) -> Result<Vec<ResponseContent>> {
...
/// Parse VAC API error and return user-friendly message
pub(crate) fn parse_vac_error(error_text: &str, status_code: u16) -> String {
        ...
        && let Some(error) = json.get("error")
    {
        ...
        if message.contains("Exceeded credits") || message.contains("balance is") {
            return format!(
                "Insufficient credits. Please top up your VAC account at https://vastar.ai. {}",
                message
            );
        }
    ...
    format!("VAC API error {}: {}", status_code, error_text)
}
