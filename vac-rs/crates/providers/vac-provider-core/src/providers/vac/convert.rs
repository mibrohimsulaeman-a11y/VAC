//! Conversion from SDK types to VAC's OpenAI-compatible request format
//!
//! VAC's backend forwards requests to Anthropic, which requires each tool_result
...
/// Convert an SDK request to a VAC-compatible OpenAI chat completion request.
///
...
pub fn to_vac_request(req: &GenerateRequest, stream: bool) -> ChatCompletionRequest {
    ...
    let messages: Vec<ChatMessage> = req.messages.iter().flat_map(to_vac_messages).collect();
...
fn to_vac_messages(msg: &Message) -> Vec<ChatMessage> {
...
mod tests {
    ...
    fn make_request(messages: Vec<Message>) -> GenerateRequest {
        GenerateRequest::new(
            Model::custom("anthropic/claude-sonnet-4-20250514", "vac"),
            messages,
        )
    }
    ...
    fn test_basic_user_message() {
        ...
        let result = to_vac_request(&req, false);
    ...
    fn test_streaming_request() {
        let req = make_request(vec![Message::new(Role::User, "Hello")]);
        let result = to_vac_request(&req, true);
