...
use crate::providers::{
    ...
    openrouter::OpenRouterProvider, vac::VacProvider,
};
...
impl ClientBuilder {
    ...
    pub fn with_inference_config(mut self, inference_config: InferenceConfig) -> Self {
        ...
        // Register VAC if configured
        if let Some(config) = inference_config.vac_config
            && let Ok(provider) = VacProvider::new(config)
        {
