...
use vac_provider_core::providers::anthropic::{AnthropicConfig, AnthropicProvider};
use vac_provider_core::providers::gemini::{GeminiConfig, GeminiProvider};
use vac_provider_core::providers::openai::{OpenAIConfig, OpenAIProvider};
use vac_provider_core::providers::vac::{VacProvider, VacProviderConfig};
use vac_provider_core::registry::ProviderRegistry;
...
#[test]
fn test_vac_provider_creates_with_tls_client() {
    let provider = VacProvider::new(VacProviderConfig::new("test-key"));
    assert!(
        provider.is_ok(),
        "VAC provider should create successfully with TLS client"
    );
}

#[test]
fn test_vac_provider_custom_base_url_creates_with_tls_client() {
    let config = VacProviderConfig::new("test-key").with_base_url("https://custom.vac.example.com");
    let provider = VacProvider::new(config);
    assert!(
        provider.is_ok(),
        "VAC provider with custom base URL should create successfully with TLS client"
    );
}
...
#[test]
fn test_vac_provider_rejects_empty_key() {
    let provider = VacProvider::new(VacProviderConfig::new(""));
    assert!(
        provider.is_err(),
        "VAC provider should reject empty API key"
    );
}
...
fn test_all_providers_create_concurrently() {
    ...
    let vac = VacProvider::new(VacProviderConfig::new("test-key"));
    ...
    assert!(vac.is_ok(), "VAC provider creation failed");
}
...
fn test_register_all_tls_providers_in_registry() {
    ...
    let vac = VacProvider::new(VacProviderConfig::new("test-key")).unwrap();
    ...
    assert!(registry.has_provider("vac"));
}
...
fn test_inference_config_creates_all_providers_with_tls() {
    ...
    assert!(providers.contains(&"vac".to_string()));
}
