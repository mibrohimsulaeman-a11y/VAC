...
pub const VASTAR_FILE_URI_PREFIX: &str = "sediment://";
pub const VASTAR_FILE_UPLOAD_LIMIT_BYTES: u64 = 512 * 1024 * 1024;

const VASTAR_FILE_REQUEST_TIMEOUT: Duration = Duration::from_secs(60);
const VASTAR_FILE_FINALIZE_TIMEOUT: Duration = Duration::from_secs(30);
const VASTAR_FILE_FINALIZE_RETRY_DELAY: Duration = Duration::from_millis(250);
const VASTAR_FILE_USE_CASE: &str = "vac";
...
pub enum OpenAiFileError {
    ...
    #[error("failed to send Vastar file request to {url}: {source}")]
    Request {
        url: String,
        #[source]
        source: reqwest::Error,
    },
    #[error("Vastar file request to {url} failed with status {status}: {body}")]
    UnexpectedStatus {
        url: String,
        status: StatusCode,
        body: String,
    },
    #[error("failed to parse Vastar file response from {url}: {source}")]
    Decode {
        url: String,
        #[source]
        source: serde_json::Error,
    },
    #[error("Vastar file upload for `{file_id}` is not ready yet")]
    UploadNotReady { file_id: String },
    #[error("Vastar file upload for `{file_id}` failed: {message}")]
...
pub fn vastar_file_uri(file_id: &str) -> String {
    format!("{VASTAR_FILE_URI_PREFIX}{file_id}")
}
...
pub async fn upload_local_file(
    ...
    if metadata.len() > VASTAR_FILE_UPLOAD_LIMIT_BYTES {
        return Err(OpenAiFileError::FileTooLarge {
            path: path.to_path_buf(),
            size_bytes: metadata.len(),
            limit_bytes: VASTAR_FILE_UPLOAD_LIMIT_BYTES,
        });
    }
    ...
    let upload_response = build_reqwest_client()
        ...
        .timeout(VASTAR_FILE_REQUEST_TIMEOUT)
        ...
        match finalize_payload.status.as_str() {
            "success" => {
                return Ok(UploadedOpenAiFile {
                    ...
                    uri: vastar_file_uri(&create_payload.file_id),
            ...
            "retry" => {
                if finalize_started_at.elapsed() >= VASTAR_FILE_FINALIZE_TIMEOUT {
                    return Err(OpenAiFileError::UploadNotReady {
                        file_id: create_payload.file_id,
                    });
                }
                tokio::time::sleep(VASTAR_FILE_FINALIZE_RETRY_DELAY).await;
            }
            _ => {
...
fn build_reqwest_client() -> reqwest::Client {
    build_reqwest_client_with_custom_ca(reqwest::Client::builder()).unwrap_or_else(|error| {
        tracing::warn!(error = %error, "failed to build Vastar file upload client");
        reqwest::Client::new()
    })
}
...
mod tests {
    ...
    fn base_url_for(server: &MockServer) -> String {
        format!("{}/provider-api", server.uri())
    }
