...
use crate::maybe_build_rustls_client_config_with_custom_ca;
...
const X_VAC_TURN_STATE_HEADER: &str = "x-vac-turn-state";
...
pub struct ResponsesWebsocketConnection {
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening) (pakrym): is this the right place for timeout?
...
async fn connect_websocket(
    ...
    // If a VAC-specific CA bundle is configured, build an explicit rustls connector so this
...
mod tests {
    ...
    #[test]
    fn websocket_config_uses_tungstenite_defaults() {
        let config = websocket_config();
        let default_config = WebSocketConfig::default();

        assert_eq!(config.read_buffer_size, default_config.read_buffer_size);
        assert_eq!(config.write_buffer_size, default_config.write_buffer_size);
        assert_eq!(
            config.max_write_buffer_size,
            default_config.max_write_buffer_size
        );
        assert_eq!(config.max_message_size, default_config.max_message_size);
        assert_eq!(config.max_frame_size, default_config.max_frame_size);
        assert_eq!(
            config.accept_unmasked_frames,
            default_config.accept_unmasked_frames
        );
    }
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-vac-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-vac-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
