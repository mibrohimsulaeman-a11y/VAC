...
const PROVIDER_REALTIME_ENV: &str = "VAC_ENABLE_PROVIDER_REALTIME";

fn provider_realtime_enabled() -> bool {
    std::env::var(PROVIDER_REALTIME_ENV)
        .map(|value| {
            matches!(
...
const MULTIPART_BOUNDARY: &str = "vac-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=vac-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    fn uses_backend_request_shape(&self) -> bool {
        self.session
            .provider()
            .base_url
            .contains("/provider-disabled-backend")
    }
    ...
    pub async fn create_with_headers(
        ...
        if !provider_realtime_enabled() {
            return Err(ApiError::InvalidRequest {
                message: format!(
                    "provider realtime calls are disabled for the local coding-agent build; set {PROVIDER_REALTIME_ENV}=1 to opt in"
                ),
            });
        }
        let resp = self
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "vac_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
        ...
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(aibrahim): Align the SIWC route with the API multipart shape and remove this branch.
...
mod tests {
    ...
    async fn sends_sdp_offer_as_raw_body() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://api.vastar.com/v1"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(request.url, "https://api.vastar.com/v1/realtime/calls");
        assert_eq!(
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://provider-realtime.local/disabled"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://provider-realtime.local/provider-disabled-backend/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
