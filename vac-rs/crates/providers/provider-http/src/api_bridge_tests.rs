...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, VACErr::ServerOverloaded));
}
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let VACErr::CyberPolicy { message } = err else {
        panic!("expected VACErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let VACErr::InvalidRequest(message) = err else {
        panic!("expected VACErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("vac_other"),
    );
    headers.insert(
        "x-vac-other-limit-name",
        http::HeaderValue::from_static("vac_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let VACErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected VACErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let VACErr::UnexpectedStatus(err) = err else {
        panic!("expected VACErr::UnexpectedStatus, got {err:?}");
    };
