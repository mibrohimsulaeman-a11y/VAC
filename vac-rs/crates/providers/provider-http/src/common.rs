...
use vac_protocol::config_types::Verbosity as VerbosityConfig;
...
pub enum ResponseEvent {
    ...
    /// Emitted when the server includes `Vastar-Model` on the stream response.
...
pub struct TextFormat {
    /// Format type used by the Vastar text controls.
...
pub fn create_text_param_for_request(
    ...
    Some(TextControls {
        ...
        format: output_schema.as_ref().map(|schema| TextFormat {
            ...
            schema: schema.clone(),
            name: "vac_output_schema".to_string(),
        }),
    })
}
