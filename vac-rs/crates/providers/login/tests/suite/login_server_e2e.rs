...
use vac_login::ServerOptions;
use vac_login::run_login_server;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = vac_home.clone();

    let opts = ServerOptions {
        vac_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: vac_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = vac_home.join("auth.json");
    ...
    assert_eq!(json["VASTAR_API_KEY"], "access-123");
...
#[tokio::test]
async fn creates_missing_vac_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let vac_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_vac_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to VAC"),
        "error body should clearly explain the VAC access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_vac_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to VAC to retry"),
    ...
    assert!(
        !body.contains("get access to VAC"),
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not vac"));
            }
        })
    };
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_vac_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        vac_home: first_vac_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: vac_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_vac_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        vac_home: second_vac_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: vac_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
