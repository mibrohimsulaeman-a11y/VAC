//! Default VAC HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`VACHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`vac_login::default_client`] from other crates in this
//! workspace.
...
use std::sync::Mutex;
use std::sync::RwLock;
use vac_client::BuildCustomCaTransportError;
use vac_client::VACHttpClient;
pub use vac_client::VACRequestBuilder;
use vac_client::build_reqwest_client_with_custom_ca;
use vac_client::with_chatgpt_cloudflare_cookie_store;
...
/// See https://github.com/vastar/vac/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by VAC. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "vac_cli_rs";
pub const VAC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "VAC_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-vastar-internal-vac-residency";

pub use vac_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(VAC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(VAC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "vac-tui"
        || originator_value == "vac_vscode"
        || originator_value.starts_with("VAC ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "vac_atlas" || originator_value == "vac_chatgpt_desktop"
}

pub fn get_vac_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized VAC user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base VAC user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default VAC originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> VACHttpClient {
    let inner = build_reqwest_client();
    VACHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary VAC HTTP traffic.
///
/// This starts from the standard VAC user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `VAC_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary VAC HTTP traffic.
///
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_vac_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("VAC_SANDBOX").as_deref() == Ok("seatbelt")
}
