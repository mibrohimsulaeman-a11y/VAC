...
use vac_protocol::auth::KnownPlan as InternalKnownPlan;
...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    assert_eq!(auth.vastar_api_key.as_deref(), Some("sk-new"));
...
async fn login_with_agent_identity_writes_only_token() {
    ...
    Mock::given(method("GET"))
        .and(path("/provider-api/wham/agent-identities/jwks"))
        .respond_with(ResponseTemplate::new(200).set_body_json(test_jwks_body()))
        .expect(1)
        .mount(&server)
        .await;
    let chatgpt_base_url = format!("{}/provider-api", server.uri());
    ...
    assert!(auth.tokens.is_none(), "tokens should be cleared");
    assert!(auth.vastar_api_key.is_none(), "API key should be cleared");
    server.verify().await;
}
...
async fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let _agent_guard = EnvVarGuard::remove(VAC_AGENT_IDENTITY_ENV_VAR);
    let auth = VACAuth::from_auth_storage(
        dir.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
    )
...
async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
    ...
    assert_eq!(AuthMode::ProviderCredential, auth.auth_mode());
...
async fn loads_api_key_from_auth_json() {
    ...
    std::fs::write(
        auth_file,
        r#"{"VASTAR_API_KEY":"sk-test-key","tokens":null,"last_refresh":null}"#,
    )
    .unwrap();
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = VACAuth::from_auth_dot_json(
        vac_home.path(),
...
struct AuthFileParams {
    vastar_api_key: Option<String>,
    chatgpt_plan_type: Option<String>,
    chatgpt_account_id: Option<String>,
}

fn write_auth_file(params: AuthFileParams, vac_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(vac_home);
    let auth_json_data = json!({
        "VASTAR_API_KEY": params.vastar_api_key,
        "tokens": {
            "id_token": fake_jwt,
...
fn fake_jwt_for_auth_file_params(params: &AuthFileParams) -> std::io::Result<String> {
    ...
    let payload = serde_json::json!({
        ...
        "email_verified": true,
        "https://api.vastar.com/auth": auth_payload,
    });
...
/// Use sparingly.
/// VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening) (gpeal): replace this with an injectable env var provider.
#[cfg(test)]
struct EnvVarGuard {
    key: &'static str,
...
async fn load_auth_reads_agent_identity_from_env() {
    ...
    Mock::given(method("POST"))
        .and(path(
            "/provider-api/v1/agent/agent-runtime-id/task/register",
        ))
        ...
        .mount(&server)
        .await;
    let _agent_guard = EnvVarGuard::set(VAC_AGENT_IDENTITY_ENV_VAR, &agent_identity);

    let chatgpt_base_url = format!("{}/provider-api", server.uri());
    let _authapi_guard = EnvVarGuard::set("VAC_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = super::load_auth(
        vac_home.path(),
        /*enable_vac_api_key_env*/ false,
        AuthCredentialsStoreMode::File,
    ...
    let VACAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(vac_home.path()).exists(),
...
#[tokio::test]
#[serial(vac_auth_env)]
async fn load_auth_keeps_vac_api_key_env_precedence() {
    let vac_home = tempdir().unwrap();
    let record = agent_identity_record("account-123");
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _agent_guard = EnvVarGuard::set(VAC_AGENT_IDENTITY_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(VAC_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    let vac_home = tempdir().unwrap();
    let _agent_guard = EnvVarGuard::remove(VAC_AGENT_IDENTITY_ENV_VAR);
    login_with_api_key(vac_home.path(), "sk-test", AuthCredentialsStoreMode::File)
        .expect("seed api key");
    ...
    assert!(
        !vac_home.path().join("auth.json").exists(),
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(VAC_API_KEY_ENV_VAR, "sk-env");
    let _agent_guard = EnvVarGuard::remove(VAC_AGENT_IDENTITY_ENV_VAR);
    let vac_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        vac_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://provider.vac.invalid/vac-backend/agent-identity",
        "aud": "vac-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let chatgpt_base_url = format!("{}/provider-api", server.uri());
    let _authapi_guard = EnvVarGuard::set("VAC_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = VACAuth::from_agent_identity_jwt(&jwt, Some(&chatgpt_base_url))
        .await
