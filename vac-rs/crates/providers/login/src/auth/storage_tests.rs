...
use vac_keyring_store::tests::MockKeyringStore;

#[tokio::test]
async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
    let vac_home = tempdir()?;
    let storage = FileAuthStorage::new(vac_home.path().to_path_buf());
    let auth_dot_json = AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        vastar_api_key: Some("test-key".to_string()),
        tokens: None,
...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(vac_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
async fn file_storage_loads_agent_identity_as_jwt() -> anyhow::Result<()> {
    ...
    let auth_file = get_auth_file(vac_home.path());
    std::fs::write(
        &auth_file,
...
fn ephemeral_storage_save_load_delete_is_in_memory_only() -> anyhow::Result<()> {
    ...
    let auth_dot_json = AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        vastar_api_key: Some("sk-ephemeral".to_string()),
        tokens: None,
        last_refresh: Some(Utc::now()),
        agent_identity: None,
    };
...
fn auth_with_prefix(prefix: &str) -> AuthDotJson {
    AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        vastar_api_key: Some(format!("{prefix}-api-key")),
...
fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_keyring_with_auth(
        &mock_keyring,
        || compute_store_key(vac_home.path()),
        &expected,
    )?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let vac_home = PathBuf::from("~/.vac");

    let key = compute_store_key(vac_home.as_path())?;
...
fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    let key = compute_store_key(vac_home.path())?;
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, &key, vac_home.path(), &auth);
    Ok(())
}
...
fn keyring_auth_storage_delete_removes_keyring_and_file() -> anyhow::Result<()> {
    ...
    let (key, auth_file) =
        seed_keyring_and_fallback_auth_file_for_delete(&mock_keyring, vac_home.path(), || {
            compute_store_key(vac_home.path())
        })?;
...
fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
    let vac_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = AutoAuthStorage::new(vac_home.path().to_path_buf(), Arc::new(mock_keyring));
...
fn auto_auth_storage_save_prefers_keyring() -> anyhow::Result<()> {
    ...
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, &key, vac_home.path(), &expected);
    Ok(())
}
