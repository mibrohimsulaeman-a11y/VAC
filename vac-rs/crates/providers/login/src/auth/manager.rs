...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(VACAuth::get_account_id);
    ...
    pub fn has_next(&self) -> bool {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(VACAuth::is_chatgpt_auth)
        {
            return false;
        }
...
impl AuthManager {
    ...
    fn auths_equal_for_refresh(a: Option<&VACAuth>, b: Option<&VACAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
    ...
    fn auths_equal(a: Option<&VACAuth>, b: Option<&VACAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    async fn load_auth_from_storage(&self) -> Option<VACAuth> {
        load_auth(
            &self.vac_home,
    ...
    fn set_cached_auth(&self, new_auth: Option<VACAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    fn external_auth_mode(&self) -> Option<CredentialMode> {
        self.external_auth()
            .as_ref()
    ...
    async fn resolve_external_api_key_auth(&self) -> Option<VACAuth> {
    ...
    fn is_stale_for_proactive_refresh(auth: &VACAuth) -> bool {
