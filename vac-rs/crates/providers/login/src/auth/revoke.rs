...
use vac_runtime_protocol::AuthMode as ApiAuthMode;
...
fn managed_chatgpt_tokens(auth_dot_json: &AuthDotJson) -> Option<&TokenData> {
    if resolved_auth_mode(auth_dot_json) == ApiAuthMode::ProviderCredential {
...
fn resolved_auth_mode(auth_dot_json: &AuthDotJson) -> ApiAuthMode {
    ...
    if auth_dot_json.vastar_api_key.is_some() {
        return ApiAuthMode::ApiKey;
    }
    ApiAuthMode::ProviderCredential
}
...
mod tests {
    ...
    async fn revoke_request_times_out() {
        ...
        let client = VACHttpClient::new(reqwest::Client::new());
