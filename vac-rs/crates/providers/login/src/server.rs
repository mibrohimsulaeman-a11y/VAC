...
use tracing::info;
use tracing::warn;
use vac_client::build_reqwest_client_with_custom_ca;
use vac_config::types::AuthCredentialsStoreMode;
use vac_runtime_protocol::AuthMode;
use vac_utils_template::Template;

const LEGACY_CHATGPT_ACCOUNT_LOGIN_ENV: &str = "VAC_ENABLE_LEGACY_CHATGPT_ACCOUNT_LOGIN";

fn legacy_chatgpt_account_login_enabled() -> bool {
    std::env::var(LEGACY_CHATGPT_ACCOUNT_LOGIN_ENV)
        .ok()
        .is_some_and(|value| matches!(value.as_str(), "1" | "true"))
}

const DEFAULT_ISSUER: &str = "https://auth.vastar.com";
...
pub struct ServerOptions {
    ...
    pub vac_streamlined_login: bool,
    ...
    match path.as_str() {
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to VAC failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "vac_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("vac_cli_simplified_flow".to_string(), "true".to_string()),
...
pub(crate) async fn persist_tokens_async(
    ...
    if !legacy_chatgpt_account_login_enabled() {
        return Err(io::Error::other(
            "legacy ChatGPT account login is disabled in the local coding-agent build; set VAC_ENABLE_LEGACY_CHATGPT_ACCOUNT_LOGIN=1 to opt in",
        ));
    }
    // Reuse existing synchronous logic but run it off the async runtime.
    let vac_home = vac_home.to_path_buf();
    tokio::task::spawn_blocking(move || {
        ...
        let auth = AuthDotJson {
            ...
            vastar_api_key: api_key,
            tokens: Some(tokens),
            last_refresh: Some(Utc::now()),
            agent_identity: None,
        };
        save_auth(&vac_home, &auth, auth_credentials_store_mode)
    })
    .await
...
fn compose_success_url(
    ...
    let platform_url = if issuer == DEFAULT_ISSUER {
        "https://platform.vastar.com"
    } else {
        "https://platform.api.vastar.org"
    };
    ...
    if vac_streamlined_login {
        params.push(("vac_streamlined_login", "true".to_string()));
    }
    let qs = params
        .drain(..)
        .map(|(k, v)| format!("{}={}", k, urlencoding::encode(&v)))
...
fn jwt_auth_claims(jwt: &str) -> serde_json::Map<String, serde_json::Value> {
    ...
    match base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(payload_b64) {
        Ok(bytes) => match serde_json::from_slice::<serde_json::Value>(&bytes) {
            Ok(mut v) => {
                if let Some(obj) = v
                    .get_mut("https://api.vastar.com/auth")
                    .and_then(|x| x.as_object_mut())
                {
                    return obj.clone();
                }
                eprintln!("JWT payload missing expected 'https://api.vastar.com/auth' object");
            }
            Err(e) => {
...
/// Returns true when the OAuth callback represents a missing VAC entitlement.
fn is_missing_vac_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_vac_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_vac_entitlement_error(error_code, error_description) {
        return "VAC is not enabled for your workspace. Contact your workspace administrator to request access to VAC.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_vac_entitlement_error(code, error_description) {
            (
                "You do not have access to VAC".to_string(),
                "This account is not currently authorized to use VAC in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to VAC.".to_string(),
                "Contact your workspace administrator to get access to VAC, then return to VAC and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to VAC to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
pub(crate) async fn obtain_api_key(
    ...
    let resp = client
        ...
        .body(format!(
            ...
            urlencoding::encode(client_id),
            urlencoding::encode("vastar-api-key"),
...
mod tests {
    ...
    use super::is_missing_vac_entitlement_error;
    ...
    fn redact_sensitive_url_parts_preserves_safe_url_shape() {
        let mut url = url::Url::parse(
            "https://user:pass@auth.vastar.com/oauth/token?code=abc123&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback#frag",
        )
        .expect("valid url");
        ...
        assert_eq!(
            url.as_str(),
            "https://auth.vastar.com/oauth/token?code=%3Credacted%3E&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback"
        );
    }
    ...
    fn compose_success_url_omits_streamlined_success_by_default() {
        ...
        assert_eq!(
            url.query_pairs()
                .find(|(key, _)| key == "vac_streamlined_login"),
            None
        );
    }
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_vac_entitlement");
        assert!(is_missing_vac_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to VAC"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_vac_entitlement"));
    }
}
