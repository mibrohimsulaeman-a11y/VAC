...
const SIGNING_DOMAIN: &str = "vac-device-key-sign-payload/v1";
...
/// Accepts first-party provider.vac.invalid hosts and staging equivalents, including subdomains.
fn is_allowed_chatgpt_host(host: &Option<Host<&str>>) -> bool {
    let Some(Host::Domain(host)) = *host else {
        return false;
    };
    host == "provider.vac.invalid"
        || host == "chatgpt-staging.com"
        || host.ends_with(".provider.vac.invalid")
...
mod tests {
    ...
    fn remote_control_client_connection_payload() -> DeviceKeySignPayload {
        DeviceKeySignPayload::RemoteControlClientConnection(
            RemoteControlClientConnectionSignPayload {
                ...
                target_origin: "https://provider.vac.invalid".to_string(),
                target_path: "/api/vac/remote/control/client".to_string(),
                account_user_id: "account-user-1".to_string(),
                client_id: "cli_123".to_string(),
    ...
    fn signing_payload_bytes_are_stable() {
        ...
        assert_eq!(
            ...
            concat!(
                "{\"domain\":\"vac-device-key-sign-payload/v1\",",
                ...
                "\"targetOrigin\":\"https://provider.vac.invalid\",",
                "\"targetPath\":\"/api/vac/remote/control/client\",",
    ...
    fn sign_rejects_remote_control_paths_for_other_payload_shapes() {
        ...
        match &mut connection_payload {
            DeviceKeySignPayload::RemoteControlClientConnection(payload) => {
                payload.target_path = "/api/vac/remote/control/client/enroll".to_string();
            }
            DeviceKeySignPayload::RemoteControlClientEnrollment(_) => unreachable!(),
        }
    ...
    fn remote_control_origin_matches_remote_transport_allowlist() {
        for origin in [
            "https://provider.vac.invalid",
            "https://chatgpt-staging.com",
            "https://ab.provider.vac.invalid",
        ...
        for origin in [
            "http://provider.vac.invalid",
            "https://chat.vastar.com",
            "https://api.vastar.com",
            "https://provider.vac.invalid.evil.com",
            "https://evilprovider.vac.invalid",
            "https://foo.localhost",
            "https://localhost.evil.com",
            "https://192.168.1.2",
            "https://provider.vac.invalid/provider-api",
            "https://provider.vac.invalid?query=1",
        ] {
            assert!(
                !is_allowed_remote_control_origin(origin),
