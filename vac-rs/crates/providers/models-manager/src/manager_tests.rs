...
use vac_login::AuthCredentialsStoreMode;
use vac_login::AuthManager;
use vac_login::ExternalAuth;
use vac_login::ExternalAuthRefreshContext;
use vac_login::ExternalAuthTokens;
use vac_login::TokenData;
use vac_login::VACAuth;
use vac_protocol::auth::AuthMode;
...
fn vastar_manager_for_tests(
    vac_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
) -> OpenAiModelsManager {
    vastar_manager_for_tests_with_auth(
        vac_home,
        endpoint_client,
...
fn vastar_manager_for_tests_with_auth(
    vac_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
    auth_manager: Option<Arc<AuthManager>>,
) -> OpenAiModelsManager {
    OpenAiModelsManager::new(vac_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(vac_home: &Path) -> VACAuth {
    let auth_dot_json = vac_login::AuthDotJson {
        auth_mode: Some(AuthMode::ProviderCredential),
        vastar_api_key: None,
        tokens: Some(TokenData {
            id_token: vac_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(vac_home).expect("vac home should be created");
    std::fs::write(
...
async fn get_model_info_tracks_fallback_usage() {
    let vac_home = tempdir().expect("temp dir");
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let manager = vastar_manager_for_tests(vac_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager = vastar_manager_for_tests(vac_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_skips_network_without_chatgpt_auth() {
    ...
    let manager = vastar_manager_for_tests_with_auth(
        vac_home.path().to_path_buf(),
        endpoint.clone(),
        /*auth_manager*/ None,
    );
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(vac_home.path()).await;
    let manager = vastar_manager_for_tests_with_auth(
        vac_home.path().to_path_buf(),
        endpoint.clone(),
