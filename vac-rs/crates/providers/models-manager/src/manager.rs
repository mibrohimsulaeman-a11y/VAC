...
/// Remote endpoint used by the Vastar-compatible model manager.
///
...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use VAC backend-only models.
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_by_auth(presets, uses_vac_backend);
...
/// Vastar-compatible model manager backed by bundled models, cache, and `/models`.
#[derive(Debug)]
...
impl OpenAiModelsManager {
    /// Construct an Vastar-compatible remote model manager.
    pub fn new(
        vac_home: PathBuf,
        endpoint_client: Arc<dyn ModelsEndpointClient>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Self {
        let cache_path = vac_home.join(MODEL_CACHE_FILE);
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_vac_backend().await || self.endpoint_client.has_command_auth()
    }
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer = vac_otel::start_global_timer("vac.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
        info!(client_version, "models cache: evaluating cache eligibility");
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(celia-oai): Include provider identity in cache eligibility so switching
...
pub(crate) fn construct_model_info_from_candidates(
    ...
    // retry for namespaced slugs like `custom/gpt-5.3-vac`.
