...
use crate::events::VACAppMentionedEventRequest;
use crate::events::VACAppServerClientMetadata;
use crate::events::VACAppUsedEventRequest;
use crate::events::VACCompactionEventRequest;
use crate::events::VACHookRunEventRequest;
use crate::events::VACPluginEventRequest;
use crate::events::VACPluginUsedEventRequest;
use crate::events::VACRuntimeMetadata;
use crate::events::VACTurnEventParams;
use crate::events::VACTurnEventRequest;
use crate::events::VACTurnSteerEventParams;
use crate::events::VACTurnSteerEventRequest;
use crate::events::plugin_state_event_type;
use crate::events::subagent_parent_thread_id;
use crate::events::subagent_source_name;
use crate::events::subagent_thread_started_event_request;
use crate::events::vac_app_metadata;
use crate::events::vac_compaction_event_params;
use crate::events::vac_hook_run_metadata;
use crate::events::vac_plugin_metadata;
use crate::events::vac_plugin_used_metadata;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a VACCompactionEvent) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<VACErrorInfo>,
    completed_at: u64,
...
impl AnalyticsReducer {
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "vac_guardian_review",
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = vac_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(VACAppMentionedEventRequest {
                event_type: "vac_app_mentioned",
                event_params,
            })
        }));
    }
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = vac_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(VACAppUsedEventRequest {
            event_type: "vac_app_used",
            event_params,
        }));
    }
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(VACHookRunEventRequest {
            event_type: "vac_hook_run",
            event_params: vac_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(VACPluginUsedEventRequest {
            event_type: "vac_plugin_used",
            event_params: vac_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = VACPluginEventRequest {
            event_type: plugin_state_event_type(state),
            event_params: vac_plugin_metadata(plugin),
        };
    ...
    fn ingest_response(
        ...
        match response {
            ...
            ClientResponse::ThreadBranch { response, .. } => {
                self.emit_thread_initialized(
                    connection_id,
                    response.thread,
                    response.model,
                    ThreadInitializationMode::Branched,
                    out,
                );
            }
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.vac_error_info),
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: vac_runtime_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn ingest_compaction(&mut self, input: VACCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            VACCompactionEventRequest {
                event_type: "vac_compaction_event",
                event_params: vac_compaction_event_params(
                    input,
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(VACTurnSteerEventRequest {
            event_type: "vac_turn_steer_event",
            event_params: VACTurnSteerEventParams {
    ...
    fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::TurnEvent(Box::new(
            VACTurnEventRequest {
                event_type: "vac_turn_event",
                event_params: vac_turn_event_params(
                    connection_state.app_server_client.clone(),
                    connection_state.runtime.clone(),
                    turn_id.to_string(),
                    turn_state,
...
fn analytics_turn_status(status: vac_runtime_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        vac_runtime_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        vac_runtime_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        vac_runtime_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        vac_runtime_protocol::TurnStatus::InProgress => None,
    }
}
