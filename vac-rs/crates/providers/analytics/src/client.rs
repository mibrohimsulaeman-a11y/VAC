...
impl AnalyticsEventsQueue {
    ...
    fn try_send(&self, input: AnalyticsFact) {
        if self.sender.try_send(input).is_err() {
            //VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): add a metric for this
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::VACCompactionEvent) {
    ...
    pub fn track_response(
        ...
        if !matches!(
            ...
            ClientResponsePayload::ThreadStart(_)
                ...
                | ClientResponsePayload::ThreadBranch(_)
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/vac/analytics-events/events");
