...
use vac_protocol::models::SandboxPermissions;
use vac_protocol::protocol::GuardianAssessmentOutcome;
use vac_protocol::protocol::GuardianCommandSource;
use vac_protocol::protocol::GuardianRiskLevel;
use vac_protocol::protocol::GuardianUserAuthorization;
use vac_protocol::protocol::HookEventName;
use vac_protocol::protocol::HookRunStatus;
...
pub(crate) enum TrackEventRequest {
    ...
    AppMentioned(VACAppMentionedEventRequest),
    AppUsed(VACAppUsedEventRequest),
    HookRun(VACHookRunEventRequest),
    Compaction(Box<VACCompactionEventRequest>),
    TurnEvent(Box<VACTurnEventRequest>),
    TurnSteer(VACTurnSteerEventRequest),
    PluginUsed(VACPluginUsedEventRequest),
    PluginInstalled(VACPluginEventRequest),
    PluginUninstalled(VACPluginEventRequest),
    PluginEnabled(VACPluginEventRequest),
    PluginDisabled(VACPluginEventRequest),
}
...
pub(crate) struct VACAppServerClientMetadata {
...
#[derive(Clone, Serialize)]
pub(crate) struct VACRuntimeMetadata {
    pub(crate) vac_rs_version: String,
...
pub(crate) struct ThreadInitializedEventParams {
    ...
    pub(crate) app_server_client: VACAppServerClientMetadata,
    pub(crate) runtime: VACRuntimeMetadata,
...
pub enum GuardianApprovalRequestSource {
    /// Approval requested directly by the main VAC turn.
    MainTurn,
...
impl GuardianReviewTrackContext {
    ...
    pub(crate) fn event_params(
        ...
        GuardianReviewEventParams {
            ...
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(rhan-oai): plumb nested Guardian review session tool-call counts.
...
#[derive(Serialize)]
pub(crate) struct VACAppMetadata {
...
#[derive(Serialize)]
pub(crate) struct VACAppMentionedEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACAppMetadata,
}

#[derive(Serialize)]
pub(crate) struct VACAppUsedEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACAppMetadata,
}

#[derive(Serialize)]
pub(crate) struct VACHookRunMetadata {
    pub(crate) thread_id: Option<String>,
    pub(crate) turn_id: Option<String>,
    pub(crate) model_slug: Option<String>,
...
#[derive(Serialize)]
pub(crate) struct VACHookRunEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACHookRunMetadata,
}

#[derive(Serialize)]
pub(crate) struct VACCompactionEventParams {
    pub(crate) thread_id: String,
...
#[derive(Serialize)]
pub(crate) struct VACCompactionEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACCompactionEventParams,
}

#[derive(Serialize)]
pub(crate) struct VACTurnEventParams {
    pub(crate) thread_id: String,
    pub(crate) turn_id: String,
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(rhan-oai): Populate once queued/default submission type is plumbed from
    ...
    pub(crate) turn_error: Option<VACErrorInfo>,
    pub(crate) steer_count: Option<usize>,
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(rhan-oai): Populate these once tool-call accounting is emitted from
...
#[derive(Serialize)]
pub(crate) struct VACTurnEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACTurnEventParams,
}

#[derive(Serialize)]
pub(crate) struct VACTurnSteerEventParams {
    pub(crate) thread_id: String,
...
#[derive(Serialize)]
pub(crate) struct VACTurnSteerEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACTurnSteerEventParams,
}

#[derive(Serialize)]
pub(crate) struct VACPluginMetadata {
...
#[derive(Serialize)]
pub(crate) struct VACPluginUsedMetadata {
    #[serde(flatten)]
    pub(crate) plugin: VACPluginMetadata,
    pub(crate) thread_id: Option<String>,
    pub(crate) turn_id: Option<String>,
    pub(crate) model_slug: Option<String>,
}
...
#[derive(Serialize)]
pub(crate) struct VACPluginEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACPluginMetadata,
}

#[derive(Serialize)]
pub(crate) struct VACPluginUsedEventRequest {
    pub(crate) event_type: &'static str,
    pub(crate) event_params: VACPluginUsedMetadata,
}

pub(crate) fn plugin_state_event_type(state: PluginState) -> &'static str {
    match state {
        PluginState::Installed => "vac_plugin_installed",
        PluginState::Uninstalled => "vac_plugin_uninstalled",
        PluginState::Enabled => "vac_plugin_enabled",
        PluginState::Disabled => "vac_plugin_disabled",
    }
}

pub(crate) fn vac_app_metadata(
...
pub(crate) fn vac_plugin_metadata(plugin: PluginTelemetryMetadata) -> VACPluginMetadata {
...
pub(crate) fn vac_compaction_event_params(
    input: VACCompactionEvent,
    app_server_client: VACAppServerClientMetadata,
    runtime: VACRuntimeMetadata,
    thread_source: Option<&'static str>,
    subagent_source: Option<String>,
    parent_thread_id: Option<String>,
) -> VACCompactionEventParams {
    VACCompactionEventParams {
...
pub(crate) fn vac_plugin_used_metadata(
    tracking: &TrackEventsContext,
    plugin: PluginTelemetryMetadata,
) -> VACPluginUsedMetadata {
    VACPluginUsedMetadata {
        plugin: vac_plugin_metadata(plugin),
        thread_id: Some(tracking.thread_id.clone()),
        turn_id: Some(tracking.turn_id.clone()),
        model_slug: Some(tracking.model_slug.clone()),
    }
...
pub(crate) fn vac_hook_run_metadata(
    tracking: &TrackEventsContext,
    hook: HookRunFact,
) -> VACHookRunMetadata {
    VACHookRunMetadata {
...
pub(crate) fn current_runtime_metadata() -> VACRuntimeMetadata {
    let os_info = os_info::get();
    VACRuntimeMetadata {
        vac_rs_version: env!("CARGO_PKG_VERSION").to_string(),
...
pub(crate) fn subagent_thread_started_event_request(
    ...
    ThreadInitializedEvent {
        event_type: "vac_thread_initialized",
        event_params,
    }
}
