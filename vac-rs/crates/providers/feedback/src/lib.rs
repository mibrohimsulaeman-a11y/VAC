...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_recovery_followup_success = tracing::field::debug(&snapshot.auth_recovery_followup_success),
        auth_recovery_followup_status = tracing::field::debug(&snapshot.auth_recovery_followup_status),
        auth_env_vastar_api_key_present = tracing::field::debug(auth_env.vastar_api_key_env_present),
        auth_env_vac_api_key_present = tracing::field::debug(auth_env.vac_api_key_env_present),
        auth_env_vac_api_key_enabled = tracing::field::debug(auth_env.vac_api_key_env_enabled),
...
#[derive(Clone)]
pub struct VACFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for VACFeedback {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("vac-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("vac-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = VACFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("vac-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = VACFeedback::new()
        ...
        let attachments_without_diagnostics = VACFeedback::new()
            .snapshot(/*session_id*/ None)
