...
fn test_deserialize_azure_model_provider_toml() {
    let azure_provider_toml = r#"
name = "Azure"
base_url = "https://xxxxx.vastar.azure.com/vastar"
env_key = "AZURE_VASTAR_API_KEY"
query_params = { api-version = "2025-04-01-preview" }
        "#;
    let expected_provider = ModelProviderInfo {
        name: "Azure".into(),
        base_url: Some("https://xxxxx.vastar.azure.com/vastar".into()),
        env_key: Some("AZURE_VASTAR_API_KEY".into()),
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
        aws: None,
...
fn test_deserialize_chat_wire_api_shows_helpful_error() {
    let provider_toml = r#"
name = "Vastar using Chat Completions"
base_url = "https://api.vastar.com/v1"
env_key = "VASTAR_API_KEY"
wire_api = "chat"
        "#;
...
#[test]
fn test_supports_remote_compaction_for_vastar() {
    let provider = ModelProviderInfo::create_vastar_provider(/*base_url*/ None);
...
fn test_supports_remote_compaction_for_azure_name() {
    let provider = ModelProviderInfo {
        name: "Azure".into(),
        base_url: Some("https://example.com/vastar".into()),
        env_key: Some("AZURE_VASTAR_API_KEY".into()),
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
...
#[test]
fn test_supports_remote_compaction_for_non_vastar_non_azure_provider() {
    let provider = ModelProviderInfo {
        name: "Example".into(),
...
fn test_create_amazon_bedrock_provider() {
    assert_eq!(
        ...
        ModelProviderInfo {
            ...
            base_url: Some("https://bedrock-mantle.us-east-1.api.aws/vastar/v1".to_string()),
            env_key: None,
            env_key_instructions: None,
            experimental_bearer_token: None,
            auth: None,
...
fn test_built_in_model_providers_include_amazon_bedrock() {
    let providers = built_in_model_providers(/*vastar_base_url*/ None);
...
fn test_merge_configured_model_providers_adds_custom_provider() {
    ...
    let mut expected = built_in_model_providers(/*vastar_base_url*/ None);
    expected.insert("custom".to_string(), custom_provider);

    assert_eq!(
        merge_configured_model_providers(
            built_in_model_providers(/*vastar_base_url*/ None),
...
fn test_merge_configured_model_providers_allows_amazon_bedrock_default_fields() {
    ...
    assert_eq!(
        ...
        Ok(built_in_model_providers(/*vastar_base_url*/ None))
    );
}
...
fn test_validate_provider_aws_rejects_conflicting_auth() {
    ...
    assert_eq!(
        provider.validate(),
        Err("provider aws cannot be combined with env_key, requires_vastar_auth".to_string())
    );
}
