...
//! detailed explanation of what "hermetic" means here lives in `vac_client::custom_ca`; these
...
use vac_utils_cargo_bin::cargo_bin;

const VAC_CA_CERT_ENV: &str = "VAC_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "VAC_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "VAC_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "VAC_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    let mut cmd = Command::new(
        cargo_bin("provider_ca_probe")
            .unwrap_or_else(|error| panic!("failed to locate provider_ca_probe: {error}")),
    );
    ...
    cmd.env_remove(VAC_CA_CERT_ENV);
...
fn run_probe(envs: &[(&str, &Path)]) -> std::process::Output {
    ...
    cmd.output()
        .unwrap_or_else(|error| panic!("failed to run provider_ca_probe: {error}"))
}
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    vac_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls13_material() -> Tls13Material {
    ...
    ca_distinguished_name.push(DnType::CommonName, "vac test CA");
...
#[test]
fn uses_vac_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(VAC_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_vac_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (VAC_CA_CERT_ENV, cert_path.as_path()),
...
fn posts_to_tls13_server_using_custom_ca_bundle() {
    ...
    let output =
        run_probe_posting_to_tls13_server(&[(VAC_CA_CERT_ENV, cert_path.as_path())], &server.url);
    let server_result = server.request_rx.recv_timeout(Duration::from_secs(5));

    assert!(
        output.status.success(),
        "provider_ca_probe failed\nstdout:\n{}\nstderr:\n{}\nserver:\n{server_result:?}",
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("VAC_CA_CERTIFICATE"));
