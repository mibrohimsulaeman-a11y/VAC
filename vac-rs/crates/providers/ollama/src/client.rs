...
use crate::url::is_vastar_compatible_base_url;
...
impl OllamaClient {
    ...
    pub(crate) async fn try_from_provider(provider: &ModelProviderInfo) -> io::Result<Self> {
        ...
        let uses_vastar_compat = is_vastar_compatible_base_url(base_url);
    ...
    async fn probe_server(&self) -> io::Result<()> {
        let url = if self.uses_vastar_compat {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_probe_server_happy_path_vastar_compat_and_native() {
        if std::env::var(vac_core::spawn::VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
                "{} set; skipping test_probe_server_happy_path_vastar_compat_and_native",
                vac_core::spawn::VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR
            );
            return;
        }
    ...
    async fn test_try_from_oss_provider_ok_when_server_running() {
        ...
        // Vastar‑compat models endpoint responds OK.
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/v1/models"))
            .respond_with(wiremock::ResponseTemplate::new(200))
            .mount(&server)
