...
pub const PLUGIN_MANIFEST_PATH: &str = ".vac-plugin/plugin.json";
...
/// plugin manifest (same `name` rules as full manifest loading in vac-core).
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".vac-plugin")).expect("mkdir manifest");
        fs::write(
