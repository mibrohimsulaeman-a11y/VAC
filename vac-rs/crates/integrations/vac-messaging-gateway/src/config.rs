...
impl GatewayConfig {
    ...
    fn apply_env_overrides(&mut self) {
        if self.server.url.trim().is_empty()
            && let Ok(value) = std::env::var("VAC_GATEWAY_URL")
        {
            self.server.url = value;
        }

        if self.server.token.trim().is_empty()
            && let Ok(value) = std::env::var("VAC_GATEWAY_TOKEN")
        {
...
fn default_store_path() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        return home.join(".vac").join("autopilot").join("gateway.db");
    }
    PathBuf::from(".vac/autopilot/gateway.db")
}
