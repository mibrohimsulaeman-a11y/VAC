...
impl GatewayStore {
    ...
    async fn configure_database_pragmas(&self) -> Result<()> {
        ...
        vac_foundation::sqlite::apply_database_pragmas(&conn)
            .await
    ...
    ///
    /// See [`vac_foundation::sqlite::apply_connection_pragmas`] for details.
    async fn connection(&self) -> Result<Connection> {
        let conn = self.connect_raw()?;
        vac_foundation::sqlite::apply_connection_pragmas(&conn)
            .await
