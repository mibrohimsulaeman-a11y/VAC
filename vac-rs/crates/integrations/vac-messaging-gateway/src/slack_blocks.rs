...
fn generate_fallback_text(text: &str) -> String {
    ...
    for event in parser {
        match event {
            ...
            Event::Start(Tag::CodeBlock(_)) if !last_was_block => {
                output.push('\n');
            }
            ...
            Event::Start(Tag::Paragraph) if !output.is_empty() && !last_was_block => {
                output.push('\n');
            }
            Event::Start(Tag::Paragraph) => {}
            Event::End(TagEnd::Paragraph) => {
            ...
            Event::Start(Tag::Heading { .. }) if !output.is_empty() && !last_was_block => {
                output.push('\n');
            }
