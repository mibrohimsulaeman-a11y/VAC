...
use vac_exec_server::ExecServerClient;
use vac_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    vac_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    InitializeRequestParams {
        ...
        client_info: Implementation {
            ...
            title: Some("VAC rmcp recovery test".into()),
            description: None,
            icons: None,
            website_url: None,
        },
...
pub(crate) async fn spawn_exec_server() -> anyhow::Result<ExecServerProcess> {
    ...
    let mut child = Command::new(vac_utils_cargo_bin::cargo_bin("vac")?)
...
/// Reads the websocket URL printed by `vac exec-server --listen`.
