...
const MEMO_URI: &str = "memo://vac/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "vac/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of VAC TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (VAC TUI):
    /// - Build this binary: `cargo build -p vac-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `vac mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in VAC TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://vac/{slug}".to_string(),
            name: "vac-memo".to_string(),
            title: Some("VAC Memo".to_string()),
            description: Some(
                "Template for memo://vac/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise VAC TUI handling of MCP image outputs.
///
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                ...
                meta.insert("vac/imageDetail".to_string(), serde_json::json!("original"));
