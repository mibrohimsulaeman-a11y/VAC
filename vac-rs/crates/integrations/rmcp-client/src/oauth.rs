...
//!
//! If the keyring is not available or fails, we fall back to VAC_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "VAC MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_vac_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    impl TempVACHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create VAC_HOME temp dir");
            // SAFETY: RMCP client boundary: this launcher path owns the process or FFI handle during this conversion/call; the multi-line unsafe block below uses inputs that remain live until the block returns.
            unsafe {
                std::env::set_var("VAC_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
