...
use vac_exec_server::ExecBackend;
use vac_exec_server::ExecEnvPolicy;
use vac_exec_server::ExecParams;
use vac_exec_server::ExecProcess;
use vac_protocol::config_types::ShellEnvironmentPolicyInherit;
#[cfg(unix)]
use vac_utils_pty::process_group::kill_process_group;
#[cfg(unix)]
use vac_utils_pty::process_group::terminate_process_group;
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from VAC. Start from `All` only so the
