...
use vac_foundation::task_manager::StartTaskOptions;

/// Default config path inside container (matches ~/.vac/config.toml convention).
const CONTAINER_CONFIG_PATH: &str = "/agent/.vac/config.toml";
...
pub struct DynamicSubagentRequest {
    ...
    /// Use tool names like: vac__view, vac__run_command, vac__search_docs, etc.
    #[schemars(
        description = "Array of tool names to grant the subagent. Follow least-privilege principle - only include tools necessary for the task. Examples: vac__view, vac__run_command, vac__search_docs, vac__str_replace"
    )]
...
- .vac session data is mounted writable at /agent/.vac
...
- Unspecified tool calls are rejected by default"
    ...
    pub async fn resume_subagent_task(
        ...
        if !matches!(
            ...
            vac_foundation::task_manager::TaskStatus::Paused
                | vac_foundation::task_manager::TaskStatus::Completed
        ) {
        ...
        // Build the vac CLI command for resuming
    ...
    fn build_dynamic_subagent_command(
        ...
        // When running in sandbox (warden container), use "vac" since it's in PATH
        ...
        // Build the vac command arguments
        ...
        // `prepare_volumes()` → `vac_agent_default_mounts()` automatically.
        if enable_sandbox {
            use vac_foundation::container::{
                ensure_named_volumes_exist, resolve_ak_store_for_sandbox, vac_agent_image,
            ...
            // Pre-create ~/.vac/knowledge on the host so docker doesn't
            ...
            {
                let default_root = Path::new(&home).join(".vac/knowledge");
...
fn default_subagent_model(provider: &str) -> Option<&'static str> {
    match provider {
        ...
        "vac" => Some("vac/anthropic/claude-haiku-4-5"),
        "github-copilot" => None,
        _ => None,
    }
}
...
mod tests {
    ...
    fn dynamic_subagent_command_shell_quotes_model_and_tools() {
        ...
        let command = container
            .build_dynamic_subagent_command(
                "do the thing",
                None,
                &["vac__view; touch /tmp/tool_pwn".to_string()],
        ...
        assert!(command.contains("-t 'vac__view; touch /tmp/tool_pwn'"));
    }
    ...
    #[test]
    fn openai_gemini_bedrock_and_vac_parents_use_builtin_defaults() {
        ...
        assert_eq!(
            default_subagent_model("vac"),
            Some("vac/anthropic/claude-haiku-4-5")
        );
    }
