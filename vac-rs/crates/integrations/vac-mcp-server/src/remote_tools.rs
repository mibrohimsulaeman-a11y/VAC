...
use vac_remote_service::models::SearchDocsRequest as ApiSearchDocsRequest;
// use vac_remote_service::models::CodeIndex;
...
If your goal requires understanding multiple distinct topics or technologies, make separate search calls rather than combining all keywords into one overly-specific search that may miss relevant documentation."
    ...
    pub async fn search_docs(
        ...
        let processed: Vec<Content> = response
            ...
            .map(|c| {
                ...
                let allowance = remaining_lines
                    .checked_div(remaining_items)
                    .unwrap_or(1)
                    .max(1);
    ...
    pub async fn load_skill(
        ...
        if path.exists() && path.is_file() {
            ...
            if is_allowed {
                match vac_remote_service::local::skills::load_skill_from_path(path) {
                    Ok((skill_dir, body)) => {
                        let response =
...
mod tests {
    ...
    fn search_docs_runtime_builds_normalized_api_payload() {
        ...
        assert_eq!(payload.keywords, "vac cli latest");
