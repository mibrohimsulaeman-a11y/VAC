...
`vac-otel` is the OpenTelemetry integration crate for VAC. It provides:

- Provider wiring for log/trace/metric exporters (`vac_otel::OtelProvider`
  and `vac_otel::provider`).
- Session-scoped business event emission via `vac_otel::SessionTelemetry`.
- Low-level metrics APIs via `vac_otel::metrics`.
- Trace-context helpers via `vac_otel::trace_context` and crate-root re-exports.
...
```rust
use vac_otel::config::OtelExporter;
use vac_otel::config::OtelHttpProtocol;
use vac_otel::config::OtelSettings;
use vac_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "vac-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    vac_home: std::path::PathBuf::from("/tmp"),
...
VAC-specific session events. Rich session/business events should go through
...
`vac-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using VAC-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use vac_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("vac.session_started", 1, &[("source", "tui")])?;
metrics.histogram("vac.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("vac.turns", 1, &[("model", "gpt-5.1")])?;
