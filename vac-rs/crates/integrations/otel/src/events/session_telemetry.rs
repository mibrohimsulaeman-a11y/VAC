...
impl SessionTelemetry {
    ...
    pub fn record_responses(&self, handle_responses_span: &Span, event: &ResponseEvent) {
        ...
        match event {
            ...
            ResponseEvent::Completed {
                ...
                handle_responses_span.record(
                    "vac.usage.reasoning_output_tokens",
                    token_usage.reasoning_output_tokens,
                );
                handle_responses_span.record("vac.usage.total_tokens", token_usage.total_tokens);
            }
            _ => {}
        }
    }
    ...
    pub fn conversation_starts(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "vac.conversation_starts",
                provider_name = %provider_name,
                auth.env_vastar_api_key_present = self.metadata.auth_env.vastar_api_key_env_present,
                auth.env_vac_api_key_present = self.metadata.auth_env.vac_api_key_env_present,
                auth.env_vac_api_key_enabled = self.metadata.auth_env.vac_api_key_env_enabled,
    ...
    pub fn record_websocket_connect(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "vac.websocket_connect",
                duration_ms = %duration.as_millis(),
                http.response.status_code = status,
                success = success_str,
                error.message = error,
    ...
    pub fn record_websocket_request(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "vac.websocket_request",
                duration_ms = %duration.as_millis(),
                success = success_str,
                error.message = error,
                auth.env_vastar_api_key_present = self.metadata.auth_env.vastar_api_key_env_present,
    ...
    pub fn record_websocket_event(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "vac.websocket_event",
                event.kind = %kind_str,
                duration_ms = %duration.as_millis(),
                success = success_str,
