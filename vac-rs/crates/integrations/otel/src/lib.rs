...
pub use vac_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on vac-core.
...
impl From<vac_runtime_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: vac_runtime_protocol::AuthMode) -> Self {
        if mode == vac_runtime_protocol::AuthMode::ApiKey {
            Self::ApiKey
        } else {
