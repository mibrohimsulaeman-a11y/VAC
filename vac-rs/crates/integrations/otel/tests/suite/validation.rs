...
use vac_otel::MetricsError;
use vac_otel::Result;

fn build_in_memory_client() -> Result<MetricsClient> {
    let exporter = InMemoryMetricExporter::default();
    let config = MetricsConfig::in_memory("test", "vac-cli", env!("CARGO_PKG_VERSION"), exporter);
...
fn counter_rejects_invalid_tag_key() -> Result<()> {
    ...
    let err = metrics
        .counter("vac.turns", /*inc*/ 1, &[("bad key", "value")])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::InvalidTagComponent { label, value }
...
fn counter_rejects_negative_increment() -> Result<()> {
    let metrics = build_in_memory_client()?;
    let err = metrics.counter("vac.turns", /*inc*/ -1, &[]).unwrap_err();
    assert!(matches!(
        err,
        MetricsError::NegativeCounterIncrement { name, inc } if name == "vac.turns" && inc == -1
    ));
    metrics.shutdown()?;
    Ok(())
}
