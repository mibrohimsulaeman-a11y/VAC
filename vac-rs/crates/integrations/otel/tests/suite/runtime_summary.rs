...
use vac_otel::Result;
use vac_otel::RuntimeMetricTotals;
...
fn runtime_metrics_summary_collects_tool_api_and_streaming_metrics() -> Result<()> {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "vac-cli", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )?;
    ...
    manager.record_duration("vac.turn.ttft.duration_ms", Duration::from_millis(95), &[]);
    manager.record_duration("vac.turn.ttfm.duration_ms", Duration::from_millis(180), &[]);
