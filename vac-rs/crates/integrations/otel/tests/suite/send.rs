...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "vac-cli"), ("env", "prod")])?;
    ...
    metrics.histogram("vac.tool_latency", /*value*/ 25, &[("tool", "shell")])?;
    metrics.shutdown()?;

    let resource_metrics = latest_metrics(&exporter);

    let counter = find_metric(&resource_metrics, "vac.turns").expect("counter metric missing");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "vac-cli".to_string()),
        ("env".to_string(), "dev".to_string()),
        ("model".to_string(), "gpt-5.1".to_string()),
    ]);
    assert_eq!(counter_attributes, expected_counter_attributes);

    let (bounds, bucket_counts, sum, count) = histogram_data(&resource_metrics, "vac.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "vac.tool_latency").and_then(|metric| {
            match metric.data() {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "vac-cli"), ("env", "prod"), ("region", "us")])?;
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "vac.alpha").expect("vac.alpha metric missing");
    ...
    let beta_metric = find_metric(&resource_metrics, "vac.beta").expect("vac.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("vac.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("vac.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
