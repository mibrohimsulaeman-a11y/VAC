//! Aggregates MCP server connections for VAC.
//!
...
use crate::vac_apps::VACAppsToolsCacheKey;
...
use vac_protocol::protocol::McpStartupCompleteEvent;
use vac_protocol::protocol::McpStartupFailure;
use vac_protocol::protocol::McpStartupStatus;
use vac_protocol::protocol::McpStartupUpdateEvent;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        vac_home: PathBuf,
        vac_apps_tools_cache_key: VACAppsToolsCacheKey,
        ...
        let vac_apps_auth_provider = auth
            .filter(|auth| auth.uses_vac_backend())
            .map(vac_model_provider::auth_provider_from_auth);
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let vac_apps_tools_cache_context = if server_name == VAC_APPS_MCP_SERVER_NAME {
                Some(VACAppsToolsCacheContext {
                    vac_home: vac_home.clone(),
                    user_key: vac_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == VAC_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    vac_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    /// Force-refresh vac apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_vac_apps_tools_cache(&self) -> Result<HashMap<String, ToolInfo>> {
        let managed_client = self
            .clients
            .get(VAC_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{VAC_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{VAC_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_vac_apps_tools_if_needed(
            VAC_APPS_MCP_SERVER_NAME,
            managed_client.vac_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
...
fn mcp_init_error_display(
        ...
        && http_headers.as_ref().map(HashMap::is_empty).unwrap_or(true)
    {
        ...
        format!("The {server_name} MCP server is not logged in. Run `vac mcp login {server_name}`.")
