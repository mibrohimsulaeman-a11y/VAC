...
use crate::mcp::VAC_APPS_MCP_SERVER_NAME;
...
use crate::vac_apps::CachedVACAppsToolsLoad;
use crate::vac_apps::VACAppsToolsCacheContext;
use crate::vac_apps::filter_disallowed_vac_apps_tools;
use crate::vac_apps::load_cached_vac_apps_tools;
use crate::vac_apps::load_startup_cached_vac_apps_tools_snapshot;
use crate::vac_apps::normalize_vac_apps_callable_name;
use crate::vac_apps::normalize_vac_apps_callable_namespace;
use crate::vac_apps::normalize_vac_apps_tool_title;
use crate::vac_apps::write_cached_vac_apps_tools_if_needed;
use anyhow::Result;
use anyhow::anyhow;
...
use vac_async_utils::OrCancelExt;
use vac_config::McpServerConfig;
use vac_config::McpServerTransportConfig;
use vac_config::types::OAuthCredentialsStoreMode;
use vac_exec_server::HttpClient;
use vac_exec_server::ReqwestHttpClient;
use vac_protocol::protocol::Event;
use vac_rmcp_client::ExecutorStdioServerLauncher;
use vac_rmcp_client::LocalStdioServerLauncher;
use vac_rmcp_client::RmcpClient;
use vac_rmcp_client::StdioServerLauncher;

/// MCP server capability indicating that VAC should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "vac/sandbox-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "vac.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "vac.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) vac_apps_tools_cache_context: Option<VACAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.vac_apps_tools_cache_context.as_ref()
            && let CachedVACAppsToolsLoad::Hit(tools) = load_cached_vac_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.vac_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        vac_apps_tools_cache_context: Option<VACAppsToolsCacheContext>,
        tool_plugin_provenance: Arc<ToolPluginProvenance>,
        runtime_environment: McpRuntimeEnvironment,
        runtime_auth_provider: Option<SharedAuthProvider>,
    ) -> Self {
        let tool_filter = ToolFilter::from_config(&config);
        let startup_snapshot = load_startup_cached_vac_apps_tools_snapshot(
            &server_name,
            vac_apps_tools_cache_context.as_ref(),
        )
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
        let annotate_tools = |tools: Vec<ToolInfo>| {
            ...
            for tool in &mut tools {
                if tool.server_name == VAC_APPS_MCP_SERVER_NAME {
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_vac_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_vac_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_vac_apps_tool_title(server_name, connector_name.as_deref(), title);
        ...
        .collect();
    if server_name == VAC_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_vac_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    write_cached_vac_apps_tools_if_needed(
        &server_name,
        vac_apps_tools_cache_context.as_ref(),
        &tools,
    );
...
struct StartServerTaskParams {
    startup_timeout: Option<Duration>, // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): cancel_token should handle this.
...
mod tests {
    ...
    fn custom_mcp_connector_metadata_is_stripped() {
        ...
        assert!(meta.0.contains_key("vastar/fileParams"));
        assert_eq!(
    ...
    #[test]
    fn vac_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
