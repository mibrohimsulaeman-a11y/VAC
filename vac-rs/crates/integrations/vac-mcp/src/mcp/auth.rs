...
use vac_rmcp_client::OAuthProviderError;
use vac_rmcp_client::determine_streamable_http_auth_status;
use vac_rmcp_client::discover_streamable_http_oauth;

use super::VAC_APPS_MCP_SERVER_NAME;
    ...
    I: IntoIterator<Item = (&'a String, &'a McpServerConfig)>,
{
    let futures = servers.into_iter().map(|(name, config)| {
        ...
        let has_runtime_auth = name == VAC_APPS_MCP_SERVER_NAME
            && auth.is_some_and(VACAuth::uses_vac_backend)
            && matches!(
                &config.transport,
