...
> Auto-generated by `vac init` on {date}. Verified by {user/auto}.
> Last updated: {date}
>
...
- **Source**: `./services/api/` or `<git remote supplied by maintainer>`
...
Write check scripts as simple shell one-liners or short scripts. Store them in `~/.vac/checks/` on the target machine. Use the `create` tool with remote path format for remote servers.
...
1. Add each schedule using `vac autopilot schedule add`
2. If Slack/Discord integration is desired, configure the channel
3. Start autopilot with `vac up`
4. Verify with `vac autopilot status`
