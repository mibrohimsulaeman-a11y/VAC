...
pub use client::{AgentClient, AgentClientConfig, DEFAULT_VAC_ENDPOINT, VacCloudConfig};
...
pub use storage::{
    ...
    SessionSummary, SessionVisibility, StorageError,
    UpdateSessionRequest as StorageUpdateSessionRequest, VACStorage,
};
...
///
/// When `use_vac` is true, the model is transformed for VAC API routing.
pub fn find_model(model_str: &str, use_vac: bool) -> Option<Model> {
...
fn find_in_provider(provider_id: &str, model_id: &str) -> Option<Model> {
    let models = vac_provider_core::load_models_for_provider(provider_id).ok()?;
...
/// Transform a model for VAC API routing
///
/// Changes the model's provider to "vac" and prefixes the model ID
/// with the original provider name for routing purposes.
pub fn transform_for_vac(model: Model) -> Model {
    Model {
