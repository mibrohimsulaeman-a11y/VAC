...
mod local_storage_tests {
    ...
    async fn test_async_no_trimming_under_threshold() {
        ...
        for msg in &result {
            if let vac_foundation::models::llm::LLMMessageContent::String(s) = &msg.content {
    ...
    async fn test_async_trimming_triggers_at_threshold() {
        ...
        match &result[0].content {
            vac_foundation::models::llm::LLMMessageContent::String(s) => {
        ...
        for (i, msg) in result.iter().enumerate() {
            if i < trimmed_idx
                && msg.role == "assistant"
                && let vac_foundation::models::llm::LLMMessageContent::String(s) = &msg.content
            {
                assert_eq!(
                    s, "[trimmed]",
    ...
    async fn test_async_trimming_state_persists_across_session_resume() {
        ...
        for (i, msg) in result1.iter().enumerate() {
            let is_trimmed = match &msg.content {
                vac_foundation::models::llm::LLMMessageContent::String(s) => s == "[trimmed]",
                vac_foundation::models::llm::LLMMessageContent::List(parts) => {
                    parts.iter().all(|p| match p {
                        vac_foundation::models::llm::LLMMessageTypedContent::Text { text } => {
                            text == "[trimmed]"
                        }
                        vac_foundation::models::llm::LLMMessageTypedContent::ToolResult {
                            content,
                            ..
                        } => content == "[trimmed]",
                        _ => true,
        ...
        // === Phase 2: Resume session (simulate `vac -s <session_id>`) ===
    ...
    async fn test_connection_applies_busy_timeout() {
        ...
        let timeout = vac_foundation::sqlite::read_busy_timeout_millis(&conn)
            .await
            .expect("read_busy_timeout_millis failed");

        assert_eq!(
            timeout,
            vac_foundation::sqlite::BUSY_TIMEOUT.as_millis() as i64,
