...
//! - VAC-first routing when API key is present
//! - Local fallback when VAC is unavailable
...
use tokio::sync::mpsc;
use uuid::Uuid;
use vac_foundation::hooks::{HookContext, LifecycleEvent};
...
impl AgentProvider for AgentClient {
    ...
    async fn get_my_account(&self) -> Result<GetMyAccountResponse, String> {
        if let Some(api) = &self.vac_remote_service {
            ...
            Ok(GetMyAccountResponse {
                ...
                email: "local@vastar.ai".to_string(),
                scope: None,
            })
        }
    }
    ...
    ) -> Result<vac_foundation::models::billing::BillingResponse, String> {
        if let Some(api) = &self.vac_remote_service {
            api.get_billing(account_username).await
        } else {
            Err("Billing info not available without VAC API key".to_string())
        }
    }
    ...
    async fn list_rulebooks(&self) -> Result<Vec<ListRuleBook>, String> {
        if let Some(api) = &self.vac_remote_service {
            ...
            let client = vac_foundation::tls_client::create_tls_client(
                vac_foundation::tls_client::TlsClientConfig::default()
                    .with_timeout(std::time::Duration::from_secs(30)),
            )?;

            let url = format!("{}/v1/rules", self.get_vac_remote_service_endpoint());
    ...
    async fn get_rulebook_by_uri(&self, uri: &str) -> Result<RuleBook, String> {
        if let Some(api) = &self.vac_remote_service {
            ...
            let url = format!(
                "{}/v1/rules/{}",
                self.get_vac_remote_service_endpoint(),
                encoded_uri
            );
            let response = client.get(&url).send().await.map_err(|e| e.to_string())?;
    ...
    async fn create_rulebook(
        ...
        if let Some(api) = &self.vac_remote_service {
            ...
            Err("Creating rulebooks requires VAC API key".to_string())
        }
    }
    ...
    async fn delete_rulebook(&self, uri: &str) -> Result<(), String> {
        if let Some(api) = &self.vac_remote_service {
            ...
            Err("Deleting rulebooks requires VAC API key".to_string())
        }
    }
    ...
    async fn search_docs(&self, input: &SearchDocsRequest) -> Result<Vec<Content>, String> {
        if let Some(api) = &self.vac_remote_service {
            api.search_docs(&crate::vac::SearchDocsRequest {
            ...
            use vac_foundation::models::integrations::search_service::*;
    ...
    async fn search_memory(&self, input: &SearchMemoryRequest) -> Result<Vec<Content>, String> {
        if let Some(api) = &self.vac_remote_service {
            api.search_memory(&crate::vac::SearchMemoryRequest {
                keywords: input.keywords.clone(),
    ...
    async fn slack_read_messages(
        ...
        if let Some(api) = &self.vac_remote_service {
            api.slack_read_messages(&crate::vac::SlackReadMessagesRequest {
                channel: input.channel.clone(),
                limit: input.limit,
            })
            .await
        } else {
            Err("Slack integration requires VAC API key".to_string())
        }
    }
    ...
    async fn slack_read_replies(
        ...
        if let Some(api) = &self.vac_remote_service {
            api.slack_read_replies(&crate::vac::SlackReadRepliesRequest {
                channel: input.channel.clone(),
                ts: input.ts.clone(),
            })
            .await
    ...
    async fn slack_send_message(
        ...
        if let Some(api) = &self.vac_remote_service {
            api.slack_send_message(&crate::vac::SlackSendMessageRequest {
                channel: input.channel.clone(),
    ...
    async fn list_models(&self) -> Vec<vac_provider_core::Model> {
        ...
        let registry = self.provider_core.registry();
...
fn sort_models_by_recency(models: &mut [vac_provider_core::Model]) {
    models.sort_by(|a, b| {
...
impl AgentClient {
    ...
    pub(crate) async fn run_agent_completion(
        ...
        let (response_message, usage) = if let Some(tx) = stream_channel_tx {
            ...
            let provider_core = self.provider_core.clone();
    ...
    async fn generate_session_title(&self, messages: &[ChatMessage]) -> Result<String, String> {
        ...
        let use_vac = self.vac.is_some();
        let providers = self.provider_core.registry().list_providers();
        let cheap_models: &[(&str, &str)] = &[
            ("vac", "claude-haiku-4-5"),
        ...
        let model = cheap_models
            ...
            .find_map(|(provider, model_id)| {
                if providers.contains(&provider.to_string()) {
                    crate::find_model(model_id, use_vac)
                } else {
                    None
                }
            })
