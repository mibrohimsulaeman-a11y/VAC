...
use super::{
    ...
    UpdateSessionRequest, UpdateSessionResponse, VACApiConfig, knowledge::AccountCacheState,
    models::*,
};
use crate::models::{
...
use uuid::Uuid;
use vac_foundation::models::billing::BillingResponse;
use vac_foundation::tls_client::{TlsClientConfig, create_tls_client};

/// Client for VAC's non-inference APIs
#[derive(Clone, Debug)]
pub struct VACApiClient {
...
impl VACApiClient {
    /// Create a new VACApiClient
    pub fn new(config: &VACApiConfig) -> Result<Self, String> {
        if config.api_key.is_empty() {
            return Err("VAC API key is required".to_string());
        }
        ...
        headers.insert(
            header::USER_AGENT,
            header::HeaderValue::from_str(&format!("VAC/{}", env!("CARGO_PKG_VERSION")))
                .map_err(|e| e.to_string())?,
        );
    ...
    async fn handle_response_error(&self, response: Response) -> Result<Response, String> {
        ...
        if let Ok(api_error) = serde_json::from_str::<ApiError>(&error_body) {
            ...
            if api_error.error.key == "EXCEEDED_API_LIMIT" {
                return Err(format!(
                    "{}. You can top up your billing at https://vastar.ai/settings/billing",
