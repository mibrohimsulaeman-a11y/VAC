---
description: Deploy & monitor OpenClaw gateway with VAC Autopilot
---

# Deploy & Monitor OpenClaw Gateway with AWS Lightsail Blueprint + VAC Autopilot
...
* Configure VAC Autopilot for continuous health monitoring with Telegram or Discord alerts
...
* Never share a Telegram bot token between OpenClaw and VAC — causes 409 Conflict polling errors (only applies if both use Telegram)
...
| VAC alert token | Always (Phase 5) | See Phase 5 — asked later | Varies |
...
Only needed if `$MESSAGING_CHANNEL` = `telegram` or if Telegram is chosen for VAC alerts (Phase 5).
...
> **⚠ If using Telegram for BOTH OpenClaw and VAC alerts**: Create TWO separate bots. Sharing a token causes 409 Conflict.

### VAC Autopilot LLM Provider

> **⚠ CRITICAL**: Before starting autopilot, verify that the VAC CLI has a working LLM provider.
> Run `vac auth list` to check. If empty, run `vac auth login` to configure one.
...
### Phase 5: VAC Autopilot Monitoring

VAC Autopilot runs on your **LOCAL machine** and monitors the Lightsail instance via SSH.

#### 5.0 Install & Configure VAC CLI
...
```bash
vac auth list   # If empty:
vac auth login  # Select provider (Anthropic, OpenAI, DeepSeek, etc.), enter API key
```
...
> **ASK THE USER** (use `ask_user` tool) which channel for VAC autopilot alerts.
>
...
>
> **If Telegram** — ask two questions:
> 1. "Paste your VAC alert Telegram bot token" (must be a DIFFERENT bot than `$OPENCLAW_BOT_TOKEN` if OpenClaw uses Telegram)
...
>
> Store as `$VAC_BOT_TOKEN` and `$VAC_CHAT_ID`.
>
> **If Discord** — ask one question:
> 1. "Paste your Discord bot token for VAC alerts"
>
> Store as `$VAC_DISCORD_TOKEN`.
>
> **If Slack** — ask two questions:
> 1. "Paste your Slack bot token for VAC alerts" (format: `xoxb-...`)
> 2. "Paste your Slack app-level token" (format: `xapp-...`)
>
> Store as `$VAC_SLACK_BOT_TOKEN` and `$VAC_SLACK_APP_TOKEN`.
...
```bash
vac autopilot channel add telegram --token $VAC_BOT_TOKEN --target $VAC_CHAT_ID
```

> **⚠ CRITICAL**: The `--target $VAC_CHAT_ID` flag is REQUIRED. Without it, autopilot runs silently (`missing gateway notifications config`). If you forgot `--target`, remove and re-add:
> ```bash
> vac autopilot channel remove telegram
> vac autopilot channel add telegram --token $VAC_BOT_TOKEN --target $VAC_CHAT_ID
> ```
...
```bash
vac autopilot channel add discord --token $VAC_DISCORD_TOKEN
```

**If `$ALERT_CHANNEL` = `slack`:**

```bash
vac autopilot channel add slack --bot-token $VAC_SLACK_BOT_TOKEN --app-token $VAC_SLACK_APP_TOKEN
```

##### Verify Channel

```bash
vac autopilot channel test
vac autopilot channel list
```
...
**Step 2**: Create local SSH wrappers in `~/.vac/checks/`:
...
for NAME in health service channels models auth-cooldown resources cron-status heartbeat queue workspace-disk orphaned-sandbox compaction presence memory-search sandbox-image security-audit version; do
  cat > ~/.vac/checks/openclaw-${NAME}.sh << EOF
#!/bin/bash
ssh -i $SSH_KEY -o StrictHostKeyChecking=no -o ConnectTimeout=10 \
  $SSH_USER@$PUBLIC_IP '/opt/openclaw/checks/${NAME}.sh'
EOF
  chmod +x ~/.vac/checks/openclaw-${NAME}.sh
done
```
...
**Example:**
```bash
vac autopilot schedule add openclaw-health \
  --cron '*/5 * * * *' \
  --check ~/.vac/checks/openclaw-health.sh \
...
# Dry-run a check:
vac autopilot schedule trigger openclaw-health --dry-run
```
...
| 6 | Telegram 409 Conflict | Two processes polling same bot | Separate bots for OpenClaw and VAC (only if both use Telegram) |
| 7 | `Provider not found` in autopilot | No LLM provider configured | Run `vac auth login` |
...
* [ ] VAC autopilot running (Quick start: 4 schedules minimum)
...
| VAC Autopilot Docs | docs/workflow-control-plane/VAC_V1_5_ARCHITECTURE.md |
