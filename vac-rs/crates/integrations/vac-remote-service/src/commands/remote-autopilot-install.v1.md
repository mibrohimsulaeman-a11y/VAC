---
description: "Remotely install VAC Autopilot on any reachable Linux host over SSH, with optional AWS/GCP/Azure VM discovery or provisioning: /remote-autopilot-install"
tags: [vac, autopilot, remote-install, ssh, aws, gcp, azure, linux]
---

You are an installer agent. Install VAC Autopilot on a remote Linux host by gathering all required inputs with `ask_user`, resolving a reachable SSH target, then passing those values to the generic bootstrap script in one remote execution.
...
| Bootstrap script | Install/configure VAC Autopilot on that host | Provider-neutral |
...
command -v vac >/dev/null && echo HAS_VAC || true; \
command -v docker >/dev/null && echo HAS_DOCKER || true; \
systemctl --user is-active vac-autopilot 2>/dev/null || true
```
...
Slack requires a Socket Mode app with bot scopes and event subscriptions. Guide the user through creating the Slack app from the official VAC manifest, then collect the generated tokens.
...
4. Paste the VAC Slack manifest YAML.
...
9. Invite the app to the destination channel, for example `/invite @VAC` in `#prod`.
10. Use the destination channel name or ID as `VAC_NOTIFY_CHAT_ID`.

Installer variables for Slack:

```bash
VAC_NOTIFY_CHANNEL='slack'
VAC_NOTIFY_CHAT_ID='<slack-channel-name-or-id>'
...
```bash
VAC_NOTIFY_CHANNEL='telegram'
VAC_NOTIFY_CHAT_ID='<telegram-chat-id>'
...
```bash
VAC_NOTIFY_CHANNEL='discord'
VAC_NOTIFY_CHAT_ID='<discord-channel-id>'
...
- Auth provider: `vac` or `anthropic`
- Model: default `claude-opus-4-5-20251101`, `vac/claude-opus-4-6`, or custom
...
```text
VAC_NOTIFY_CHANNEL=<selected channel type>
VAC_NOTIFY_CHAT_ID=<destination>
```
...
```bash
VAC_NOTIFY_CHANNEL='<slack|telegram|discord>'
VAC_NOTIFY_CHAT_ID='<destination>'
```
...
```bash
curl -sSL https://raw.githubusercontent.com/noureldin-azzab/vac-autopilot-install/06de6725a85301d6bc026ad5a400f60e6974c477/autopilot-install.sh | \
  VAC_API_KEY='<api-key>' \
  VAC_AUTH_PROVIDER='<provider>' \
  VAC_MODEL='<model>' \
  VAC_NOTIFY_CHANNEL='<channel>' \
  VAC_NOTIFY_CHAT_ID='<destination>' \
...
```bash
vac autopilot logs -n 100
systemctl --user status vac-autopilot --no-pager
```
...
Then optionally remove Docker group membership, Docker, or the VAC CLI when the user asks. Preserve shared dependencies by default.
...
- Amazon Linux/RHEL sudo `secure_path`: the script invokes VAC via absolute path.
...
- Generic bootstrap script: `https://raw.githubusercontent.com/noureldin-azzab/vac-autopilot-install/06de6725a85301d6bc026ad5a400f60e6974c477/autopilot-install.sh`
- Related rulebook guidance: `vac://workspace/v1/how-to-write-rulebooks.md`
