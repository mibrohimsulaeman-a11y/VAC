...
`vac-network-proxy` is VAC's local network policy enforcement proxy. It runs:
...
`vac-network-proxy` reads from VAC's merged `config.toml` (via `vac-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $VAC_HOME/proxy/ (ca.pem + ca.key).
...
# Use exact hosts or scoped wildcards like `*.vastar.com` or `**.vastar.com`.
...
`vac-network-proxy` can be embedded as a library with a thin API:

```rust
use vac_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `vac-network-proxy` is embedded in managed VAC runtime, policy decisions emit structured
OTEL-compatible events with `target=vac_otel.network_proxy`.

Event name:

- `vac.network_proxy.policy_decision`
...
This section documents the protections implemented by `vac-network-proxy`, and the boundaries of
