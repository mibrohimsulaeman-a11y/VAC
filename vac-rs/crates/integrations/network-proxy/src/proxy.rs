...
#[command(name = "vac-network-proxy", about = "VAC network sandbox proxy")]
pub struct Args {}
...
impl NetworkProxyBuilder {
    ...
    pub fn managed_by_vac(mut self, managed_by_vac: bool) -> Self {
        self.managed_by_vac = managed_by_vac;
        self
    }
    ...
    pub async fn build(self) -> Result<NetworkProxy> {
        ...
        let (requested_http_addr, requested_socks_addr, reserved_listeners) = if self.managed_by_vac
        {
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "VAC_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "VAC_NETWORK_ALLOW_LOCAL_BINDING";
...
#[cfg(target_os = "macos")]
pub const VAC_PROXY_GIT_SSH_COMMAND_MARKER: &str = "VAC_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const VAC_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "VAC_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const VAC_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn vac_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{VAC_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{VAC_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_vac_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(VAC_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(VAC_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected VAC fallback so it cannot point
        // at a stale proxy port after the proxy is restarted.
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_vac_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    vac_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_vac_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"VAC_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_vac_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            vac_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&vac_proxy_git_ssh_command(SocketAddr::new(
