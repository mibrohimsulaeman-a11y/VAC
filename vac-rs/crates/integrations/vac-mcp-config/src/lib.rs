...
use vac_foundation::paths::vac_home_dir;
...
/// 2. `.vac/mcp.{toml,json}` (current directory)
/// 3. `<vac_dir>/mcp.{toml,json}`
pub fn find_config_file() -> Result<String, String> {
    ...
    // Priority 2: .vac/mcp.{toml,json} in current directory
    let cwd_vac = PathBuf::from(".vac");

    let cwd_vac_toml = cwd_vac.join("mcp.toml");
    if cwd_vac_toml.exists() {
        return Ok(cwd_vac_toml.to_string_lossy().to_string());
    }

    let cwd_vac_json = cwd_vac.join("mcp.json");
    if cwd_vac_json.exists() {
        return Ok(cwd_vac_json.to_string_lossy().to_string());
    }

    let vac_dir = vac_home_dir();

    // Priority 3: vac dir (typically ~/.vac/)
    let home_toml = vac_dir.join("mcp.toml");
    if home_toml.exists() {
        return Ok(home_toml.to_string_lossy().to_string());
    }

    let home_json = vac_dir.join("mcp.json");
    if home_json.exists() {
...
pub fn add_server(
    ...
    if config.servers.contains_key(name) {
        return Err(format!(
            "Server '{name}' already exists. Use 'vac mcp remove {name}' first."
        ));
    }
...
mod tests {
    ...
    fn temp_test_path(filename: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("vac-mcp-config-{unique}-{filename}"))
    }
