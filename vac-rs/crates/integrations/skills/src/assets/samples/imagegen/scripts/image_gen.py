...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("VASTAR_API_KEY"):
        print("VASTAR_API_KEY is set.", file=sys.stderr)
        return
    if dry_run:
        _warn("VASTAR_API_KEY is not set; dry-run only.")
        return
    _die("VASTAR_API_KEY is not set. Export it before running.")
...
def _create_client():
    try:
        from vastar import Vastar
    except ImportError:
        _die(f"vastar SDK not installed in the active environment. {_dependency_hint('vastar')}")
    return Vastar()


def _create_async_client():
    try:
        from vastar import AsyncVastar
    except ImportError:
        try:
            import vastar as _vastar  # noqa: F401
        except ImportError:
            _die(
                f"vastar SDK not installed in the active environment. {_dependency_hint('vastar')}"
            )
        _die(
            "AsyncVastar not available in this vastar SDK version. "
            f"{_dependency_hint('vastar', upgrade=True)}"
        )
    return AsyncVastar()
...
def _extract_retry_after_seconds(exc: Exception) -> Optional[float]:
    # Best-effort: vastar SDK errors vary by version. Prefer a conservative fallback.
