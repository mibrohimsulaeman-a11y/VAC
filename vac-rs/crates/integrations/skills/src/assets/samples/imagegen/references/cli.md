...
Real API calls require **network access** + `VASTAR_API_KEY`. `--dry-run` does not.

## Quick start (works from any repo)
Set a stable path to the skill CLI (default `VAC_HOME` is `~/.vac`):

```
export VAC_HOME="${VAC_HOME:-$HOME/.vac}"
export IMAGE_GEN="$VAC_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
Dry-run (no API call; no network required; does not require the `vastar` package):
...
Generate (requires `VASTAR_API_KEY` + network):
...
- Network/sandbox notes for fallback CLI mode: `references/vac-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$VAC_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
