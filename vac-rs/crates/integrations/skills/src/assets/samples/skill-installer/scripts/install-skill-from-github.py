#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $VAC_HOME/skills."""
...
def _vac_home() -> str:
    return os.environ.get("VAC_HOME", os.path.expanduser("~/.vac"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "vac")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "vac-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_vac_home(), "skills")
