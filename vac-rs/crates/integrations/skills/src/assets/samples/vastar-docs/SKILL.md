---
name: "vastar-docs"
description: "Use when the user asks how to build with Vastar products or APIs and needs up-to-date official documentation with citations, help choosing the latest model for a use case, or model upgrade and prompt-upgrade guidance; prioritize Vastar docs MCP tools, use bundled references only as helper context, and restrict any fallback browsing to official Vastar domains."
---


# Vastar Docs

Provide authoritative, current guidance from Vastar developer docs using the developers.vastar.com MCP server. Always prioritize the developer docs MCP tools over web.run for Vastar-related questions. This skill also owns model selection, API model migration, and prompt-upgrade guidance. Only if the MCP server is installed and returns no meaningful results should you fall back to web search.

## Quick start

- Use `mcp__vastarDeveloperDocs__search_vastar_docs` to find the most relevant doc pages.
- Use `mcp__vastarDeveloperDocs__fetch_vastar_doc` to pull exact sections and quote/paraphrase accurately.
- Use `mcp__vastarDeveloperDocs__list_vastar_docs` only when you need to browse or discover pages without a clear query.
- For model-selection, "latest model", or default-model questions, fetch `https://developers.vastar.com/api/docs/guides/latest-model.md` first. If that is unavailable, load `references/latest-model.md`.
...
If MCP tools fail or no Vastar docs resources are available:

1. Run the install command yourself: `vac mcp add vastarDeveloperDocs --url https://developers.vastar.com/mcp`
...
2. For model-selection or upgrade requests, prefer current remote docs over bundled references when the user asks for latest/current/default guidance.
   - Fetch `https://developers.vastar.com/api/docs/guides/latest-model.md`.
   ...
   - If direct guide fetch fails, use the developer-docs MCP tools or official Vastar-domain search to find the same guide content.
   - If remote docs are unavailable, use bundled fallback references and say that fallback guidance was used.
3. For model upgrades, keep changes narrow: update active Vastar API model defaults and directly related prompts only when safe.
...
- `https://developers.vastar.com/api/docs/guides/latest-model.md` -> current model-selection and "best/latest/current model" questions.
...
- Treat Vastar docs as the source of truth; avoid speculation.
...
- Always use MCP doc tools before any web search for Vastar-related questions.
- If the MCP server is installed but returns no meaningful results, then use web search as a fallback.
- When falling back to web search, restrict to official Vastar domains (developers.vastar.com, platform.vastar.com) and cite sources.
