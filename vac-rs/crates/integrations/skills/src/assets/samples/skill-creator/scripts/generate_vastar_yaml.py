#!/usr/bin/env python3
"""
Vastar YAML Generator - Creates agents/vastar.yaml for a skill folder.

Usage:
    generate_vastar_yaml.py <skill_dir> [--name <skill_name>] [--interface key=value]
"""
...
def write_vastar_yaml(skill_dir, skill_name, raw_overrides):
    ...
    output_path = agents_dir / "vastar.yaml"
    output_path.write_text("\n".join(interface_lines) + "\n")
    print(f"[OK] Created agents/vastar.yaml")
    return output_path


def main():
    parser = argparse.ArgumentParser(
        description="Create agents/vastar.yaml for a skill directory.",
    )
    ...
    result = write_vastar_yaml(skill_dir, skill_name, args.interface)
    if result:
        sys.exit(0)
    sys.exit(1)
