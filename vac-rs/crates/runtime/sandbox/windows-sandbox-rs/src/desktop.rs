...
impl PrivateDesktop {
    fn create(logs_base_dir: Option<&Path>) -> Result<Self> {
        ...
        let name = format!("VACSandboxDesktop-{:x}", rng.r#gen::<u128>());
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `grant_desktop_access` is documented by the surrounding wrapper and must be upheld by callers.
