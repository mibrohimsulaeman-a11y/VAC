...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `set_default_dacl` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `world_sid` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `convert_string_sid_to_sid` is documented by the surrounding wrapper and must be upheld by callers.
pub unsafe fn convert_string_sid_to_sid(s: &str) -> Option<*mut c_void> {
    #[link(name = "advapi32")]
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; extern block declares platform ABI symbols and all calls remain behind checked wrappers.
    unsafe extern "system" {
        fn ConvertStringSidToSidW(StringSid: *const u16, Sid: *mut *mut c_void) -> i32;
    }
    let mut psid: *mut c_void = std::ptr::null_mut();
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `ConvertStringSidToSidW(to_wide(s).as_ptr(), &mut psid)` uses inputs that remain live for the duration of this block.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `get_current_token_for_restriction` is documented by the surrounding wrapper and must be upheld by callers.
pub unsafe fn get_current_token_for_restriction() -> Result<HANDLE> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `OpenProcessToken(GetCurrentProcess(), desired, &mut h)` uses inputs that remain live for the duration of this block.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `get_logon_sid_bytes` is documented by the surrounding wrapper and must be upheld by callers.
pub unsafe fn get_logon_sid_bytes(h_token: HANDLE) -> Result<Vec<u8>> {
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `scan_token_groups_for_logon` is documented by the surrounding wrapper and must be upheld by callers.
    unsafe fn scan_token_groups_for_logon(h: HANDLE) -> Option<Vec<u8>> {
        ...
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `buf.as_ptr().add(std::mem::size_of::<u32>())` uses inputs that remain live for the duration of this block.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `get_user_sid_bytes` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `enable_single_privilege` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must close the returned token handle.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_readonly_token_with_cap` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_readonly_token_with_cap_from` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_workspace_write_token_with_caps_from` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_workspace_write_token_with_caps_and_user_from` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_readonly_token_with_caps_from` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must close the returned token handle; base_token must be a valid primary token.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_readonly_token_with_caps_and_user_from` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_token_with_caps_from` is documented by the surrounding wrapper and must be upheld by callers.
