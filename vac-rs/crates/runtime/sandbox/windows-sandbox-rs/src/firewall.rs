...
const OFFLINE_BLOCK_RULE_NAME: &str = "vac_sandbox_offline_block_outbound";
const OFFLINE_BLOCK_LOOPBACK_TCP_RULE_NAME: &str = "vac_sandbox_offline_block_loopback_tcp";
const OFFLINE_BLOCK_LOOPBACK_UDP_RULE_NAME: &str = "vac_sandbox_offline_block_loopback_udp";

// Friendly text shown in the firewall UI.
const OFFLINE_BLOCK_RULE_FRIENDLY: &str = "VAC Sandbox Offline - Block Non-Loopback Outbound";
const OFFLINE_BLOCK_LOOPBACK_TCP_RULE_FRIENDLY: &str =
    "VAC Sandbox Offline - Block Loopback TCP (Except Proxy)";
const OFFLINE_BLOCK_LOOPBACK_UDP_RULE_FRIENDLY: &str = "VAC Sandbox Offline - Block Loopback UDP";
const OFFLINE_PROXY_ALLOW_RULE_NAME: &str = "vac_sandbox_offline_allow_loopback_proxy";
...
pub fn ensure_offline_proxy_allowlist(
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `CoInitializeEx(None, COINIT_APARTMENTTHREADED)` uses inputs that remain live for the duration of this block.
...
fn remove_rule_if_present(rules: &INetFwRules, internal_name: &str, log: &mut File) -> Result<()> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `rules.Item(&name)` uses inputs that remain live for the duration of this block.
    if unsafe { rules.Item(&name) }.is_ok() {
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `rules.Remove(&name)` uses inputs that remain live for the duration of this block.
    ...
    let rule: INetFwRule3 = match unsafe { rules.Item(&name) } {
        ...
        Err(_) => {
            let new_rule: INetFwRule3 =
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `CoCreateInstance(&NetFwRule, None, CLSCTX_INPROC_SERVER)` uses inputs that remain live for the duration of this block.
            ...
            // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `new_rule.SetName(&name)` uses inputs that remain live for the duration of this block.
            ...
            // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `rules.Add(&new_rule)` uses inputs that remain live for the duration of this block.
...
fn configure_rule(rule: &INetFwRule3, spec: &BlockRuleSpec<'_>) -> Result<()> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `rule.LocalUserAuthorizedList()` uses inputs that remain live for the duration of this block.
