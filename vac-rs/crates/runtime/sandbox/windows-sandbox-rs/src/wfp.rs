...
const SESSION_NAME: &str = "VAC Windows Sandbox WFP";
const PROVIDER_NAME: &str = "VAC Windows Sandbox WFP";
const PROVIDER_DESCRIPTION: &str = "Persistent WFP provider for VAC Windows sandbox filters";
const SUBLAYER_NAME: &str = "VAC Windows Sandbox WFP";
const SUBLAYER_DESCRIPTION: &str = "Persistent WFP sublayer for VAC Windows sandbox filters";

// WFP identifies persistent providers, sublayers, and filters by stable GUIDs.
// These values are VAC-owned identities; do not regenerate them unless we
...
/// Installs the persistent VAC WFP filters for `account`.
///
...
impl Engine {
    fn open() -> Result<Self> {
        ...
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `zeroed()` uses inputs that remain live for the duration of this block.
    ...
    fn begin_transaction(&self) -> Result<Transaction<'_>> {
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `FwpmTransactionBegin0(self.handle, 0)` uses inputs that remain live for the duration of this block.
...
impl Transaction<'_> {
    fn commit(&mut self) -> Result<()> {
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `FwpmTransactionCommit0(self.engine.handle)` uses inputs that remain live for the duration of this block.
...
/// Ensures the persistent VAC WFP provider exists.
fn ensure_provider(engine: HANDLE) -> Result<()> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `FwpmProviderAdd0(engine, &provider, null_mut())` uses inputs that remain live for the duration of this block.
...
/// Ensures the persistent VAC sublayer exists under the VAC provider.
fn ensure_sublayer(engine: HANDLE) -> Result<()> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `FwpmSubLayerAdd0(engine, &sublayer, null_mut())` uses inputs that remain live for the duration of this block.
...
fn add_filter(engine: HANDLE, spec: &FilterSpec, user_condition: &UserMatchCondition) -> Result<()> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `FwpmFilterAdd0(engine, &filter, null_mut(), &mut filter_id)` uses inputs that remain live for the duration of this block.
...
fn delete_filter_if_present(engine: HANDLE, key: &GUID) -> Result<()> {
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `FwpmFilterDeleteByKey0(engine, key)` uses inputs that remain live for the duration of this block.
