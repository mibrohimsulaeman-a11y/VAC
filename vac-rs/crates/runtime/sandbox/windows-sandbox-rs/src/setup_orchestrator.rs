...
pub const OFFLINE_USERNAME: &str = "VACSandboxOffline";
pub const ONLINE_USERNAME: &str = "VACSandboxOnline";
...
pub fn sandbox_dir(vac_home: &Path) -> PathBuf {
    vac_home.join(".sandbox")
}

pub fn sandbox_bin_dir(vac_home: &Path) -> PathBuf {
    vac_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(vac_home: &Path) -> PathBuf {
    vac_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(vac_home: &Path) -> PathBuf {
    sandbox_dir(vac_home).join("setup_marker.json")
}

pub fn sandbox_users_path(vac_home: &Path) -> PathBuf {
    sandbox_secrets_dir(vac_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, vac_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        vac_home: request.vac_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.vac_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.vac_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(vac_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(vac_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(vac_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    gather_legacy_full_read_roots(command_cwd, policy, vac_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<vac_otel::StatsigMetricsSettings>,
    real_user: String,
    #[serde(default)]
    refresh_only: bool,
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "VAC_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    ...
    {
        let candidate = dir.join("vac-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `vac-resources/` next to `vac.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("vac-resources")
            .join("vac-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("vac-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(vac_home) {
...
fn run_setup_exe(payload: &ElevationPayload, needs_elevation: bool, vac_home: &Path) -> Result<()> {
    ...
    let cleared_report = match clear_setup_error_report(vac_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(vac_home) {
            log_note(
                &format!(
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `std::mem::zeroed()` uses inputs that remain live for the duration of this block.
    ...
    sei.nShow = 0; // SW_HIDE
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `ShellExecuteExW(&mut sei)` uses inputs that remain live for the duration of this block.
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.vac_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: vac_otel::global_statsig_metrics_settings(),
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.vac_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.vac_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.vac_home);
        ...
        gather_read_roots(request.command_cwd, request.policy, request.vac_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, vac_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to VAC_HOME or anything under VAC_HOME/.sandbox,
    // VAC_HOME/.sandbox-bin, or VAC_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let vac_home_key = canonical_path_key(vac_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(vac_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(vac_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(vac_home));
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "VAC_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_vac_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let vac_home = user_profile.join("VACHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&vac_home).expect("create vac home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &vac_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let vac_home = tmp.path().join("vac-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &vac_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&vac_home)).expect("canonical helper dir");
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_vac).expect("canonical extra .vac"),
                explicit_deny,
            ]
            .into_iter()
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &vac_home);
