...
fn sid_bytes_from_string(sid_str: &str) -> Result<Vec<u8>> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `CopySid(sid_len, out.as_mut_ptr() as *mut std::ffi::c_void, psid)` uses inputs that remain live for the duration of this block.
