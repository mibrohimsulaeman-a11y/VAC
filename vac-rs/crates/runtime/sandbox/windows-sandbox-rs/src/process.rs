...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `ensure_inheritable_stdio` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_process_as_user` is documented by the surrounding wrapper and must be upheld by callers.
