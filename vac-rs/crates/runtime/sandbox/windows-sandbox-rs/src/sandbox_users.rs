...
use vac_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "VACSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "VAC sandbox internal group (managed)";
...
fn sid_bytes_from_string(sid_str: &str) -> Result<Vec<u8>> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `ConvertStringSidToSidW(sid_w.as_ptr(), &mut psid)` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `GetLengthSid(psid)` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `CopySid(sid_len, out.as_mut_ptr() as *mut c_void, psid)` uses inputs that remain live for the duration of this block.
...
fn write_secrets(
    ...
    let sandbox_dir = sandbox_dir(vac_home);
    ...
    let secrets_dir = sandbox_secrets_dir(vac_home);
