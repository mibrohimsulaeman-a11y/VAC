...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "vac-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(vac_home: &Path) -> PathBuf {
    sandbox_bin_dir(vac_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, vac_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(
    ...
    let destination = helper_bin_dir(vac_home).join(file_name);
    match copy_from_source_if_needed(&source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(vac_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), vac_home.display());
    ...
    let destination = helper_destination_for_source(kind, vac_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(vac_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let vac_home = Path::new(r"C:\Users\example\.vac");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.vac\.sandbox-bin"),
            helper_bin_dir(vac_home)
        );
    }
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("vac.exe");
        let helper = resources_dir.join("vac-command-runner.exe");
        fs::write(&exe, b"vac").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            source_path_for_exe(&exe, /*file_name*/ "vac-command-runner.exe").expect("helper path");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("vac.exe");
        let sibling_helper = release_dir.join("vac-command-runner.exe");
        let resource_helper = resources_dir.join("vac-command-runner.exe");
        fs::write(&exe, b"vac").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "vac-command-runner-test-suffix.exe");
    }
}
