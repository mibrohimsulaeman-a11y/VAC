...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.vac`)
    /// without permanently affecting other workspaces.
    #[serde(default)]
    pub workspace_by_cwd: HashMap<String, String>,
}

pub fn cap_sid_file(vac_home: &Path) -> PathBuf {
    vac_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(vac_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(vac_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(vac_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(vac_home);
    let mut caps = load_or_create_cap_sids(vac_home)?;
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let vac_home = temp.path().join("vac-home");
        std::fs::create_dir_all(&vac_home).expect("create vac home");
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&vac_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&vac_home, alt_spelling.as_path()).expect("second sid");

        assert_eq!(first_sid, second_sid);

        let caps = load_or_create_cap_sids(&vac_home).expect("load caps");
