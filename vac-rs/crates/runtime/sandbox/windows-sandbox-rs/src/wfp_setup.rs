...
const WFP_SETUP_SERVICE_NAME: &str = "vac-windows-sandbox-setup";
const WFP_SETUP_SUCCESS_METRIC: &str = "vac.windows_sandbox.wfp_setup_success";
const WFP_SETUP_FAILURE_METRIC: &str = "vac.windows_sandbox.wfp_setup_failure";
...
fn build_wfp_metrics_provider(
    ...
    // The setup helper cannot call vac-core's OTEL builder because core
...
fn emit_wfp_setup_metric(
    ...
    let Some(provider) = build_wfp_metrics_provider(vac_home, otel)? else {
        return Ok(());
    };
    ...
    F: FnMut(&str),
{
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        emit_wfp_setup_metric(vac_home, otel, metric)
    }));
    match result {
        Ok(Ok(())) => {}
