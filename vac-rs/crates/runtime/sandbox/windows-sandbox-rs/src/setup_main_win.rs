...
use std::sync::mpsc;
use vac_otel::StatsigMetricsSettings;
use vac_windows_sandbox::LOG_FILE_NAME;
use vac_windows_sandbox::SETUP_VERSION;
use vac_windows_sandbox::SetupErrorCode;
use vac_windows_sandbox::SetupErrorReport;
use vac_windows_sandbox::SetupFailure;
use vac_windows_sandbox::add_deny_write_ace;
use vac_windows_sandbox::canonicalize_path;
use vac_windows_sandbox::convert_string_sid_to_sid;
use vac_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use vac_windows_sandbox::ensure_allow_write_aces;
use vac_windows_sandbox::extract_setup_failure;
use vac_windows_sandbox::hide_newly_created_users;
use vac_windows_sandbox::install_wfp_filters;
use vac_windows_sandbox::is_command_cwd_root;
use vac_windows_sandbox::load_or_create_cap_sids;
use vac_windows_sandbox::log_note;
use vac_windows_sandbox::path_mask_allows;
use vac_windows_sandbox::sandbox_bin_dir;
use vac_windows_sandbox::sandbox_dir;
use vac_windows_sandbox::sandbox_secrets_dir;
use vac_windows_sandbox::string_from_sid_bytes;
use vac_windows_sandbox::to_wide;
use vac_windows_sandbox::workspace_cap_sid_for_cwd;
use vac_windows_sandbox::write_setup_error_report;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(vac_home) = std::env::var("VAC_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&vac_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.vac_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.vac_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.vac_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.vac_home, &payload.command_cwd)?;
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; the multi-line unsafe block below uses inputs that remain live until the block returns.
        ...
        for root in grant_tasks {
            ...
            scope.spawn(move || {
                ...
                for sid_str in &sid_strings {
                    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(sid_str)` uses inputs that remain live for the duration of this block.
                ...
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `ensure_allow_write_aces(&root, &psids)` uses inputs that remain live for the duration of this block.
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.vac`, and `.agents`.
        //
        ...
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `add_deny_write_ace(path, deny_psid)` uses inputs that remain live for the duration of this block.
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.vac_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.vac_home).display()
            ),
        ))
    })?;
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.vac_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.vac_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.vac_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.vac_home).join("sandbox_users.json");
...
mod tests {
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "VACSandboxOffline",
            "online_username": "VACSandboxOnline",
            "vac_home": "C:\\vac-home",
