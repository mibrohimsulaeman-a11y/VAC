...
use crate::sandbox_utils::ensure_vac_home_exists;
...
use crate::workspace_acl::protect_workspace_vac_dir;
use anyhow::Result;
...
impl LocalSid {
    pub fn from_string(sid: &str) -> Result<Self> {
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(sid)` uses inputs that remain live for the duration of this block.
    ...
    let (h_token, psid_generic, psid_workspace, cap_sid_str) = unsafe {
        match policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                ...
                let workspace_sid = workspace_cap_sid_for_cwd(vac_home, cwd)?;
    ...
    unsafe {
        ...
        if let Some(psid_workspace) = psid_workspace {
            ...
            if persist_aces && matches!(policy, SandboxPolicy::WorkspaceWrite { .. }) {
                let _ = protect_workspace_vac_dir(current_dir, psid_workspace.as_ptr());
...
pub(crate) fn prepare_elevated_spawn_context(
    ...
    let (psid_to_use, cap_sids) = match &common.policy {
        ...
        SandboxPolicy::WorkspaceWrite { .. } => {
            let cap_sid = workspace_cap_sid_for_cwd(vac_home, cwd)?;
            (
