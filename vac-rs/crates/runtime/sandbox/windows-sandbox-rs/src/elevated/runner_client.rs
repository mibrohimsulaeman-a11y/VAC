...
fn try_take_completed_connect_result(
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `WaitForSingleObject(thread_handle, 0)` uses inputs that remain live for the duration of this block.
...
fn connect_pipe_with_timeout(
    ...
    let mut connect_thread = Some(
        thread::Builder::new()
            .name(format!("vac-runner-connect-{pipe_label}"))
            .spawn(move || {
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `GetCurrentProcess()` uses inputs that remain live for the duration of this block.
    ...
    let result = match connect_result_rx.recv_timeout(RUNNER_PIPE_CONNECT_TIMEOUT) {
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            if let Some(result) = try_take_completed_connect_result(
                ...
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `CancelSynchronousIo(thread_handle)` uses inputs that remain live for the duration of this block.
...
pub(crate) fn spawn_runner_transport(
    ...
    let runner_exe = find_runner_exe(vac_home, log_dir);
    let runner_cmdline = runner_exe
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "vac-command-runner.exe".to_string());
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `SetErrorMode(RUNNER_ERROR_MODE_FLAGS)` uses inputs that remain live for the duration of this block.
    ...
    if let Err(err) = connect_result {
        ...
        unsafe {
            ...
            // that child instead of leaking a stray `vac-command-runner.exe`.
            if pi.hProcess != 0 {
    ...
    let mut transport = RunnerTransport {
        ...
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `File::from_raw_handle(h_pipe_in as _)` uses inputs that remain live for the duration of this block.
        pipe_write: unsafe { File::from_raw_handle(h_pipe_in as _) },
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `File::from_raw_handle(h_pipe_out as _)` uses inputs that remain live for the duration of this block.
    ...
    let startup_result = (|| -> Result<()> {
        ...
        // runner instead of leaving a stray `vac-command-runner.exe` behind.
