...
pub fn find_runner_exe(vac_home: &Path, log_dir: Option<&Path>) -> PathBuf {
    resolve_helper_for_launch(HelperExecutable::CommandRunner, vac_home, log_dir)
}
...
pub fn pipe_pair() -> (String, String) {
    ...
    let base = format!(r"\\.\pipe\vac-runner-{nonce:x}");
...
pub fn connect_pipe(h: HANDLE, expected_runner_pid: u32) -> io::Result<()> {
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `ConnectNamedPipe(h, ptr::null_mut())` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `GetNamedPipeClientProcessId(h, &mut client_pid)` uses inputs that remain live for the duration of this block.
