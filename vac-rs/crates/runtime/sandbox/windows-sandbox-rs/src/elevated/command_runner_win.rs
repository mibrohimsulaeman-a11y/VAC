...
use vac_windows_sandbox::ErrorPayload;
use vac_windows_sandbox::ExitPayload;
use vac_windows_sandbox::FramedMessage;
use vac_windows_sandbox::LaunchDesktop;
use vac_windows_sandbox::LocalSid;
use vac_windows_sandbox::Message;
use vac_windows_sandbox::OutputPayload;
use vac_windows_sandbox::OutputStream;
use vac_windows_sandbox::PipeSpawnHandles;
use vac_windows_sandbox::ResizePayload;
use vac_windows_sandbox::SandboxPolicy;
use vac_windows_sandbox::SpawnReady;
use vac_windows_sandbox::SpawnRequest;
use vac_windows_sandbox::StderrMode;
use vac_windows_sandbox::StdinMode;
use vac_windows_sandbox::allow_null_device;
use vac_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use vac_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use vac_windows_sandbox::decode_bytes;
use vac_windows_sandbox::encode_bytes;
use vac_windows_sandbox::get_current_token_for_restriction;
use vac_windows_sandbox::hide_current_user_profile_dir;
use vac_windows_sandbox::log_note;
use vac_windows_sandbox::parse_policy;
use vac_windows_sandbox::read_frame;
use vac_windows_sandbox::read_handle_loop;
use vac_windows_sandbox::spawn_process_with_pipes;
use vac_windows_sandbox::to_wide;
use vac_windows_sandbox::write_frame;
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `create_job_kill_on_close` is documented by the surrounding wrapper and must be upheld by callers.
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.vac_home.clone();
    hide_current_user_profile_dir(req.vac_home.as_path());
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `get_current_token_for_restriction()?` uses inputs that remain live for the duration of this block.
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, conpty) = vac_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
...
pub fn main() -> Result<()> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `File::from_raw_handle(h_pipe_in.into_raw() as _)` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `create_job_kill_on_close().ok()` uses inputs that remain live for the duration of this block.
    ...
    let msg = FramedMessage {
        ...
        message: Message::SpawnReady {
            payload: SpawnReady {
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `GetProcessId(pi.hProcess)` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `WaitForSingleObject(pi.hProcess, timeout)` uses inputs that remain live for the duration of this block.
