...
mod windows_impl {
    ...
    /// Creates the sandbox user's VAC home directory if it does not already exist.
    fn ensure_vac_home_exists(p: &Path) -> Result<()> {
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = vac_home.join(".sandbox");
        ensure_vac_home_exists(&sandbox_base)?;
        ...
        let (psid_to_use, cap_sids) = match &policy {
            SandboxPolicy::ReadOnly { .. } => {
                ...
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(&caps.readonly).unwrap()` uses inputs that remain live for the duration of this block.
                let psid = unsafe { convert_string_sid_to_sid(&caps.readonly).unwrap() };
                (psid, vec![caps.readonly])
            }
            SandboxPolicy::WorkspaceWrite { .. } => {
                #[allow(clippy::unwrap_used)]
                // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(&caps.workspace).unwrap()` uses inputs that remain live for the duration of this block.
                let psid = unsafe { convert_string_sid_to_sid(&caps.workspace).unwrap() };
                (
                    psid,
                    vec![
                        caps.workspace,
                        crate::cap::workspace_cap_sid_for_cwd(vac_home, cwd)?,
                    ],
                )
            }
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        (|| -> Result<CaptureResult> {
            let spawn_request = SpawnRequest {
                ...
                vac_home: sandbox_base.clone(),
                real_vac_home: vac_home.to_path_buf(),
                cap_sids,
                timeout_ms,
                tty: false,
                stdin_open: false,
