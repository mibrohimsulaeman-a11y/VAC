...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `path_has_world_write_allow` is documented by the surrounding wrapper and must be upheld by callers.
...
pub fn audit_everyone_writable(
    ...
    let check_world_writable = |path: &Path| -> bool {
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `path_has_world_write_allow(path)` uses inputs that remain live for the duration of this block.
...
pub fn apply_capability_denies_for_world_writable(
    ...
    let cap_path = cap_sid_file(vac_home);
    ...
    let (active_sid, workspace_roots): (*mut c_void, Vec<PathBuf>) = match sandbox_policy {
        ...
        SandboxPolicy::ReadOnly { .. } => (
            // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(&caps.readonly)` uses inputs that remain live for the duration of this block.
    ...
    for path in flagged {
        ...
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `add_deny_write_ace(path, active_sid)` uses inputs that remain live for the duration of this block.
