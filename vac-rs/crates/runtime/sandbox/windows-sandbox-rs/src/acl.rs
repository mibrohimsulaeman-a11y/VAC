...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `fetch_dacl_handle` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `dacl_mask_allows` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `dacl_has_write_allow_for_sid` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `dacl_has_write_deny_for_sid` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `ensure_allow_mask_aces_with_inheritance_impl` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `ensure_allow_mask_aces_with_inheritance` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must pass valid SID pointers and an existing path; free the returned security descriptor with `LocalFree`.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `ensure_allow_mask_aces` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must pass valid SID pointers and an existing path; free the returned security descriptor with `LocalFree`.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `ensure_allow_write_aces` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `add_allow_ace` is documented by the surrounding wrapper and must be upheld by callers.
...
/// # Safety
/// Caller must ensure `psid` points to a valid SID and `path` refers to an existing file or directory.
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `add_deny_write_ace` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `revoke_ace` is documented by the surrounding wrapper and must be upheld by callers.
...
// SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `allow_null_device` is documented by the surrounding wrapper and must be upheld by callers.
