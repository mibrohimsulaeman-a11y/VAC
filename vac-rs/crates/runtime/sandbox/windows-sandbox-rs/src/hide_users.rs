...
fn hide_directory(path: &Path) -> anyhow::Result<bool> {
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `GetFileAttributesW(wide.as_ptr())` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `SetFileAttributesW(wide.as_ptr(), new_attrs)` uses inputs that remain live for the duration of this block.
