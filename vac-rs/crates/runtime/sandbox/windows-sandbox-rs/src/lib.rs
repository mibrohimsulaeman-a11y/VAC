...
mod windows_impl {
    ...
    use super::sandbox_utils::ensure_vac_home_exists;
    ...
    // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; unsafe function contract for `setup_stdio_pipes` is documented by the surrounding wrapper and must be upheld by callers.
    ...
    pub fn run_windows_sandbox_capture_with_extra_deny_write_paths(
        ...
        let caps = load_or_create_cap_sids(vac_home)?;
        ...
        let (h_token, psid_generic, psid_workspace): (HANDLE, *mut c_void, Option<*mut c_void>) = unsafe {
            match &policy {
                ...
                SandboxPolicy::WorkspaceWrite { .. } => {
                    ...
                    let ws_sid = workspace_cap_sid_for_cwd(vac_home, cwd)?;
                    #[allow(clippy::expect_used)]
                    let psid_workspace =
        ...
        // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `setup_stdio_pipes()?` uses inputs that remain live for the duration of this block.
    ...
    pub fn run_windows_sandbox_legacy_preflight(
        ...
        ensure_vac_home_exists(vac_home)?;
        let caps = load_or_create_cap_sids(vac_home)?;
        #[allow(clippy::expect_used)]
        let psid_generic =
            // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(&caps.workspace)` uses inputs that remain live for the duration of this block.
            unsafe { convert_string_sid_to_sid(&caps.workspace) }.expect("valid workspace SID");
        let ws_sid = workspace_cap_sid_for_cwd(vac_home, cwd)?;
        #[allow(clippy::expect_used)]
        let psid_workspace =
            // SAFETY: Win32 sandbox boundary: adjacent checked API calls create or validate the raw handle/SID/ACL/token/pipe pointer used here; `convert_string_sid_to_sid(&ws_sid)` uses inputs that remain live for the duration of this block.
