...
# Run a suite of smoke tests against the Windows sandbox via the VAC CLI
...
def _resolve_vac_cmd() -> List[str]:
    """Resolve the VAC CLI to invoke `vac sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run VAC.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "vac.exe",
        ws_root / "target" / "release" / "vac.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "vac.exe",
            cargo_base / "release" / "vac.exe",
        ])
    ...
    raise FileNotFoundError(
        "VAC CLI not found. Build it first, e.g.\n"
        "  cargo build -p vac-surface-cli --release\n"
        "or for debug:\n"
        "  cargo build -p vac-surface-cli\n"
    )

VAC_CMD = _resolve_vac_cmd()
print(VAC_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to vac CLI overrides.
    ...
    argv = [*VAC_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"VAC_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\vac_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.vac artifacts) denied
    vac_home = Path(os.environ["USERPROFILE"]) / ".vac"
    cap_sid_target = vac_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .vac\\policy.json"], WS_ROOT)
    add("WS: .vac cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .vac policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > vac_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://vac-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
