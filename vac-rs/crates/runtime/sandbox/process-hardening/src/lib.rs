...
pub(crate) fn pre_main_hardening_linux() {
    ...
    // SAFETY: process hardening boundary: constants and checked pid/fd values are prepared immediately before the kernel call; `libc::prctl(libc::PR_SET_DUMPABLE, 0, 0, 0, 0)` uses inputs that remain live for the duration of this block.
    ...
    // Official VAC releases are MUSL-linked, which means that variables such
...
pub(crate) fn pre_main_hardening_macos() {
    ...
    // SAFETY: process hardening boundary: constants and checked pid/fd values are prepared immediately before the kernel call; `libc::ptrace(libc::PT_DENY_ATTACH, 0, std::ptr::null_mut(), 0)` uses inputs that remain live for the duration of this block.
    ...
    // VAC or inherited child processes do not get sprayed into the TUI:
    // https://github.com/vastar/vac/issues/11555
...
fn set_core_file_size_limit_to_zero() {
    ...
    // SAFETY: process hardening boundary: constants and checked pid/fd values are prepared immediately before the kernel call; `libc::setrlimit(libc::RLIMIT_CORE, &rlim)` uses inputs that remain live for the duration of this block.
...
pub(crate) fn pre_main_hardening_windows() {
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(mbolin): Perform the appropriate configuration for Windows.
}

#[cfg(unix)]
fn remove_env_vars_with_prefix(prefix: &[u8]) {
    for key in env_keys_with_prefix(std::env::vars_os(), prefix) {
        // SAFETY: process hardening boundary: constants and checked pid/fd values are prepared immediately before the kernel call; the multi-line unsafe block below uses inputs that remain live until the block returns.
        unsafe {
