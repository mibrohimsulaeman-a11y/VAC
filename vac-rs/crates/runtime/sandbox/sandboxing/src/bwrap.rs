...
const MISSING_BWRAP_WARNING: &str = concat!(
    "VAC could not find bubblewrap on PATH. ",
    "Install bubblewrap with your OS package manager. ",
    "See the sandbox prerequisites: ",
    "https://developers.vastar.com/vac/concepts/sandboxing#prerequisites. ",
    "VAC will use the vendored bubblewrap in the meantime.",
);
const USER_NAMESPACE_WARNING: &str =
    "VAC's Linux sandbox uses bubblewrap and needs access to create user namespaces.";
pub(crate) const WSL1_BWRAP_WARNING: &str = concat!(
    "VAC's Linux sandbox uses bubblewrap, which is not supported on WSL1 ",
