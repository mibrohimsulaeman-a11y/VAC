...
use vac_network_proxy::PROXY_URL_ENV_KEYS;
use vac_network_proxy::has_proxy_url_env_vars;
use vac_network_proxy::proxy_url_env_value;
...
use vac_protocol::protocol::WritableRoot;
...
fn build_seatbelt_access_policy(
    ...
    for (index, access_root) in roots.into_iter().enumerate() {
            ...
            access_root.excluded_subpaths.into_iter().enumerate()
        {
            ...
            // protected directory itself, such as `mkdir .vac`.
...
fn confstr(name: libc::c_int) -> Option<String> {
    ...
    // SAFETY: Sandbox policy boundary: immutable Rust policy data backs the pointers used for this sandbox call; `libc::confstr(name, buf.as_mut_ptr(), buf.len())` uses inputs that remain live for the duration of this block.
    let len = unsafe { libc::confstr(name, buf.as_mut_ptr(), buf.len()) };
    if len == 0 {
        return None;
    }
    // confstr guarantees NUL-termination when len > 0.
    // SAFETY: Sandbox policy boundary: immutable Rust policy data backs the pointers used for this sandbox call; `CStr::from_ptr(buf.as_ptr())` uses inputs that remain live for the duration of this block.
