...
fn transform_linux_seccomp_request(
    vac_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: cwd.as_path(),
            vac_linux_sandbox_exe: Some(vac_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let vac_linux_sandbox_exe = std::path::PathBuf::from("/tmp/vac-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&vac_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(vac_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let vac_linux_sandbox_exe = std::path::PathBuf::from("/tmp/vac");
    let exec_request = transform_linux_seccomp_request(&vac_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("vac-linux-sandbox".to_string()));
}
