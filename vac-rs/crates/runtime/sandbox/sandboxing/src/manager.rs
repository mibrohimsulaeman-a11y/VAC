...
use crate::landlock::VAC_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(viyatb): Evaluate switching this to Option<Arc<NetworkProxy>>
    // to make shared ownership explicit across runtime/sandbox plumbing.
    pub network: Option<&'a NetworkProxy>,
    pub sandbox_policy_cwd: &'a Path,
    pub vac_linux_sandbox_exe: Option<&'a Path>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing vac-linux-sandbox executable path")
            }
            #[cfg(target_os = "linux")]
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(VAC_LINUX_SANDBOX_ARG0) {
