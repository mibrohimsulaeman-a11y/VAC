...
use vac_network_proxy::ConfigReloader;
use vac_network_proxy::ConfigState;
use vac_network_proxy::NetworkMode;
use vac_network_proxy::NetworkProxy;
use vac_network_proxy::NetworkProxyConfig;
use vac_network_proxy::NetworkProxyConstraints;
use vac_network_proxy::NetworkProxyState;
use vac_network_proxy::build_config_state;
use vac_protocol::permissions::FileSystemAccessMode;
use vac_protocol::permissions::FileSystemPath;
use vac_protocol::permissions::FileSystemSandboxEntry;
...
use vac_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/vac-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.vac".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/vac-readable");
    let unreadable = absolute_path("/tmp/vac-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/vac-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/vac-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/vac-proxy-use";
    let explicit_socket = "/tmp/vac-browser-use";
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .managed_by_vac(/*managed_by_vac*/ false)
        .build()
        .await?;
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_vac_subpaths() {
    ...
    // write to .vac/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.vac carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_vac_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .vac/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_vac_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .vac is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_vac_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_vac = repo_root.join(".vac");
    let config_toml = dot_vac.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_vac.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_vac = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_vac_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_vac),
        "missing {expected_dot_vac}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .vac/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(mbolin): Should also support the case where `.git` is a file
    // with a gitdir: ... line.
    Command::new("git")
        .arg("init")
        .arg(".")
    ...
    fs::create_dir_all(vulnerable_root.join(".vac")).expect("create .vac");
    fs::write(
        vulnerable_root.join(".vac").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .vac/config.toml");
    ...
    let dot_vac_canonical = vulnerable_root_canonical.join(".vac");
