...
const PROXY_SOCKET_DIR_PREFIX: &str = "vac-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(vac_home) = std::env::var_os("VAC_HOME") {
        let candidate = PathBuf::from(vac_home).join("tmp");
...
fn spawn_proxy_socket_dir_cleanup_worker(
    ...
    if pid == 0 {
        ...
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::_exit(0)` uses inputs that remain live for the duration of this block.
        unsafe { libc::_exit(0) };
    }
...
fn spawn_host_bridge(endpoint: SocketAddr, uds_path: &Path) -> io::Result<libc::pid_t> {
    ...
    if pid == 0 {
        if close_fd(read_fd).is_err() {
            // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::_exit(1)` uses inputs that remain live for the duration of this block.
            unsafe { libc::_exit(1) };
        }
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `File::from_raw_fd(read_fd)` uses inputs that remain live for the duration of this block.
...
fn run_host_bridge(endpoint: SocketAddr, uds_path: &Path, ready_fd: libc::c_int) -> io::Result<()> {
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `File::from_raw_fd(ready_fd)` uses inputs that remain live for the duration of this block.
...
fn ensure_loopback_interface_up() -> io::Result<()> {
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::socket(libc::AF_INET, libc::SOCK_DGRAM | libc::SOCK_CLOEXEC, 0)` uses inputs that remain live for the duration of this block.
    let fd = unsafe { libc::socket(libc::AF_INET, libc::SOCK_DGRAM | libc::SOCK_CLOEXEC, 0) };
    if fd < 0 {
        return Err(io::Error::last_os_error());
    }

    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `std::mem::zeroed::<libc::ifreq>()` uses inputs that remain live for the duration of this block.
    ...
    let read_flags_result =
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::ioctl(fd, libc::SIOCGIFFLAGS as libc::Ioctl, &mut ifreq)` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `ifreq.ifr_ifru.ifru_flags` uses inputs that remain live for the duration of this block.
    ...
    if (current_flags & up_flag) != up_flag {
        ...
        let set_flags_result =
            // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::ioctl(fd, libc::SIOCSIFFLAGS as libc::Ioctl, &ifreq)` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::ioctl(fd, libc::SIOCSIFADDR as libc::Ioctl, &addr_req)` uses inputs that remain live for the duration of this block.
...
fn harden_bridge_process() -> io::Result<()> {
    ...
    vac_process_hardening::disable_process_dumping()
}
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("vac-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("vac-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("vac-linux-sandbox-proxy-x"),
            None
        );
