...
use vac_protocol::protocol::NetworkSandboxPolicy;
use vac_sandboxing::landlock::VAC_LINUX_SANDBOX_ARG0;
...
fn apply_inner_command_argv0_for_launcher(
    ...
    if supports_argv0 {
        argv.splice(
            ...
            ["--argv0".to_string(), VAC_LINUX_SANDBOX_ARG0.to_string()],
        );
        return;
    }
...
fn run_bwrap_in_child_with_synthetic_mount_cleanup(bwrap_args: crate::bwrap::BwrapArgs) -> ! {
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::getpid()` uses inputs that remain live for the duration of this block.
    let parent_pid = unsafe { libc::getpid() };
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::fork()` uses inputs that remain live for the duration of this block.
    let pid = unsafe { libc::fork() };
    if pid < 0 {
        let err = std::io::Error::last_os_error();
        panic!("failed to branch for bubblewrap: {err}");
    }

    if pid == 0 {
        reset_forwarded_signal_handlers_to_default();
        setup_signal_mask.restore();
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::setpgid(0, 0)` uses inputs that remain live for the duration of this block.
...
impl ProtectedCreateWatcher {
    fn new(targets: &[crate::bwrap::ProtectedCreateTarget]) -> Option<Self> {
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::inotify_init1(libc::IN_NONBLOCK | libc::IN_CLOEXEC)` uses inputs that remain live for the duration of this block.
        ...
        for target in targets {
            ...
            // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::inotify_add_watch(fd, parent_cstr.as_ptr(), mask)` uses inputs that remain live for the duration of this block.
        ...
        if watches.is_empty() {
            // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; the multi-line unsafe block below uses inputs that remain live until the block returns.
            unsafe {
                libc::close(fd);
            }
            return None;
    ...
    fn wait_for_create_event(&self, stop: &AtomicBool) {
        ...
        while !stop.load(Ordering::SeqCst) {
            // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::poll(&mut poll_fd, 1, 10)` uses inputs that remain live for the duration of this block.
    ...
    fn drain_events(&self) {
        ...
        loop {
            // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::read(self.fd, buf.as_mut_ptr().cast(), buf.len())` uses inputs that remain live for the duration of this block.
...
fn create_exec_start_pipe(enabled: bool) -> [libc::c_int; 2] {
    ...
    let mut pipe = [-1, -1];
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::pipe2(pipe.as_mut_ptr(), libc::O_CLOEXEC)` uses inputs that remain live for the duration of this block.
...
fn wait_for_parent_exec_start(read_fd: libc::c_int, write_fd: libc::c_int) {
    ...
    let mut byte = [0_u8; 1];
    loop {
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::read(read_fd, byte.as_mut_ptr().cast(), byte.len())` uses inputs that remain live for the duration of this block.
...
impl ForwardedSignalMask {
    fn block() -> Self {
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `std::mem::zeroed()` uses inputs that remain live for the duration of this block.
...
fn terminate_with_parent(parent_pid: libc::pid_t) {
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::prctl(libc::PR_SET_PDEATHSIG, libc::SIGTERM)` uses inputs that remain live for the duration of this block.
    let res = unsafe { libc::prctl(libc::PR_SET_PDEATHSIG, libc::SIGTERM) };
    if res < 0 {
        let err = std::io::Error::last_os_error();
        panic!("failed to set bubblewrap child parent-death signal: {err}");
    }
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::getppid()` uses inputs that remain live for the duration of this block.
...
fn wait_for_bwrap_child(pid: libc::pid_t) -> libc::c_int {
    loop {
        ...
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::waitpid(pid, &mut status as *mut libc::c_int, 0)` uses inputs that remain live for the duration of this block.
...
fn process_is_active(pid: libc::pid_t) -> bool {
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::kill(pid, 0)` uses inputs that remain live for the duration of this block.
...
fn with_synthetic_mount_registry_lock<T>(f: impl FnOnce() -> T) -> T {
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::flock(lock_file.as_raw_fd(), libc::LOCK_EX)` uses inputs that remain live for the duration of this block.
    ...
    let result = f();
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::flock(lock_file.as_raw_fd(), libc::LOCK_UN)` uses inputs that remain live for the duration of this block.
...
fn synthetic_mount_registry_root() -> PathBuf {
    std::env::temp_dir().join("vac-bwrap-synthetic-mount-targets")
}
...
fn run_bwrap_in_child_capture_stderr(bwrap_args: crate::bwrap::BwrapArgs) -> String {
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::pipe2(pipe_fds.as_mut_ptr(), libc::O_CLOEXEC)` uses inputs that remain live for the duration of this block.
...
fn close_fd_or_panic(fd: libc::c_int, context: &str) {
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::close(fd)` uses inputs that remain live for the duration of this block.
