...
pub(crate) fn apply_permission_profile_to_current_thread(
    ...
    if apply_landlock_fs && !file_system_sandbox_policy.has_full_disk_write_access() {
        if !file_system_sandbox_policy.has_full_disk_read_access() {
            return Err(VACErr::UnsupportedOperation(
...
fn set_no_new_privs() -> Result<()> {
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `libc::prctl(libc::PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0)` uses inputs that remain live for the duration of this block.
...
///
/// # Errors
/// Returns [`VACErr::Sandbox`] variants when the ruleset fails to apply.
///
...
fn install_filesystem_landlock_rules_on_current_thread(
    ...
    if status.ruleset == landlock::RulesetStatus::NotEnforced {
        return Err(VACErr::Sandbox(SandboxErr::LandlockRestrict));
    }
