...
mod imp {
    ...
    // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; extern block declares platform ABI symbols and all calls remain behind checked wrappers.
    unsafe extern "C" {
    ...
    pub(crate) fn run_vendored_bwrap_main(
        ...
        // SAFETY: Linux sandbox boundary: this function validates or owns the fd, pid, argv/env pointer, or namespace input before the syscall/exec boundary; `bwrap_main(cstrings.len() as libc::c_int, argv_ptrs.as_ptr())` uses inputs that remain live for the duration of this block.
...
VAC first tries a system bubblewrap binary at runtime. The vendored launcher is optional and is not compiled by default.
Action:
- install bubblewrap so `bwrap` is available on PATH, or
- rebuild with VAC_BUILD_VENDORED_BWRAP=1 and vendored sources at vac-rs/vendor/bubblewrap, or
- set VAC_BWRAP_SOURCE_DIR to an existing bubblewrap checkout for a hardened/release build."#
        );
    }
