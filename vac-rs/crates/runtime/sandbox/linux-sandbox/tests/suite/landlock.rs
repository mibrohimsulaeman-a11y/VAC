...
use vac_core::exec::ExecCapturePolicy;
use vac_core::exec::ExecParams;
use vac_core::exec::process_exec_tool_call;
use vac_core::exec_env::create_env;
...
fn vac_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_vac-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> vac_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<vac_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let vac_linux_sandbox_exe = Some(vac_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &vac_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(VACErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(VACErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<vac_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> vac_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(VACErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let vac_linux_sandbox_exe: Option<PathBuf> = Some(vac_linux_sandbox_exe());
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://vastar.com"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://vastar.com"]).await;
}
...
#[tokio::test]
async fn sandbox_blocks_git_and_vac_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_vac = tmpdir.path().join(".vac");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_vac).expect("create .vac");

    let git_target = dot_git.join("config");
    let vac_target = dot_vac.join("config.toml");
    ...
    let vac_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", vac_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".vac write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(vac_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_vac_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-vac");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_vac = tmpdir.path().join(".vac");
    symlink(&decoy, &dot_vac).expect("create .vac symlink");
    ...
    let vac_output = expect_denied(
        ...
        .await,
        ".vac symlink replacement should be denied",
    );
    assert_ne!(vac_output.exit_code, 0);
}
...
if grep -E '(^|[[:space:]])\.(git|vac|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_vac_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".vac"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .vac directory creation should be denied",
    );
    assert_ne!(mkdir_vac_output.exit_code, 0);
    assert!(!subdir.join(".vac").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = vac_linux_sandbox_exe()
        .parent()
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent", "ahosts", "vastar.com"]).await;
}
