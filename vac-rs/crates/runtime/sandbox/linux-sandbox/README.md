...
- a `vac-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the VAC CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `vac-exec` CLI can check if its arg0 is `vac-linux-sandbox` and, if so, execute as if it were `vac-linux-sandbox`
  - this should also be true of the `vac` multitool CLI

On Linux, VAC prefers the first `bwrap` found on `PATH`
...
binary.
VAC also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the vendored helper. VAC surfaces the same startup warning
...
it cannot create the required user namespaces, so VAC rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the vendored bubblewrap
  path.
- If `bwrap` is missing, VAC also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, VAC surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; VAC rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.vac`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface still uses legacy names like `vac debug landlock`.
