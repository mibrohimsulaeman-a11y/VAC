...
pub const CHECKPOINT_FORMAT_V1: &str = "provider_core_message_v1";
...
fn migrate_legacy_checkpoint(value: &serde_json::Value) -> Option<CheckpointEnvelopeV1> {
    if value.is_array() {
        let messages: Vec<vac_provider_core::Message> =
    ...
    let messages: Vec<vac_provider_core::Message> =
        serde_json::from_value(messages_value.clone()).ok()?;
...
mod tests {
    ...
    fn roundtrip_v1_envelope() {
        ...
        let first_message_text = parsed
            .messages
            .first()
            .and_then(vac_provider_core::Message::text);
