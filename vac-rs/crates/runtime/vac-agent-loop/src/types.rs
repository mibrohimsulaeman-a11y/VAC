...
pub type TokenUsage = vac_provider_core::Usage;
...
pub struct AgentConfig {
    ...
    pub provider_options: Option<vac_provider_core::ProviderOptions>,
...
/// Strip MCP server prefix from tool name (e.g. "vac__run_command" -> "run_command").
...
pub enum AgentCommand {
    ...
    Steering(String),
    FollowUp(String),
    SwitchModel(vac_provider_core::Model),
    Cancel,
}
...
pub struct AgentLoopResult {
    ...
    pub messages: Vec<vac_provider_core::Message>,
...
mod tests {
    ...
    fn from_allowlist_handles_prefix() {
        ...
        assert_eq!(
            policy.action_for("vac__view", None),
            ToolApprovalAction::Approve
        );
        assert_eq!(
            policy.action_for("vac__run_command", None),
            ToolApprovalAction::Deny
        );
    }
    ...
    fn strip_tool_prefix_cases() {
        assert_eq!(strip_tool_prefix("vac__run_command"), "run_command");
    ...
    fn e2e_nested_sh_c_resolves_inner_commands() {
        ...
        // vac_shell_approval recursively extracts inner commands from "sh -c '...'"
        assert_eq!(
            policy.action_for(
                "run_command",
