use crate::{
    ...
    bound_runtime::GateDecision,
    bound_tool::BoundRuntimeToolBoundary,
...
use vac_provider_core::{ContentPart, FinishReasonKind, Message, MessageContent, Role};
...
pub async fn run_agent(
    run: AgentRunContext,
    inference: &vac_provider_core::Inference,
    config: &AgentConfig,
    ...
    let mut vac_boundary = BoundRuntimeToolBoundary::from_context_metadata(&run, context_metadata);

    let mut current_model = config.model.clone();
    let mut queues = RuntimeQueues::default();
    let mut total_usage = vac_provider_core::Usage::default();
    ...
    'run_loop: loop {
        ...
        let response = loop {
            ...
            let mut request =
                vac_provider_core::GenerateRequest::new(current_model.clone(), reduced_messages);
        ...
        let mut stop_reason = map_stop_reason(finish_reason);
        if let Some(completion_gate) = vac_boundary.complete_before_run_completed(context_metadata)
        {
            let status = format!(
                "VAC completion lock requires operator input: gate={:?} blockers={:?} warnings={:?}",
                completion_gate.gate, completion_gate.blockers, completion_gate.warnings
            );
            if matches!(completion_gate.decision, GateDecision::NeedsDiscussion) {
                emit(
                    &event_tx,
                    AgentEvent::PausedForDiscussion {
                        run_id: run.run_id,
                        gate: format!("{:?}", completion_gate.gate),
                        explicit_open_question: completion_gate.warnings.first().cloned().unwrap_or_else(|| "Clarify the unresolved VAC completion-lock item before marking the session done.".to_string()),
                        blocking_reason: completion_gate.blockers.first().cloned().unwrap_or_else(|| "VAC completion lock entered needs_discussion.".to_string()),
                        operator_visible_status: status.clone(),
                    },
                )
                .await;
                messages.push(Message::new(Role::Assistant, status));
                stop_reason = StopReason::NeedsOperatorInput;
            } else {
                let error = format!(
                    "VAC completion lock blocked run closeout: gate={:?} blockers={:?} warnings={:?}",
                    completion_gate.gate, completion_gate.blockers, completion_gate.warnings
                );
                emit(
                    &event_tx,
                ...
                .await;
                messages.push(Message::new(Role::Assistant, error));
...
async fn run_tool_cycle(
    ...
    current_model: &mut vac_provider_core::Model,
    messages: &mut Vec<Message>,
    proposed_tool_calls: Vec<ProposedToolCall>,
    vac_boundary: &mut BoundRuntimeToolBoundary,
    context_metadata: &mut serde_json::Value,
    ...
    loop {
        ...
        if let Some(resolved) = approvals.next_ready() {
            ...
            match resolved.decision {
                ToolDecision::Accept => {
                    let gate = vac_boundary.gate_tool_call(&resolved.tool_call, context_metadata);
                    let Some(bound_tool_call) = gate.stamped_tool_call.clone() else {
                        if gate.approval_request.is_some() {
                            approvals.pause_for_vac_runtime_approval(&tool_call_id)?;
                            emit(
                                event_tx,
                                AgentEvent::WaitingForToolApproval {
                                    run_id: run.run_id,
                                    pending_tool_call_ids: vec![tool_call_id.clone()],
                                },
                            )
                            .await;
                            drain_runtime_commands_nonblocking(
                        ...
                        let result = gate.tool_error_payload();
                        append_tool_result_message(messages, &tool_call_id, json!(result.clone()));
                    ...
                    for hook in hooks {
                        hook.before_tool_execution(run, &bound_tool_call, messages)
                            .await?;
                    }

                    match tools
                        .execute_tool_call(run, &bound_tool_call, cancel)
                        .await?
                    {
                    ...
                    for hook in hooks {
                        hook.after_tool_execution(run, &bound_tool_call, messages)
                            .await?;
                    }
                }
                ToolDecision::Reject => {
        ...
        match command {
            AgentCommand::ResolveTool {
                ...
                if current_tool_ids.contains(&tool_call_id) {
                    vac_boundary.record_operator_tool_decision(
                        &tool_call_id,
                        &decision,
                        context_metadata,
                    );
            ...
            AgentCommand::ResolveTools { decisions } => {
                ...
                if !apply_now.is_empty() {
                    for (tool_call_id, decision) in &apply_now {
                        vac_boundary.record_operator_tool_decision(
                            tool_call_id,
                            decision,
                            context_metadata,
...
fn map_finish_reason(reason: &vac_provider_core::FinishReason) -> TurnFinishReason {
...
fn add_usage(total: &mut vac_provider_core::Usage, usage: &vac_provider_core::Usage) {
