...
mod tests {
    ...
    fn arg_single_segment_does_not_match_later_positions() {
        let mut rules = HashMap::new();
        rules.insert("run_command::vac::ak".to_string(), Action::Approve);

        let resolved = resolve_hierarchical_policy(
            "vac browser ak visit example.com",
            "run_command",
            &[],
            &rules,
            Action::Ask,
    ...
    fn arg_multi_segment_matches_exact_path() {
        ...
        let resolved =
            resolve_hierarchical_policy("vac config show", "run_command", &[], &rules, Action::Ask);
    ...
    fn arg_multi_segment_does_not_match_short_command() {
        ...
        let resolved =
            resolve_hierarchical_policy("vac config", "run_command", &[], &rules, Action::Ask);
    ...
    fn arg_multi_segment_does_not_match_different_middle() {
        ...
        let resolved =
            resolve_hierarchical_policy("vac config list", "run_command", &[], &rules, Action::Ask);
    ...
    fn ak_verb_specific_rules_can_all_auto_approve() {
        ...
        for verb in ["search", "read", "write", "remove", "skill"] {
            rules.insert(format!("run_command::vac::ak::{verb}"), Action::Approve);
        }

        for command in [
            "vac ak search services --tree",
            "vac ak read notes.md",
            "vac ak write notes.md",
            "vac ak remove notes.md",
            "vac ak skill usage",
        ] {
            let resolved =
