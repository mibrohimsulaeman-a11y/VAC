...
HTTP/SSE runtime shell around `vac-agent-loop`.
...
- MCP tool execution bridge (through `vac-mcp-client`)
- Runtime config exposure (`/v1/config` and `session.config`)

## What this crate does **not** own

- Core agent loop logic (delegated to `vac-agent-loop::run_agent`)
...
- `message_bridge.rs` — temporary storage edge adapters (`ChatMessage` <-> `vac_provider_core::Message`)
...
1. Client calls `POST /v1/sessions/{id}/messages` with a `vac_provider_core::Message`.
...
5. Actor calls `run_agent` from `vac-agent-loop`.
...
- **VAC Provider Core-native boundary**: session message input/output is `vac_provider_core::Message`.
...
```bash
# Add channels
vac autopilot channel add slack --bot-token "$SLACK_BOT_TOKEN" --app-token "$SLACK_APP_TOKEN"
...
`vac-broker` should be treated as a runtime adapter + API shell.
All agent behavior (turn loop, approvals, retries, compaction, event semantics) should remain centralized in `vac-agent-loop`.
