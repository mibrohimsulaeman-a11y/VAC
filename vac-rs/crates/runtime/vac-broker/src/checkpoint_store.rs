...
use uuid::Uuid;
use vac_agent_loop::{CheckpointEnvelopeV1, deserialize_checkpoint, serialize_checkpoint};
...
impl CheckpointStore {
    ...
    pub fn default_local() -> Self {
        let root = std::env::var("HOME")
            ...
            .unwrap_or_else(|_| PathBuf::from(".vac").join("server").join("checkpoints"));
...
mod tests {
    ...
    async fn save_and_load_checkpoint_envelope_roundtrip() {
        let root = std::env::temp_dir().join(format!("vac-checkpoint-test-{}", Uuid::new_v4()));
