...
//!
//! When sandbox mode is enabled for a session, a `vac mcp start` server
...
use vac_foundation::cert_utils::{CertificateChain, MtlsIdentity};
use vac_mcp_client::McpClient;
use vac_mcp_proxy::client::{ClientPoolConfig, ServerConfig};
use vac_mcp_proxy::server::start_proxy_server;

/// Environment variable used to pass the client CA cert PEM to the container.
const TRUSTED_CLIENT_CA_ENV: &str = "VAC_MCP_CLIENT_CA";
...
/// root (via `--user 0:0`) with `VAC_TARGET_UID`/`VAC_TARGET_GID` env
...
pub struct SandboxConfig {
    ...
    /// Container image for the sandbox (e.g., `vac-local:dev`).
    pub image: String,
...
impl PersistentSandbox {
    ...
    pub async fn tools(&self) -> Vec<vac_provider_core::Tool> {
    ...
    loop {
        ...
        let check_result = {
            ...
            tokio::time::timeout(
                ...
                vac_mcp_client::get_tools(&sandbox.client),
            )
            .await
        };
...
pub struct SandboxedMcpServer {
    ...
    pub tools: Vec<vac_provider_core::Tool>,
...
impl SandboxedMcpServer {
    ...
    /// 3. Spawns `warden wrap <image> -- vac mcp start`
    ...
    pub async fn spawn(config: &SandboxConfig) -> Result<Self, String> {
        ...
        // 9. Get tools
        let mcp_tools = vac_mcp_client::get_tools(&client)
            .await
...
fn build_warden_argv(
    ...
    use vac_foundation::container::{expand_volume_path, is_named_volume};
    ...
    match &config.user_mapping {
        SandboxUserMapping::HostUser { uid, gid } => {
            ...
            args.push(format!("VAC_TARGET_UID={uid}"));
            args.push("--env".to_string());
            args.push(format!("VAC_TARGET_GID={gid}"));
        }
    ...
    args.push("--env".to_string());
    args.push("VAC_SKIP_WARDEN=1".to_string());

    args.push("--env".to_string());
    args.push("VAC_MCP_PORT=8080".to_string());
    ...
    // the container is started as root with VAC_TARGET_UID/GID set.
    args.push("--".to_string());
    args.push(CONTAINER_ENTRYPOINT.to_string());
    args.push("/usr/local/bin/vac".to_string());
...
async fn spawn_warden_container(
    ...
    for var in &["VAC_API_KEY", "VAC_PROFILE", "VAC_API_ENDPOINT"] {
...
async fn parse_server_ca_from_stdout(process: &mut Child) -> Result<String, String> {
    ...
    loop {
        ...
        if trimmed == "---BEGIN VAC SERVER CA---" {
            in_server_ca = true;
            continue;
        }

        if trimmed == "---END VAC SERVER CA---" {
...
fn build_sandbox_proxy_config(
    ...
    // Register the sandboxed MCP server under the same name ("vac") so
    // tool names like `vac__run_command` route correctly through the proxy.
    servers.insert(
        "vac".to_string(),
        ServerConfig::Http {
    ...
    servers.insert(
        "paks".to_string(),
        ServerConfig::Http {
            url: "https://api.vastar.ai/vac/v1/paks/mcp".to_string(),
            headers: None,
            certificate_chain: Arc::new(None),
    ...
    servers.insert(
        "aap".to_string(),
        ServerConfig::Http {
            url: "https://api.vastar.ai/vac/v1/aap/mcp".to_string(),
            headers: None,
            certificate_chain: Arc::new(None),
            client_tls_config: None,
        },
...
async fn connect_to_proxy(
    ...
    for attempt in 1..=MAX_RETRIES {
        match vac_mcp_client::connect_https(proxy_url, Some(cert_chain.clone()), None).await {
...
-----END CERTIFICATE-----";
    ...
    fn mtls_identity_cross_trust() {
        use vac_foundation::cert_utils::MtlsIdentity;
    ...
    fn expand_volume_path_leaves_named_volumes_unchanged() {
        use vac_foundation::container::expand_volume_path;
        let named = "vac-aqua-cache:/home/agent/.local/share/aquaproj-aqua";
    ...
    fn named_volume_is_detected_correctly() {
        use vac_foundation::container::is_named_volume;
        let cases = vec![
            ("vac-aqua-cache", true),
            ("my-volume", true),
            ("./relative/path", false),
    ...
    fn host_user_mapping_starts_as_root_with_target_env() {
        ...
        assert!(argv.contains(&"VAC_TARGET_UID=1001".to_string()));
        assert!(argv.contains(&"VAC_TARGET_GID=1001".to_string()));
    }
    ...
    fn warden_argv_options_before_image() {
        ...
        // Command after `--` must be `entrypoint.sh vac mcp start`
        ...
        assert!(argv.contains(&"0:0".to_string()));
        assert!(argv.contains(&"VAC_TARGET_UID=1000".to_string()));
        assert!(argv.contains(&"VAC_TARGET_GID=1001".to_string()));
    ...
    fn warden_argv_no_user_when_image_default() {
        ...
        assert!(!argv.iter().any(|a| a.starts_with("VAC_TARGET_UID")));
        assert!(!argv.iter().any(|a| a.starts_with("VAC_TARGET_GID")));
    }
