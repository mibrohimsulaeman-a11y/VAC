...
use vac_protocol::protocol::HookHandlerType;
...
fn append_plugin_hook_sources(
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(abhinav): check enabled/trusted state here before plugin hooks become runnable.
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_vac_folder } => dot_vac_folder.join(CONFIG_TOML_FILE),
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: vac_protocol::protocol::HookEventName,
        ...
        for (handler_index, handler) in group.hooks.into_iter().enumerate() {
            match handler {
                HookHandlerConfig::Command {
                    ...
                    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(abhinav): replace this positional suffix with a durable hook id.
                    let key = format!(
                        "{}:{}:{}:{}",
                        source.key_source,
...
fn hook_event_key_label(event_name: vac_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        vac_protocol::protocol::HookEventName::PreToolUse => "pre_tool_use",
        vac_protocol::protocol::HookEventName::PermissionRequest => "permission_request",
        vac_protocol::protocol::HookEventName::PostToolUse => "post_tool_use",
        vac_protocol::protocol::HookEventName::SessionStart => "session_start",
        vac_protocol::protocol::HookEventName::UserPromptSubmit => "user_prompt_submit",
        vac_protocol::protocol::HookEventName::Stop => "stop",
    }
}
...
mod tests {
    ...
    fn hook_source_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.vac/config.toml").abs();
        let dot_vac_folder = test_path_buf("/tmp/worktree/.vac").abs();
        ...
        assert_eq!(
            super::hook_source_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.vastar.vac".to_string(),
