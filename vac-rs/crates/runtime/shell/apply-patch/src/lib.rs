...
/// Special argv[1] flag used when the VAC executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `vac-apply-patch` (to avoid forcing
/// `vac-arg0` to depend on `vac-core`), it remains part of the "vac core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const VAC_CORE_APPLY_PATCH_ARG1: &str = "--vac-run-as-apply-patch";
