...
fn get_escalate_client() -> anyhow::Result<AsyncDatagramSocket> {
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): we should defensively require only calling this once, since AsyncSocket will take ownership of the fd.
    ...
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `AsyncDatagramSocket::from_raw_fd(client_fd)` uses inputs that remain live for the duration of this block.
...
pub async fn run_shell_escalation_execve_wrapper(
    ...
    match message.action {
        EscalateAction::Escalate => {
            ...
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): also forward signals over the super-exec socket
...
mod tests {
    ...
    fn duplicate_fd_for_transfer_does_not_close_original() {
        ...
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::fcntl(original_fd, libc::F_GETFD)` uses inputs that remain live for the duration of this block.
