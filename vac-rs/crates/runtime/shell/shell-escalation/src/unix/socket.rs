...
fn assume_init<T>(buf: &[MaybeUninit<T>]) -> &[T] {
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `std::slice::from_raw_parts(buf.as_ptr().cast(), buf.len())` uses inputs that remain live for the duration of this block.
    unsafe { std::slice::from_raw_parts(buf.as_ptr().cast(), buf.len()) }
}

fn assume_init_slice<T, const N: usize>(buf: &[MaybeUninit<T>; N]) -> &[T; N] {
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `&*(buf as *const [MaybeUninit<T>; N] as *const [T; N])` uses inputs that remain live for the duration of this block.
    unsafe { &*(buf as *const [MaybeUninit<T>; N] as *const [T; N]) }
}

fn assume_init_vec<T>(mut buf: Vec<MaybeUninit<T>>) -> Vec<T> {
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; the multi-line unsafe block below uses inputs that remain live until the block returns.
    unsafe {
...
fn control_space_for_fds(count: usize) -> usize {
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::CMSG_SPACE((count * size_of::<RawFd>()) as _) as usize` uses inputs that remain live for the duration of this block.
...
fn extract_fds(control: &[u8]) -> Vec<OwnedFd> {
    let mut fds = Vec::new();
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `std::mem::zeroed()` uses inputs that remain live for the duration of this block.
    let mut hdr: libc::msghdr = unsafe { std::mem::zeroed() };
    hdr.msg_control = control.as_ptr() as *mut libc::c_void;
    hdr.msg_controllen = control.len() as _;
    let hdr = hdr; // drop mut

    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::CMSG_FIRSTHDR(&hdr) as *const libc::cmsghdr` uses inputs that remain live for the duration of this block.
    let mut cmsg = unsafe { libc::CMSG_FIRSTHDR(&hdr) as *const libc::cmsghdr };
    while !cmsg.is_null() {
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `(*cmsg).cmsg_level` uses inputs that remain live for the duration of this block.
        let level = unsafe { (*cmsg).cmsg_level };
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `(*cmsg).cmsg_type` uses inputs that remain live for the duration of this block.
        let ty = unsafe { (*cmsg).cmsg_type };
        if level == libc::SOL_SOCKET && ty == libc::SCM_RIGHTS {
            // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::CMSG_DATA(cmsg).cast::<RawFd>()` uses inputs that remain live for the duration of this block.
            ...
            let fd_count: usize = {
                ...
                // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `(*cmsg).cmsg_len` uses inputs that remain live for the duration of this block.
                let cmsg_data_len = usize::try_from(unsafe { (*cmsg).cmsg_len })
                    .expect("cmsghdr length fits")
                    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::CMSG_LEN(0) as usize` uses inputs that remain live for the duration of this block.
                    - unsafe { libc::CMSG_LEN(0) as usize };
                cmsg_data_len / size_of::<RawFd>()
            };
            for i in 0..fd_count {
                // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `data_ptr.add(i).read()` uses inputs that remain live for the duration of this block.
                let fd = unsafe { data_ptr.add(i).read() };
                // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `OwnedFd::from_raw_fd(fd)` uses inputs that remain live for the duration of this block.
                fds.push(unsafe { OwnedFd::from_raw_fd(fd) });
            }
        }
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::CMSG_NXTHDR(&hdr, cmsg)` uses inputs that remain live for the duration of this block.
...
impl AsyncDatagramSocket {
    ...
    // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; unsafe function contract for `from_raw_fd` is documented by the surrounding wrapper and must be upheld by callers.
    pub unsafe fn from_raw_fd(fd: RawFd) -> std::io::Result<Self> {
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `Socket::from_raw_fd(fd)` uses inputs that remain live for the duration of this block.
...
mod tests {
    ...
    async fn async_socket_round_trips_payload_and_fds() -> std::io::Result<()> {
        ...
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::fcntl(received_fds[0].as_raw_fd(), libc::F_GETFD)` uses inputs that remain live for the duration of this block.
