...
mod tests {
    ...
    use vac_protocol::approvals::EscalationPermissions;
    use vac_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    use vac_protocol::models::NetworkPermissions;
    use vac_utils_absolute_path::AbsolutePathBuf;
    ...
    fn process_exists(pid: i32) -> bool {
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::kill(pid, 0)` uses inputs that remain live for the duration of this block.
    ...
    impl ShellCommandExecutor for AfterSpawnAssertingShellCommandExecutor {
        async fn run(
            ...
            let socket_fd = env_overlay
                ...
                .parse::<i32>()?;
            // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::fcntl(socket_fd, libc::F_GETFD)` uses inputs that remain live for the duration of this block.
    ...
    /// executable paths.
    ///
    /// The `/bin/zsh` and `/tmp/vac-execve-wrapper` values here are
    ...
    async fn start_session_exposes_wrapper_env_overlay() -> anyhow::Result<()> {
        ...
        let execve_wrapper = PathBuf::from("/tmp/vac-execve-wrapper");
    ...
    async fn exec_closes_parent_socket_after_shell_spawn() -> anyhow::Result<()> {
        ...
        let server = EscalateServer::new(
            ...
            PathBuf::from("/tmp/vac-execve-wrapper"),
            DeterministicEscalationPolicy {
                decision: EscalationDecision::run(),
            },
        );
    ...
    async fn handle_escalate_session_respects_run_in_sandbox_decision() -> anyhow::Result<()> {
        ...
        for i in 0..10 {
            ...
            env.insert(format!("VAC_TEST_VAR{i}"), value);
        }
    ...
    impl RestoredFd {
        ...
        fn close_temporarily(target_fd: i32) -> anyhow::Result<Self> {
            // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::dup(target_fd)` uses inputs that remain live for the duration of this block.
            let original_fd = unsafe { libc::dup(target_fd) };
            if original_fd == -1 {
                return Err(std::io::Error::last_os_error().into());
            }
            // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::close(target_fd)` uses inputs that remain live for the duration of this block.
            ...
            Ok(Self {
                target_fd,
                // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `std::os::fd::OwnedFd::from_raw_fd(original_fd)` uses inputs that remain live for the duration of this block.
    ...
    -> anyhow::Result<()> {
        ...
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::pipe(pipe_fds.as_mut_ptr())` uses inputs that remain live for the duration of this block.
        if unsafe { libc::pipe(pipe_fds.as_mut_ptr()) } == -1 {
            return Err(std::io::Error::last_os_error().into());
        }
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `std::os::fd::OwnedFd::from_raw_fd(pipe_fds[0])` uses inputs that remain live for the duration of this block.
    ...
    -> anyhow::Result<()> {
        ...
        let socket_fd = session
            ...
            .expect("session should export shell escalation socket")
            .parse::<i32>()?;
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::dup(socket_fd)` uses inputs that remain live for the duration of this block.
        ...
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `libc::dup(server_stream_guard.as_raw_fd())` uses inputs that remain live for the duration of this block.
        ...
        // SAFETY: Unix escalation boundary: this module owns the socket/fd buffer for the immediate libc or socket operation; `std::os::fd::OwnedFd::from_raw_fd(dup_server_stream_fd)` uses inputs that remain live for the duration of this block.
