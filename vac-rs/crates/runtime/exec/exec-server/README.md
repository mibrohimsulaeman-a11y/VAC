...
`vac-exec-server` is the library backing `vac exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`vac-utils-pty`.

It provides:

- a CLI entrypoint: `vac exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `vac` binary owns hidden helper dispatch for sandboxed
filesystem operations and `vac-linux-sandbox`.

## Transport

The server speaks the shared `vac-app-server-protocol` message envelope on
the wire.
...
- `arg0`: optional argv0 override forwarded to `vac-utils-pty`.
...
hidden helper process launched from the top-level `vac` executable and
...
`vac exec-server` command builds these paths from the `vac` arg0 dispatch
state.
