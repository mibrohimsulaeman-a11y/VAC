...
use vac_utils_absolute_path::canonicalize_preserving_symlinks;
...
use crate::fs_helper::VAC_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.vac_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![VAC_FS_HELPER_ARG1.to_string()],
            cwd: cwd.clone(),
        ...
        sandbox_manager
            .transform(SandboxTransformRequest {
                ...
                vac_linux_sandbox_exe: self.runtime_paths.vac_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.vac_self_exe.as_path())
        .chain(runtime_paths.vac_linux_sandbox_exe.as_deref().into_iter())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let vac_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(vac_self_exe, /*vac_linux_sandbox_exe*/ None)
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/vac".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let vac_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths = ExecServerRuntimePaths::new(vac_self_exe.clone(), Some(vac_self_exe))
            .expect("runtime paths");
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let vac_self_exe = root.path().join("bin").join("vac");
        let vac_linux_sandbox_exe = root.path().join("aliases").join("vac-linux-sandbox");
        let runtime_paths = ExecServerRuntimePaths::new(vac_self_exe, Some(vac_linux_sandbox_exe))
            .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let vac_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute vac parent");
        ...
        assert!(policy.can_read_path_with_cwd(vac_parent.as_path(), cwd.as_path()));
