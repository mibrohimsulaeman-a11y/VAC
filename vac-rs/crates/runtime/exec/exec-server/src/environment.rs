...
pub const VAC_EXEC_SERVER_URL_ENV_VAR: &str = "VAC_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the VAC runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `VAC_EXEC_SERVER_URL` behavior
/// while provider-based construction accepts a provider-supplied snapshot.
///
/// Setting `VAC_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `VAC_EXEC_SERVER_URL` and local runtime paths
    /// used when creating local filesystem helpers.
    pub async fn new(args: EnvironmentManagerArgs) -> Self {
        let EnvironmentManagerArgs {
            local_runtime_paths,
        } = args;
        let exec_server_url = std::env::var(VAC_EXEC_SERVER_URL_ENV_VAR).ok();
        Self::from_default_provider_url(exec_server_url, local_runtime_paths).await
    }
    ...
    async fn from_default_provider_url(
        ...
        if environment_disabled {
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): Remove this legacy `VAC_EXEC_SERVER_URL=none` crutch once
    ...
    fn from_environments(
        ...
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): Stop deriving a default environment here once omitted
...
impl Environment {
    /// Builds an environment from the raw `VAC_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `VAC_EXEC_SERVER_URL` value and
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = vac_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            vac_protocol::models::PermissionProfile::from_runtime_permissions(
                &vac_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                vac_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
