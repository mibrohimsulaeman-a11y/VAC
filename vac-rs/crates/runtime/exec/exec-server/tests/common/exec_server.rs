...
use vac_runtime_protocol::JSONRPCRequest;
...
pub(crate) struct ExecServerHarness {
    _vac_home: TempDir,
    _helper_paths: TestVACHelperPaths,
    child: Child,
...
pub(crate) struct TestVACHelperPaths {
    pub(crate) vac_exe: PathBuf,
    pub(crate) vac_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_vac_helper_paths() -> anyhow::Result<TestVACHelperPaths> {
    let (helper_binary, vac_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestVACHelperPaths {
        vac_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.vac_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("VAC_HOME", vac_home.path());
    child.envs(env);
