...
- The legacy rule matcher lives in `vac-execpolicy-legacy`.
...
- From the VAC CLI, run `vac execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
vac execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
vac execpolicy check \
...
```bash
cargo run -p vac-execpolicy -- check --rules path/to/policy.rules git status
```
