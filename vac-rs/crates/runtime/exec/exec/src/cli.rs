...
#[command(
    version,
    override_usage = "vac exec [OPTIONS] [PROMPT]\n       vac exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running VAC outside a Git repository.
    ...
    /// Do not load `$VAC_HOME/config.toml`; auth still uses `VAC_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "vac resume --last <prompt>" treats the positional as a prompt, not a session id.
