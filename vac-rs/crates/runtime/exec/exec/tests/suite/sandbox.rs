...
use vac_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use vac_core::exec::build_exec_request;
    ...
    let vac_linux_sandbox_exe = None;
...
async fn spawn_command_under_sandbox(
    ...
    use vac_core::spawn_command_under_linux_sandbox;
    use vac_protocol::models::PermissionProfile;

    let vac_linux_sandbox_exe = core_test_support::find_vac_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_vac_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .vac && echo 'sandbox_mode = \"danger-full-access\"' > .vac/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &policy,
    ...
    .await
    .expect("should spawn command creating .vac");

    let status = child.wait().await.expect("should wait for .vac command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .vac creation: {status:?}"
    );
    let dot_vac_metadata = tokio::fs::symlink_metadata(&dot_vac).await;
    if let Ok(metadata) = dot_vac_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_vac.display()
        );
    } else if let Err(err) = &dot_vac_metadata {
        assert_eq!(
            err.kind(),
