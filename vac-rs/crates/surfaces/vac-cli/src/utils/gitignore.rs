...
/// Appends `.vac/session*` to the `.gitignore` file in the current directory if:
/// 1. The feature is enabled in config (auto_append_gitignore is true)
/// 2. We're in a git repository
/// 3. `.vac/session*` is not already in the `.gitignore` file
pub fn ensure_vac_in_gitignore(config: &AppConfig) -> Result<bool, String> {
    ...
    if gitignore_path.exists() {
        ...
        // Create new .gitignore with .vac/session* entry
        append_vac_to_gitignore(&gitignore_path)?;
        Ok(true)
    }
}

/// Checks and updates the `.gitignore` file:
/// - Returns Ok(false) if `.vac/session*` already exists
/// - Updates `.vac` or `.vac/` to `.vac/session*` and returns Ok(true)
/// - Appends `.vac/session*` if no vac entry exists and returns Ok(true)
fn check_and_update_gitignore(gitignore_path: &Path) -> Result<bool, String> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if trimmed == ".vac/session*" {
            has_session_pattern = true;
            break;
        } else if trimmed == ".vac" || trimmed == ".vac/" {
    ...
    // No vac entry exists, append new one
    append_vac_to_gitignore(gitignore_path)?;
    Ok(true)
}

/// Appends `.vac/session*` to the `.gitignore` file, creating it if it doesn't exist
fn append_vac_to_gitignore(gitignore_path: &Path) -> Result<(), String> {
    ...
    // Add comment and .vac/session* entry
    writeln!(file, "# VAC session files")
        .map_err(|e| format!("Failed to write comment to .gitignore: {}", e))?;
    writeln!(file, ".vac/session*")
        .map_err(|e| format!("Failed to write .vac/session* to .gitignore: {}", e))?;
