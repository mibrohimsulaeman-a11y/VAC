...
/// (up to 3 parent levels), falling back to the global `~/.vac/APPS.md`.
///
...
pub fn discover_apps_md(start_dir: &Path) -> Option<AppsMdInfo> {
    ...
    // Fall back to global ~/.vac/APPS.md
    if let Some(home) = std::env::home_dir() {
        let global_apps = home.join(".vac").join("APPS.md");
...
mod tests {
    ...
    fn test_discover_apps_md_not_found() {
        let temp_dir = TempDir::new().unwrap();
        let result = discover_apps_md(temp_dir.path());
        // May find global ~/.vac/APPS.md if it exists, otherwise None
        if let Some(info) = result {
            assert!(
                info.path.to_string_lossy().contains(".vac"),
