...
use vac_foundation::auth_manager::AuthManager;
use vac_foundation::models::auth::ProviderAuth;
use vac_foundation::models::integrations::anthropic::AnthropicConfig;
use vac_foundation::models::integrations::gemini::GeminiConfig;
...
use super::{VAC_API_ENDPOINT, VAC_CONFIG_PATH};
...
pub struct AppConfig {
    ...
    /// Whether to auto-append .vac to .gitignore
...
impl AppConfig {
    ...
    pub(crate) fn build(
        ...
        AppConfig {
            api_endpoint: std::env::var("VAC_API_ENDPOINT").unwrap_or(
                profile_config
                    .api_endpoint
                    .unwrap_or_else(|| VAC_API_ENDPOINT.into()),
            ),
            api_key: std::env::var("VAC_API_KEY").ok().or(profile_config.api_key),
    ...
    pub(crate) fn load_config_file<P: AsRef<Path>>(
        ...
        match std::fs::read_to_string(config_path.as_ref()) {
            Ok(content) => {
                ...
                let config_file = match toml::from_str::<ConfigFile>(&content) {
                    Ok(config_file) => config_file,
                    Err(new_format_error) => {
                        println!(
                            "Failed to parse config file in new format: {}",
                            new_format_error
                        );
                        match Self::migrate_old_config(config_path.as_ref(), &content) {
                            Ok(config_file) => config_file,
                            Err(old_format_error)
                                if Self::looks_like_foreign_agent_config(&content) =>
                            {
                                Self::archive_foreign_agent_config(
                                    config_path.as_ref(),
                                    &new_format_error,
                                    &old_format_error,
                                )?;
                                ConfigFile::with_default_profile()
                            }
                            Err(old_format_error) => return Err(old_format_error),
                        }
                    }
                };
    ...
    fn looks_like_foreign_agent_config(content: &str) -> bool {
        let Ok(value) = content.parse::<toml::Value>() else {
            return false;
        };
        let table = value.as_table();
        let has_foreign_model_provider = table
            .and_then(|table| table.get("model_provider"))
            .and_then(|value| value.as_str())
            .is_some();
        let has_foreign_model_providers_table = table
            .and_then(|table| table.get("model_providers"))
            .and_then(|value| value.as_table())
            .is_some_and(|providers| !providers.is_empty());
        has_foreign_model_provider || has_foreign_model_providers_table
    }

    fn archive_foreign_agent_config(
        config_path: &Path,
        new_format_error: &toml::de::Error,
        old_format_error: &ConfigError,
    ) -> Result<(), ConfigError> {
        ...
        let archive_path =
            config_path.with_extension(format!("toml.foreign-agent-backup-{timestamp}"));
        std::fs::rename(config_path, &archive_path).map_err(|error| {
            ConfigError::Message(format!(
                "Failed to archive unsupported agent config '{}' after VAC parse failed ({new_format_error}) and legacy parse failed ({old_format_error}): {error}",
                config_path.display()
            ))
        })?;
        eprintln!(
            "Warning: archived unsupported agent config '{}' to '{}'; starting VAC onboarding with a fresh config.",
            config_path.display(),
    ...
    pub fn get_config_dir(&self) -> PathBuf {
        ...
        // Default to ~/.vac/
        std::env::home_dir().unwrap_or_default().join(".vac")
    }
    ...
    pub async fn refresh_provider_auth_if_needed(
        ...
        use vac_foundation::oauth::{OAuthFlow, ProviderRegistry};
        ...
        let tokens = flow.refresh_token(refresh_token).await.map_err(|e| {
            format!(
                "Token refresh failed: {}. Please re-authenticate with 'vac auth login'.",
                e
            )
        })?;
    ...
    /// Get VAC API key with resolved credentials from auth.toml fallback chain.
    /// Returns None if the API key is empty or not set.
    pub fn get_vac_remote_service_key(&self) -> Option<String> {
        ...
        if let Some(ProviderAuth::Api { key }) = self.resolve_provider_auth("vac")
            && !key.is_empty()
        {
            return Some(key);
        }
    ...
    pub fn get_default_model(&self, cli_override: Option<&str>) -> vac_remote_service::Model {
        ...
        // First, find the model without VAC transform to determine its native provider
        let model = vac_remote_service::find_model(model_str, false).unwrap_or_else(|| {
            ...
            let (provider, model_id) = if let Some(idx) = model_str.find('/') {
                ...
                // Default to vac (which can resolve arbitrary model names)
                // if the user has a VAC key; otherwise fall back to anthropic
                let default_provider = if has_vac_key { "vac" } else { "anthropic" };
                (default_provider, model_str)
            };

            vac_remote_service::Model::custom(model_id.to_string(), provider)
        });
        ...
        // Transform for VAC routing only if:
        // 1. User has a VAC API key
        ...
        // If the user has a direct provider key, use it instead of routing through VAC.
        ...
        {
            return vac_remote_service::transform_for_vac(model);
        }
