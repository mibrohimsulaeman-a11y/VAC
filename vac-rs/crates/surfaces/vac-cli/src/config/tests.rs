...
[profiles.dev]
api_endpoint = "https://api.example.invalid/new"
api_key = "dev-key"
allowed_tools = ["read"]

[profiles.a]
api_endpoint = "https://new-api.vac.a"
api_key = "a-key"
...
fn sample_app_config(profile_name: &str) -> AppConfig {
    AppConfig {
        api_endpoint: "https://api.example.invalid/custom".into(),
        ...
        config_path: "/tmp/vac/config.toml".into(),
        allowed_tools: Some(vec!["git".into(), "curl".into()]),
        auto_approve: Some(vec!["git status".into()]),
        subagent: None,
        rulebooks: Some(RulebookConfig {
            include: Some(vec!["https://rules.example.invalid/security/*".into()]),
            exclude: Some(vec!["https://rules.example.invalid/internal/*".into()]),
...
fn create_test_rulebook(uri: &str, tags: Vec<String>) -> vac_remote_service::models::ListRuleBook {
    vac_remote_service::models::ListRuleBook {
...
fn get_config_path_returns_custom_path_when_provided() {
    let custom_path = PathBuf::from("/tmp/vac/custom.toml");
...
fn get_config_path_defaults_to_home_directory() {
    ...
    let expected = home_dir.join(VAC_CONFIG_PATH);
...
fn config_file_with_default_profile_contains_built_in_profile() {
    ...
    assert_eq!(default.api_endpoint.as_deref(), Some(VAC_API_ENDPOINT));
...
fn resolved_profile_config_merges_all_profile_defaults() {
    ...
    config.profiles.insert(
        "all".into(),
        ProfileConfig {
            api_endpoint: Some("https://api.example.invalid/shared".into()),
            api_key: Some("shared-key".into()),
            allowed_tools: Some(vec!["git".into()]),
            auto_approve: Some(vec!["git status".into()]),
            rulebooks: Some(RulebookConfig {
                include: Some(vec!["https://rules.example.invalid/shared/*".into()]),
                exclude: None,
                include_tags: None,
                exclude_tags: None,
            }),
    ...
    config.profiles.insert(
        "dev".into(),
        ProfileConfig {
            api_endpoint: Some("https://api.example.invalid/dev".into()),
            api_key: None,
            allowed_tools: None,
    ...
    assert_eq!(
        ...
        Some("https://api.example.invalid/dev")
    );
...
fn insert_and_set_app_config_update_profiles_and_settings() {
    ...
    assert_eq!(
        ...
        Some("https://api.example.invalid/custom")
    );
...
#[test]
fn load_config_file_archives_foreign_agent_config_for_onboarding() {
    let temp = tempfile::TempDir::new().unwrap();
    let path = temp.path().join("config.toml");
    std::fs::write(
        &path,
        r#"model_provider = "kilo"
model = "gpt-test"
...
"#,
    ...
    assert!(
        !path.exists(),
        "foreign config should be archived before onboarding fallback"
    );
    let archived: Vec<_> = std::fs::read_dir(temp.path())
        .unwrap()
    ...
    assert!(
        archived
            .iter()
            .any(|name| name.starts_with("config.toml.foreign-agent-backup-")),
        "archive not found: {archived:?}"
    );
}
...
fn load_config_file_for_new_formats() {
    ...
    assert_eq!(
        ...
        Some("https://api.example.invalid/new")
    );
...
fn save_writes_profile_and_settings() {
    ...
    assert_eq!(
        ...
        &vec!["https://rules.example.invalid/security/*".to_string()]
    );
    assert_eq!(
        rulebooks.exclude.as_ref().unwrap(),
        &vec!["https://rules.example.invalid/internal/*".to_string()]
    );
...
fn test_glob_pattern_matching() {
    assert!(RulebookConfig::matches_pattern(
        "https://rules.example.invalid/security/auth",
        "https://rules.example.invalid/security/*"
    ));

    assert!(RulebookConfig::matches_pattern(
        "https://rules.example.invalid/security/network",
        "https://rules.example.invalid/security/*"
    ));

    assert!(!RulebookConfig::matches_pattern(
        "https://rules.example.invalid/performance/v1",
        "https://rules.example.invalid/security/*"
    ));

    assert!(RulebookConfig::matches_pattern(
        "https://rules.example.invalid/performance/v2",
        "https://rules.example.invalid/performance/v2"
    ));
    ...
    assert!(RulebookConfig::matches_pattern(
        "https://rules.example.invalid/performance/v1",
        "https://rules.example.invalid/performance/v?"
    ));
}
...
fn test_rulebook_filtering_include_patterns() {
    let config = RulebookConfig {
        include: Some(vec![
            "https://rules.example.invalid/security/*".to_string(),
    ...
    let rulebooks = vec![
        create_test_rulebook("https://rules.example.invalid/security/auth", vec![]),
        create_test_rulebook("https://rules.example.invalid/performance/v1", vec![]),
    ...
    assert!(
        filtered
            .iter()
            .any(|r| r.uri == "https://rules.example.invalid/security/auth")
    );
    assert!(
        filtered
            .iter()
...
fn test_rulebook_filtering_exclude_patterns() {
    let config = RulebookConfig {
        include: None,
        exclude: Some(vec![
            "https://rules.example.invalid/*/beta".to_string(),
    ...
    let rulebooks = vec![
        create_test_rulebook("https://rules.example.invalid/security/stable", vec![]),
        create_test_rulebook("https://rules.example.invalid/security/beta", vec![]),
        create_test_rulebook("https://internal.company.com/team1/stable", vec![]),
        create_test_rulebook("https://experimental.rules.dev/test", vec![]),
    ];
    ...
    assert!(
        filtered
            .iter()
            .any(|r| r.uri == "https://rules.example.invalid/security/stable")
    );
    assert!(
        filtered
            .iter()
...
fn test_rulebook_filtering_include_tags() {
    ...
    let rulebooks = vec![
        create_test_rulebook(
            "https://rules.example.invalid/r1",
            vec!["security".to_string()],
        ),
        create_test_rulebook(
            "https://rules.example.invalid/r2",
            vec!["performance".to_string()],
        ),
        create_test_rulebook(
            "https://rules.example.invalid/r3",
            vec!["stable".to_string()],
        ),
        create_test_rulebook("https://rules.example.invalid/r4", vec!["beta".to_string()]),
    ];
    ...
    assert!(
        filtered
            .iter()
            .any(|r| r.uri == "https://rules.example.invalid/r1")
    );
    assert!(
        filtered
            .iter()
            .any(|r| r.uri == "https://rules.example.invalid/r3")
    );
}
...
fn test_rulebook_filtering_exclude_tags() {
    ...
    let rulebooks = vec![
        ...
        create_test_rulebook("https://rules.example.invalid/r2", vec!["beta".to_string()]),
        create_test_rulebook(
            "https://rules.example.invalid/r3",
            vec!["stable".to_string()],
        ),
        create_test_rulebook(
            "https://rules.example.invalid/r4",
...
"#
    ...
    match openai.backend {
        OpenAIBackendProfile::Codex(codex) => {
            assert_eq!(
                codex.base_url,
                vac_foundation::models::integrations::openai::OpenAIConfig::OPENAI_CODEX_BASE_URL
            );
            assert_eq!(codex.chatgpt_account_id, "acct_test_789");
            assert_eq!(codex.originator, "vac");
        }
...
"#
    ...
    assert!(matches!(
        ...
        vac_provider_core::types::OpenAIApiConfig::Responses(_)
    ));
}
