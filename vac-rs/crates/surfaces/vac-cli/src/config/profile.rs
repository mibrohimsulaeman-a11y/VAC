...
use vac_foundation::models::integrations::openai::OpenAIConfig;
...
pub struct ProfileConfig {
    ...
    /// Stores model IDs which may include provider prefix for VAC API routing
...
impl ProfileConfig {
    ...
    /// config `model` field's provider, or `"vac"` as a fallback.
    ///
    /// Entries with an unknown provider prefix (not a known cloud provider, not `"vac"`,
    /// and not explicitly configured in the user's `providers` HashMap) are re-mapped to
    /// `"vac/short_name"` so they route through the VAC API instead of failing
    ...
    pub fn migrate_recent_models(&mut self) -> bool {
        ...
        // and re-map unknown provider entries to "vac/short_name".
            ...
            .filter_map(|id| {
                if let Some((provider, _rest)) = id.split_once('/') {
                    if known_providers.contains(&provider) {
                        ...
                        // Unknown provider (e.g., "fireworks-ai/...") — re-map to vac
                        changed = true;
                        let short_name = id.rsplit('/').next().unwrap_or(id);
                        Some(format!("vac/{}", short_name))
                    }
...
/// like `"vac/glm-5"`, `"anthropic/claude-sonnet-4-5"`, or `"z.ai/glm-4.6"`.
