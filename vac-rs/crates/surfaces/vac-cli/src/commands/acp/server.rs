...
use vac_foundation::models::llm::LLMTokenUsage;
use vac_mcp_client::McpClient;
use vac_remote_service::models::ApiStreamError;
use vac_remote_service::storage::CreateSessionRequest;
use vac_remote_service::{AgentClient, AgentClientConfig, AgentProvider, VacCloudConfig};
use vac_remote_service::{Model, ModelLimit};

pub struct VACAcpAgent {
...
impl VACAcpAgent {
    ...
    pub async fn new(
        ...
        let client: Arc<dyn AgentProvider> = {
            let vac_remote_service_key = config.get_vac_remote_service_key();
            if vac_remote_service_key.is_none() {
                log::warn!("No VAC API key found. Running in local mode.");
            }

            // Use credential resolution with auth.toml fallback chain
            let vac = vac_remote_service_key.map(|api_key| VacCloudConfig {
                api_key,
        ...
        let model = if let Some(model_str) = &config.model {
            ...
            models.into_iter().next().unwrap_or_else(|| {
                // Fallback default: Claude Opus via VAC
                Model::new(
    ...
    pub async fn run_stdio(&self) -> Result<(), String> {
        ...
        local_set
            .run_until(async move {
                ...
                // Start up the VACAcpAgent connected to stdio.
                let (conn, handle_io) =
...
impl acp::Agent for VACAcpAgent {
    async fn initialize(
        &self,
        ...
        // Only advertise VAC auth if the user has no credentials at all
        // (no VAC API key AND no local provider keys configured)
        ...
        let auth_methods = if !has_any_credentials {
            vec![acp::AuthMethod::new(
                acp::AuthMethodId::new("vac"),
                "Login to VAC",
            )
            .description("Authenticate via browser to get your VAC API key. A browser window will open for you to sign in.")]
        } else {
            Vec::new()
        };
        ...
        Ok(acp::InitializeResponse::new(acp::ProtocolVersion::V1)
            .agent_info(acp::Implementation::new("vac", version).title("VAC Agent"))
            .agent_capabilities(
    ...
    async fn authenticate(
        ...
        // Handle VAC authentication via browser redirect (ACP Agent Auth)
        if method_id == "vac" {
            log::info!("VAC auth method selected, initiating browser-based authentication");
            ...
            // requests use the authenticated VAC provider
            {
                let config = self.config.read().await;
                let vac = Some(VacCloudConfig {
        ...
        // Legacy support: check for VAC_API_KEY environment variable
        if method_id == "github" {
            log::info!(
                "Legacy github auth method selected, checking for VAC_API_KEY environment variable"
            );
            match std::env::var("VAC_API_KEY") {
                Ok(_api_key) => {
                    log::info!("VAC_API_KEY found in environment");
                    return Ok(acp::AuthenticateResponse::new());
                }
                Err(_) => {
                    log::error!("VAC_API_KEY environment variable is not set");
                    return Err(
                        acp::Error::auth_required().data("VAC_API_KEY is not set. Use the 'vac' auth method for browser-based authentication.".to_string())
                    );
                }
            }
        }
    ...
    async fn new_session(
        ...
        // Check if we have valid credentials: either a VAC API key OR local provider keys
        // In-memory config is now always up-to-date since authenticate() updates it directly
        let config = self.config.read().await;
        let has_api_key = config.api_key.is_some() || std::env::var("VAC_API_KEY").is_ok() || {
        ...
        if !has_api_key && !has_provider_keys {
            ...
            return Err(acp::Error::auth_required().data(
                "Authentication required. Configure a provider with `vac auth login` or use the 'vac' auth method to authenticate via browser.".to_string()
            ));
        }
...
mod tests {
    ...
    use vac_remote_service::ModelLimit;
    ...
    fn test_model_to_acp_model_info_basic() {
        ...
        let model_info = VACAcpAgent::model_to_acp_model_info(&model);
    ...
    fn test_model_to_acp_model_info_with_provider_prefix() {
        ...
        assert!(model_info.description.as_ref().unwrap().contains("vac"));
    }
    ...
    fn test_session_model_state_creation() {
        ...
        let acp_models: Vec<ModelInfo> = models
            .iter()
            .map(VACAcpAgent::model_to_acp_model_info)
            .collect();
    ...
    fn test_model_selection_with_provider_prefixed_ids() {
        // Models with vac provider prefix format
        let models = [
            Model::new(
        ...
        assert_eq!(found.unwrap().provider, "vac");
    }
    ...
    fn test_new_session_response_serialization_with_models() {
        let models = [
            ModelInfo::new("anthropic/claude-sonnet-4-5", "Claude Sonnet 4.5")
                .description("Provider: vac".to_string()),
            ModelInfo::new("openai/gpt-4o", "GPT-4o").description("Provider: vac".to_string()),
        ];
