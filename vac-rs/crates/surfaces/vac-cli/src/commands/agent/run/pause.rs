...
use vac_foundation::models::async_manifest::{AsyncManifest, PauseReason};
use vac_foundation::models::integrations::openai::{ChatMessage, Role, ToolCall};
...
/// Write the async manifest to `.vac/session/pause.json`.
    ...
    match pause_reason {
        PauseReason::ToolApprovalRequired { pending_tool_calls } => {
            if pending_tool_calls.len() == 1 {
                ...
                format!("vac -c {} --approve-all", checkpoint_id)
            }
        }
        PauseReason::InputRequired => {
            format!("vac -c {} \"your input here\"", checkpoint_id)
        }
    }
}
