...
pub async fn validate_profile_switch(
    ...
    // Only inherit the VAC API key for remote profiles; local profiles use their own
    // provider credentials and inheriting a VAC key would cause model routing confusion
    // (e.g., use_vac=true would transform model IDs for VAC proxy format).
    ...
    for attempt in 1..=MAX_RETRIES {
        match client.get_my_account().await {
            ...
            Err(e) => {
                // If no VAC key, the local stub account is returned, so this should succeed
