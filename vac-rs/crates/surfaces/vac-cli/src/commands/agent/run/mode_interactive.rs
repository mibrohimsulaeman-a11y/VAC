...
use vac_remote_service::models::{ApiStreamError, Skill};
use vac_remote_service::{AgentClient, AgentClientConfig, AgentProvider, Model};
...
use vac_foundation::models::llm::{LLMTokenUsage, PromptTokensDetails};
use vac_foundation::secret_manager::SecretManager;
...
const INIT_PROMPT: &str =
    include_str!("../../../../../../integrations/vac-remote-service/src/prompts/init.v4.md");
use std::sync::Arc;
use uuid::Uuid;
use vac_foundation::telemetry::{TelemetryEvent, capture_event};
use vac_tui::{InputEvent, LoadingOperation, OutputEvent};
...
async fn send_next_tool_from_queue(
    ...
    if tool_name == "ask_user"
        && let Ok(request) = serde_json::from_str::<
            vac_foundation::models::integrations::openai::AskUserRequest,
...
pub struct RunInteractiveConfig {
    ...
    pub model: Model,
    /// When true, send init_prompt_content as first user message on session start (vac init)
    pub send_init_prompt_on_start: bool,
    /// Theme override: None = auto-detect, Some(theme) = use specified theme
    pub theme: Option<vac_tui::services::detect_term::Theme>,
}
...
pub async fn run_interactive(
    ...
    vac_tui::services::detect_term::init_theme(config.theme);
    ...
    'profile_switch_loop: loop {
        ...
        let api_key = ctx.get_vac_remote_service_key();
        let api_endpoint = ctx.api_endpoint.clone();
        let has_vac_key = api_key.is_some();
        ...
        let rulebook_config_for_tui = ctx.rulebooks.clone().map(|rb| vac_tui::RulebookConfig {
            include: rb.include,
            exclude: rb.exclude,
        ...
        // Always run discovery probes so both `vac init` and `/init` get pre-calculated analysis results.
        ...
        let banner_message = if agent_context
            ...
            .unwrap_or(true)
        {
            Some(vac_tui::BannerMessage::persistent_with_action(
                "Run /init to auto-discover your apps, infra, and dependencies. VAC works better with context.",
                vac_tui::BannerStyle::Info,
                "/init",
            ))
        } else {
            None
        ...
        let client_handle: tokio::task::JoinHandle<ClientTaskResult> = tokio::spawn(async move {
            ...
            if let Some(ref key) = api_key_for_client {
                client_config = client_config.with_vac(
                    vac_remote_service::VacCloudConfig::new(key.clone())
            ...
            // Fetch billing info (only when VAC API key is present)
            if has_vac_key {
            ...
            if config.plan_mode {
                ...
                if vac_tui::services::plan::plan_file_exists(session_dir) {
                    // Existing plan found — let the TUI show the modal
                    let meta = vac_tui::services::plan::read_plan_file(session_dir).map(|(m, _)| m);
                    send_input_event(
                        &input_tx,
                        InputEvent::ExistingPlanFound(vac_tui::ExistingPlanPrompt {
                            inline_prompt: None,
                            metadata: meta,
                        }),
                    )
            ...
            while let Some(output_event) = output_rx.recv().await {
                match output_event {
                    OutputEvent::SwitchToModel(new_model) => {
                        ...
                        let has_direct_provider_key = ctx_clone
                            ...
                            .is_some();
                        let should_transform = has_vac_key
                            && !has_direct_provider_key
                            && new_model.provider != "vac"
                            && known_cloud_providers.contains(&new_model.provider.as_str());

                        model = if should_transform {
                            vac_remote_service::transform_for_vac(new_model.clone())
                        } else {
                            new_model.clone()
                        };
                    ...
                    OutputEvent::UserMessage(
                        ...
                        let user_msg = if image_parts.is_empty() {
                            ...
                            if !redacted_user_input.trim().is_empty() {
                                parts.push(
                                    vac_foundation::models::integrations::openai::ContentPart {
                        ...
                        // Capture telemetry when not using VAC API (local mode)
                        if !has_vac_key
                                ...
                                Ok(_) => {
                                    ...
                                    send_input_event(
                                        ...
                                        InputEvent::ToolResult(
                                            vac_foundation::models::integrations::openai::ToolCallResult {
                ...
                match response_result {
                    Ok(response) => {
                        ...
                        // Refresh billing info after each assistant message (only when using VAC API)
                        if has_vac_key {
                            refresh_billing_info(client.as_ref(), &input_tx).await;
                        }
        ...
        if let Some(new_config) = profile_switch_config {
            ...
            if let Some(api_key) = new_config.get_vac_remote_service_key() {
                new_client_config = new_client_config.with_vac(
            ...
            if should_use_warden {
                ...
                if let Err(e) =
                    warden::run_vac_in_warden(ctx, &std::env::args().collect::<Vec<_>>()).await
                {
                    return Err(format!("Failed to run vac in warden: {}", e));
                }
        ...
        if let Some(api_key) = ctx.get_vac_remote_service_key() {
            final_client_config = final_client_config.with_vac(
                vac_remote_service::VacCloudConfig::new(api_key)
...
https://vastar.ai/{}/agent-sessions/{}",
                username, session_id
            );
        }
...
mod tests {
    ...
    fn test_tool_call(id: &str) -> ToolCall {
        ToolCall {
            ...
            function: vac_foundation::models::integrations::openai::FunctionCall {
                name: format!("{}_fn", id),
                arguments: "{}".to_string(),
            },
            metadata: None,
