...
use vac_remote_service::storage::ListSessionsQuery;
use vac_tui::SessionInfo;
    ...
    if tool_exists {
        ...
        let metadata = Some({
            ...
            if let Some(session_id) = session_id {
                ...
                // Legacy key — consumed by the local vac MCP server (ctx.meta.get("session_id"))
                meta.insert(
        ...
        let handle = match vac_mcp_client::call_tool(
            mcp_client,
