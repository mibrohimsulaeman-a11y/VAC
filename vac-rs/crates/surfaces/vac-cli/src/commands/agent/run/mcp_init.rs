...
use vac_foundation::cert_utils::CertificateChain;
use vac_foundation::models::integrations::openai::ToolCallResultProgress;
...
use vac_mcp_server::{EnabledToolsConfig, MCPServerConfig, SubagentConfig, ToolMode, start_server};
use vac_remote_service::local::skills::default_skill_directories;
...
pub struct McpInitResult {
    ...
    pub tools: Vec<vac_foundation::models::integrations::openai::Tool>,
...
fn build_proxy_config(
    ...
    // id is forwarded via the JSON-RPC `_meta["dev.vac/session-id"]` field
    // (set in vac-rs/crates/surfaces/vac-cli/src/commands/agent/run/tooling.rs), not as an HTTP header.
    ...
    if let Ok(config_path) = vac_mcp_config::find_config_file() {
        match load_external_servers(&config_path) {
            Ok(external_servers) => {
                let mut loaded_servers = 0;
                for (name, config) in external_servers {
                    if name == "vac" || name == "paks" || name == "aap" {
                        tracing::warn!(
                            "Skipping external MCP server {} (reserved for vac's internal use)",
                            name
                        );
                        continue;
                    }
...
fn load_external_servers(config_path: &str) -> Result<HashMap<String, ServerConfig>, String> {
    let config = vac_mcp_config::load_config(config_path.as_ref())?;
...
async fn connect_to_proxy(
    ...
    for attempt in 1..=MAX_RETRIES {
        match vac_mcp_client::connect_https(
            proxy_url,
...
pub async fn initialize_mcp_server_and_tools(
    ...
    let mcp_tools = vac_mcp_client::get_tools(&mcp_client)
        .await
