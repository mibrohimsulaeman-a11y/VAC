...
use vac_tui::{InputEvent, LoadingOperation};

/// Accumulates streaming tool call deltas into complete tool calls.
///
/// Handles two different streaming behaviors:
/// - **ID-based matching**: When delta has an ID, match by ID only (used by Anthropic/VAC Provider Core)
/// - **Index-based matching**: When delta has no ID, fall back to index (used by OpenAI)
///
/// This distinction is important because some providers (like Anthropic via VAC Provider Core adapter)
        ...
        match &response {
            Ok(response) => {
                ...
                if !response.model.is_empty() {
                    ...
                    // Use false for use_vac since the response already has the resolved model ID
                    let model = vac_remote_service::find_model(&response.model, false)
...
mod tests {
    ...
    async fn test_consecutive_tool_calls_with_same_index_different_ids() {
        ...
        // Simulate two tool calls with index 0 but different IDs (provider-core behavior)
        let responses = vec![
