...
use uuid::Uuid;
use vac_foundation::local_store::LocalStore;
use vac_foundation::models::async_manifest::{AsyncManifest, PauseReason, PendingToolCall};
use vac_foundation::models::integrations::openai::{ChatMessage, MessageContent, Role};
...
use vac_remote_service::{AgentClient, AgentClientConfig, AgentProvider, Model, SessionStorage};
...
impl AsyncAutoApproveConfig {
    fn new(auto_approve_tools: Option<&Vec<String>>) -> Self {
        ...
        let config_path = std::path::Path::new(".vac/session/auto_approve.json");
...
pub async fn run_async(ctx: AppConfig, mut config: RunAsyncConfig) -> Result<AsyncOutcome, String> {
    ...
    print!("{}", renderer.render_title("VAC Agent - Async Mode"));
    print!(
        "{}",
    ...
    let mut plan_previous_status: Option<vac_tui::services::plan::PlanStatus> = None;

    // If plan mode, handle existing plan file
    if config.plan_mode && config.plan_new {
        let session_dir = std::path::Path::new(".vac/session");
        if let Some(archive_path) = vac_tui::services::plan::archive_plan_file(session_dir) {
            print!(
                "{}",
                renderer.render_info(&format!(
    ...
    if config.plan_mode && !chat_messages.is_empty() {
        ...
        if let Some(last_user_msg) = chat_messages
            ...
            && let Some(vac_foundation::models::integrations::openai::MessageContent::String(
                ref mut text,
    ...
    loop {
        ...
        if config.plan_mode {
            let session_dir = std::path::Path::new(".vac/session");
            if let Some((meta, _content)) = vac_tui::services::plan::read_plan_file(session_dir) {
                ...
                if plan_previous_status != Some(new_status) {
                    ...
                    match new_status {
                        vac_tui::services::plan::PlanStatus::PendingReview => {
                        ...
                        vac_tui::services::plan::PlanStatus::Approved => {
                            if old == Some(vac_tui::services::plan::PlanStatus::PendingReview) {
                                print!(
                                    "{}",
                                    renderer.render_info(
                        ...
                        vac_tui::services::plan::PlanStatus::Drafting => {
...
mod tests {
    ...
    fn build_resume_command_prefers_session_id() {
        ...
        assert_eq!(resume_command, Some(format!("vac -s {}", session_id)));
    }
    ...
    fn build_resume_command_uses_checkpoint_when_no_session() {
        ...
        assert_eq!(resume_command, Some(format!("vac -c {}", checkpoint_id)));
    }
    ...
    fn auto_approve_canonicalizes_legacy_tool_names() {
        ...
        assert!(
            config.should_auto_approve("vac__read_rulebook"),
    ...
    fn auto_approve_canonicalizes_prefixed_prompt_tool_names() {
        let config = AsyncAutoApproveConfig::new(None);
        assert!(
            !config.should_auto_approve("vac__run_command()"),
