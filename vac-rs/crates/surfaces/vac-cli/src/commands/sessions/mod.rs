//! `vac sessions` — list and inspect past sessions.
//!
...
//! with whatever profile backend is configured (SQLite or VAC API) and
...
use vac_remote_service::{
    AgentClient, ListSessionsQuery, SessionStorage, StorageError, VacCloudConfig,
};
...
pub enum SessionsCommands {
    ...
    ///
    /// Uses the VAC API when the active profile has an API key, otherwise the local SQLite store; honours `--profile`.
    List {
...
async fn build_storage(config: &AppConfig) -> Result<Arc<dyn SessionStorage>, String> {
    ...
    AgentClient::build_session_storage(vac, None, Some(config.profile_name.clone())).await
}
