//! End-to-end tests for `vac sessions` driven against an in-memory
...
fn remote_backend() -> BackendInfo {
    BackendInfo::vac_remote_service(Some("dev".to_string()), "https://api.vac.test")
}

fn render_list(sessions: &[vac_remote_service::SessionSummary], mode: OutputMode) -> String {
...
// =============================================================================
// 5.2 — `vac sessions list --json` end-to-end
// =============================================================================
...
fn render_show_human_includes_remote_backend_header() {
    ...
    assert!(
        rendered.starts_with(
            "Backend: vac-remote-service (profile: dev, endpoint: https://api.vac.test)\n"
        ),
...
// =============================================================================
// 5.3 — `vac sessions show <id> --role assistant --limit 1 --json` end-to-end
// =============================================================================
...
fn render_show_human_resume_hint_omits_default_profile() {
    ...
    assert!(rendered.contains(&format!("Resume: vac --session {}", session.id)));
...
fn render_show_human_resume_hint_includes_non_default_profile() {
    ...
    assert!(rendered.contains(&format!(
        "Resume: vac --profile local --session {}",
        session.id
    )));
}
