//! Human + JSON renderers for the `vac sessions` subcommands.

use serde::Serialize;
use vac_foundation::models::integrations::openai::ChatMessage;
...
fn render_show_human(
    ...
    let resume = match profile {
        Some(p) if !p.is_empty() && p != "default" => {
            format!("vac --profile {} --session {}", p, session.id)
        }
        _ => format!("vac --session {}", session.id),
    };
    out.push_str(&format!("\nResume: {}\n", resume));
...
fn render_backend_header(backend: &BackendInfo) -> String {
    match (
        ...
        (BackendKind::VACApi, Some(profile), Some(endpoint), _) => {
            format!("Backend: vac-remote-service (profile: {profile}, endpoint: {endpoint})")
        }
        (BackendKind::VACApi, None, Some(endpoint), _) => {
            format!("Backend: vac-remote-service (endpoint: {endpoint})")
        }
