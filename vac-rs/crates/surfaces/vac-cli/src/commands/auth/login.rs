            ...
            >| {
                ...
                async move {
                    ...
                    let (body, status) = match result {
                        Ok(_) => (
                            "Authentication complete. You can return to VAC.",
                            axum::http::StatusCode::OK,
                        ),
                        Err(_) => (
                            "Authentication failed. You can return to VAC for details.",
...
async fn handle_non_interactive_api_setup(
    ...
    let mut profile_config = match provider_id {
        "vac" => {
            // VAC API key -> Remote provider (key stored in config.toml)
            ProfileConfig {
        ...
        "github-copilot" => {
            return Err("GitHub Copilot does not support API key authentication.\n\
                ...
                • Interactive login:\n\
                vac auth login --provider github-copilot\n\n\
                • Provide an OAuth access token:\n\
                vac auth login --access <ACESS_TOKEN>"
                .to_string());
        }
        _ => {
            return Err(format!(
                "Unsupported provider '{}'. Supported: anthropic, openai, gemini, openrouter, vac, amazon-bedrock, github-copilot\n\
                 For bedrock, use: vac auth login --provider amazon-bedrock --region <region>\n\
...
async fn handle_bedrock_setup(
    ...
    println!();
    println!("No AWS credentials are stored by vac.");
...
mod tests {
    ...
    #[tokio::test]
    async fn non_interactive_vac_sets_profile_api_endpoint() {
        let temp_dir = temp_dir();
