...
use vac_foundation::oauth::ProviderRegistry;

/// Handle the auth status command.
///
/// Prints configured profiles/providers (mirroring `auth list`), highlights the
/// currently active profile, and pings the VAC API to verify reachability
...
pub async fn handle_status(
    ...
    if all_credentials.is_empty() {
        ...
        println!();
        println!("Run 'vac auth login' to add credentials.");
        return Ok(());
    }
    ...
    let active_vac_auth = all_credentials
        .get(probe_profile)
        .and_then(|providers| providers.get("vac"))
        .or_else(|| {
            all_credentials
                .get("all")
                .and_then(|providers| providers.get("vac"))
    ...
    let Some(auth) = active_vac_auth else {
        println!(
            "API check:      skipped (no vac credential on profile '{}')",
            probe_profile
        );
        return Ok(());
    };
    ...
    if auth.is_expired() {
        let msg = "access token expired (run `vac auth login`)";
