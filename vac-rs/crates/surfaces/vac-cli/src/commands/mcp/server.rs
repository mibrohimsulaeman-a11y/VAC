...
pub async fn run_server(
    ...
    let (bind_address, listener) = if let Ok(fixed_port) = std::env::var("VAC_MCP_PORT") {
        let port: u16 = fixed_port
            .parse()
            .map_err(|e| format!("Invalid VAC_MCP_PORT '{fixed_port}': {e}"))?;
    ...
    let (certificate_chain, server_tls_config) =
        if let Ok(trusted_client_ca_pem) = std::env::var(TRUSTED_CLIENT_CA_ENV) {
            ...
            println!("---BEGIN VAC SERVER CA---");
            println!("{server_ca_pem}");
            println!("---END VAC SERVER CA---");
    ...
    println!(
        "⚠️  Secret redaction is handled by the proxy layer. Run behind 'vac mcp proxy' for secret protection."
    );
