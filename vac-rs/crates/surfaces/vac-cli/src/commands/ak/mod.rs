...
use std::rc::Rc;
use vac_ak::search::SearchEngine;
use vac_ak::skills::{SKILL_MAINTAIN, SKILL_RETROSPECT, SKILL_USAGE};
use vac_ak::{
    GrepResult, LocalFsBackend, PeekResult, RemoteBackend, StorageBackend, TreeNavEngine,
};
use vac_remote_service::vac::VACApiConfig;
...
default store root: ~/.vac/knowledge
override: AK_STORE
...
pub const AK_AFTER_HELP: &str = "Examples:
  vac ak search
  vac ak search services --tree
  vac ak search --glob 'services/**/*.md'
  vac ak search --grep 'rate.limit'
  vac ak search --grep 'rate.limit' --glob '**/*.md'
  vac ak read services/rate-limits.md
  vac ak read services/rate-limits.md services/auth-flow.md
  echo 'Rate limit is 1000/min' | vac ak write services/rate-limits.md
  vac ak write notes.md --file /tmp/notes.md
  vac ak remove services/rate-limits.md";
...
Default output is peek body per file (frontmatter, if present, plus the first paragraph). Use `--tree` for structure-only output, `--glob` to filter by path pattern, and `--grep` to filter by content regex. `--grep` matches frontmatter too. `-i` makes `--grep` case-insensitive.",
        after_help = "Examples:
  vac ak search
  vac ak search services
  vac ak search services --tree
  vac ak search --glob 'services/**/*.md'
  vac ak search --grep 'rate.limit'
  vac ak search --grep 'rate.limit' -i
  vac ak search --grep 'rate.limit' --glob '**/*.md'"
    )]
    Search {
...
- paths are relative to the store root",
  ...
  echo 'Rate limit is 1000/min' | vac ak write services/rate-limits.md
  vac ak write notes.md --file /tmp/notes.md
  vac ak write --force summaries/auth-overview.md"
    )]
    Write {
...
Use `usage` to teach an agent how to navigate and write to the store. Use `maintain` to teach an agent how to audit, deduplicate, and clean up stored knowledge. Use `retrospect` to teach an agent how to turn past sessions into durable entries in the store (pipe its output into `vac autopilot schedule add --prompt ...` to run it on cron)."
    )]
    Skill {
...
fn run_write(
    ...
    if force {
        ...
        backend.as_ref().create(&path, &content).map_err(|error| match error {
            vac_ak::Error::AlreadyExists(existing) => {
                format!(
                    "destination already exists: {}. next action: choose a new path or rerun with `vac ak write --force {path}` if overwrite is intentional",
                    existing.display()
                )
            }
...
fn run_remove(backend: Rc<dyn StorageBackend>, path: &str) -> Result<(), String> {
    backend.as_ref().remove(path).map_err(|error| match error {
        vac_ak::Error::NotFound(missing) => format!("path not found: {}", missing.display()),
        other => other.to_string(),
    })?;
...
fn ensure_path_is_not_directory(
    ...
    match backend.as_ref().list(path) {
        ...
        Err(vac_ak::Error::NotADirectory(_) | vac_ak::Error::NotFound(_)) => Ok(()),
...
/// - If VAC API key is available, use RemoteBackend
...
fn create_backend(config: &AppConfig) -> Result<Rc<dyn StorageBackend>, String> {
    ...
    if let Some(api_key) = config.get_vac_remote_service_key()
        && !api_key.is_empty()
    {
        let api_config = VACApiConfig::new(api_key).with_endpoint(config.api_endpoint.clone());
...
mod tests {
    ...
    fn unknown_skill_error_lists_valid_values() {
        ...
        let config = AppConfig::load("default", Some(config_path.as_path())).expect("load config");
