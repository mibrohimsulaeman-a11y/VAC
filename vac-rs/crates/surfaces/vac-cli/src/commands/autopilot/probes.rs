...
fn docker_install_remediation(os_id: Option<&str>) -> (String, String) {
    match os_id {
        Some("amzn" | "rhel" | "fedora" | "rocky" | "almalinux" | "centos") => (
            "Install Docker, then rerun vac up".to_string(),
        ...
        _ => (
            "Install Docker for your distribution, then rerun vac up".to_string(),
...
pub fn probe_credentials(ctx: &AutopilotProbeContext<'_>) -> ProbeResult {
    let has_vac_key = ctx.app_config.get_vac_remote_service_key().is_some();
    ...
    if has_vac_key || has_provider_keys {
        ...
        ProbeResult {
            ...
            details: Some(
                "Autopilot needs either a VAC API key or a configured local provider.".to_string(),
            ),
            remediation: Some(Remediation::Manual {
                summary: "Authenticate before starting autopilot".to_string(),
                command: Some("vac auth login --api-key <key>".to_string()),
            }),
        }
    }
}
    ...
    match env.can_bind_addr(bind_addr) {
        ...
        Err(error) => ProbeResult {
            ...
            remediation: Some(Remediation::Manual {
                ...
                command: Some("vac down && systemctl --user status vac-autopilot".to_string()),
            }),
        },
    }
}
...
pub fn probe_sandbox_mount_inputs(
    ...
    for volume in volumes {
        let expanded = vac_foundation::container::expand_volume_path(&volume);
        let (host_path, container_path) = match parse_volume_mapping(&expanded) {
            Some(parts) => parts,
            None => continue,
        };

        if vac_foundation::container::is_named_volume(host_path)
    ...
    ProbeResult {
        ...
        remediation: Some(Remediation::Suggested {
            summary:
                "Verify these files/directories are readable by the invoking user. VAC maps the host UID/GID into the sandbox; do not loosen secret file permissions globally."
                    .to_string(),
        }),
    }
}
...
mod tests {
    ...
    fn sandbox_mount_inputs_warn_when_critical_mount_is_unreadable() {
        ...
        config.warden = Some(WardenConfig {
            enabled: true,
            volumes: vec!["/tmp/vac-config.toml:/home/agent/.vac/config.toml:ro".to_string()],
        });

        let env = MockProbeEnvironment::default()
            .with_path_access("/tmp/vac-config.toml", Err("Permission denied".to_string()));
        let ctx = AutopilotProbeContext {
            app_config: &config,
            bind_addr: None,
            server_reachable: false,
        ...
        assert!(details.contains("/tmp/vac-config.toml"));
