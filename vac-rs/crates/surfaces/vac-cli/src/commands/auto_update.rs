...
fn is_vac_homebrew_install() -> bool {
...
fn update_success_message(version: &str) -> String {
    format!(
        "✓ Updated to {}. Restart any long-running vac processes to pick up the new binary.",
        version
    )
}
...
fn update_via_brew_with_command(
    ...
    if !upgrade_status.success() {
        return Err("brew upgrade vac failed".to_string());
    }
...
fn is_current_binary_homebrew_managed() -> Result<bool, String> {
    ...
    // Get Homebrew's vac path
    match Command::new("brew").arg("--prefix").arg("vac").output() {
        Ok(output) if output.status.success() => {
            let homebrew_path_lossy = String::from_utf8_lossy(&output.stdout);
            let homebrew_path = homebrew_path_lossy.trim();
            let homebrew_binary = std::path::Path::new(homebrew_path).join("bin").join("vac");
        ...
        Ok(_) => {
            // brew --prefix vac failed (vac not installed via brew)
            Ok(false)
        }
        Err(_) => {
...
mod tests {
    ...
    fn failed_binary_update_removes_downloaded_binary() {
        ...
        let missing_current_exe = temp_dir.path().join("missing-vac");
        let extracted_binary = temp_dir.path().join("vac_downloaded");
    ...
    fn atomic_binary_update_returns_ok_without_exiting() {
        let temp_dir = TempDir::new().expect("temp dir");
        let current_exe = temp_dir.path().join("vac");
        let extracted_binary = temp_dir.path().join("vac_downloaded");
    ...
    fn brew_update_returns_ok_without_exiting() {
        ...
        assert!(log.contains("upgrade vac"));
    }
}
