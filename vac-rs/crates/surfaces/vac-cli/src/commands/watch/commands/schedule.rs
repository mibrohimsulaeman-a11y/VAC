...
pub async fn show_schedule(name: &str) -> Result<(), String> {
        ...
        && let Ok(db) = ScheduleDb::new(db_path_str).await
    {
            ...
            && !runs.is_empty()
        {
            ...
            println!();
            println!(
                "Use 'vac autopilot schedule history {}' to inspect a run.",
                schedule.name
            );
        }
    }
...
pub async fn fire_schedule(name: &str, dry_run: bool) -> Result<(), String> {
    ...
    match autopilot_state {
        None => {
            return Err(
                "Autopilot is not running. Start it with 'vac autopilot up' first, or use --dry-run to preview."
                    .to_string(),
            );
        }
        Some(state) => {
            ...
            if !is_process_running(state.pid as u32) {
                return Err(
                    "Autopilot is not running (stale state). Start it with 'vac autopilot up' first, or use --dry-run to preview."
                        .to_string(),
                );
            }
        }
    ...
    println!(
        "  Use 'vac autopilot schedule history {}' to monitor progress.",
        name
    );
