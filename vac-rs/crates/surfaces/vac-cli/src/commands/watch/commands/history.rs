...
pub async fn show_run(run_id: i64) -> Result<(), String> {
    ...
    if run.agent_woken {
            ...
            && !stdout.trim().is_empty()
        {
            if let Some(manifest) =
                vac_foundation::models::async_manifest::AsyncManifest::try_parse(stdout)
            {
                // Outcome
                ...
                if let Some(pause_reason) = &manifest.pause_reason {
                    ...
                    match pause_reason {
                        vac_foundation::models::async_manifest::PauseReason::ToolApprovalRequired { pending_tool_calls } => {
                            println!("  Waiting for tool approval:");
                            for tc in pending_tool_calls {
                                println!("    • {} ({})", tc.name, tc.id);
                            }
                        }
                        vac_foundation::models::async_manifest::PauseReason::InputRequired => {
        ...
        && (run.status == RunStatus::Failed || run.status == RunStatus::TimedOut)
    {
        println!();
        println!(
            "\x1b[33mTip: Resume this run with 'vac autopilot schedule resume {}'\x1b[0m",
            run.id
        );
    }
