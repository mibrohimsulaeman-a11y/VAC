...
//!
//! Spawns the vac agent via the co-hosted agent server API.

use crate::commands::agent::run::pause::EXIT_CODE_PAUSED;
use std::collections::{HashMap, HashSet};
use std::time::Duration;
use tracing::{debug, info, warn};
use vac_foundation::models::async_manifest::PauseReason;
use vac_messaging_gateway::client::{
    CallerContextInput, ClientError, RunOverrides, SendMessageOptions, ToolDecisionAction,
    ToolDecisionInput, VACClient,
};
...
/// Spawn the vac agent via the co-hosted agent server API.
///
...
pub async fn spawn_agent(config: SpawnConfig) -> Result<AgentResult, AgentError> {
    ...
    let client = VACClient::new(server.url.clone(), server.token.clone());
...
fn build_tool_decisions(
    ...
    tool_calls
        ...
        .map(|(tool_call_id, tool_name)| {
            let normalized = vac_broker::strip_tool_prefix(tool_name).to_string();
...
async fn run_server_session(
    ...
    let message = vac_provider_core::Message {
        role: vac_provider_core::Role::User,
        content: vac_provider_core::MessageContent::Text(config.prompt.clone()),
        name: None,
    ...
    loop {
        ...
        if let Some(proposed) = event.as_tool_calls_proposed() {
            if config.pause_on_approval {
                paused = true;
                let pending: Vec<vac_foundation::models::async_manifest::PendingToolCall> =
                    proposed
                        .tool_calls
                        .iter()
                        .map(
                            |tc| vac_foundation::models::async_manifest::PendingToolCall {
                                id: tc.id.clone(),
                                name: tc.name.clone(),
...
mod tests {
    ...
    fn test_tool_decision_accepts_listed() {
        ...
        let tool_calls = vec![("1".to_string(), "vac__view".to_string())];
    ...
    fn test_tool_decision_rejects_unlisted() {
        ...
        let tool_calls = vec![("1".to_string(), "vac__run_command".to_string())];
        let decisions = build_tool_decisions(&tool_calls, &allowed_tools);
    ...
    fn test_agent_result_paused() {
        let result = AgentResult {
            ...
            resume_hint: Some("vac -c test-checkpoint --approve-all".to_string()),
            stdout: String::new(),
            stderr: String::new(),
        };
