...
pub const VAC_AUTOPILOT_CONFIG_PATH: &str = "~/.vac/autopilot.toml";
...
fn default_db_path() -> String {
    "~/.vac/autopilot/autopilot.db".to_string()
}

fn default_log_dir() -> String {
    "~/.vac/autopilot/logs".to_string()
}
...
impl ScheduleConfig {
    /// Load configuration from the default path (~/.vac/autopilot.toml).
    pub fn load_default() -> Result<Self, ConfigError> {
        let path = expand_tilde_for_path(Path::new(VAC_AUTOPILOT_CONFIG_PATH))?;
        Self::load(&path)
    }
    ...
    ///
    /// The gateway auth token is generated fresh each `vac up` and never
...
mod tests {
    ...
    fn test_parse_valid_config() {
        let config_str = r#"
[watch]
db_path = "~/.vac/autopilot/autopilot.db"
log_dir = "~/.vac/autopilot/logs"
...
"#;
        ...
        assert_eq!(config.watch.db_path, "~/.vac/autopilot/autopilot.db");
