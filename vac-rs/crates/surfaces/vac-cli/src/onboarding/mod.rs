...
//! through setting up their VAC configuration, including:
//! - VAC API authentication (OAuth flow)
...
pub async fn run_onboarding(config: &mut AppConfig, mode: OnboardingMode) {
    let profile_name = match mode {
        OnboardingMode::Default => {
            ...
            print!("\r\n");
            crate::onboarding::styled_output::render_title("Welcome to VAC");
            print!("\r\n");
            render_profile_name(&profile);
            print!("\r\n");
            crate::onboarding::styled_output::render_info(
                "Configuring vac. You can connect to VAC API or use your own model/API keys.",
            );
            print!("\r\n");
    ...
    // Initial decision: VAC API or Own Keys
    loop {
        print!("\x1b[u");
        print!("\x1b[0J");
        print!("\x1b[K");
        ...
        let initial_choice = select_option(
            ...
            &[
                (InitialChoice::VacApi, "Use VAC API (recommended)", true),
                (
        ...
        match initial_choice {
            NavResult::Forward(InitialChoice::VacApi) => {
                print!("\x1b[u");
                print!("\x1b[0J");
                let _ = io::stdout().flush();
                ...
                match mode {
                    ...
                    OnboardingMode::New => {
                        handle_vac_remote_service_for_new_profile(config, &profile_name).await;
                    }
                }
                break;
            }
...
/// Handle VAC API setup for a new profile
/// Saves API key to the new profile, copying endpoint from default but using new API key
async fn handle_vac_remote_service_for_new_profile(config: &AppConfig, profile_name: &str) {
    use crate::apikey_auth::prompt_for_api_key;
