...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_vac_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let custom_catalog_path = vac_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(vac_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        vac_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let vac = if let Ok(path) = vac_utils_cargo_bin::cargo_bin("vac") {
        path
    } else {
        ...
        if fallback.is_file() {
            ...
            eprintln!("skipping integration test because vac binary is unavailable");
            return Ok(());
        }
    };

    let exec_output = std::process::Command::new(&vac)
        .arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(&repo_root)
        .arg("seed session for resume")
        .env("VAC_HOME", vac_home.path())
        .env("VASTAR_API_KEY", "dummy")
        .env("VAC_RS_SSE_FIXTURE", fixture_path)
        .output()
        .context("failed to execute vac exec")?;
    anyhow::ensure!(
    ...
    env.insert(
        "VAC_HOME".to_string(),
        vac_home.path().display().to_string(),
    );
    env.insert("VASTAR_API_KEY".to_string(), "dummy".to_string());
    ...
    let spawned = vac_utils_pty::spawn_pty_process(
        vac.to_string_lossy().as_ref(),
        &args,
        &repo_root,
        &env,
        &None,
        vac_utils_pty::TerminalSize::default(),
    )
    .await?;
    ...
    let mut output_rx = vac_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for vac resume to exit");
        }
    };
    ...
    anyhow::ensure!(
        ...
        "unexpected exit code from vac resume: {exit_code}; output: {output_text}",
    );

    let config_contents = std::fs::read_to_string(vac_home.path().join("config.toml"))?;
