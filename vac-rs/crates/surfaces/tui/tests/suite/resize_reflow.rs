...
#[test]
#[ignore = "requires tmux and a locally built vac binary; run with --ignored for manual resize smoke"]
fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let vac = vac_binary(&repo_root)?;
    let vac_home = tempdir()?;
    ...
    write_auth(vac_home.path())?;

    let session_name = format!("vac-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("VAC_HOME={}", vac_home.path().display()))
            .arg("VASTAR_API_KEY=dummy")
            .arg(format!("VAC_RS_SSE_FIXTURE={}", fixture.display()))
            .arg(vac)
            .arg("-c")
    ...
    let vac_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!vac_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&vac_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&vac_pane)?;
    let final_row =
...
fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("vac-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&vac_pane)?;
    let restored_row =
...
fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let fixture = fixture_dir.path().join("resize-reflow.sse");
    write_fixture(&fixture)?;
    write_config(vac_home.path(), &repo_root, terminal_resize_reflow_enabled)?;
    write_auth(vac_home.path())?;
    ...
    let session_name = format!("vac-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn vac_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = vac_utils_cargo_bin::cargo_bin("vac") {
        return Ok(path);
    }

    let fallback = repo_root.join("vac-rs/target/debug/vac");
    anyhow::ensure!(
        fallback.is_file(),
        "vac binary is unavailable; run `cargo build -p vac-surface-cli` first"
    );
    Ok(fallback)
}
...
"#
    ...
    std::fs::write(vac_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(vac_home: &Path) -> Result<()> {
    std::fs::write(
        vac_home.join("auth.json"),
        r#"{"VASTAR_API_KEY":"dummy","tokens":null,"last_refresh":null}"#,
    )?;
    Ok(())
}
