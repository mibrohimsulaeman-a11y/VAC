...
pub(crate) struct CommandPopup {
    ...
    route_catalog: crate::surface_route_catalog::SurfaceRouteCatalog,
    state: ScrollState,
}
...
impl CommandPopup {
    pub(crate) fn new(flags: CommandPopupFlags) -> Self {
        let route_catalog = std::env::current_dir()
            .ok()
            .map(crate::surface_route_catalog::SurfaceRouteCatalog::load)
            .unwrap_or_default();
        // Keep built-in availability in sync with the composer, then drop commands
        // that are not declared in the slash surface manifest.
        let builtins: Vec<(&'static str, SlashCommand)> =
            slash_commands::builtins_for_input_with_catalog(flags.into(), &route_catalog)
                .into_iter()
    ...
    fn rows_from_matches(
        ...
        matches
            ...
            .map(|(item, indices)| {
                ...
                let mut category_tag = None;
                let mut disabled_reason = None;
                let mut is_disabled = false;
                if let Some(route) = self.route_catalog.slash_route(cmd.command()) {
                    category_tag = Some(format!("{} / {}", route.status, route.capability));
                    if !route.visible {
                        is_disabled = true;
                        disabled_reason = Some(
                            route
                                .reason
                                .clone()
                                .unwrap_or_else(|| "route marked unavailable".to_string()),
                        );
                    }
                }
                GenericDisplayRow {
