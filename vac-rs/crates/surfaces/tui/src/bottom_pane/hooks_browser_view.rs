use crate::session_protocol::HookErrorInfo;
use crate::session_protocol::HookEventName;
use crate::session_protocol::HookMetadata;
use crate::session_protocol::HookSource;
...
impl HooksBrowserView {
        ...
        vac_protocol::protocol::HookEventName::iter()
            .map(|event_name| {
                ...
                let installed = self
                    .hooks
                    .iter()
                    .filter(|hook| hook.event_name.to_core() == event_name)
                    .count();
                let active = self
                    .hooks
                    .iter()
                    .filter(|hook| {
                        hook.event_name.to_core() == event_name && (hook.enabled || hook.is_managed)
                    })
                    .count();
                EventRow {
                    event_name,
    ...
    fn handlers_for_event(&self, event_name: HookEventName) -> impl Iterator<Item = &HookMetadata> {
        self.hooks
            .iter()
            .filter(move |hook| hook.event_name.to_core() == event_name)
    }
    ...
    fn selected_hook_index(&self, event_name: HookEventName) -> Option<usize> {
        ...
        self.hooks
            .iter()
            .enumerate()
            .filter(|(_, hook)| hook.event_name.to_core() == event_name)
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => vac_protocol::protocol::HookEventName::iter().count(),
...
fn event_description(event_name: HookEventName) -> &'static str {
    match event_name {
        ...
        HookEventName::Stop => "Right before VAC ends its turn",
    }
}
...
fn hook_source_summary(hook: &HookMetadata) -> String {
    match hook.source.to_core() {
        HookSource::Plugin => hook
            .plugin_id
            .as_deref()
            .map(|plugin_id| format!("Plugin - {plugin_id}"))
            .unwrap_or_else(|| "Plugin".to_string()),
        _ => config_source_label(hook.source.to_core()).to_string(),
    }
}
...
fn detail_source_value(hook: &HookMetadata) -> String {
    match hook.source.to_core() {
        ...
        | HookSource::LegacyManagedConfigMdm => {
            config_source_label(hook.source.to_core()).to_string()
        }
        _ => format!(
            "{} - {}",
            config_source_label(hook.source.to_core()),
...
mod tests {
    ...
    use crate::render::renderable::Renderable;
    use crate::session_protocol::HookEventName;
    use crate::session_protocol::HookHandlerType;
    use crate::session_protocol::HookMetadata;
    use crate::session_protocol::HookSource;
    ...
    fn hook(
        ...
        HookMetadata {
            ...
            handler_type: HookHandlerType::Command.into(),
            is_managed,
        ...
        HooksBrowserView::new(
            vec![
                hook(
                    ...
                    HookSource::Plugin,
                    Some("superpowers@vastar-curated"),
                    "${VAC_PLUGIN_ROOT}/hooks/pre-tool-use-check.sh",
                    /*enabled*/ true,
                    /*is_managed*/ false,
                    /*display_order*/ 0,
                ),
