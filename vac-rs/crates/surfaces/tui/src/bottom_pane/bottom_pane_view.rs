...
pub(crate) trait BottomPaneView: Renderable {
    ...
    /// Called by the bottom pane before drawing, allowing live views to refresh
    /// file-backed/runtime state without waiting for another keypress.
    fn pre_draw_tick(&mut self) -> bool {
        false
    }

    /// Optional cadence for live views. Returning a delay schedules the next
    /// redraw even when the terminal is otherwise idle.
