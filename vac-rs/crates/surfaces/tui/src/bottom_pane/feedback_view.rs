...
use vac_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use vac_feedback::FeedbackDiagnostics;
...
const BASE_CLI_BUG_ISSUE_URL: &str = "https://github.com/vastar/vac/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const VAC_FEEDBACK_INTERNAL_URL: &str = "http://go/vac-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::InternalTester => {
            format!("{prefix} Please report this in #vac-feedback:")
        }
        Some(_) => format!("{prefix} Please open an issue using the following URL:"),
        None => format!("{prefix} Thanks for the feedback!"),
    })];
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::InternalTester => {
            lines.extend([
                "".into(),
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/vac-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
fn issue_url_for_category(
    ...
    match category {
        ...
        | FeedbackCategory::Other => Some(match feedback_audience {
            FeedbackAudience::InternalTester => slack_feedback_url(thread_id),
...
fn slack_feedback_url(_thread_id: &str) -> String {
    VAC_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "vac-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current VAC session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use vac_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/vac-feedback-internal".to_string();
        ...
        let expected_external_url = "https://github.com/vastar/vac/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #vac-feedback:\n\n  http://go/vac-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/vac-feedback/thread-2"
        );
    }
