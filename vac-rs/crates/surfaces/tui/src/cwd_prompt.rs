...
impl CwdPromptAction {
    ...
    fn past_participle(self) -> &'static str {
        match self {
            ...
            CwdPromptAction::Branch => "branched",
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn cwd_prompt_branch_snapshot() {
        let screen = CwdPromptScreen::new(
            FrameRequester::test_dummy(),
            CwdPromptAction::Branch,
            "/Users/example/current".to_string(),
            "/Users/example/session".to_string(),
        );
        let mut terminal =
        ...
        terminal
            .draw(|frame| frame.render_widget_ref(&screen, frame.area()))
            .expect("render cwd prompt");
        insta::assert_snapshot!("cwd_prompt_branch_modal", terminal.backend());
    }
