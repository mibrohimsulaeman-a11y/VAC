...
use vac_config::ManagedHooksRequirementsToml;
use vac_config::NetworkConstraints;
use vac_config::NetworkDomainPermissionToml;
...
use vac_config::WebSearchModeRequirement;
...
pub(crate) fn new_debug_config_output(
    ...
    lines.extend(render_control_plane_registry_lines(config.cwd.as_path()));
...
fn format_rules_requirements(rules: &RequirementsExecPolicyToml) -> String {
    ...
    parts.push(format!("prefix_rules={}", rules.prefix_rules.len()));
    for (index, rule) in rules.prefix_rules.iter().take(3).enumerate() {
        let pattern = rule
            .pattern
            .iter()
            .map(|token| {
                token
                    .token
                    .clone()
                    .or_else(|| token.any_of.as_ref().map(|values| values.join("|")))
        ...
        let decision = rule
            ...
            .unwrap_or_else(|| "missing-decision".to_string());
        parts.push(format!("#{index}:{decision}:{pattern}"));
    }
    if rules.prefix_rules.len() > 3 {
        parts.push(format!("+{} more", rules.prefix_rules.len() - 3));
    }
    parts.join("; ")
}

fn render_control_plane_registry_lines(cwd: &Path) -> Vec<Line<'static>> {
    load_control_plane_registry_report(cwd)
        .render_tui_lines()
        .into_iter()
...
fn render_debug_config_lines(stack: &ConfigLayerStack) -> Vec<Line<'static>> {
    ...
    if let Some(rules) = requirements_toml.rules.as_ref() {
        requirement_lines.push(requirement_line(
            "rules",
            format_rules_requirements(rules),
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_vac_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_vac_folder.as_path().display()
            )
        }
...
#[cfg(test)]
mod tests {
    use super::render_control_plane_registry_lines;
    ...
    use vac_config::FilesystemConstraints;
    use vac_config::HookEventsToml;
    use vac_config::HookHandlerConfig;
    use vac_config::ManagedHooksRequirementsToml;
    use vac_config::MatcherGroup;
    use vac_config::McpServerIdentity;
    use vac_config::McpServerRequirement;
    use vac_config::NetworkConstraints;
    use vac_config::NetworkDomainPermissionToml;
    use vac_config::NetworkDomainPermissionsToml;
    use vac_config::NetworkUnixSocketPermissionToml;
    ...
    #[test]
    fn debug_config_output_shows_registry_empty_report_when_vac_root_exists_but_registry_is_empty()
    {
        let tempdir = tempfile::tempdir().expect("tempdir");
        fs::create_dir(tempdir.path().join(".vac")).expect("vac root");

        let rendered = render_to_text(&render_control_plane_registry_lines(tempdir.path()));
        assert!(rendered.contains("registry: empty"));
        assert!(!rendered.contains("unable to locate `.vac` root"));
    }

    #[test]
    fn debug_config_output_shows_registry_warning_when_vac_root_missing() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        ...
        assert!(
            rendered
                .contains("create a `.vac/registry/product.yaml` file to enable the control plane")
        );
    }
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\vac\\config.toml")
        } else {
            absolute_path("/etc/vac/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.vac")
        } else {
            absolute_path("/repo/.vac")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\Vastar\\VAC\\requirements.toml")
        } else {
            absolute_path("/etc/vac/requirements.toml")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.vac\\config.toml")
        } else {
            absolute_path("/home/alice/.vac/config.toml")
        };
        let stack = ConfigLayerStack::new(
        ...
        assert!(
            rendered.contains("enforce_residency: us (source: MDM managed_config.toml (legacy))")
        );
        assert!(rendered.contains(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "experimental_network: unix_sockets={/tmp/blocked.sock=none, /tmp/vac.sock=allow} (source: MDM managed_config.toml (legacy))"
        ));
    }
...
"#;
    ...
    fn debug_config_output_normalizes_empty_web_search_mode_list() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "allowed_web_search_modes: disabled (source: MDM managed_config.toml (legacy))"
        ));
    }
    ...
    fn debug_config_output_lists_managed_hooks_requirement() {
        ...
        assert!(rendered.contains("(source: MDM managed_config.toml (legacy))"));
    }
