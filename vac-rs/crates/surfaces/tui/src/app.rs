...
use crate::local_runtime_session::TypedRequestError;
use crate::local_runtime_session::app_server_rate_limit_snapshots;
...
use crate::session_protocol::ConfigBatchWriteParams;
use crate::session_protocol::ConfigLayerSource;
use crate::session_protocol::ConfigValueWriteParams;
use crate::session_protocol::ConfigWriteResponse;
use crate::session_protocol::FeedbackUploadParams;
use crate::session_protocol::FeedbackUploadResponse;
use crate::session_protocol::GetAccountRateLimitsResponse;
use crate::session_protocol::HooksListParams;
use crate::session_protocol::HooksListResponse;
use crate::session_protocol::ListMcpServerStatusParams;
use crate::session_protocol::ListMcpServerStatusResponse;
#[cfg(test)]
use crate::session_protocol::McpAuthStatus;
use crate::session_protocol::McpServerStatus;
use crate::session_protocol::McpServerStatusDetail;
use crate::session_protocol::MergeStrategy;
use crate::session_protocol::PluginInstallParams;
use crate::session_protocol::PluginInstallResponse;
use crate::session_protocol::PluginListParams;
...
use crate::session_protocol::PluginUninstallParams;
use crate::session_protocol::PluginUninstallResponse;
use crate::session_protocol::RateLimitSnapshot;
use crate::session_protocol::SendAddCreditsNudgeEmailParams;
...
use crate::session_protocol::TurnError as AppServerTurnError;
...
use uuid::Uuid;
use vac_ansi_escape::ansi_escape_line;
...
use vac_models_manager::model_presets::HIDE_GPT_5_1_VAC_MAX_MIGRATION_PROMPT_CONFIG;
use vac_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
use vac_protocol::vastar_models::ModelAvailabilityNux;
use vac_protocol::vastar_models::ModelPreset;
use vac_protocol::vastar_models::ModelUpgrade;
...
fn default_exec_approval_decisions(
    network_approval_context: bool,
    proposed_execpolicy_amendment: Option<&crate::session_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<&[crate::session_protocol::NetworkPolicyAmendment]>,
    additional_permissions: Option<&crate::session_protocol::AdditionalPermissionProfile>,
) -> Vec<crate::session_protocol::CommandExecutionApprovalDecision> {
    use crate::session_protocol::CommandExecutionApprovalDecision;
    use crate::session_protocol::NetworkPolicyRuleAction;
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.vac_error_info,
        Some(AppServerVACErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    pub fn chatwidget_init_for_branched_or_resumed_thread(
        &self,
        tui: &mut tui::Tui,
        ...
        S: LocalRuntimeSession,
    {
        ...
        let (app_event_tx, mut app_event_rx) = mpsc::channel(AppEventSender::QUEUE_CAPACITY);
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            vac_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("vac.status_line", /*inc*/ 1, &[]);
        }
        ...
        let (mut chat_widget, initial_started_thread) = match session_selection {
            ...
            SessionSelection::Branch(target_session) => {
                session_telemetry.counter(
                    "vac.thread.branch",
                    /*inc*/ 1,
                    &[("source", "cli_subcommand")],
                );
                let branched = app_server
                    .branch_thread(config.clone(), target_session.thread_id)
                    .await
                    .wrap_err_with(|| {
                        let target_label = target_session.display_label();
                        format!("Failed to branch session from {target_label}")
                    })?;
                let init = crate::chatwidget::ChatWidgetInit {
                    config: config.clone(),
                    frame_requester: tui.frame_requester(),
                ...
                (ChatWidget::new_with_app_event(init), Some(branched))
            }
        };
...
See the VAC keymap documentation for supported actions and examples."
        ...
        if let Some(started) = initial_started_thread {
            let (session, turns) =
                crate::local_runtime_session::LocalRuntimeStartedThread::into_parts(started);
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.vac_home.clone();
    ...
    pub(crate) async fn handle_tui_event<S>(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut S,
        event: TuiEvent,
        ...
        if self.overlay.is_some() {
            ...
            match event {
                ...
                TuiEvent::Draw | TuiEvent::Resize => {
                    ...
                    // 00D-8: Render the Activity sidebar as a floating
                    ...
                    if terminal_resize_reflow_enabled {
                        tui.draw_with_resize_reflow(desired_height, |frame| {
                            ...
                            if let Some(rect) = sidebar_area {
                                crate::activity_sidebar::ActivitySidebar::new(
                                    self.chat_widget.runtime_activity(),
                                )
                                .render(rect, frame.buffer);
                            }
                            if let Some((x, y)) = self.chat_widget.cursor_pos(main_area) {
                                frame.set_cursor_style(self.chat_widget.cursor_style(main_area));
