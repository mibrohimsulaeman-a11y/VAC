...
// `vac-file-search` session for the current search root, updates the query
...
impl FileSearchManager {
    ...
    pub fn update_search_dir(&mut self, new_dir: PathBuf) {
        ...
        let Ok(mut st) = self.state.lock() else {
            tracing::warn!("file search state lock poisoned while updating search directory");
            return;
        };
        st.session.take();
    ...
    pub fn on_user_query(&self, query: String) {
        let Ok(mut st) = self.state.lock() else {
            tracing::warn!("file search state lock poisoned while applying user query");
            return;
        };
...
impl TuiSessionReporter {
    fn send_snapshot(&self, snapshot: &file_search::FileSearchSnapshot) {
        let Ok(st) = self.state.lock() else {
            tracing::warn!("file search state lock poisoned while reporting snapshot");
            return;
        };
