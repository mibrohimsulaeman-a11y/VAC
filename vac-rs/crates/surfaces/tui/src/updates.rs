...
use vac_login::default_client::create_client;

use crate::version::VAC_CLI_VERSION;

pub fn get_upgrade_version(config: &Config) -> Option<String> {
    if !config.check_for_update_on_startup || is_source_build_version(VAC_CLI_VERSION) {
        return None;
    }
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, VAC_CLI_VERSION).unwrap_or(false) {
...
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/vac.json";
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/vastar/vac/releases/latest";
...
fn version_filepath(config: &Config) -> PathBuf {
    config.vac_home.join(VERSION_FILENAME).into_path_buf()
}
