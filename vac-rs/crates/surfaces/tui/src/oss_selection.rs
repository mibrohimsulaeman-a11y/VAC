...
use vac_model_provider_info::DEFAULT_LMSTUDIO_PORT;
use vac_model_provider_info::DEFAULT_OLLAMA_PORT;
...
pub async fn select_oss_provider(vac_home: &std::path::Path) -> io::Result<String> {
    ...
    if let Ok(ref provider) = result
        && let Err(e) = set_default_oss_provider(vac_home, provider)
    {
