...
const FRAME_SCHEDULE_QUEUE_CAPACITY: usize = 1024;
...
impl FrameRequester {
    ...
    pub fn new(draw_tx: broadcast::Sender<()>) -> Self {
        let (tx, rx) = mpsc::channel(FRAME_SCHEDULE_QUEUE_CAPACITY);
    ...
    pub fn schedule_frame_in(&self, dur: Duration) {
        let _ = self.frame_schedule_tx.try_send(Instant::now() + dur);
    }
}
...
impl FrameRequester {
    ...
    pub(crate) fn test_dummy() -> Self {
        let (tx, _rx) = mpsc::channel(FRAME_SCHEDULE_QUEUE_CAPACITY);
        FrameRequester {
            frame_schedule_tx: tx,
        }
    }
