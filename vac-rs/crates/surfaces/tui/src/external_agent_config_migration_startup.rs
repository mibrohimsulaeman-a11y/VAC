...
fn external_agent_config_migration_success_message(
    ...
    if items.iter().any(|item| {
        item.item_type == crate::session_protocol::ExternalAgentConfigMigrationItemType::Plugins
    }) {
...
async fn persist_external_agent_config_migration_prompt_shown(
    ...
    ConfigEditsBuilder::new(&config.vac_home)
        .with_edits(edits)
        .apply()
        .await
...
pub(crate) async fn handle_external_agent_config_migration_prompt_if_needed<S>(
    tui: &mut tui::Tui,
    app_server: &mut S,
    config: &mut Config,
...
mod tests {
    use super::*;
    use crate::session_protocol::ExternalAgentConfigMigrationItemType;
    ...
    async fn visible_external_agent_config_migration_items_omits_hidden_scopes() {
        let vac_home = tempdir().expect("temp vac home");
