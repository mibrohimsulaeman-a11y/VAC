...
// under `{VAC_HOME}/themes/`.  It provides:
//
...
const MIN_THEME_CONTRAST_RATIO: f32 = 4.5;

#[derive(Debug, Clone, Copy, PartialEq)]
pub(crate) struct ThemeContrastReport {
    pub(crate) foreground_background: f32,
    pub(crate) selected_row: f32,
    pub(crate) muted_text: f32,
    pub(crate) warning_text: f32,
...
pub(crate) fn validate_theme_contrast(
    foreground: (u8, u8, u8),
    background: (u8, u8, u8),
    selected: (u8, u8, u8),
    muted: (u8, u8, u8),
    ...
    let foreground_background = contrast_ratio(foreground, background);
...
fn theme_picker_subtitle(vac_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = vac_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    let entries = highlight::list_available_themes(vac_home);
    let vac_home_owned = vac_home.map(Path::to_path_buf);
    ...
    let preview_home = vac_home_owned.clone();
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*vac_home*/ None, /*terminal_width*/ None,
        );
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_vac_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let vac_home = home.join(".vac");

        let subtitle = theme_picker_subtitle(Some(&vac_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let vac_home = home.join(long_segment).join(".vac");

        let subtitle = theme_picker_subtitle(Some(&vac_home), Some(140));
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle = theme_picker_subtitle(/*vac_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&vac_home), Some(94));
    ...
    fn unavailable_configured_theme_falls_back_to_configured_or_default_selection() {
        ...
        let params =
            build_theme_picker_params(Some("not-a-real-theme"), /*vac_home*/ None, Some(120));
