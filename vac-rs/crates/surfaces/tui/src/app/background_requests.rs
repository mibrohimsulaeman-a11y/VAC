...
use crate::session_protocol::MarketplaceAddParams;
use crate::session_protocol::MarketplaceAddResponse;
use crate::session_protocol::MarketplaceRemoveParams;
use crate::session_protocol::MarketplaceRemoveResponse;
use crate::session_protocol::MarketplaceUpgradeParams;
...
impl App {
    pub(super) fn fetch_mcp_inventory<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
    ...
    pub(super) fn refresh_rate_limits<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
    ...
    pub(super) fn send_add_credits_nudge_email<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
    ...
    pub(super) fn refresh_startup_skills<S: LocalRuntimeSession>(&mut self, app_server: &S) {
        let request_handle = app_server.request_handle();
        let app_event_tx = self.app_event_tx.clone();
    ...
    pub(super) fn fetch_plugins_list<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
    ) {
    ...
    pub(super) fn fetch_hooks_list<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
    ) {
    ...
    pub(super) fn fetch_plugin_detail<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
    ...
    pub(super) fn fetch_marketplace_add<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
        source: String,
    ...
    pub(super) fn fetch_marketplace_remove<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
    ...
    pub(super) fn fetch_marketplace_upgrade<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
    ...
    pub(super) fn fetch_plugin_install<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
    ...
    pub(super) fn fetch_plugin_uninstall<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
        plugin_id: String,
    ...
    pub(super) fn set_plugin_enabled<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        cwd: PathBuf,
        plugin_id: String,
    ...
    pub(super) fn spawn_plugin_enabled_write(
        &mut self,
        app_server: &impl LocalRuntimeSession,
        cwd: PathBuf,
        plugin_id: String,
        enabled: bool,
    ) {
    ...
    pub(super) fn set_hook_enabled<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
        key: String,
        enabled: bool,
    ...
    pub(super) fn refresh_plugin_mentions(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            let plugins = PluginsManager::new(config.vac_home.to_path_buf())
    ...
    pub(super) fn submit_feedback<S: LocalRuntimeSession>(
        &mut self,
        app_server: &S,
...
pub(super) async fn fetch_all_mcp_server_statuses<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    detail: McpServerStatusDetail,
...
pub(super) async fn fetch_account_rate_limits<R: LocalRuntimeRequestHandle>(
    request_handle: R,
...
pub(super) async fn send_add_credits_nudge_email<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<crate::session_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: crate::session_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
pub(super) async fn fetch_skills_list<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    cwd: PathBuf,
    ...
    // extending a borrow of the session boundary across the first frame render.
    request_handle
...
pub(super) async fn fetch_plugins_list<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    cwd: PathBuf,
...
pub(super) async fn fetch_hooks_list<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    cwd: PathBuf,
...
const CLI_HIDDEN_PLUGIN_MARKETPLACES: &[&str] = &["vastar-bundled"];
...
pub(super) async fn fetch_plugin_detail<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    params: PluginReadParams,
...
pub(super) async fn fetch_marketplace_add<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    cwd: PathBuf,
    source: String,
...
pub(super) async fn fetch_marketplace_remove<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    marketplace_name: String,
...
pub(super) async fn fetch_marketplace_upgrade<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    marketplace_name: Option<String>,
...
pub(super) async fn fetch_plugin_install<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    marketplace_path: AbsolutePathBuf,
    plugin_name: String,
...
pub(super) async fn fetch_plugin_uninstall<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    plugin_id: String,
...
pub(super) async fn write_plugin_enabled<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    plugin_id: String,
    enabled: bool,
...
pub(super) async fn write_hook_enabled<R: LocalRuntimeRequestHandle>(
    request_handle: R,
    key: String,
    enabled: bool,
) -> Result<ConfigWriteResponse> {
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![crate::session_protocol::ConfigEdit {
...
pub(super) async fn fetch_feedback_upload<R: LocalRuntimeRequestHandle>(
    request_handle: R,
...
pub(super) type McpInventoryMaps = (
    HashMap<String, vac_protocol::mcp::Tool>,
    HashMap<String, Vec<vac_protocol::mcp::Resource>>,
    HashMap<String, Vec<vac_protocol::mcp::ResourceTemplate>>,
...
pub(super) fn mcp_inventory_maps_from_statuses(statuses: Vec<McpServerStatus>) -> McpInventoryMaps {
    ...
    for status in statuses {
        ...
        auth_statuses.insert(server_name.clone(), status.auth_status.to_core());
...
mod tests {
    use super::*;
    use crate::session_protocol::PluginMarketplaceEntry;
    ...
    #[test]
    fn hide_cli_only_plugin_marketplaces_removes_vastar_bundled() {
        let mut response = PluginListResponse {
            marketplaces: vec![
                PluginMarketplaceEntry {
                    name: "vastar-bundled".to_string(),
                    path: Some(test_absolute_path("/marketplaces/vastar-bundled")),
                    interface: None,
                    plugins: Vec::new(),
                },
                PluginMarketplaceEntry {
                    name: "vastar-curated".to_string(),
                    path: Some(test_absolute_path("/marketplaces/vastar-curated")),
                    interface: None,
                    plugins: Vec::new(),
                },
            ],
    ...
    fn mcp_inventory_maps_prefix_tool_names_by_server() {
        let statuses = vec![
            McpServerStatus {
                ...
                auth_status: crate::session_protocol::McpAuthStatus::Unsupported.into(),
            },
            McpServerStatus {
