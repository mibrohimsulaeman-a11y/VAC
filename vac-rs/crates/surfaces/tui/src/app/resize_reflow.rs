...
// the stored cells as source, clears the VAC-owned terminal history, and re-emits the transcript
...
impl App {
    ...
    ///
    /// Width changes need a rebuild because transcript wrapping changes. Height-only changes do
    /// not change wrapping, so they must not trigger an O(N cell × width) transcript rebuild. The
    /// inline viewport is repaired by the regular draw path and status refresh below. The first
    /// observed width initializes resize tracking without scheduling a rebuild, because there is no
    /// previously emitted width to repair yet.
    pub(super) fn handle_draw_size_change(
        ...
        let transcript_width = crate::activity_sidebar::main_width_for_activity_sidebar(size.width);
        let width = self.transcript_reflow.note_width(transcript_width);
        ...
        if should_rebuild_transcript {
            if self.terminal_resize_reflow_enabled() {
                ...
                let target_width = reflow_needed.then_some(transcript_width);
    ...
    fn reflow_transcript_now(&mut self, tui: &mut tui::Tui) -> Result<u16> {
        let width =
            crate::activity_sidebar::main_width_for_activity_sidebar(tui.terminal.size()?.width);
