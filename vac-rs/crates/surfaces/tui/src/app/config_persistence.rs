...
impl App {
    pub(super) async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            .vac_home(self.config.vac_home.to_path_buf())
    ...
    pub(super) async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        let mut builder = ConfigEditsBuilder::new(&self.config.vac_home)
    ...
    pub(super) async fn update_memory_settings(
        ...
        if let Err(err) = ConfigEditsBuilder::new(&self.config.vac_home)
            .with_edits(edits)
            .apply()
            .await
        {
    ...
    pub(super) async fn update_memory_settings_with_app_server<S>(
        &mut self,
        app_server: &mut S,
        use_memories: bool,
        generate_memories: bool,
    ...
    pub(super) async fn reset_memories_with_app_server<S>(&mut self, app_server: &mut S)
    where
        S: crate::local_runtime_session::LocalRuntimeSession,
    {
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("vac-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
    ...
    pub(super) fn on_update_reasoning_effort(&mut self, effort: Option<ReasoningEffortConfig>) {
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(aibrahim): Remove this and don't use config as a state object.
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.vac_home))
        {
...
mod tests {
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        app.config.vac_home = vac_home.path().to_path_buf().abs();
        ...
        ConfigEditsBuilder::new(&app.config.vac_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    -> Result<()> {
        ...
        let vac_home = tempdir()?;
        app.config.vac_home = vac_home.path().to_path_buf().abs();
        std::fs::write(vac_home.path().join("config.toml"), "[broken")?;
    ...
    async fn refresh_in_memory_config_from_disk_updates_resize_reflow_config() -> Result<()> {
        ...
        app.config.vac_home = vac_home.path().to_path_buf().abs();
        std::fs::write(
            vac_home.path().join("config.toml"),
            r#"
[tui]
