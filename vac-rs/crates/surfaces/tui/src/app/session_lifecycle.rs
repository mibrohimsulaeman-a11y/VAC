// Session, resume, branch, and subagent selection lifecycle for the TUI app.
//
// This module owns the high-level transitions between app-server threads: starting fresh sessions,
// resuming/branching saved sessions, replacing ChatWidget instances, and maintaining the agent picker
// cache used for multi-agent navigation.

use super::*;

impl App {
    pub(super) async fn open_agent_picker<S>(&mut self, app_server: &mut S)
    where
    ...
    pub(super) async fn refresh_agent_picker_thread_liveness<S>(
        &mut self,
        app_server: &mut S,
        thread_id: ThreadId,
    ) -> bool
            ...
            .await
        {
            Ok(thread) => {
                let is_closed = matches!(
                    thread.status,
                    crate::session_protocol::ThreadStatus::NotLoaded
                );
            ...
            Err(err) => {
                ...
                tracing::debug!(
                    thread_id = %thread_id,
                    error = %err,
                    "marking locally replayable picker thread closed after liveness read failed"
                );
    ...
    pub(super) async fn attach_live_thread_for_selection<S>(
        &mut self,
        app_server: &mut S,
        thread_id: ThreadId,
    ) -> Result<bool>
                    ...
                    .await
                {
                            ...
                            .await
                        {
                            ...
                            Err(err) => {
                                tracing::warn!(
                                    thread_id = %thread_id,
                                    error = %err,
                                    "failed to read fallback thread without turns for selection"
                                );
                    ...
                    Err(err) => {
                        tracing::warn!(
                            thread_id = %thread_id,
                            error = %err,
                            "failed to read fallback thread with turns for selection"
                        );
                        return Err(color_eyre::eyre::eyre!(
                            "Agent thread {thread_id} is not yet available for replay or live attach."
                        ));
    ...
    pub(super) async fn select_agent_thread<S>(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut S,
        thread_id: ThreadId,
        ...
        S: crate::local_runtime_session::LocalRuntimeSession,
    {
        ...
        let init = self.chatwidget_init_for_branched_or_resumed_thread(
            tui,
            self.config.clone(),
    ...
    pub(super) async fn start_fresh_session_with_summary_hint<S>(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut S,
    ...
    pub(super) async fn replace_chat_widget_with_app_server_thread<S>(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut S,
        started: S::StartedThread,
        initial_user_message: Option<crate::chatwidget::UserMessage>,
    ) -> Result<()>
    where
        S: crate::local_runtime_session::LocalRuntimeSession,
    {
        ...
        // resume/branch flows pass `None` so they cannot replay old history and then auto-submit a new
    ...
    ///
    /// Called after `replace_chat_widget_with_app_server_thread` during resume, branch, and new
    ...
    pub(super) async fn backfill_loaded_subagent_threads<S>(&mut self, app_server: &mut S) -> bool
    where
        S: crate::local_runtime_session::LocalRuntimeSession,
    {
    ...
    pub(super) async fn adjacent_thread_id_with_backfill<S>(
        &mut self,
        app_server: &mut S,
    ...
    pub(super) async fn resume_target_session<S>(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut S,
        ...
        S: crate::local_runtime_session::LocalRuntimeSession,
    {
        ...
        let resume_cwd = {
            match crate::session_resume::resolve_cwd_for_resume_or_branch(
                tui,
                &self.config,
                &current_cwd,
