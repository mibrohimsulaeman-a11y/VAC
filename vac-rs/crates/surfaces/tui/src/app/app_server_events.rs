...
use crate::local_runtime_session::status_account_display_from_auth_mode;
...
impl App {
    ...
    {
        match event {
            TuiSessionEvent::Lagged { skipped } => {
                tracing::warn!(
                    skipped,
            ...
            TuiSessionEvent::ServerRequest(request) => {
    ...
    async fn handle_server_notification_event<S>(
        &mut self,
        app_server_client: &mut S,
    ...
    async fn handle_server_request_event<S>(
        &mut self,
        app_server_client: &mut S,
