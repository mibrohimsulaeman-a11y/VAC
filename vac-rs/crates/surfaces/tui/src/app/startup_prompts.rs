...
pub(super) fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_vac_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_vac_folder.as_path().display().to_string(),
...
pub(super) fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_VAC_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_vac_max_migration_prompt
            .unwrap_or(false),
...
pub(super) async fn prepare_startup_tooltip_override(
    ...
    if let Err(err) = ConfigEditsBuilder::new(&config.vac_home)
