...
impl App {
    pub(super) async fn shutdown_current_thread<S>(&mut self, app_server: &mut S)
    where
    ...
    async fn thread_file_change_changes(
        ...
        turn_id: &str,
        item_id: &str,
    ) -> Option<Vec<crate::session_protocol::FileUpdateChange>> {
        let channel = self.thread_event_channels.get(&thread_id)?;
        let store = channel.store.lock().await;
        ...
        match request {
            ServerRequest::CommandExecutionRequestApproval { params, .. } => {
                ...
                let additional_permissions: Option<
                    crate::session_protocol::AdditionalPermissionProfile,
                    ...
                    Some(ThreadInteractiveRequest::Approval(
                        ApprovalRequest::McpElicitation {
                            ...
                            message: match &params.request {
                                crate::session_protocol::McpServerElicitationRequest::Form {
                                    message,
                                    ..
                                }
                                | crate::session_protocol::McpServerElicitationRequest::Url {
                                    message,
                                    ..
                                } => message.clone(),
                            },
    ...
    pub(super) async fn submit_active_thread_op<S>(
        &mut self,
        app_server: &mut S,
        op: AppCommand,
    ) -> Result<()>
    ...
    pub(super) async fn submit_thread_op<S>(
        &mut self,
        app_server: &mut S,
        thread_id: ThreadId,
        op: AppCommand,
                    ...
                    app_event_tx.send(AppEvent::ThreadHistoryEntryResponse {
                        ...
                        event: HistoryLookupResponse {
                            ...
                            entry: entry_opt.map(|entry| {
                                vac_protocol::message_history::HistoryEntry {
    ...
    pub(super) async fn try_submit_active_thread_op_via_app_server<S>(
        &mut self,
        app_server: &mut S,
        thread_id: ThreadId,
        op: &AppCommand,
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(crate::session_protocol::SkillsListParams {
                            cwds: cwds.clone(),
    ...
    pub(super) async fn try_resolve_app_server_request<S>(
        &mut self,
        app_server: &mut S,
        thread_id: ThreadId,
        op: &AppCommand,
    ...
    /// this change.
    pub(super) async fn hydrate_collab_agent_metadata_for_notification<S>(
        &mut self,
        app_server: &mut S,
        notification: &ServerNotification,
    ) where
    ...
    pub(super) async fn refresh_snapshot_session_if_needed<S>(
        &mut self,
        app_server: &mut S,
        thread_id: ThreadId,
        is_replay_only: bool,
    ...
    pub(super) async fn apply_refreshed_snapshot_thread(
        &mut self,
        thread_id: ThreadId,
        started: impl crate::local_runtime_session::LocalRuntimeStartedThread,
        snapshot: &mut ThreadEventSnapshot,
    ) {
        let (session, turns) =
    ...
    pub(super) async fn handle_active_thread_event<S>(
        &mut self,
        tui: &mut tui::Tui,
        app_server: &mut S,
