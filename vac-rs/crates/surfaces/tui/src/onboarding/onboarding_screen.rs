...
use crate::local_runtime_session::LOCAL_FS;
use crate::local_runtime_session::TuiSessionEvent;
...
pub(crate) struct OnboardingScreenArgs {
    ...
    pub app_server_request_handle: Option<TuiSessionRequestHandle>,
    pub config: Config,
}
...
impl OnboardingScreen {
    pub(crate) async fn new(tui: &mut Tui, args: OnboardingScreenArgs) -> Self {
        ...
        let vac_home = config.vac_home.to_path_buf();
        ...
        if show_login_screen {
            ...
            if let Some(app_server_request_handle) = app_server_request_handle {
                ...
                crate::auth_state_banner::write_auth_state_runtime_event(
                    &config.cwd,
                    crate::auth_state_banner::AuthStateBanner::AuthUnavailable,
                    "onboarding login step skipped because app-server request handle is unavailable",
                );
                let _ = tui.notify(
                    "Auth unavailable: VAC is running in local provider-neutral mode; configure API-key/provider auth from /status or config."
                );
...
pub(crate) async fn run_onboarding_app<S>(
    args: OnboardingScreenArgs,
    mut app_server: Option<&mut S>,
    tui: &mut Tui,
    ...
    while !onboarding_screen.is_done() {
        tokio::select! {
            ...
            event = async {
                ...
                if let Some(event) = event {
                    match event {
                        TuiSessionEvent::ServerNotification(notification) => {
                            onboarding_screen.handle_app_server_notification(notification);
                        }
                        TuiSessionEvent::Disconnected { message } => {
                            return Err(color_eyre::eyre::eyre!(message));
                        }
                        TuiSessionEvent::Lagged { .. }
                        | TuiSessionEvent::ServerRequest(_) => {}
                    }
                }
            }
        }
