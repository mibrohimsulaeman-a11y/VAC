...
use crate::session_protocol::HookOutputEntry;
use crate::session_protocol::HookOutputEntryKind;
use crate::session_protocol::HookRunStatus;
use crate::session_protocol::HookRunSummary;
use ratatui::prelude::*;
...
impl HookCell {
    ...
    pub(crate) fn start_run(&mut self, run: HookRunSummary) {
        ...
        if let Some(existing) = self.runs.iter_mut().find(|existing| existing.id == run.id) {
            existing.event_name = run.event_name.to_core();
    ...
    pub(crate) fn complete_run(&mut self, run: HookRunSummary) -> bool {
        ...
        existing.event_name = event_name.to_core();
...
mod tests {
    ...
    fn hook_run_summary(id: &str) -> HookRunSummary {
        HookRunSummary {
            id: id.to_string(),
            event_name: HookEventName::PostToolUse.into(),
