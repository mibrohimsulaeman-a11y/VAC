...
use vac_config::types::NotificationCondition;
use vac_config::types::NotificationMethod;
...
/// Restore the terminal after VAC is exiting.
///
...
fn flush_terminal_input_buffer() {
    ...
    // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `libc::tcflush(libc::STDIN_FILENO, libc::TCIFLUSH)` uses inputs that remain live for the duration of this block.
...
fn flush_terminal_input_buffer() {
    ...
    // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `GetStdHandle(STD_INPUT_HANDLE)` uses inputs that remain live for the duration of this block.
    let handle = unsafe { GetStdHandle(STD_INPUT_HANDLE) };
    if handle == INVALID_HANDLE_VALUE || handle == 0 {
        // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `GetLastError()` uses inputs that remain live for the duration of this block.
        let err = unsafe { GetLastError() };
        tracing::warn!("failed to get stdin handle for flush: error {err}");
        return;
    }

    // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `FlushConsoleInputBuffer(handle)` uses inputs that remain live for the duration of this block.
...
impl Tui {
    ...
    /// terminal modes (optionally keeping raw mode enabled), then re-applies VAC TUI modes and
    ...
    fn update_inline_viewport(
        ...
        if area != terminal.viewport_area {
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(nornagon): probably this could be collapsed with the clear + set_viewport_area above.
            terminal.clear()?;
