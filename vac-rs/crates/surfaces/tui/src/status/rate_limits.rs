// Rate-limit display shaping for status surfaces.
//
...
use crate::session_protocol::CreditsSnapshot as CoreCreditsSnapshot;
...
pub(crate) struct StatusRateLimitRow {
    /// Human-readable row label, such as `"5h limit"`.
    pub label: String,
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `vac` or `vac_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "vac".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("vac");
        ...
        let combine_non_vac_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_vac_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_vac_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_vac_single_limit_renders_combined_row() {
        let now = Local::now();
        let vac = RateLimitSnapshotDisplay {
            limit_name: "vac".to_string(),
            captured_at: now,
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "vac-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[vac, other], now) {
        ...
        assert_eq!(
            labels,
            vec!["5h limit".to_string(), "vac-other 5h limit".to_string()]
        );
        assert!(
            rows.iter()
                .all(|row| !row.label.eq_ignore_ascii_case("credits"))
        );
    }

    #[test]
    fn non_vac_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "vac-other".to_string(),
            captured_at: now,
