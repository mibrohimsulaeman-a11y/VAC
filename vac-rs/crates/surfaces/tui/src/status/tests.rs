...
use crate::session_protocol::RateLimitWindow;
...
async fn test_config(temp_home: &TempDir) -> Config {
    let mut config = ConfigBuilder::default()
        .vac_home(temp_home.path().to_path_buf())
        .build()
        .await
        .expect("load config");
...
fn removed_credit_label() -> String {
    ["Cred", "its:"].concat()
}
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.model = Some("gpt-5.1-vac-max".to_string());
    config.model_provider_id = "vastar".to_string();
...
async fn status_model_provider_uses_bedrock_runtime_base_url() {
    ...
    config.model_provider.base_url =
        Some("https://bedrock-mantle.us-east-1.api.aws/vastar/v1".to_string());
    let usage = TokenUsage::default();
    let captured_at = chrono::Local
        .with_ymd_and_hms(2024, 1, 2, 3, 4, 5)
        .single()
        .expect("timestamp");
    let model_slug = crate::legacy_core::test_support::get_model_offline(config.model.as_deref());
    let runtime_base_url = "https://bedrock-mantle.eu-west-1.api.aws/vastar/v1";
...
#[tokio::test]
async fn status_snapshot_includes_branched_from() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-vac-max".to_string());
    config.model_provider_id = "vastar".to_string();
...
#[tokio::test]
async fn status_snapshot_omits_unlimited_credit_display() {
    let temp_home = TempDir::new().expect("temp home");
    ...
    assert!(
        rendered
            .iter()
            .all(|line| !line.contains(&removed_credit_label())),
        "expected no credit display line, got {rendered:?}"
    );
}

#[tokio::test]
async fn status_snapshot_omits_positive_credit_display() {
    let temp_home = TempDir::new().expect("temp home");
    let config = test_config(&temp_home).await;
    let account_display = test_status_account_display();
    let usage = TokenUsage::default();
...
async fn status_snapshot_hides_when_has_no_credits_flag() {
    ...
    assert!(
        rendered
            .iter()
            .all(|line| !line.contains(&removed_credit_label())),
        "expected no credit display line when has_credits is false, got {rendered:?}"
    );
}
...
async fn status_snapshot_includes_credits_and_limits() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-vac".to_string());
    config.cwd = test_path_buf("/workspace/tests").abs();
