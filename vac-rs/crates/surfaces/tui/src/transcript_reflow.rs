// Tracks when VAC-owned transcript scrollback must be repaired after terminal resize.
//
// Terminal scrollback is not a retained widget tree: once VAC writes wrapped lines into the
// terminal, the terminal owns those rows. Width resize reflow treats the in-memory transcript cells
// as the source of truth, clears VAC-owned history, and re-emits the cells at the current width.
...
/// VAC has already rebuilt scrollback. Keeping those widths distinct lets the next draw request a
...
impl TranscriptReflowState {
    ...
    #[cfg(test)]
    pub(crate) fn last_reflow_width_for_test(&self) -> Option<u16> {
