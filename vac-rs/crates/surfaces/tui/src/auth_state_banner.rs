    ...
    pub(crate) fn guidance(self) -> &'static str {
        match self {
            ...
            Self::AuthUnavailable => {
                "Auth UI could not attach to the app-server request handle; provider setup can continue from /status or config."
            }
            Self::ProviderDisabled => {
                "The selected provider is disabled by feature/config; choose another provider or enable it explicitly."
            }
        }
    }
