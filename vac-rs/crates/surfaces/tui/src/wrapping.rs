    ...
    O: Into<Options<'a>>,
{
    ...
    let estimated_lines = (text.len() / opts.width.max(1)).saturating_add(1);
    let mut lines: Vec<Range<usize>> = Vec::with_capacity(estimated_lines);
    let mut cursor = 0usize;
    for (line_index, line) in textwrap::wrap(text, &opts).iter().enumerate() {
        match line {
            std::borrow::Cow::Borrowed(slice) => {
                // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `slice.as_ptr().offset_from(text.as_ptr()) as usize` uses inputs that remain live for the duration of this block.
