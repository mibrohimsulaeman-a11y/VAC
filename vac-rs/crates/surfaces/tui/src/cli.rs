...
use clap::Parser;
use vac_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `vac resume` subcommand.
    // These are not exposed as user flags on the base `vac` command.
    #[clap(skip)]
    ...
    /// top-level `vac resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `vac branch` subcommand.
    // These are not exposed as user flags on the base `vac` command.
    #[clap(skip)]
    pub branch_picker: bool,

    #[clap(skip)]
    pub branch_last: bool,

    /// Internal: branch a specific recorded session by id (UUID). Set by the
    /// top-level `vac branch <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    pub branch_session_id: Option<String>,

    /// Internal: show all sessions (disables cwd filtering and shows CWD column).
    #[clap(skip)]
    pub branch_show_all: bool,
