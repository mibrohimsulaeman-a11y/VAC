...
use crate::slash_command::CommandAvailability;
...
const TUI_VAC_INIT_OPERATOR_CHOICES: &str = "schema_version: 1\nkind: init.operator_choices\nid: init.operator_choices\nsurface: tui\nsource: tui_slash_init\npartition_strategy: layered_control_plane\nsynthesize_missing_manifests: yes\nverify_after_synthesis: yes\nrequires_cli_interactive_flag: false\n";

impl ChatWidget {
    fn write_tui_vac_init_operator_choices(&self) -> std::io::Result<std::path::PathBuf> {
        let init_dir = self.config.cwd.join(".vac/.init");
        std::fs::create_dir_all(&init_dir)?;
        let operator_choices_path = init_dir.join("operator_choices.yaml");
        std::fs::write(&operator_choices_path, TUI_VAC_INIT_OPERATOR_CHOICES)?;
        Ok(operator_choices_path.to_path_buf())
    }
    ...
    pub(super) fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd.availability() {
            ...
            CommandAvailability::FeatureOff => {
                self.add_info_message(
                    format!("'/{}' is feature-off by default.", cmd.command()),
                    Some("Enable the matching provider feature explicitly; local coding mode keeps it disabled.".to_string()),
                );
                self.request_redraw();
                return;
            }
            CommandAvailability::Experimental => {
                self.add_info_message(
                    format!("'/{}' is experimental.", cmd.command()),
                    Some("Proceeding with guarded experimental behavior.".to_string()),
                );
            }
        ...
        match cmd {
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("vac.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.session_telemetry.counter(
                        "vac.windows_sandbox.setup_elevated_sandbox_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
            ...
            SlashCommand::Activity => {
                // 00D-8: Toggle the floating Activity sidebar open or
                // closed without changing transcript width.
                // On narrow terminals the sidebar cannot render, so we fall
                // back to a transcript timeline snapshot.
                self.toggle_runtime_activity_expanded();
                let sidebar_can_render = self.last_rendered_width.get().is_some_and(|width| {
                    width >= crate::activity_sidebar::ACTIVITY_SIDEBAR_MIN_TOTAL_WIDTH as usize
                });
                if !sidebar_can_render {
                    let lines: Vec<ratatui::text::Line<'static>> =
                        crate::activity_sidebar::activity_timeline_strings(self.runtime_activity())
                            .into_iter()
                            .map(ratatui::text::Line::from)
                            .collect();
                    let cell = crate::history_cell::PlainHistoryCell::new(lines);
                    self.add_to_history(cell);
                }
                self.request_redraw();
            }
            SlashCommand::Status => {
                // Hardening 3: /status is an operator-local inventory surface.
                // It must not initiate rate-limit or credit refreshes, and it must
                // not render account limit rows. Ambient rate-limit warnings remain
                // owned by the status-line/nudge surfaces.
                debug_assert!(!crate::status::status_command_requests_rate_limit_refresh());
            ...
            SlashCommand::Runtime => {
                self.add_runtime_jobs_output();
            }
            SlashCommand::Capabilities => {
                self.add_capability_dashboard_output();
            }
    ...
    fn handle_workflow_command_args(&mut self, trimmed: &str) {
        ...
        match command {
            "filter" => {
                ...
                if parts.next().is_some() {
                    self.add_error_message(
                        "Usage: /workflow filter <capability>|clear".to_string(),
                    );
                    return;
                }
                let state = match filter {
                    None | Some("clear") | Some("none") => {
                        crate::workflow_browser::WorkflowBrowserState::default()
                    }
                    Some(capability) => crate::workflow_browser::WorkflowBrowserState::default()
                        .with_capability_filter(capability.to_string()),
                };
                self.add_workflow_browser_output_with_state(state);
            }
            "inspect" => {
                let Some(workflow_id) = parts.next() else {
                    self.add_error_message("Usage: /workflow inspect <workflow-id>".to_string());
                    return;
                };
                if parts.next().is_some() {
                    self.add_error_message("Usage: /workflow inspect <workflow-id>".to_string());
                ...
                self.add_workflow_browser_output_with_state(
                    crate::workflow_browser::WorkflowBrowserState::default()
                        .with_selected_workflow(workflow_id.to_string()),
                );
            }
            "run" => {
                let Some(workflow_id) = parts.next() else {
                    self.add_error_message("Usage: /workflow run <workflow-id>".to_string());
                    return;
                };
                if parts.next().is_some() {
                    self.add_error_message("Usage: /workflow run <workflow-id>".to_string());
            ...
            _ => {
                self.add_error_message(
                    "Usage: /workflow [filter <capability>|filter clear|inspect <workflow-id>|run <workflow-id>]"
                        .to_string(),
                );
            }
        }
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Workflow if !trimmed.is_empty() => {
                self.handle_workflow_command_args(trimmed);
            }
    ...
    pub(super) fn submit_queued_slash_prompt(&mut self, user_message: UserMessage) -> QueueDrain {
        ...
        let route_catalog = SurfaceRouteCatalog::load(&self.config.cwd);
        let Some(cmd) = slash_commands::find_builtin_command_with_catalog(
            name,
