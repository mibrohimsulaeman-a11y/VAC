...
impl ChatWidget {
    ...
    /// Clears the terminal title VAC most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_vac_folder } => {
                    dot_vac_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::VACVersion => Some(VAC_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            ...
            StatusSurfacePreviewItem::VACVersion => StatusLineItem::VACVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("vac".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::VACVersion => self
                .status_line_value_for_item(StatusLineItem::VACVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
