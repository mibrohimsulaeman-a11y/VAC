...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::VACOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::VACOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            crate::session_protocol::ThreadNameUpdatedNotification {
...
async fn slash_init_prepares_tui_vac_init_without_cli_interactive_flag() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no VAC op to be sent, got {other:?}"),
    }

    let choices_path = tempdir.path().join(".vac/.init/operator_choices.yaml");
    let choices = std::fs::read_to_string(&choices_path).unwrap();
    assert!(choices.contains("surface: tui"));
    assert!(choices.contains("requires_cli_interactive_flag: false"));
    assert_eq!(
    ...
    assert!(
        rendered.contains("VAC-Init guided setup is ready"),
        "info message should describe TUI VAC-Init readiness: {rendered:?}"
    );
    assert!(
        rendered.contains("run `vac init` without `--interactive`"),
        "info message should make the no-flag TUI contract explicit: {rendered:?}"
    );
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::VACOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn usage_error_slash_command_is_available_from_local_recall() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3-vac")).await;
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(vac_config::types::KeybindingsSpec::One(
        vac_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
...
async fn active_goal_without_follow_up_suppresses_agent_turn_complete_notification() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            crate::session_protocol::ThreadGoalUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: None,
                goal: crate::session_protocol::ThreadGoal {
                    thread_id: vac_protocol::ThreadId::new(),
                    objective: "finish the benchmark".to_string(),
                    status: crate::session_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
#[tokio::test]
async fn slash_branch_requests_current_branch() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.dispatch_command(SlashCommand::Branch);

    assert_matches!(rx.try_recv(), Ok(AppEvent::BranchCurrentSession));
}

#[tokio::test]
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/vac-test-rollout.jsonl");
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5.3-vac")).await;
    chat.set_feature_enabled(Feature::FastMode, /*enabled*/ true);
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::VACOp(Op::OverrideTurnContext {
...
async fn compact_queues_user_messages_snapshot() {
    ...
    handle_error(
        ...
        Some(VACErrorInfo::ActiveTurnNotSteerable {
