...
async fn exec_approval_uses_approval_id_when_present() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        ...
        } = app_ev
        {
            ...
            assert_matches!(
                decision,
                crate::session_protocol::CommandExecutionApprovalDecision::Accept
            );
            found = true;
            break;
        }
...
async fn unified_exec_wait_after_final_agent_message_snapshot() {
    ...
    begin_unified_exec_startup(&mut chat, "call-wait", "proc-1", "cargo test -p vac-core");
...
async fn unified_exec_wait_status_renders_command_in_single_details_row_snapshot() {
    ...
    begin_unified_exec_startup(
        ...
        "call-wait-ui",
        "proc-ui",
        "cargo test -p vac-core -- --exact some::very::long::test::name",
    );
...
async fn apply_patch_approval_sends_op_with_call_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        ...
        } = app_ev
        {
            ...
            assert_matches!(
                decision,
                crate::session_protocol::FileChangeApprovalDecision::Accept
            );
            found = true;
            break;
        }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto vac_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to vac channel");
    match forwarded {
...
async fn apply_patch_untrusted_shows_approval_modal() -> anyhow::Result<()> {
    ...
    for y in 0..area.height {
        ...
        if row.contains("the agent wants to modify files") {
            contains_title = true;
            break;
        }
    }
    assert!(
        contains_title,
        "expected approval modal to be visible with subtitle 'the agent wants to modify files'"
    );
