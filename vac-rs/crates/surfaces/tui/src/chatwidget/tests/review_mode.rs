...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::VACOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::VACOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::VACOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        crate::session_protocol::ServerNotification::TurnStarted(
            crate::session_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: crate::session_protocol::Turn {
                    id: "turn-1".to_string(),
                    items: Vec::new(),
                    status: crate::session_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        crate::session_protocol::ServerNotification::ThreadGoalUpdated(
            crate::session_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: crate::session_protocol::ThreadGoal {
                    ...
                    status: crate::session_protocol::ThreadGoalStatus::BudgetLimited,
    ...
    chat.handle_server_notification(
        crate::session_protocol::ServerNotification::TurnCompleted(
            crate::session_protocol::TurnCompletedNotification {
                thread_id: "thread-1".to_string(),
                turn: crate::session_protocol::Turn {
                    id: "turn-1".to_string(),
                    items: Vec::new(),
                    status: crate::session_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
