    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                vac_error_info: Some(VACErrorInfo::ServerOverloaded),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                vac_error_info: Some(VACErrorInfo::CyberPolicy),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
...
async fn live_app_server_model_verification_renders_warning() {
    ...
    assert!(rendered.contains("Trusted Access for Cyber"));
    assert!(rendered.contains("https://provider.vac.invalid/cyber"));
}
...
async fn live_app_server_thread_name_update_shows_resume_hint() {
    ...
    assert!(rendered.contains("vac resume review-fix"));
}
