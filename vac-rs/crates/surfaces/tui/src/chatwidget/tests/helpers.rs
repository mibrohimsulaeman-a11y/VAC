use super::*;
use crate::session_protocol::PluginAvailability;
use pretty_assertions::assert_eq;

pub(super) async fn test_config() -> Config {
    // Start from the built-in defaults so tests do not inherit host/system config.
    let vac_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_vac_home(vac_home.clone(), Vec::new())
            .await
            .expect("config");
    config.vac_home = vac_home.abs();
    config.sqlite_home = vac_home.clone();
    config.log_dir = vac_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        placeholder_text: "Ask VAC to do anything".to_string(),
    ...
    let mut widget = ChatWidget {
        app_event_tx,
        vac_op_target: super::VACOpTarget::Direct(op_tx),
        bottom_pane: bottom,
        active_cell: None,
        ...
        normal_placeholder_text: "Ask VAC to do anything".to_string(),
...
fn test_model_info(slug: &str, priority: i32, supports_fast_mode: bool) -> ModelInfo {
    let additional_speed_tiers = if supports_fast_mode {
        vec![vac_protocol::vastar_models::SPEED_TIER_FAST]
    } else {
        Vec::new()
    };
...
fn token_usage_breakdown(usage: TokenUsage) -> crate::session_protocol::TokenUsageBreakdown {
    crate::session_protocol::TokenUsageBreakdown {
        ...
        Some(info) => {
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    crate::session_protocol::ThreadTokenUsageUpdatedNotification {
                        thread_id: thread_id(chat),
                        turn_id: chat
                            .last_turn_id
                            .clone()
                            .unwrap_or_else(|| "turn-1".to_string()),
                        token_usage: crate::session_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    vac_error_info: Option<VACErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn handle_agent_message_delta(chat: &mut ChatWidget, delta: impl Into<String>) {
    chat.handle_server_notification(
        ServerNotification::AgentMessageDelta(
            crate::session_protocol::AgentMessageDeltaNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .last_turn_id
                    .clone()
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = vac_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: vac_shell_command::parse_command::shlex_join(&command),
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            crate::session_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .last_turn_id
                    .clone()
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        name: VASTAR_CURATED_MARKETPLACE_NAME.to_string(),
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: crate::session_protocol::HookEventName,
    status: crate::session_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<crate::session_protocol::HookOutputEntry>,
) -> crate::session_protocol::HookRunSummary {
    crate::session_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name: event_name.into(),
        handler_type: crate::session_protocol::HookHandlerType::Command.into(),
        execution_mode: crate::session_protocol::HookExecutionMode::Sync.into(),
        scope: crate::session_protocol::HookScope::Turn.into(),
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: crate::session_protocol::HookSource::User.into(),
        display_order: 0,
        status,
        ...
        completed_at: matches!(
            status,
            crate::session_protocol::HookRunStatus::Completed
                | crate::session_protocol::HookRunStatus::Failed
                | crate::session_protocol::HookRunStatus::Blocked
                | crate::session_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            crate::session_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                crate::session_protocol::HookOutputEntry {
                    kind: crate::session_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                crate::session_protocol::HookOutputEntry {
                    kind: crate::session_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: crate::session_protocol::HookEventName) -> &'static str {
    match event_name {
        crate::session_protocol::HookEventName::PreToolUse => "PreToolUse",
        crate::session_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        crate::session_protocol::HookEventName::PostToolUse => "PostToolUse",
        crate::session_protocol::HookEventName::SessionStart => "SessionStart",
        crate::session_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        crate::session_protocol::HookEventName::Stop => "Stop",
    }
}
