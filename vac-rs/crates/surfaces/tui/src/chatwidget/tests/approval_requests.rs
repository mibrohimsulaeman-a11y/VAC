...
fn app_server_exec_approval_request_preserves_permissions_context() {
    ...
    let request = exec_approval_request_from_params(
        AppServerCommandExecutionRequestApprovalParams {
            ...
            approval_id: Some("approval-1".to_string()),
            reason: None,
            network_approval_context: Some(AppServerNetworkApprovalContext {
    ...
    assert_eq!(
        ...
        Some(CoreNetworkApprovalContext {
            host: "example.com".to_string(),
            ...
            file_system: Some(FileSystemPermissions {
                entries: vec![
                    FileSystemSandboxEntry {
                        path: FileSystemPath::Path { path: read_path },
                        access: FileSystemAccessMode::Read,
                    },
                    FileSystemSandboxEntry {
                        path: FileSystemPath::Path { path: write_path },
...
fn app_server_request_permissions_preserves_file_system_permissions() {
    ...
    let request = request_permissions_from_params(AppServerPermissionsRequestApprovalParams {
        ...
        permissions: crate::session_protocol::RequestPermissionProfile {
            network: Some(
                AppServerAdditionalNetworkPermissions {
                    enabled: Some(true),
                }
                .into(),
            ),
