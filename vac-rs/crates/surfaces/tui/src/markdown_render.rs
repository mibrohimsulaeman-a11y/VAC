...
static MARKDOWN_RENDER_CACHE: LazyLock<Mutex<MarkdownRenderCache>> =
    LazyLock::new(|| Mutex::new(MarkdownRenderCache::default()));
const MARKDOWN_RENDER_CACHE_CAPACITY: usize = 1024;

fn markdown_render_cache_key(input: &str, width: Option<usize>, cwd: Option<&Path>) -> u64 {
...
fn markdown_render_cache_get(key: u64) -> Option<Text<'static>> {
    let mut cache = MARKDOWN_RENDER_CACHE.lock().ok()?;
    let rendered = cache.entries.get(&key).cloned()?;
    cache.lru.retain(|existing| *existing != key);
    cache.lru.push_back(key);
    Some(rendered)
}

fn markdown_render_cache_put(key: u64, rendered: &Text<'static>) {
    if let Ok(mut cache) = MARKDOWN_RENDER_CACHE.lock() {
        cache.entries.insert(key, rendered.clone());
        cache.lru.retain(|existing| *existing != key);
        cache.lru.push_back(key);
        while cache.entries.len() > MARKDOWN_RENDER_CACHE_CAPACITY {
...
pub(crate) fn render_markdown_text_with_width_and_cwd(
    ...
    let cache_key = markdown_render_cache_key(input, width, cwd);
    if let Some(rendered) = markdown_render_cache_get(cache_key) {
        return rendered;
    }
    ...
    w.run();
    markdown_render_cache_put(cache_key, &w.text);
    w.text
}
...
///
/// This accepts the path shapes VAC emits today: `file://` URLs, absolute and relative paths,
