...
use vac_install_context::InstallContext;
#[cfg(any(not(debug_assertions), test))]
use vac_install_context::StandalonePlatform;

/// Update action the CLI should perform after the TUI exits.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum UpdateAction {
    /// Update via `npm install -g @vastar/vac@latest`.
    NpmGlobalLatest,
    /// Update via `bun install -g @vastar/vac@latest`.
    BunGlobalLatest,
    /// Update via `brew upgrade vac`.
    BrewUpgrade,
    /// Update via `curl -fsSL https://developers.vastar.com/vac/install.sh | sh`.
    StandaloneUnix,
    /// Update via `irm https://developers.vastar.com/vac/install.ps1|iex`.
    StandaloneWindows,
}
...
impl UpdateAction {
    ...
    pub fn command_args(self) -> (&'static str, &'static [&'static str]) {
        match self {
            UpdateAction::NpmGlobalLatest => ("npm", &["install", "-g", "@vastar/vac"]),
            UpdateAction::BunGlobalLatest => ("bun", &["install", "-g", "@vastar/vac"]),
    ...
    fn maps_install_context_to_update_action() {
        ...
        assert_eq!(
            UpdateAction::from_install_context(&InstallContext::Standalone {
                ...
                resources_dir: Some(native_release_dir.join("vac-resources")),
            }),
