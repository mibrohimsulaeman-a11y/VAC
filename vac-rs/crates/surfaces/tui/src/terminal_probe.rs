...
mod imp {
    ...
    impl Tty {
        ...
        fn new(reader: File, writer: File) -> io::Result<Self> {
            ...
            // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `libc::fcntl(fd, libc::F_GETFL)` uses inputs that remain live for the duration of this block.
            let original_flags = unsafe { libc::fcntl(fd, libc::F_GETFL) };
            if original_flags == -1 {
                return Err(io::Error::last_os_error());
            }
            // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `libc::fcntl(fd, libc::F_SETFL, original_flags | libc::O_NONBLOCK)` uses inputs that remain live for the duration of this block.
        ...
        fn read_available(&mut self, buffer: &mut Vec<u8>) -> io::Result<()> {
            ...
            loop {
                // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; the multi-line unsafe block below uses inputs that remain live until the block returns.
                let count = unsafe {
                    libc::read(
    ...
    impl Drop for Tty {
        fn drop(&mut self) {
            let _ =
                // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `libc::fcntl(self.reader.as_raw_fd(), libc::F_SETFL, self.original_flags)` uses inputs that remain live for the duration of this block.
    ...
    fn dup_file(fd: libc::c_int) -> io::Result<File> {
        // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `libc::dup(fd)` uses inputs that remain live for the duration of this block.
        let duplicated = unsafe { libc::dup(fd) };
        if duplicated == -1 {
            return Err(io::Error::last_os_error());
        }
        // SAFETY: Terminal UI boundary: this adapter validates the terminal, clipboard, or editor handle before the platform call; `File::from_raw_fd(duplicated)` uses inputs that remain live for the duration of this block.
