...
use vac_shell_command::bash::extract_bash_command;
use vac_utils_elapsed::format_duration;
...
impl ExecCell {
    ...
    fn exploring_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let omitted_call_count = self
            .calls
            .len()
            .saturating_sub(crate::operator_ui::TOOL_TIMELINE_LIMIT);
        let mut calls = self
            .calls
            .iter()
            .skip(omitted_call_count)
            .cloned()
            .collect::<Vec<_>>();
        ...
        if omitted_call_count > 0 {
            out_indented.push(
                format!(
                    "tool timeline limited to last 5 · +{omitted_call_count} earlier tool calls"
                )
                .dim()
                .into(),
            );
