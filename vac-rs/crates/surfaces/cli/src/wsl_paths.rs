...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\vac.zip").as_deref(),
            Some("/mnt/c/Temp/vac.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/vac.tgz").as_deref(),
            Some("/mnt/d/Work/vac.tgz")
        );
        assert!(win_path_to_wsl("/home/user/vac").is_none());
    }
