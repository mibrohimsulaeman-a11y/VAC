...
mod tests {
    ...
    fn test_strip_residue_no_prefix() {
        ...
        let line = Line::from(vec![Span::raw("    name: VAC Dev")]);
        let extracted = " name: VAC Dev"; // after border filter (no borders here, but simulating)
        let result = strip_decorative_prefix_residue(extracted, 0, &line);
        assert_eq!(result, " name: VAC Dev");
    }
    ...
    fn test_extract_code_block_preserves_indentation() {
        ...
        let lines = vec![
            Line::from(vec![Span::raw("name: VAC Dev")]),
