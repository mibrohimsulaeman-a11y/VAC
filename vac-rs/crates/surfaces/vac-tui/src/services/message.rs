...
use uuid::Uuid;
#[cfg(test)]
use vac_foundation::models::integrations::openai::FunctionCall;
...
pub enum MessageContent {
    ...
    RenderTaskWaitBlock(
        Vec<vac_foundation::models::integrations::openai::TaskUpdate>,
        f64,
        Vec<String>,
    ),
    ...
    RenderSubagentResumePendingBlock(
        ToolCall,
        bool,
        Option<vac_foundation::models::integrations::openai::TaskPauseInfo>,
    ),
    ...
    RenderAskUserBlock {
        questions: Vec<vac_foundation::models::integrations::openai::AskUserQuestion>,
...
impl Message {
    ...
    pub fn render_task_wait_block(
        task_updates: Vec<vac_foundation::models::integrations::openai::TaskUpdate>,
        progress: f64,
...
fn compute_cache_key(state: &AppState, width: usize) -> u64 {
    ...
    state.shell_popup_state.is_visible.hash(&mut hasher);
    state.side_panel_state.is_shown.hash(&mut hasher);
...
pub fn get_cached_line_count(state: &AppState, width: usize) -> Option<usize> {
    ...
    if let Some((cached_key, ref cached_lines, _)) =
...
pub fn get_visible_lines_arc(
    ...
    let visible = if let Some((_, ref cached_lines, _)) =
        state.messages_scrolling_state.assembled_lines_cache
    {
...
pub fn get_wrapped_message_lines_cached(state: &mut AppState, width: usize) -> Vec<Line<'static>> {
    ...
    if let Some((cached_key, cached_lines, _)) =
        &state.messages_scrolling_state.assembled_lines_cache
        && *cached_key == cache_key
    {
    ...
    let message_refs: Vec<&Message> = if state.shell_popup_state.is_visible {
        state
            .messages_scrolling_state
            .messages
            .iter()
    ...
    state.messages_scrolling_state.cache_generation = state
        .messages_scrolling_state
        .cache_generation
        .wrapping_add(1);
...
fn render_single_message_internal(msg: &Message, width: usize) -> Vec<(Line<'static>, Style)> {
    ...
    match &msg.content {
        ...
        MessageContent::RenderFullContentMessage(tool_call_result) => {
            let tool_name =
                vac_foundation::utils::strip_tool_name(&tool_call_result.call.function.name);
...
pub fn cleanup_message_cache(state: &mut AppState) {
    let valid_ids: std::collections::HashSet<Uuid> = state
        .messages_scrolling_state
        .messages
        .iter()
        .map(|m| m.id)
...
pub fn get_wrapped_collapsed_message_lines_cached(
    ...
    let cache_valid = if let Some((cached_messages, cached_width, _)) =
