...
use crate::services::helper_block::{
    ...
    push_styled_message, push_support_message, push_usage_message, render_system_message,
    welcome_messages,
};
use crate::services::layout::centered_rect;
use crate::services::message::{Message, MessageContent};
use crate::services::vac_operator::VacOperatorRoute;
...
impl CommandAction {
    ...
    pub fn to_command_id(&self) -> Option<&'static str> {
        match self {
            ...
            CommandAction::OpenRuntimeJobs => Some("/runtime"),
            CommandAction::OpenCapabilities => Some("/capabilities"),
            CommandAction::OpenAssessment => Some("/assessment"),
            CommandAction::OpenSpecSync => Some("/spec-sync"),
            CommandAction::OpenFirstLaunch => Some("/first-launch"),
            CommandAction::OpenIdle => Some("/idle"),
            CommandAction::OpenChat => Some("/chat"),
...
pub fn get_all_commands() -> Vec<Command> {
    vec![
        ...
        Command::new(
            "Status",
            "Show VAC startup/status console",
            "/status",
            CommandAction::ShowStatus,
        ),
        Command::new(
            "Runtime Jobs",
            "Open VAC one-shot, cron, and filewatch monitor",
            "/runtime",
            CommandAction::OpenRuntimeJobs,
        ),
        Command::new(
            "Capability Dashboard",
            "Inspect declared/computed/effective readiness",
            "/capabilities",
            CommandAction::OpenCapabilities,
        ),
        Command::new(
            "Assessment",
            "Inspect assessment-grounded gap report",
            "/assessment",
            CommandAction::OpenAssessment,
        ),
        Command::new(
            "SpecSync",
            "Review spec/manifest/code drift proposals",
            "/spec-sync",
            CommandAction::OpenSpecSync,
        ),
        Command::new(
            "Submit Issue",
...
pub fn commands_to_helper_commands() -> Vec<HelperCommand> {
    ...
    vec![
        ...
        HelperCommand {
            command: "/status".into(),
            description: "Show VAC workspace, provider, model, and token usage".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
        ...
        HelperCommand {
            ...
            description: "Open the right context/tool timeline panel".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
            command: "/timeline".into(),
            description: "Open the right tool timeline/context panel".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
            command: "/issue".into(),
        ...
        HelperCommand {
            ...
            description: "Open VAC runtime jobs monitor".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
            command: "/capabilities".into(),
            description: "Open VAC capability dashboard".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
            command: "/assessment".into(),
            description: "Open VAC assessment findings".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
            command: "/spec-sync".into(),
            description: "Review VAC spec drift proposals".into(),
            source: CommandSource::BuiltIn,
        },
        HelperCommand {
            command: "/init".into(),
...
pub fn execute_command(command_id: CommandId<'_>, ctx: CommandContext) -> Result<(), String> {
    ...
    match command_id {
        ...
        "/status" => {
            set_vac_route(ctx.state, VacOperatorRoute::FirstLaunch);
            Ok(())
        }

        "/runtime" => {
            set_vac_route(ctx.state, VacOperatorRoute::RuntimeJobs);
            Ok(())
        }
        "/capabilities" => {
            set_vac_route(ctx.state, VacOperatorRoute::Capabilities);
            Ok(())
        }
        "/assessment" => {
            set_vac_route(ctx.state, VacOperatorRoute::Assessment);
            Ok(())
        }
        "/spec-sync" => {
            set_vac_route(ctx.state, VacOperatorRoute::SpecSync);
            Ok(())
        }
        "/first-launch" => {
            set_vac_route(ctx.state, VacOperatorRoute::FirstLaunch);
        ...
        "/idle" => {
            set_vac_route(ctx.state, VacOperatorRoute::Idle);
            Ok(())
        }
        "/chat" => {
            set_vac_route(ctx.state, VacOperatorRoute::Chat);
            Ok(())
        }
        "/sessions" => {
        ...
        "/context" | "/timeline" | "/tools" => {
            ctx.state.side_panel_state.is_shown = true;
            ctx.state.input_state.text_area.set_text("");
            ctx.state.input_state.show_helper_dropdown = false;
            Ok(())
        }
        ...
        _ => {
            ...
            //   - Custom: user-defined .md commands from .vac/commands/
            //
...
fn set_vac_route(state: &mut AppState, route: VacOperatorRoute) {
    state.vac_operator_state.route = route;
