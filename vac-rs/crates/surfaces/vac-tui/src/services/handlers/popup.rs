...
/// down to just the last segment, producing clean entries like `"vac/glm-5"`.
...
/// `"vac/glm-5"` matches a catalog model with
/// `id: "fireworks-ai/accounts/fireworks/models/glm-5", provider: "vac"`.
...
pub fn handle_available_models_loaded(
    ...
    // "vac" provider always first, then alphabetically
    let mut sorted_models = models;
    sorted_models.sort_by(|a, b| {
        match (a.provider.as_str() == "vac", b.provider.as_str() == "vac") {
...
pub fn handle_model_switcher_select(state: &mut AppState, output_tx: &Sender<OutputEvent>) {
            ...
            >= state.model_switcher_state.available_models.len()
    {
        ...
        push_error_message(
            state,
            "No selectable models are available yet. Try /model again after the model catalog loads.",
            None,
        );
        return;
    }

    {
        ...
        if !filtered.contains(&state.model_switcher_state.is_selected) {
            state.model_switcher_state.is_visible = false;
            state.model_switcher_state.search.clear();
            push_error_message(state, "No model matches the current filter.", None);
            return;
        }
