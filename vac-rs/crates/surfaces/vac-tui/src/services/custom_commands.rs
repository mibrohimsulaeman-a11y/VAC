...
//! - `~/.vac/commands/` (global)
//! - `.vac/commands/`   (project-local, higher priority on name clash)
//!
...
///
/// Project-local commands (`.vac/commands/`) take priority over global
/// commands (`~/.vac/commands/`) when both define the same filename.
///
...
Check the health of all services."#;
    ...
    fn test_parse_command_file_with_front_matter() {
        let dir = std::env::temp_dir().join("vac_test_custom_cmds");
    ...
    fn test_parse_command_file_no_front_matter() {
        let dir = std::env::temp_dir().join("vac_test_custom_cmds_2");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn test_parse_command_file_empty() {
        let dir = std::env::temp_dir().join("vac_test_custom_cmds_3");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn test_description_truncation() {
        let dir = std::env::temp_dir().join("vac_test_custom_cmds_4");
        let _ = std::fs::create_dir_all(&dir);
