...
/// Example prompts that showcase VAC's strengths
const VAC_PROMPTS: &[&str] = &[
    "Dockerize my app",
...
static VAC_INDEX: OnceLock<usize> = OnceLock::new();
...
pub fn get_placeholder_prompt(textarea: &TextArea) -> &'static str {
    if textarea.is_shell_mode() {
        ...
        // Return a random VAC prompt (selected once per session)
        let index = VAC_INDEX.get_or_init(|| {
            use std::collections::hash_map::DefaultHasher;
            use std::hash::{Hash, Hasher};
            use std::time::{SystemTime, UNIX_EPOCH};
            ...
            SystemTime::now()
                ...
                .as_nanos()
                .hash(&mut hasher);
            hasher.finish() as usize % VAC_PROMPTS.len()
        });
        VAC_PROMPTS[*index]
    }
}
