...
use tokio::sync::mpsc;
use vac_foundation::models::integrations::openai::ToolCall;
use vac_foundation::utils::{backward_compatibility_mapping, strip_tool_name};
...
impl Default for AutoApproveConfig {
    fn default() -> Self {
        ...
        tools.insert(
            "run_command::vac::version".to_string(),
            AutoApprovePolicy::Auto,
        );
        tools.insert(
            "run_command::vac::account".to_string(),
            AutoApprovePolicy::Auto,
        );
        tools.insert(
            "run_command::vac::completion".to_string(),
            AutoApprovePolicy::Auto,
        );
        tools.insert("run_command::vac::ak".to_string(), AutoApprovePolicy::Auto);
        tools.insert(
            "run_command::vac::sessions".to_string(),
            AutoApprovePolicy::Auto,
        );
        tools.insert(
            "run_command::vac::config::show".to_string(),
            AutoApprovePolicy::Auto,
        );
        tools.insert(
            "run_command::vac::config::sample".to_string(),
            AutoApprovePolicy::Auto,
        );
...
impl AutoApproveManager {
    ...
    /// Persist the current config to the project-local `.vac/session/auto_approve.json`.
...
fn resolve_shell_scope(
    ...
    match vac_shell_approval::resolve_hierarchical_policy(
        command_str,
        tool_name,
        &fallback_scopes,
        &effective_rules,
...
mod tests {
    ...
    async fn test_error_handling_with_invalid_config_file() {
        ...
        let config_dir = temp_dir.path().join(".vac/session");
    ...
    use vac_foundation::models::integrations::openai::{FunctionCall, ToolCall};
    ...
    #[test]
    fn default_config_includes_vac_allowlist_entries() {
        let config = AutoApproveConfig::default();

        for key in [
            "run_command::vac::version",
            "run_command::vac::account",
            "run_command::vac::completion",
            "run_command::vac::ak",
            "run_command::vac::sessions",
            "run_command::vac::config::show",
            "run_command::vac::config::sample",
        ] {
            assert_eq!(
    ...
    #[test]
    fn default_config_does_not_include_vac_scope_level_entry() {
        let config = AutoApproveConfig::default();
        assert_eq!(config.tools.get("run_command::vac"), None);
    }
    ...
    #[test]
    fn resolve_shell_scope_vac_version_auto_approved() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac version");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Auto));
    }

    #[test]
    fn resolve_shell_scope_vac_ak_namespace_matches_trailing_args() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac ak write notes.md");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Auto));
    }

    #[test]
    fn resolve_shell_scope_vac_config_show_two_segment_match() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac config show");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Auto));
    }

    #[test]
    fn resolve_shell_scope_vac_config_list_still_prompts() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac config list");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Prompt));
    }

    #[test]
    fn resolve_shell_scope_vac_update_still_prompts() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac update");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Prompt));
    }

    #[test]
    fn resolve_shell_scope_vac_bare_still_prompts() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Prompt));
    }

    #[test]
    fn resolve_shell_scope_vac_browser_does_not_bypass_ak_allowlist() {
        let config = AutoApproveConfig::default();
        let tc = make_run_command_tool_call("vac browser ak visit example.com");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Prompt));
    }
    ...
    #[test]
    fn resolve_shell_scope_run_command_task_inherits_vac_ak_allowlist() {
        let config = AutoApproveConfig::default();
        let tc = make_tool_call("run_command_task", "vac ak tree");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Auto));
    }
    ...
    fn resolve_shell_scope_user_override_prompt_wins_over_allowlist() {
        let mut config = AutoApproveConfig::default();
        config.tools.insert(
            "run_command::vac::ak".to_string(),
            AutoApprovePolicy::Prompt,
        );

        let tc = make_run_command_tool_call("vac ak search --tree");
        let result = resolve_shell_scope(&tc, &config.tools, &config.default_policy);
        assert_eq!(result, Some(AutoApprovePolicy::Prompt));
    }
    ...
    fn resolve_shell_scope_user_subrule_tightens_write_default_only() {
        let mut config = AutoApproveConfig::default();
        config.tools.insert(
            "run_command::vac::ak::write".to_string(),
            AutoApprovePolicy::Prompt,
        );

        let write_tc = make_run_command_tool_call("vac ak write notes.md");
        ...
        let search_tc = make_run_command_tool_call("vac ak search --tree");
    ...
    #[test]
    fn should_auto_approve_vac_ak_search_with_fresh_defaults() {
    ...
    #[test]
    fn should_auto_approve_vac_ak_write_with_fresh_defaults() {
        let manager = AutoApproveManager {
            original_config: AutoApproveConfig::default(),
            config: AutoApproveConfig::default(),
            config_path: PathBuf::from(AUTO_APPROVE_CONFIG_PATH),
    ...
    #[test]
    fn should_not_auto_approve_vac_update_with_fresh_defaults() {
        let manager = AutoApproveManager {
            original_config: AutoApproveConfig::default(),
            config: AutoApproveConfig::default(),
            config_path: PathBuf::from(AUTO_APPROVE_CONFIG_PATH),
    ...
    #[test]
    fn should_not_auto_approve_vac_browser_ak_visit_with_fresh_defaults() {
        let manager = AutoApproveManager {
            original_config: AutoApproveConfig::default(),
            config: AutoApproveConfig::default(),
            config_path: PathBuf::from(AUTO_APPROVE_CONFIG_PATH),
    ...
    fn matches_pattern_exact() {
        assert!(vac_shell_approval::matches_pattern("push", "push"));
        assert!(!vac_shell_approval::matches_pattern("push", "pull"));
    }

    #[test]
    fn matches_pattern_glob() {
        assert!(vac_shell_approval::matches_pattern(
            "*.prod.*",
    ...
    fn matches_pattern_regex() {
        assert!(vac_shell_approval::matches_pattern("re:^push$", "push"));
        assert!(!vac_shell_approval::matches_pattern(
            "re:^push$",
            "push-force"
        ));
