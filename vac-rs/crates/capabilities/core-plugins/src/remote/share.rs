...
pub async fn list_remote_plugin_shares(
    ...
    let local_plugin_paths =
        local_paths::load_plugin_share_local_paths(vac_home).unwrap_or_else(|err| {
...
pub async fn delete_remote_plugin_share(
    ...
    if let Err(err) = local_paths::remove_plugin_share_local_path(vac_home, remote_plugin_id) {
        warn!(
...
fn archive_plugin_for_upload_with_limit(
    ...
    if !plugin_path.join(".vac-plugin/plugin.json").is_file() {
        return Err(RemotePluginCatalogError::InvalidPluginPath {
            path: plugin_path.to_path_buf(),
            reason: "missing .vac-plugin/plugin.json".to_string(),
        });
    }
