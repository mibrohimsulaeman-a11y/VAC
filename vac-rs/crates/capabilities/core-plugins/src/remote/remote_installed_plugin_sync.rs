...
pub fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&vac_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(vac_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = vac_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
        ...
        for entry in fs::read_dir(&marketplace_root).map_err(|err| {
            ...
            if is_remote_plugin_cache_mutation_in_flight(vac_home, marketplace_name, &plugin_name) {
                continue;
            }
...
fn remote_plugin_cache_root(vac_home: &Path) -> PathBuf {
    vac_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(vac_home.path()),
        };
