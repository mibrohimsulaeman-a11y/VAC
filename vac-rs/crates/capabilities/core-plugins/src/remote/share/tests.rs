...
fn test_config(server: &MockServer) -> RemotePluginServiceConfig {
    RemotePluginServiceConfig {
        chatgpt_base_url: format!("{}/provider-api", server.uri()),
    }
}

fn test_auth() -> VACAuth {
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> PathBuf {
    ...
    write_file(
        &plugin_path.join(".vac-plugin/plugin.json"),
...
fn write_plugin_share_local_path_mapping(
    ...
    write_file(
        &vac_home.join(".tmp/plugin-share-local-paths-v1.json"),
        &format!(
            "{}\n",
...
async fn save_remote_plugin_share_creates_workspace_plugin() {
    ...
    Mock::given(method("POST"))
        .and(path("/provider-api/public/plugins/workspace/upload-url"))
    ...
    Mock::given(method("PUT"))
        ...
        .await;
    Mock::given(method("POST"))
        .and(path("/provider-api/public/plugins/workspace"))
        .and(header("authorization", "Bearer Access Token"))
        .and(header("provider-account-id", "account_id"))
        .and(body_json(json!({
            "file_id": "file_123",
    ...
    assert_eq!(
        local_paths::load_plugin_share_local_paths(vac_home.path()).unwrap(),
...
fn archive_plugin_for_upload_places_manifest_at_archive_root() {
    ...
    assert_eq!(
        ...
        vec![
            ".vac-plugin/plugin.json".to_string(),
...
async fn save_remote_plugin_share_updates_existing_workspace_plugin() {
    ...
    Mock::given(method("PUT"))
        ...
        .await;
    Mock::given(method("POST"))
        .and(path("/provider-api/public/plugins/workspace/plugins_123"))
        .and(body_json(json!({
            "file_id": "file_456",
            "etag": "\"upload_etag_456\"",
        })))
...
async fn list_remote_plugin_shares_fetches_created_workspace_plugins() {
    ...
    let local_plugin_path =
        AbsolutePathBuf::try_from(vac_home.path().join("local-plugin")).unwrap();
    write_plugin_share_local_path_mapping(vac_home.path(), "plugins_123", &local_plugin_path);
    let server = MockServer::start().await;
    let config = test_config(&server);
    let auth = test_auth();

    Mock::given(method("GET"))
        .and(path("/provider-api/ps/plugins/workspace/created"))
        .and(header("authorization", "Bearer Access Token"))
        .and(header("provider-account-id", "account_id"))
        .and(query_param(
            "limit",
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/provider-api/ps/plugins/installed"))
    ...
    let result = list_remote_plugin_shares(&config, Some(&auth), vac_home.path())
        .await
        .unwrap();
...
async fn delete_remote_plugin_share_deletes_workspace_plugin() {
    ...
    delete_remote_plugin_share(&config, Some(&auth), vac_home.path(), "plugins_123")
        .await
        .unwrap();
    assert_eq!(
        local_paths::load_plugin_share_local_paths(vac_home.path()).unwrap(),
