...
pub(crate) fn load_plugin_share_local_paths(
    ...
    read_plugin_share_local_paths(vac_home)
}
...
pub(crate) fn record_plugin_share_local_path(
    ...
    let _guard = lock_plugin_share_local_paths()?;
    let mut mapping = read_plugin_share_local_paths_for_update(vac_home)?;
    mapping.insert(remote_plugin_id.to_string(), plugin_path);
    write_plugin_share_local_paths(vac_home, mapping)
}
...
fn read_plugin_share_local_paths(vac_home: &Path) -> io::Result<BTreeMap<String, AbsolutePathBuf>> {
    let path = plugin_share_local_paths_path(vac_home);
...
fn read_plugin_share_local_paths_for_update(
    ...
    match read_plugin_share_local_paths(vac_home) {
...
fn plugin_share_local_paths_path(vac_home: &Path) -> std::path::PathBuf {
    vac_home.join(PLUGIN_SHARE_LOCAL_PATHS_FILE)
}
