...
pub(crate) fn start_startup_remote_plugin_sync_once(
    ...
    let marker_path = startup_remote_plugin_sync_marker_path(vac_home.as_path());
    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(vac_home.as_path()).await {
            warn!(
                vac_home = %vac_home.display(),
            ...
            .await
        {
            Ok(sync_result) => {
                ...
                if let Err(err) = write_startup_remote_plugin_sync_marker(vac_home.as_path()).await
                {
                    warn!(
                        error = %err,
...
fn startup_remote_plugin_sync_marker_path(vac_home: &Path) -> PathBuf {
    vac_home.join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE)
}

async fn wait_for_startup_remote_plugin_sync_prerequisites(vac_home: &Path) -> bool {
...
async fn write_startup_remote_plugin_sync_marker(vac_home: &Path) -> std::io::Result<()> {
    let marker_path = startup_remote_plugin_sync_marker_path(vac_home);
