...
const LEGACY_CHATGPT_CLOUD_PLUGIN_ENV: &str = "VAC_ENABLE_LEGACY_CHATGPT_CLOUD_PLUGINS";
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(vac_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(vac_home.clone())
    ...
    let legacy_remote_plugin_cache_root = vac_home
...
fn legacy_chatgpt_cloud_plugins_enabled() -> bool {
    std::env::var(LEGACY_CHATGPT_CLOUD_PLUGIN_ENV)
        .ok()
        .is_some_and(|value| matches!(value.as_str(), "1" | "true"))
}

fn ensure_chatgpt_auth(auth: Option<&VACAuth>) -> Result<&VACAuth, RemotePluginCatalogError> {
    if !legacy_chatgpt_cloud_plugins_enabled() {
        return Err(RemotePluginCatalogError::LegacyCloudDisabled);
    }
