...
fn write_vastar_curated_marketplace(root: &Path, plugin_names: &[&str]) {
...
fn write_curated_plugin_sha(vac_home: &Path) {
    write_file(
        &vac_home.join(".tmp/plugins.sha"),
        &format!("{TEST_CURATED_PLUGIN_SHA}\n"),
    );
}

fn has_plugins_clone_dirs(vac_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(vac_home.join(".tmp")) else {
        return false;
    };
...
async fn mount_github_repo_and_ref(server: &MockServer, sha: &str) {
    Mock::given(method("GET"))
        .and(path("/repos/vastar/plugins"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"{"default_branch":"main"}"#))
        .mount(server)
        .await;
    Mock::given(method("GET"))
        .and(path("/repos/vastar/plugins/git/ref/heads/main"))
        .respond_with(
...
async fn mount_github_zipball(server: &MockServer, sha: &str, bytes: Vec<u8>) {
    Mock::given(method("GET"))
        .and(path(format!("/repos/vastar/plugins/zipball/{sha}")))
        .respond_with(
            ResponseTemplate::new(200)
...
async fn mount_export_archive(server: &MockServer, bytes: Vec<u8>) -> String {
    let export_api_url = format!("{}/provider-api/plugins/export/curated", server.uri());
    Mock::given(method("GET"))
        .and(path("/provider-api/plugins/export/curated"))
...
async fn run_sync_with_transport_overrides(
    ...
    tokio::task::spawn_blocking(move || {
        sync_vastar_plugins_repo_with_transport_overrides(
            vac_home.as_path(),
            &git_binary,
            &api_base_url,
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_vastar_plugins_repo_via_http(vac_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.vac-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_vac_home_tmp_dir() {
...
#[cfg(unix)]
#[test]
fn sync_vastar_plugins_repo_prefers_git_when_available() {
    let tmp = tempdir().expect("tempdir");
...
if [ "$1" = "clone" ]; then
  dest="$5"
  mkdir -p "$dest/.git" "$dest/.agents/plugins" "$dest/plugins/gmail/.vac-plugin"
  cat > "$dest/.agents/plugins/marketplace.json" <<'EOF'
{{"name":"vastar-curated","plugins":[{{"name":"gmail","source":{{"source":"local","path":"./plugins/gmail"}}}}]}}
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$dest/plugins/gmail/.vac-plugin/plugin.json"
  exit 0
fi
...
"#
    ...
    let synced_sha = sync_vastar_plugins_repo_with_transport_overrides(
        tmp.path(),
        git_path.to_str().expect("utf8 path"),
        "http://127.0.0.1:9",
        "http://127.0.0.1:9/provider-api/plugins/export/curated",
    )
...
#[cfg(unix)]
#[test]
fn sync_vastar_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    let tmp = tempdir().expect("tempdir");
    ...
    let remote_repo = repo_root.path().join("remotes/vastar/plugins.git");
    std::fs::create_dir_all(work_repo.join(".agents/plugins")).expect("create marketplace dir");
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.vac-plugin"))
        .expect("create plugin dir");
    std::fs::write(
        work_repo.join(".agents/plugins/marketplace.json"),
        r#"{"name":"vastar-curated","plugins":[{"name":"gmail","source":{"source":"local","path":"./plugins/gmail"}}]}"#,
    )
    .expect("write marketplace");
    std::fs::write(
        work_repo.join("plugins/gmail/.vac-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    let commit_status = Command::new("git")
        ...
        .arg(&work_repo)
        .arg("-c")
        .arg("user.name=VAC Test")
        .arg("-c")
        .arg("user.email=vac@example.com")
        .arg("commit")
        .arg("-m")
        .arg("init")
        .status()
    ...
    let synced_sha =
        sync_vastar_plugins_repo_via_git(tmp.path(), git_wrapper.to_str().expect("utf8 path"))
            .expect("git sync should succeed");
...
#[tokio::test]
async fn sync_vastar_plugins_repo_falls_back_to_http_when_git_is_unavailable() {
    let tmp = tempdir().expect("tempdir");
...
#[cfg(unix)]
#[tokio::test]
async fn sync_vastar_plugins_repo_falls_back_to_http_when_git_sync_fails() {
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder::new()
...
#[cfg(unix)]
#[test]
fn sync_vastar_plugins_repo_via_git_cleans_up_staged_dir_on_clone_failure() {
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder::new()
...
"#
    ...
    let err = sync_vastar_plugins_repo_via_git(tmp.path(), git_path.to_str().expect("utf8 path"))
...
#[tokio::test]
async fn sync_vastar_plugins_repo_via_http_cleans_up_staged_dir_on_extract_failure() {
    let tmp = tempdir().expect("tempdir");
    let server = MockServer::start().await;
    let sha = "0123456789abcdef0123456789abcdef01234567";
...
#[tokio::test]
async fn sync_vastar_plugins_repo_falls_back_to_export_archive_when_no_snapshot_exists() {
    let tmp = tempdir().expect("tempdir");
    let server = MockServer::start().await;
...
#[tokio::test]
async fn sync_vastar_plugins_repo_skips_export_archive_when_snapshot_exists() {
    let tmp = tempdir().expect("tempdir");
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_vastar_curated_marketplace(&curated_root, &["linear"]);
    write_curated_plugin_sha(tmp.path());

    let plugin_manifest_path = curated_root.join("plugins/linear/.vac-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
    ...
    let root = format!("vastar-plugins-{sha}");
    writer
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.vac-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.vac-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");
