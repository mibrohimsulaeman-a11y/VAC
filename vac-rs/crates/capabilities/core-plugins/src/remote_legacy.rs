...
const DEFAULT_REMOTE_MARKETPLACE_NAME: &str = "vastar-curated";
...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::VAC).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_vac_backend()) {
        request =
            request.headers(vac_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_vac_backend_auth(auth: Option<&VACAuth>) -> Result<&VACAuth, RemotePluginMutationError> {
    let Some(auth) = auth else {
