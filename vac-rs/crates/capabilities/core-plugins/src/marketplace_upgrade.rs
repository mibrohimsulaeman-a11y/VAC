...
use vac_config::MarketplaceConfigUpdate;
use vac_config::record_user_marketplace;
use vac_config::types::MarketplaceConfig;
use vac_config::types::MarketplaceSourceType;
use vac_plugin::validate_plugin_segment;
...
pub fn upgrade_configured_git_marketplaces(
    ...
    for marketplace in marketplaces {
        match upgrade_configured_git_marketplace(vac_home, &install_root, &marketplace) {
...
fn marketplace_install_root(vac_home: &Path) -> PathBuf {
    vac_home.join(INSTALLED_MARKETPLACES_DIR)
}
...
fn upgrade_configured_git_marketplace(
    ...
    activate_marketplace_root(&destination, staged_dir, || {
        ensure_configured_git_marketplace_unchanged(vac_home, marketplace)?;
        record_user_marketplace(vac_home, &marketplace.name, &update).map_err(|err| {
            format!(
...
fn ensure_configured_git_marketplace_unchanged(
    ...
    let current = read_configured_git_marketplace(vac_home, &expected.name)?;
    match current {
