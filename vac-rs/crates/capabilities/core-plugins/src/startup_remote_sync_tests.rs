...
use crate::test_support::write_vastar_curated_marketplace;
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/provider-api/plugins/list"))
        .and(header("authorization", "Bearer Access Token"))
        .and(header("provider-account-id", "account_id"))
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"linear","marketplace_name":"vastar-curated","version":"1.0.0","enabled":true}
]"#,
        ))
        .mount(&server)
        .await;
    ...
    assert!(
        tmp.path()
            .join(format!(
                "plugins/cache/vastar-curated/linear/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
            ))
            .is_dir()
    );
    let config =
        std::fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).expect("config should exist");
    assert!(config.contains(r#"[plugins."linear@vastar-curated"]"#));
