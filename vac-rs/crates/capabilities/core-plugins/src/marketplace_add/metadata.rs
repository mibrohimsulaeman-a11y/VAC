...
pub(super) fn record_added_marketplace_entry(
    ...
    record_user_marketplace(vac_home, marketplace_name, &update).map_err(|err| {
...
mod tests {
    ...
    fn installed_marketplace_root_for_source_propagates_config_read_errors() {
        ...
        let config_path = vac_home.path().join(CONFIG_TOML_FILE);
        fs::create_dir(&config_path).unwrap();

        let install_root = vac_home.path().join("marketplaces");
    ...
    fn installed_marketplace_root_for_source_uses_local_source_root() {
        let vac_home = TempDir::new().unwrap();
        let install_root = vac_home.path().join("marketplaces");
        let source_root = vac_home.path().join("source");
        ...
        let install_metadata = MarketplaceInstallMetadata::from_source(&source, &[]);
        record_added_marketplace_entry(vac_home.path(), "debug", &install_metadata).unwrap();
