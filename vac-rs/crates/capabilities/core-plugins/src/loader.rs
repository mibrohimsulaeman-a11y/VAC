...
use vac_config::HooksFile;
use vac_config::types::McpServerConfig;
use vac_config::types::PluginConfig;
use vac_config::types::PluginMcpServerConfig;
use vac_core_skills::SkillMetadata;
use vac_core_skills::config_rules::SkillConfigRules;
use vac_core_skills::config_rules::resolve_disabled_skill_paths;
use vac_core_skills::config_rules::skill_config_rules_from_stack;
use vac_core_skills::loader::SkillRoot;
use vac_core_skills::loader::load_skills_from_roots;
use vac_exec_server::LOCAL_FS;
use vac_plugin::AppConnectorId;
use vac_plugin::LoadedPlugin;
use vac_plugin::PluginCapabilitySummary;
use vac_plugin::PluginHookSource;
use vac_plugin::PluginId;
use vac_plugin::PluginIdError;
use vac_plugin::PluginLoadOutcome;
...
use vac_utils_plugins::find_plugin_manifest_path;
...
pub fn remote_installed_plugins_to_config(
    ...
    plugins
        ...
        .filter_map(|plugin| {
            ...
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(remote plugins): download or update missing local bundles during remote
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(vac_home.to_path_buf()).map_err(|err| err.to_string())?;
    ...
    for plugin in curated_marketplace.plugins {
        ...
        if plugin_sources.contains_key(&plugin_name) {
            warn!(
                plugin = plugin_name,
                marketplace = VASTAR_CURATED_MARKETPLACE_NAME,
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_vac_home(
            vac_home,
    ...
    for marketplace in marketplace_outcome.marketplaces {
        if marketplace.name == VASTAR_CURATED_MARKETPLACE_NAME {
            continue;
        }
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(vac_home, &source).map_err(|err| {
...
fn configured_plugins_from_vac_home(
    vac_home: &Path,
    read_error_message: &str,
    parse_error_message: &str,
) -> HashMap<String, PluginConfig> {
    let config_path = vac_home.join(CONFIG_TOML_FILE);
...
fn curated_plugin_ids_from_config_keys(
    ...
    .into_iter()
    .filter(|plugin_id| plugin_id.marketplace_name == VASTAR_CURATED_MARKETPLACE_NAME)
    .collect::<Vec<_>>();
...
pub fn configured_curated_plugin_ids_from_vac_home(vac_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_vac_home(
        vac_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".vac-plugin/plugin.json"));
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(vac_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
fn normalize_plugin_mcp_server_value(
        ...
        && oauth.contains_key("callbackPort")
    {
        warn!(
            plugin = %plugin_root.display(),
            "plugin MCP server OAuth callbackPort is ignored; VAC uses global MCP OAuth callback settings"
        );
    }
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = vac_home.join("plugins/.marketplace-plugin-source-staging");
