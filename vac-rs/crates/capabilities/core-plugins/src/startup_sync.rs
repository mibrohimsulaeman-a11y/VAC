...
use tracing::warn;
use vac_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use vac_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use zip::ZipArchive;
...
// Keep this comfortably above a normal sync attempt so we do not race another VAC process.
...
pub fn curated_plugins_repo_path(vac_home: &Path) -> PathBuf {
    vac_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn read_curated_plugins_sha(vac_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(vac_home).as_path())
}

fn curated_plugins_sha_path(vac_home: &Path) -> PathBuf {
    vac_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_vastar_plugins_repo(vac_home: &Path) -> Result<String, String> {
...
fn sync_vastar_plugins_repo_with_transport_overrides(
    vac_home: &Path,
    git_binary: &str,
    api_base_url: &str,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    match sync_vastar_plugins_repo_via_git(vac_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_vastar_plugins_repo_via_http(vac_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(vac_home) {
                        ...
                        // The export archive is a lagging backup path. It is disabled by default
                        // and only reachable with explicit legacy opt-in.
                        warn!(
                            error = %http_err,
                            backup_archive_api_url,
                            "GitHub HTTP sync failed for curated plugin sync; legacy ChatGPT export archive fallback explicitly enabled"
                        );
                        let result = sync_vastar_plugins_repo_via_backup_archive(
                            vac_home,
                            backup_archive_api_url,
                        );
...
fn sync_vastar_plugins_repo_via_git(vac_home: &Path, git_binary: &str) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(vac_home);
    let sha_path = vac_home.join(CURATED_PLUGINS_SHA_FILE);
    ...
    let clone_output = run_git_command_with_timeout(
        Command::new(git_binary)
            ...
            .arg("--depth")
            .arg("1")
            .arg("https://github.com/vastar/plugins.git")
...
fn sync_vastar_plugins_repo_via_http(
    vac_home: &Path,
    api_base_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(vac_home);
...
fn sync_vastar_plugins_repo_via_backup_archive(
    vac_home: &Path,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(vac_home);
    let sha_path = curated_plugins_sha_path(vac_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(vac_home: &Path) -> bool {
    curated_plugins_repo_path(vac_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && vac_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
...
fn emit_curated_plugins_startup_sync_counter(
    ...
    let Some(metrics) = vac_otel::global() else {
        return;
    };
...
async fn fetch_curated_repo_remote_sha(api_base_url: &str) -> Result<String, String> {
    ...
    let repo_url = format!("{api_base_url}/repos/{VASTAR_PLUGINS_OWNER}/{VASTAR_PLUGINS_REPO}");
