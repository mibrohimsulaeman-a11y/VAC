...
#[test]
fn try_new_rejects_relative_vac_home() {
    let err =
        PluginStore::try_new(PathBuf::from("relative")).expect_err("relative vac home should fail");
...
fn install_copies_plugin_into_default_marketplace() {
    ...
    assert!(installed_path.join(".vac-plugin/plugin.json").is_file());
...
fn install_with_version_uses_requested_cache_version() {
    ...
    write_plugin(tmp.path(), "sample-plugin", "sample-plugin");
    let plugin_id =
        PluginId::new("sample-plugin".to_string(), "vastar-curated".to_string()).unwrap();
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/vastar-curated/sample-plugin/{plugin_version}"
    ));
    assert_eq!(
        result,
        PluginInstallResult {
