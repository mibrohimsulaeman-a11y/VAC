...
pub async fn run(context: Arc<MemoryStartupContext>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.vac_home);
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            vac_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            vac_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            vac_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            vac_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[vac_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
...
mod agent {
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<vac_state::Stage1Output>,
        memory_root: vac_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<vac_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[vac_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
