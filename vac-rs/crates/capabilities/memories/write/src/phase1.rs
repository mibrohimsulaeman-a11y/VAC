...
use vac_rollout::INTERACTIVE_SESSION_SOURCES;
use vac_rollout::should_persist_response_item_for_memories;
use vac_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<vac_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            vac_state::Stage1StartupClaimParams {
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: vac_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> vac_protocol::error::Result<String> {
        let filtered = items
            .iter()
            .filter_map(|item| {
        ...
        let serialized = serde_json::to_string(&filtered).map_err(|err| {
            VACErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })?;
        ...
        let serialized =
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: vac_protocol::models::FunctionCallOutputPayload {
                        body: vac_protocol::models::FunctionCallOutputBody::Text(
