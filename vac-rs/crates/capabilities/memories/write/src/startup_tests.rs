...
use vac_git_utils::diff_since_latest_init;
use vac_git_utils::reset_git_repository;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_vac(&server, home.clone()).await?;
...
async fn memories_startup_phase1_uses_live_thread_service_tier() -> anyhow::Result<()> {
    let server = start_mock_server().await;
    let home = Arc::new(TempDir::new()?);
    let test = build_test_vac(&server, home).await?;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<vac_state::StateRuntime>> {
    let db =
        vac_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into()).await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn trigger_memories_startup(test: &TestVAC) {
    let config_snapshot = test.vac.config_snapshot().await;
...
async fn seed_stage1_output(
    ...
    let mut metadata_builder = vac_state::ThreadMetadataBuilder::new(
        thread_id,
        vac_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = vac_home.join(format!("workspace-{rollout_slug}"));
...
) -> anyhow::Result<vac_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.vac.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        vac_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_vac(test: &TestVAC) -> anyhow::Result<()> {
    test.vac.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.vac, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
