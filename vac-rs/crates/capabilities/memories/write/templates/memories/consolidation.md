...
The folder `{{ memory_root }}/` is a git repository managed by VAC. Read
`{{ phase2_workspace_diff_file }}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by VAC for
...
- Retrieval bias: preserve distinctive nouns and verbatim strings that a future grep/search
  ...
  `api.vastar.org/v1/files`, `Vastar Internal Slack`, etc.).
...
- Typical workflows and tools (how they like to work, how they use VAC/agents, preferred formats)
...
- context / agent / model: optional; use only when truly needed (e.g., context: branch)
