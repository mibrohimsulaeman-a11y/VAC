...
use futures::prelude::*;
use tracing::error;
use vac_analytics::CompactionImplementation;
use vac_analytics::CompactionPhase;
use vac_analytics::CompactionReason;
use vac_analytics::CompactionStatus;
use vac_analytics::CompactionStrategy;
use vac_analytics::CompactionTrigger;
use vac_analytics::VACCompactionEvent;
use vac_analytics::now_unix_seconds;
use vac_protocol::error::Result as VACResult;
use vac_protocol::error::VACErr;
use vac_protocol::items::ContextCompactionItem;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(VACErr::Interrupted) => {
                return Err(VACErr::Interrupted);
            }
            Err(e @ VACErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(VACCompactionEvent {
...
pub(crate) fn compaction_status_from_result<T>(result: &VACResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(VACErr::Interrupted | VACErr::TurnAborted) => CompactionStatus::Interrupted,
