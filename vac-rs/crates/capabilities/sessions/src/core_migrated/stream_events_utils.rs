...
use vac_utils_stream_parser::strip_citations;
...
use vac_memories_read::citations::parse_memory_citation;
use vac_memories_read::citations::thread_ids_from_memory_citation;
use vac_protocol::error::Result;
use vac_protocol::error::VACErr;
use vac_protocol::models::FunctionCallOutputBody;
...
use vac_utils_stream_parser::strip_proposed_plan_blocks;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<vac_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                vac_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            VACErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(vac_home, session_id, call_id);
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(ctx.sess.as_ref(), item.clone()).await {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(VACErr::Fatal(message));
        }
    }
                ...
                .await
                {
                    Ok(path) => {
                        ...
                        let image_output_dir = image_output_path
                            .parent()
                            .unwrap_or_else(|| turn_context.config.vac_home.clone());
