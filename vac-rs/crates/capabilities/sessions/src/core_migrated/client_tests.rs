...
use super::X_VAC_INSTALLATION_ID_HEADER;
use super::X_VAC_PARENT_THREAD_ID_HEADER;
use super::X_VAC_TURN_METADATA_HEADER;
use super::X_VAC_WINDOW_ID_HEADER;
use super::X_VASTAR_SUBAGENT_HEADER;
...
use vac_api::ResponseEvent;
use vac_runtime_protocol::AuthMode;
use vac_model_provider::BearerAuthProvider;
use vac_model_provider_info::WireApi;
use vac_model_provider_info::create_oss_provider_with_base_url;
...
use vac_rollout_trace::ExecutionStatus;
use vac_rollout_trace::InferenceTraceAttempt;
use vac_rollout_trace::InferenceTraceContext;
use vac_rollout_trace::RawTraceEventPayload;
use vac_rollout_trace::RolloutTrace;
use vac_rollout_trace::TraceWriter;
use vac_rollout_trace::replay_bundle;
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::VACTurnStarted {
        vac_turn_id: "turn-1".to_string(),
        thread_id: "thread-root".to_string(),
    })?;
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    assert_eq!(
        ...
        std::collections::HashMap::from([
            (
                X_VAC_INSTALLATION_ID_HEADER.to_string(),
                "11111111-1111-4111-8111-111111111111".to_string(),
            ),
            (
                X_VAC_WINDOW_ID_HEADER.to_string(),
                format!("{conversation_id}:1"),
            ),
            (
                X_VASTAR_SUBAGENT_HEADER.to_string(),
                "collab_spawn".to_string(),
            ),
            (
                X_VAC_PARENT_THREAD_ID_HEADER.to_string(),
                parent_thread_id.to_string(),
            ),
            (
                X_VAC_TURN_METADATA_HEADER.to_string(),
...
fn auth_request_telemetry_context_tracks_attached_auth_and_retry_phase() {
    ...
    assert_eq!(auth_context.auth_mode, Some("ProviderCredential"));
