...
use tracing::info;
use tracing::warn;
use vac_api::ApiError;
use vac_api::Provider as ApiProvider;
use vac_api::RealtimeAudioFrame;
use vac_api::RealtimeEvent;
use vac_api::RealtimeEventParser;
use vac_api::RealtimeSessionConfig;
use vac_api::RealtimeSessionMode;
use vac_api::RealtimeWebsocketClient;
use vac_api::RealtimeWebsocketEvents;
use vac_api::RealtimeWebsocketWriter;
use vac_api::map_api_error;
use vac_runtime_protocol::AuthMode;
use vac_config::config_toml::RealtimeWsMode;
use vac_config::config_toml::RealtimeWsVersion;
use vac_login::VACAuth;
use vac_login::default_client::default_headers;
...
use vac_protocol::protocol::ConversationStartTransport;
...
use vac_protocol::protocol::RealtimeConversationClosedEvent;
use vac_protocol::protocol::RealtimeConversationRealtimeEvent;
use vac_protocol::protocol::RealtimeConversationSdpEvent;
use vac_protocol::protocol::RealtimeConversationStartedEvent;
use vac_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> VACResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> VACResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> VACResult<()> {
        let sender = {
        ...
        let Some(sender) = sender else {
            return Err(VACErr::InvalidRequest(
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(VACErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> VACResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| VACErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> VACResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> VACResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> VACResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> VACResult<PreparedRealtimeConversationStart> {
...
) -> VACResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> VACResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), VACErrorInfo::BadRequest).await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&VACAuth>, provider: &ModelProviderInfo) -> VACResult<String> {
    ...
    if let Some(api_key) = auth.and_then(VACAuth::api_key) {
        return Ok(api_key.to_string());
    }

    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(aibrahim): Remove this temporary fallback once realtime auth no longer
    // requires API key auth for ChatGPT/SIWC sessions.
    if provider.is_vastar()
        && let Some(api_key) = read_vastar_api_key_from_env()
    {
        return Ok(api_key);
    }
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            VACErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
