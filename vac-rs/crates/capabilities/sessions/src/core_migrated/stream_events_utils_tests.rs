...
use vac_protocol::models::LocalShellAction;
use vac_protocol::models::LocalShellExecAction;
use vac_protocol::models::LocalShellStatus;
...
async fn handle_non_tool_response_item_strips_citations_from_assistant_message() {
    ...
    let text = agent_message
        ...
        .map(|entry| match entry {
            vac_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect::<String>();
...
#[tokio::test]
async fn save_image_generation_result_saves_base64_to_png_in_vac_home() {
    let vac_home = tempfile::tempdir().expect("create vac home");
    let vac_home = vac_home.path().abs();
    let expected_path = image_generation_artifact_path(&vac_home, "session-1", "ig_save_base64");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path = save_image_generation_result(&vac_home, "session-1", "ig_save_base64", "Zm9v")
        .await
...
async fn save_image_generation_result_rejects_data_url_payload() {
    ...
    let err = save_image_generation_result(&vac_home, "session-1", "ig_456", result)
        .await
        .expect_err("data url payload should error");
    assert!(matches!(err, VACErr::InvalidRequest(_)));
}
...
async fn save_image_generation_result_overwrites_existing_file() {
    let vac_home = tempfile::tempdir().expect("create vac home");
    let vac_home = vac_home.path().abs();
    let existing_path = image_generation_artifact_path(&vac_home, "session-1", "ig_overwrite");
    ...
    let saved_path = save_image_generation_result(&vac_home, "session-1", "ig_overwrite", "Zm9v")
        .await
        .expect("image should be saved");
...
#[tokio::test]
async fn save_image_generation_result_sanitizes_call_id_for_vac_home_output_path() {
    let vac_home = tempfile::tempdir().expect("create vac home");
    let vac_home = vac_home.path().abs();
    let expected_path = image_generation_artifact_path(&vac_home, "session-1", "../ig/..");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path = save_image_generation_result(&vac_home, "session-1", "../ig/..", "Zm9v")
        .await
        .expect("image should be saved");
...
async fn save_image_generation_result_rejects_non_standard_base64() {
    let vac_home = tempfile::tempdir().expect("create vac home");
    let vac_home = vac_home.path().abs();
    let err = save_image_generation_result(&vac_home, "session-1", "ig_urlsafe", "_-8")
        .await
