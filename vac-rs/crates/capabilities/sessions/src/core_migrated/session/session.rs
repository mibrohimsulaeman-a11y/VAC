...
use vac_otel::LEGACY_NOTIFY_CONFIGURED_METRIC;
...
pub(crate) struct Session {
    ...
    pub(super) idle_pending_input: Mutex<Vec<ResponseInputItem>>, // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening) (jif) merge with mailbox!
...
pub(crate) struct SessionConfiguration {
    /// Provider identifier ("vastar", "openrouter", ...).
    ...
    /// Directory containing all VAC state for this session.
    pub(super) vac_home: AbsolutePathBuf,
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(pakrym): Remove config from here
...
impl SessionConfiguration {
    pub(crate) fn vac_home(&self) -> &AbsolutePathBuf {
        &self.vac_home
    }
    ...
    pub(super) fn sandbox_policy(&self) -> SandboxPolicy {
        self.permission_profile()
            ...
            .unwrap_or_else(|_| {
                ...
                vac_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let branched_from_id = initial_history.branched_from_id();
        ...
        let conversation_id = match &initial_history {
            InitialHistory::New | InitialHistory::Cleared | InitialHistory::Branched(_) => {
                ThreadId::default()
            }
        ...
        let window_generation = match &initial_history {
            ...
            .unwrap_or(u64::MAX),
            InitialHistory::New | InitialHistory::Cleared | InitialHistory::Branched(_) => 0,
        };
        ...
        let auth_and_mcp_fut = async move {
            if is_subagent {
                (
                    None,
                    HashMap::<String, vac_config::types::McpServerConfig>::new(),
                    HashMap::new(),
                )
            } else {
        ...
        let session_result: anyhow::Result<Arc<Self>> = Box::pin(async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(vac_protocol::AgentPath::root);
            let trace_task_name =
                    ...
                    msg: EventMsg::DeprecationNotice(DeprecationNoticeEvent {
                        summary:
                            "`notify` is deprecated and will be removed in a future release."
                                .to_string(),
                        details: Some(
                            "Switch to a `Stop` hook for end-of-turn automation. See https://developers.vastar.com/vac/hooks."
                                .to_string(),
                        ),
                    }),
                });
            ...
            let config_path = config.vac_home.join(CONFIG_TOML_FILE);
            ...
            let use_zsh_branch_shell = config.features.enabled(Feature::ShellZshBranch);
            let mut default_shell = if let Some(user_shell_override) =
                session_configuration.user_shell_override.clone()
            {
                user_shell_override
            } else if use_zsh_branch_shell {
                let zsh_path = config.zsh_path.as_ref().ok_or_else(|| {
                    anyhow::anyhow!(
                        "zsh branch feature enabled, but `zsh_path` is not configured; set `zsh_path` in config.toml"
                    )
                })?;
                let zsh_path = zsh_path.to_path_buf();
                shell::get_shell(shell::ShellType::Zsh, Some(&zsh_path)).ok_or_else(|| {
                    anyhow::anyhow!(
                        "zsh branch feature enabled, but zsh_path `{}` is not usable; set `zsh_path` to a valid zsh executable",
                        zsh_path.display()
                    )
                })?
            } else {
            ...
            let thread_name = if session_configuration.session_source.is_non_root_agent() {
                None
            } else {
                thread_title_from_state_db(state_db_ctx.as_ref(), &config.vac_home, conversation_id)
                    .instrument(info_span!(
            ...
            let analytics_events_client = if is_subagent {
                AnalyticsEventsClient::disabled()
            } else {
                // Local coding-agent builds must not create an implicit legacy
                // ChatGPT/backend analytics client from `chatgpt_base_url`.
                // A caller that owns an explicit local analytics sink may pass
                // it via `VACSpawnArgs`; otherwise analytics is fail-closed.
            ...
            if !is_subagent
                && let Some(network_policy_decider_session) = network_policy_decider_session
            {
            ...
            .map_err(|err| {
                VACErr::InvalidRequest(err.to_string().replace(
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => vac_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Branched(_) => {
                    vac_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => vac_hooks::SessionStartSource::Clear,
            };

            // record_initial_history can emit events. We record only after the SessionConfiguredEvent is emitted.
            if !is_subagent || matches!(initial_history, InitialHistory::Branched(_)) {
