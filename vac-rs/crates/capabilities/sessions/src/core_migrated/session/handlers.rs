...
use vac_mcp::collect_mcp_snapshot_from_manager;
...
use vac_protocol::protocol::ListSkillsResponseEvent;
use vac_protocol::protocol::McpServerRefreshConfig;
use vac_protocol::protocol::Op;
use vac_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
...
use vac_protocol::protocol::SkillsListEntry;
use vac_protocol::protocol::ThreadMemoryMode;
use vac_protocol::protocol::ThreadNameUpdatedEvent;
...
pub async fn resolve_elicitation(
    ...
    decision: vac_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        vac_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        vac_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        vac_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    tokio::spawn(async move {
        ...
        let event = Event {
            ...
            msg: EventMsg::GetHistoryEntryResponse(
                vac_protocol::protocol::GetHistoryEntryResponseEvent {
                    offset,
                    log_id,
                    entry: entry_opt.map(|e| vac_protocol::message_history::HistoryEntry {
...
pub async fn list_skills(sess: &Session, sub_id: String, cwds: Vec<PathBuf>, force_reload: bool) {
    ...
    let vac_home = sess.vac_home().await;
    ...
    if num_turns == 0 {
        sess.send_event_raw(Event {
            ...
            msg: EventMsg::Error(ErrorEvent {
                ...
                vac_error_info: Some(VACErrorInfo::ThreadRollbackFailed),
            }),
        })
        .await;
        return;
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. VAC will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
    ...
    let vac_home = sess.vac_home().await;
    if let Err(err) =
        crate::rollout::append_thread_name(&vac_home, sess.conversation_id, &name).await
    {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down VAC instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(vac_rollout_trace::RolloutStatus::Completed);
    true
}
