...
use vac_config::types::MemoriesToml;
...
pub(crate) async fn validate_config_lock_if_configured(
    ...
    let options = ConfigLockReplayOptions {
        allow_vac_version_mismatch: config.config_lock_allow_vac_version_mismatch,
    };
...
/// VAC actually ran with, not only the user-authored TOML inputs.
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        for spec in vac_features::FEATURES {
            assert_eq!(
    ...
    async fn lock_skips_session_values_when_model_catalog_fields_are_not_saved() {
        ...
        sc.service_tier = Some(vac_protocol::config_types::ServiceTier::Flex);
    ...
    #[tokio::test]
    async fn lock_validation_rejects_vac_version_mismatch_by_default() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.vac_version = "older-version".to_string();
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock VAC version mismatch"),
            "{message}"
        );
        assert!(
            message.contains("debug.config_lockfile.allow_vac_version_mismatch=true"),
            "{message}"
        );
    }

    #[tokio::test]
    async fn lock_validation_can_ignore_vac_version_mismatch() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.vac_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
        ...
        validate_config_lock_replay(
            ...
            ConfigLockReplayOptions {
                allow_vac_version_mismatch: true,
            },
        )
