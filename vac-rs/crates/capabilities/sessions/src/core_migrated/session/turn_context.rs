...
use vac_model_provider::SharedModelProvider;
...
use vac_sandboxing::policy_transforms::effective_network_sandbox_policy;

pub(super) fn image_generation_tool_auth_allowed(auth_manager: Option<&AuthManager>) -> bool {
    auth_manager.is_some_and(AuthManager::current_auth_uses_vac_backend)
}
...
pub(crate) struct TurnContext {
    ...
    pub(crate) vac_self_exe: Option<PathBuf>,
    pub(crate) vac_linux_sandbox_exe: Option<PathBuf>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_vac_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_vac_backend);
        self.features.apps_enabled_for_auth(uses_vac_backend)
    }
    ...
    pub(crate) async fn with_model(
        ...
        Self {
            ...
            vac_self_exe: self.vac_self_exe.clone(),
    ...
    fn turn_context_network_item(&self) -> Option<TurnContextNetworkItem> {
        ...
        Some(TurnContextNetworkItem {
            allowed_domains: network
                .domains
                .as_ref()
                .and_then(vac_config::NetworkDomainPermissionsToml::allowed_domains)
                .unwrap_or_default(),
            denied_domains: network
                .domains
                .as_ref()
                .and_then(vac_config::NetworkDomainPermissionsToml::denied_domains)
                .unwrap_or_default(),
        })
    }
}
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        TurnContext {
            ...
            vac_self_exe: per_turn_config.vac_self_exe.clone(),
            vac_linux_sandbox_exe: per_turn_config.vac_linux_sandbox_exe.clone(),
            tool_call_gate: Arc::new(ReadinessFlag::new()),
    ...
    pub(crate) async fn new_turn_with_sub_id(
    ...
    ) -> VACResult<Arc<TurnContext>> {
        let update_result: VACResult<_> = {
            ...
            match state.session_configuration.clone().apply(&updates) {
                Ok(next) => {
                    ...
                    let vac_home = next.vac_home.clone();
                ...
                Err(err) => Err(VACErr::InvalidRequest(err.to_string())),
            }
        };
        ...
        let (
            ...
            Err(err) => {
                ...
                .await;
                return Err(VACErr::InvalidRequest(message));
            }
        };
    ...
    pub(crate) async fn maybe_emit_unknown_model_warning_for_turn(&self, tc: &TurnContext) {
        ...
        // Kilo is intentionally a free-tier passthrough provider whose
        // catalog is not first-class metadata in vac. Suppress the warning
        // for that provider so the operator surface stays clean; other
        // providers still surface the metadata-missing notice.
        let provider_id = tc.config.model_provider_id.to_ascii_lowercase();
        let provider_name = tc.config.model_provider.name.to_ascii_lowercase();
        let provider_base_url = tc
            .config
            .model_provider
            .base_url
            .as_deref()
            ...
            .to_ascii_lowercase();
        if provider_id == "kilo"
            || provider_name.contains("kilo")
            || provider_base_url.contains("kilo.ai")
        {
            return;
        }
        self.send_event(
