...
use core_test_support::vac_linux_sandbox_exe_or_skip;
...
use vac_protocol::models::function_call_output_content_items_to_text;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                vac_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_additional_permissions_requests_past_policy_validation() {
    ...
    let (mut session, mut turn_context_raw) = make_session_and_context().await;
    turn_context_raw.vac_linux_sandbox_exe = vac_linux_sandbox_exe_or_skip!();
    turn_context_raw
        .approval_policy
        .set(AskForApproval::OnRequest)
        .expect("test setup should allow updating approval policy");
    ...
    turn_context_raw.permission_profile = vac_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    let config = Arc::new(config);
    let models_manager = models_manager_with_provider(
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: vac_tools::ToolName::plain("shell"),
...
async fn guardian_allows_unified_exec_additional_permissions_requests_past_policy_validation() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: vac_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let vac_home = tempdir().expect("create vac home");
    ...
    let mut config = build_test_config(vac_home.path()).await;
    config.cwd = project_dir.abs();
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_vac_folder: project_dir.path().abs(),
            },
    ...
    let auth_manager = AuthManager::from_auth_for_testing(VACAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.vac_home.to_path_buf(),
        auth_manager.clone(),
        config.model_provider.clone(),
    );
    let plugins_manager = Arc::new(PluginsManager::new(config.vac_home.to_path_buf()));
    ...
    let thread_store = Arc::new(vac_thread_store::LocalThreadStore::new(
        vac_thread_store::LocalThreadStoreConfig::from_config(&config),
    ));

    let VACSpawnOk { vac, .. } = VAC::spawn(VACSpawnArgs {
        config,
        auth_manager,
        models_manager,
