...
// the resume/branch hydration metadata derived from the same replay.
#[derive(Debug)]
...
enum TurnReferenceContextItem {
    ...
    /// invalidated it. Only the latter must emit an explicit clearing segment for resume/branch
    /// hydration.
    #[default]
    NeverSet,
            ...
            match item {
                ...
                RolloutItem::Compacted(compacted) => {
                    if let Some(replacement_history) = &compacted.replacement_history {
                        ...
                        // prompt shape.
                        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(ccunningham): if we drop support for None replacement_history compaction items,
