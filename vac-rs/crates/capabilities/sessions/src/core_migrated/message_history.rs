...
//!
//! The history is stored at `~/.vac/history.jsonl` with **one JSON object per
...
use vac_config::types::HistoryPersistence;
...
/// Filename that stores the message history inside `~/.vac`.
...
fn history_filepath(config: &Config) -> AbsolutePathBuf {
    config.vac_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): check `text` for sensitive patterns

    // Resolve `~/.vac/history.jsonl` and ensure the parent directory exists.
