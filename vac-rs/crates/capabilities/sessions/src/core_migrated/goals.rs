...
use vac_protocol::protocol::ThreadGoalUpdatedEvent;
...
use vac_rollout::state_db::reconcile_rollout;
...
pub(crate) enum GoalRuntimeEvent<'a> {
    ...
    ExternalSet {
        status: vac_state::ThreadGoalStatus,
    },
    ExternalClear,
    ThreadResumed,
}
...
impl Session {
    ...
    pub(crate) fn goal_runtime_apply<'a>(
        ...
        match event {
            ...
            GoalRuntimeEvent::ToolCompleted {
                ...
                if tool_name != vac_tools::UPDATE_GOAL_TOOL_NAME {
    ...
    pub(crate) async fn set_thread_goal(
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            vac_state::ThreadGoalAccountingMode::ActiveOnly,
        )
        .await?;
            ...
            if same_nonterminal_goal {
                    ...
                    .update_thread_goal(
                        ...
                        vac_state::ThreadGoalUpdate {
                            status: status
                                ...
                                .or(Some(vac_state::ThreadGoalStatus::Active)),
                            token_budget,
                ...
                state_db
                    .replace_thread_goal(
                        ...
                        status
                            .map(state_goal_status_from_protocol)
                            .unwrap_or(vac_state::ThreadGoalStatus::Active),
        ...
        *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        let newly_active_goal = goal_status == vac_state::ThreadGoalStatus::Active
            && (replacing_goal
                || previous_status
                    .is_some_and(|status| status != vac_state::ThreadGoalStatus::Active));
        if newly_active_goal {
            ...
            .await;
        } else if goal_status != vac_state::ThreadGoalStatus::Active {
    ...
    pub(crate) async fn create_thread_goal(
        ...
        let goal = state_db
            .insert_thread_goal(
                self.conversation_id,
                objective,
                vac_state::ThreadGoalStatus::Active,
                token_budget,
            )
            .await?
            .ok_or_else(|| {
    ...
    async fn apply_external_thread_goal_status(
        ...
        match status {
                ...
                match self.state_db_for_thread_goals().await {
                    Ok(Some(state_db)) => {
                        match state_db.get_thread_goal(self.conversation_id).await {
                            Ok(Some(goal))
                                if goal.status == vac_state::ThreadGoalStatus::Active =>
                            {
                                let turn_id = self
                                    .active_turn_context()
                                    .await
            ...
            vac_state::ThreadGoalStatus::BudgetLimited => {
                if self.active_turn_context().await.is_none() {
                    self.clear_stopped_thread_goal_runtime_state().await;
                }
            }
            vac_state::ThreadGoalStatus::Paused | vac_state::ThreadGoalStatus::Complete => {
                self.clear_stopped_thread_goal_runtime_state().await;
            }
        }
    }
    ...
    async fn account_thread_goal_progress(
        ...
        let goal = match outcome {
            vac_state::ThreadGoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    vac_state::ThreadGoalStatus::Active => false,
                    vac_state::ThreadGoalStatus::BudgetLimited => {
            ...
            vac_state::ThreadGoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == vac_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != vac_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        ...
        mode: vac_state::ThreadGoalAccountingMode,
            ...
            .await?
        {
            ...
            vac_state::ThreadGoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
    ...
    async fn pause_active_thread_goal_for_interrupt(&self) -> anyhow::Result<()> {
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            vac_state::ThreadGoalAccountingMode::ActiveStatusOnly,
        )
        .await?;
    ...
    async fn activate_paused_thread_goal_after_resume(&self) -> anyhow::Result<bool> {
        ...
        if goal.status != vac_state::ThreadGoalStatus::Paused {
            let goal_id = goal.goal_id.clone();
            let is_active = goal.status == vac_state::ThreadGoalStatus::Active;
            if is_active {
                self.goal_runtime
                    .accounting
                    .lock()
        ...
        let Some(goal) = state_db
            .update_thread_goal(
                self.conversation_id,
                vac_state::ThreadGoalUpdate {
                    status: Some(vac_state::ThreadGoalStatus::Active),
                    token_budget: None,
            ...
            Ok(Some(state_db)) => match state_db.get_thread_goal(self.conversation_id).await {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == vac_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
