...
/// Return the indices of branch-turn boundaries in a rollout.
///
/// A branch-turn boundary is either:
...
/// of simply truncating the mixed branch-boundary list.
pub(crate) fn branch_turn_positions_in_rollout(items: &[RolloutItem]) -> Vec<usize> {
    let mut rollback_turn_positions = Vec::new();
    let mut branch_turn_positions = Vec::new();
    for (idx, item) in items.iter().enumerate() {
        match item {
            RolloutItem::ResponseItem(item) => {
                ...
                if is_real_user_message_boundary(item) || is_trigger_turn_boundary(item) {
                    branch_turn_positions.push(idx);
                }
            }
            RolloutItem::EventMsg(EventMsg::ThreadRolledBack(rollback)) => {
                let num_turns = usize::try_from(rollback.num_turns).unwrap_or(usize::MAX);
                ...
                branch_turn_positions.retain(|position| *position < rollback_start_idx);
            }
            _ => {}
        }
    }
    branch_turn_positions
}
...
/// Return a suffix of `items` that keeps the last `n_from_end` branch turns.
///
/// If fewer than or equal to `n_from_end` branch turns exist, this returns the full rollout.
pub(crate) fn truncate_rollout_to_last_n_branch_turns(
    items: &[RolloutItem],
    n_from_end: usize,
) -> Vec<RolloutItem> {
    if n_from_end == 0 {
    ...
    let branch_turn_positions = branch_turn_positions_in_rollout(items);
    if branch_turn_positions.len() <= n_from_end {
        return items.to_vec();
    }

    let keep_idx = branch_turn_positions[branch_turn_positions.len() - n_from_end];
