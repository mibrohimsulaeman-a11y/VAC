...
use vac_analytics::TurnTokenUsageFact;
use vac_login::AuthManager;
use vac_models_manager::manager::SharedModelsManager;
use vac_otel::SessionTelemetry;
use vac_otel::TURN_E2E_DURATION_METRIC;
use vac_otel::TURN_MEMORY_METRIC;
use vac_otel::TURN_NETWORK_PROXY_METRIC;
use vac_otel::TURN_TOKEN_USAGE_METRIC;
use vac_otel::TURN_TOOL_CALL_METRIC;
...
///
/// Implementations encapsulate a specific VAC workflow (regular chat,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            vac.turn.token_usage.input_tokens = field::Empty,
            vac.turn.token_usage.cached_input_tokens = field::Empty,
            vac.turn.token_usage.non_cached_input_tokens = field::Empty,
            vac.turn.token_usage.output_tokens = field::Empty,
            vac.turn.token_usage.reasoning_output_tokens = field::Empty,
            vac.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; VAC will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub async fn on_task_finished(
        ...
        if let Some(token_usage_at_turn_start) = token_usage_at_turn_start {
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(jif): drop this
            let tmp_mem = (
                "tmp_mem_enabled",
            ...
            if records_turn_token_usage_on_span {
                ...
                current_span.record(
                    "vac.turn.token_usage.input_tokens",
                    turn_token_usage.input_tokens,
                );
                current_span.record(
                    "vac.turn.token_usage.cached_input_tokens",
                    turn_token_usage.cached_input(),
                );
                current_span.record(
                    "vac.turn.token_usage.non_cached_input_tokens",
                    turn_token_usage.non_cached_input(),
                );
                current_span.record(
                    "vac.turn.token_usage.output_tokens",
                    turn_token_usage.output_tokens,
                );
                current_span.record(
                    "vac.turn.token_usage.reasoning_output_tokens",
                    turn_token_usage.reasoning_output_tokens,
                );
                current_span.record(
                    "vac.turn.token_usage.total_tokens",
