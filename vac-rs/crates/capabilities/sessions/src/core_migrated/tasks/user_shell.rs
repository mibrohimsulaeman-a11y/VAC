...
pub(crate) async fn execute_user_shell_command(
    ...
    session
        .services
        .session_telemetry
        .counter("vac.task.user_shell", /*inc*/ 1, &[]);
    ...
    if mode == UserShellCommandMode::StandaloneTurn {
        ...
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(ccunningham): After TurnStarted, emit model-visible turn context diffs for
    ...
    if exec_env_map.contains_key(PROXY_ACTIVE_ENV_KEY) {
        ...
        if exec_env_map
            ...
            .is_some_and(|value| {
                value.starts_with(vac_network_proxy::VAC_PROXY_GIT_SSH_COMMAND_MARKER)
            })
        {
    ...
    let exec_env = ExecRequest {
        ...
        network: None,
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(zhao-oai): Now that we have ExecExpiration::Cancellation, we
