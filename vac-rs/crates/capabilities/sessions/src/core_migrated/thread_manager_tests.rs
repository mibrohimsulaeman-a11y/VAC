...
use vac_protocol::models::ReasoningItemReasoningSummary;
use vac_protocol::models::ResponseItem;
use vac_protocol::protocol::AgentMessageEvent;
...
fn truncates_before_requested_user_message() {
    ...
    let truncated = truncate_before_nth_user_message(
        InitialHistory::Branched(initial),
        /*n*/ 1,
        &SnapshotTurnState {
            ends_mid_turn: false,
            active_turn_id: None,
    ...
    let truncated2 = truncate_before_nth_user_message(
        InitialHistory::Branched(initial2.clone()),
        /*n*/ 2,
        &SnapshotTurnState {
            ends_mid_turn: false,
            active_turn_id: None,
...
fn out_of_range_truncation_drops_only_unfinished_suffix_mid_turn() {
    ...
    let truncated = truncate_before_nth_user_message(
        InitialHistory::Branched(items.clone()),
        usize::MAX,
        &SnapshotTurnState {
            ends_mid_turn: true,
            active_turn_id: None,
...
#[test]
fn branch_thread_accepts_legacy_usize_snapshot_argument() {
    fn assert_legacy_snapshot_callsite(
        manager: &ThreadManager,
        config: Config,
        path: std::path::PathBuf,
    ) {
        let _future = manager.branch_thread(
            usize::MAX,
            config,
            path,
...
fn out_of_range_truncation_drops_pre_user_active_turn_prefix() {
    ...
    let snapshot_state = snapshot_turn_state(&InitialHistory::Branched(items.clone()));
    assert_eq!(
        snapshot_state,
        SnapshotTurnState {
            ends_mid_turn: true,
...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.vac_home = temp_dir.path().join("vac-home").abs();
    config.cwd = config.vac_home.abs();
    std::fs::create_dir_all(&config.vac_home).expect("create vac home");

    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        VACAuth::from_api_key("dummy"),
        config.model_provider.clone(),
        config.vac_home.to_path_buf(),
        Arc::new(vac_exec_server::EnvironmentManager::default_for_tests()),
    );
...
async fn start_thread_accepts_explicit_environment_when_default_environment_is_disabled() {
    ...
    let runtime_paths = vac_exec_server::ExecServerRuntimePaths::new(
        std::env::current_exe().expect("current exe path"),
        /*vac_linux_sandbox_exe*/ None,
    )
    .expect("runtime paths");
    let environment_manager = Arc::new(
        vac_exec_server::EnvironmentManager::create_for_tests(
            Some("none".to_string()),
            runtime_paths,
        )
        .await,
...
#[tokio::test]
async fn resume_and_branch_do_not_restore_thread_environments_from_rollout() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.vac_home = temp_dir.path().join("vac-home").abs();
    config.cwd = config.vac_home.abs();
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(VACAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let branched = manager
        ...
        .await
        .expect("branch source thread");
    let branched_turn = branched
        .thread
        .vac
        .session
        .new_turn_with_sub_id("branch-turn".to_string(), SessionSettingsUpdate::default())
        .await
        .expect("build branched turn context");
    assert_eq!(branched_turn.environments.len(), 1);
    assert_eq!(branched_turn.environments[0].cwd, default_cwd);
    assert_ne!(branched_turn.environments[0].cwd, selected_cwd);
}
...
#[test]
fn interrupted_branch_snapshot_appends_interrupt_boundary() {
    let committed_history =
        InitialHistory::Branched(vec![RolloutItem::ResponseItem(user_msg("hello"))]);
    ...
    assert_eq!(
        ...
        .expect("serialize interrupted branch history"),
        ...
        .expect("serialize expected interrupted branch history"),
    );
    assert_eq!(
        serde_json::to_value(
            append_interrupted_boundary(
        ...
        .expect("serialize interrupted empty branch history"),
        serde_json::to_value(vec![
            RolloutItem::ResponseItem(contextual_user_interrupted_marker()),
            RolloutItem::EventMsg(EventMsg::TurnAborted(TurnAbortedEvent {
                turn_id: None,
...
#[test]
fn disabled_interrupted_branch_snapshot_appends_only_interrupt_event() {
    let committed_history =
        InitialHistory::Branched(vec![RolloutItem::ResponseItem(user_msg("hello"))]);
    ...
    assert_eq!(
        ...
        .expect("serialize disabled interrupted branch history"),
        serde_json::to_value(vec![
            RolloutItem::ResponseItem(user_msg("hello")),
            RolloutItem::EventMsg(EventMsg::TurnAborted(TurnAbortedEvent {
                turn_id: None,
        ...
        .expect("serialize expected disabled interrupted branch history"),
    );
    assert_eq!(
        serde_json::to_value(
            append_interrupted_boundary(
        ...
        .expect("serialize disabled interrupted empty branch history"),
        ...
        .expect("serialize expected disabled interrupted empty branch history"),
    );
}

#[test]
fn interrupted_snapshot_is_not_mid_turn() {
    let interrupted_history = InitialHistory::Branched(vec![
        RolloutItem::ResponseItem(user_msg("hello")),
        RolloutItem::ResponseItem(assistant_msg("partial")),
        RolloutItem::ResponseItem(contextual_user_interrupted_marker()),
        RolloutItem::EventMsg(EventMsg::TurnAborted(TurnAbortedEvent {
...
fn completed_legacy_event_history_is_not_mid_turn() {
    let completed_history = InitialHistory::Branched(vec![
...
fn mixed_response_and_legacy_user_event_history_is_mid_turn() {
    let mixed_history = InitialHistory::Branched(vec![
        RolloutItem::ResponseItem(user_msg("hello")),
        RolloutItem::EventMsg(EventMsg::UserMessage(UserMessageEvent {
            message: "hello".to_string(),
            images: None,
...
#[tokio::test]
async fn interrupted_branch_snapshot_does_not_synthesize_turn_id_for_legacy_history() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.vac_home = temp_dir.path().join("vac-home").abs();
    config.cwd = config.vac_home.abs();
    ...
    let branched = manager
        ...
        .await
        .expect("branch interrupted snapshot");
    let branched_path = branched
        .thread
        .rollout_path()
        .expect("branched rollout path should exist");
    let history = RolloutRecorder::get_rollout_history(&branched_path)
        .await
        .expect("read branched rollout history");
...
#[tokio::test]
async fn interrupted_branch_snapshot_preserves_explicit_turn_id() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.vac_home = temp_dir.path().join("vac-home").abs();
    config.cwd = config.vac_home.abs();
...
#[tokio::test]
async fn interrupted_branch_snapshot_uses_persisted_mid_turn_history_without_live_source() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.vac_home = temp_dir.path().join("vac-home").abs();
    config.cwd = config.vac_home.abs();
    ...
    let branched_rollout_items: Vec<_> = history
        .get_rollout_items()
        .into_iter()
        .filter(|item| !matches!(item, RolloutItem::SessionMeta(_)))
        .collect();
    ...
    .expect("serialize interrupted marker");
    assert_eq!(
        branched_rollout_items
            .iter()
            .filter(|item| {
                serde_json::to_value(item).expect("serialize branched rollout item")
                    == interrupted_marker_json
            })
            .count(),
        1,
    ...
    manager.remove_thread(&branched.thread_id).await;
    let rebranched = manager
        .branch_thread(
            BranchSnapshot::Interrupted,
            config.clone(),
            branched_path,
        ...
        .await
        .expect("re-branch interrupted snapshot");
    let rebranched_path = rebranched
        .thread
        .rollout_path()
        .expect("re-branched rollout path should exist");
    let rebranched_history = RolloutRecorder::get_rollout_history(&rebranched_path)
        .await
        .expect("read re-branched rollout history");
    let rebranched_rollout_items: Vec<_> = rebranched_history
        .get_rollout_items()
        .into_iter()
        .filter(|item| !matches!(item, RolloutItem::SessionMeta(_)))
        .collect();
    ...
    assert_eq!(
        rebranched_rollout_items
            .iter()
            .filter(|item| {
                serde_json::to_value(item).expect("serialize re-branched rollout item")
                    == interrupted_marker_json
            })
            .count(),
        1,
...
async fn resumed_thread_activates_paused_goal_and_continues_on_request() -> anyhow::Result<()> {
    ...
    let source = manager
        .resume_thread_with_history(
            config.clone(),
            InitialHistory::Branched(vec![RolloutItem::ResponseItem(user_msg("keep working"))]),
            auth_manager.clone(),
            /*persist_extended_history*/ false,
            /*parent_trace*/ None,
        )
    ...
    state_db
        .replace_thread_goal(
            ...
            vac_state::ThreadGoalStatus::Paused,
            /*token_budget*/ None,
        )
        .await?;
    ...
    assert_eq!(vac_state::ThreadGoalStatus::Active, goal.status);
    assert!(
        resumed
            .thread
            .vac
