...
use vac_thread_store::StoredThreadHistory;
use vac_thread_store::ThreadStoreError;
use vac_thread_store::ThreadStoreResult;
use vac_utils_absolute_path::AbsolutePathBuf;

use vac_rollout::state_db::StateDbHandle;
...
pub struct VACThreadTurnContextOverrides {
...
/// (formerly called a conversation) in VAC.
impl VACThread {
    pub(crate) fn new(
        vac: VAC,
        session_configured: SessionConfiguredEvent,
    ...
    pub async fn submit(&self, op: Op) -> VACResult<String> {
        self.vac.submit(op).await
    }

    pub async fn shutdown_and_wait(&self) -> VACResult<()> {
        self.vac.shutdown_and_wait().await
    }

    /// Wait until the underlying session loop has terminated.
    pub async fn wait_until_terminated(&self) {
        self.vac.session_loop_termination.clone().await;
    }
    ...
    pub async fn apply_external_goal_set(&self, status: vac_state::ThreadGoalStatus) {
        if let Err(err) = self
            .vac
            .session
    ...
    pub async fn ensure_rollout_materialized(&self) {
        self.vac.session.ensure_rollout_materialized().await;
    }

    #[doc(hidden)]
    pub async fn flush_rollout(&self) -> std::io::Result<()> {
        self.vac.session.flush_rollout().await
    }
    ...
    pub async fn validate_turn_context_overrides(
        &self,
        overrides: VACThreadTurnContextOverrides,
    ) -> ConstraintResult<()> {
        let VACThreadTurnContextOverrides {
            cwd,
            approval_policy,
            approvals_reviewer,
            sandbox_policy,
        ...
        self.vac.session.validate_settings(&updates).await
    }

    /// Use sparingly: this is intended to be removed soon.
    pub async fn submit_with_id(&self, sub: Submission) -> VACResult<()> {
        self.vac.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> VACResult<Event> {
        self.vac.next_event().await
    }

    pub async fn agent_status(&self) -> AgentStatus {
        self.vac.agent_status().await
    }

    pub(crate) fn subscribe_status(&self) -> watch::Receiver<AgentStatus> {
        self.vac.agent_status.clone()
    }
    ...
    /// app-server lifecycle paths replay restored usage after resume or branch without
    ...
    pub async fn token_usage_info(&self) -> Option<TokenUsageInfo> {
        self.vac.session.token_usage_info().await
    }
    ...
    pub(crate) async fn inject_user_message_without_turn(&self, message: String) {
        ...
        if self
            ...
            .is_err()
        {
            let turn_context = self.vac.session.new_default_turn().await;
            self.vac
                .session
    ...
    #[cfg(test)]
    pub(crate) async fn append_message(&self, message: ResponseItem) -> VACResult<String> {
            ...
            .await
        {
            self.vac
                ...
                .await;
            self.vac.session.maybe_start_turn_for_pending_work().await;
        }
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> VACResult<()> {
        if items.is_empty() {
        ...
        let turn_context = self.vac.session.new_default_turn().await;
        if self.vac.session.reference_context_item().await.is_none() {
            self.vac
                .session
    ...
    pub async fn config_snapshot(&self) -> ThreadConfigSnapshot {
        self.vac.thread_config_snapshot().await
    }

    pub async fn config(&self) -> Arc<crate::config::Config> {
        self.vac.session.get_config().await
    }
    ...
    pub fn enabled(&self, feature: Feature) -> bool {
        self.vac.enabled(feature)
    }

    pub async fn increment_out_of_band_elicitation_count(&self) -> VACResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        let was_zero = *guard == 0;
        *guard = guard
            .checked_add(1)
            .ok_or_else(|| VACErr::Fatal("out-of-band elicitation count overflowed".to_string()))?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> VACResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        if *guard == 0 {
            return Err(VACErr::InvalidRequest(
...
fn pending_message_input_item(message: &ResponseItem) -> VACResult<ResponseInputItem> {
    match message {
