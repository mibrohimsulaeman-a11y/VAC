...
const VAC_ARC_MONITOR_ENDPOINT_OVERRIDE: &str = "VAC_ARC_MONITOR_ENDPOINT_OVERRIDE";
const VAC_ARC_MONITOR_TOKEN: &str = "VAC_ARC_MONITOR_TOKEN";
...
pub(crate) async fn monitor_action(
    ...
    let env_token = read_non_empty_env_var(VAC_ARC_MONITOR_TOKEN);
    let Some(url) = read_non_empty_env_var(VAC_ARC_MONITOR_ENDPOINT_OVERRIDE) else {
        // Legacy ChatGPT/backend ARC monitor routing is disabled for the local
        // coding-agent build. Safety monitoring can still be wired explicitly
        // by setting VAC_ARC_MONITOR_ENDPOINT_OVERRIDE to a local/owned service.
...
async fn build_arc_monitor_request(
    ...
    ArcMonitorRequest {
        metadata: ArcMonitorMetadata {
            vac_thread_id: conversation_id.clone(),
            vac_turn_id: turn_context.sub_id.clone(),
