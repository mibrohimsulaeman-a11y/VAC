...
use vac_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use vac_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: JoinHandle<VACResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<VACResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
