You are running retrospection: turn recent `vac sessions` into
...
with what `vac ak skill usage` says, `ak skill usage` wins.

## Step 1 — internalize ak's conventions

Run `vac ak skill usage` and read it end-to-end, including the
...
Run `vac ak search --tree` to see the full structure. Then use
`vac ak search` on likely containers — any `_schema.md`, project
...
Use `vac ak read` when `search` is not enough to judge whether a
specific existing entry already covers a topic you are about to touch.

## Step 3 — list candidate sessions

Run `vac sessions list --json`, paginating until you have the
...
`vac sessions show <id> --json` to fetch the full message history.
...
file's existing `sources:` list and use `vac ak write --force` to
...
`vac ak write --force`, date the update via the new source's
...
After you finish writing, run `vac ak skill maintain` and apply
