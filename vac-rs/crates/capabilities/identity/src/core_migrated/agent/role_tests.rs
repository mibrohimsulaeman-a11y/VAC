...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.vac_linux_sandbox_exe = Some(PathBuf::from("/tmp/vac-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/vac-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.vac_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/vac-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/vac-execve-wrapper"))
    );
}
...
"#,
    ...
    let outcome = skills_manager
        .skills_for_config(&skills_input, Some(Arc::clone(&vac_exec_server::LOCAL_FS)))
        .await;
    let skill = outcome
        .skills
        .iter()
