...
use crate::thread_rollout_truncation::truncate_rollout_to_last_n_branch_turns;
...
pub(crate) enum SpawnAgentBranchMode {
    FullHistory,
    LastNTurns(usize),
}
...
pub(crate) struct SpawnAgentOptions {
    pub(crate) branch_parent_spawn_call_id: Option<String>,
    pub(crate) branch_mode: Option<SpawnAgentBranchMode>,
...
fn keep_branched_rollout_item(item: &RolloutItem) -> bool {
    match item {
        ...
        // A branched child gets its own runtime config, including spawned-agent
...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> VACThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn spawn_agent_with_metadata(
        ...
        initial_operation: Op,
        session_source: Option<SessionSource>,
        options: SpawnAgentOptions, // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(jif) drop with new branch.
    ) -> VACResult<LiveAgent> {
    ...
    async fn spawn_agent_internal(
        ...
        let new_thread = match (session_source, options.branch_mode.as_ref()) {
            (Some(session_source), Some(_)) => {
                self.spawn_branched_thread(
                    &state,
                    config,
                    session_source,
                    &options,
        ...
        )) = notification_source.as_ref()
        {
            let client_metadata = match state.get_thread(*parent_thread_id).await {
                Ok(parent_thread) => parent_thread.vac.session.app_server_client_metadata().await,
                Err(error) => {
                    tracing::warn!(
                        error = %error,
            ...
            let thread_config = new_thread.thread.vac.thread_config_snapshot().await;
        ...
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(jif) add helper for drain
    ...
    async fn spawn_branched_thread(
        &self,
    ...
    ) -> VACResult<crate::thread_manager::NewThread> {
        if options.branch_parent_spawn_call_id.is_none() {
            return Err(VACErr::Fatal(
                "spawn_agent branch requires a parent spawn call id".to_string(),
            ));
        }
        let Some(branch_mode) = options.branch_mode.as_ref() else {
            return Err(VACErr::Fatal(
                "spawn_agent branch requires a branch mode".to_string(),
            ));
        };
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return Err(VACErr::Fatal(
                "spawn_agent branch requires a thread-spawn session source".to_string(),
            ));
        };
        ...
        if let Some(parent_thread) = parent_thread.as_ref() {
            ...
            // Flush/materialize the live parent before snapshotting JSONL for a branch.
            parent_thread
                .vac
                .session
                .ensure_rollout_materialized()
                .await;
            parent_thread.vac.session.flush_rollout().await?;
        }
        ...
        let parent_history = state
            ...
            .ok_or_else(|| {
                VACErr::Fatal(format!(
                    "parent thread history unavailable for branch: {parent_thread_id}"
                ))
            })?;

        let mut branched_rollout_items = parent_history.items;
        if let SpawnAgentBranchMode::LastNTurns(last_n_turns) = branch_mode {
            branched_rollout_items =
                truncate_rollout_to_last_n_branch_turns(&branched_rollout_items, *last_n_turns);
        }
        // MultiAgentV2 root/subagent usage hints are injected as standalone developer
        // messages at thread start. When branching history, drop hints from the parent
        ...
        branched_rollout_items.retain(|item| {
            ...
            keep_branched_rollout_item(item)
        });

        state
            .branch_thread_with_source(
                config.clone(),
                InitialHistory::Branched(branched_rollout_items),
                self.clone(),
                session_source,
                /*persist_extended_history*/ false,
                inherited_shell_snapshot,
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let history = stored_thread
            .history
            .ok_or_else(|| VACErr::ThreadNotFound(thread_id))?
            .items;
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> VACResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: VACResult<String>,
    ) -> VACResult<String> {
        if matches!(result, Err(VACErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> VACResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.vac.session.ensure_rollout_materialized().await;
            thread.vac.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> VACResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> VACResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(VACErr::ThreadNotFound(_)) | Err(VACErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        let agent_path = current_agent_path
            ...
            .map_err(VACErr::UnsupportedOperation)?;
        if let Some(thread_id) = self.state.agent_id_for_path(&agent_path) {
            return Ok(thread_id);
        }
        Err(VACErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> VACResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> VACResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> VACResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| VACErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_shell_snapshot_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        parent_thread.vac.session.user_shell().shell_snapshot()
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.vac.session.get_config().await;
        if !crate::exec_policy::child_uses_parent_exec_policy(&parent_config, child_config) {
            return None;
        }

        Some(Arc::clone(&parent_thread.vac.session.services.exec_policy))
    }

    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> VACResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> VACResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
