...
use vac_mcp::ToolInfo as McpToolInfo;
use vac_mcp::VAC_APPS_MCP_SERVER_NAME;
use vac_mcp::filter_non_vac_apps_mcp_tools_only;
...
pub(crate) fn build_mcp_tool_exposure(
    ...
    let mut deferred_tools = filter_non_vac_apps_mcp_tools_only(all_mcp_tools);
    if let Some(connectors) = connectors {
        deferred_tools.extend(filter_vac_apps_mcp_tools(all_mcp_tools, connectors, config));
    }
    ...
    let direct_tools =
        filter_vac_apps_mcp_tools(all_mcp_tools, explicitly_enabled_connectors, config);
...
fn filter_vac_apps_mcp_tools(
    ...
    mcp_tools
        ...
        .filter(|(_, tool)| {
            ...
            allowed.contains(connector_id) && connectors::vac_app_tool_is_enabled(config, tool)
        })
