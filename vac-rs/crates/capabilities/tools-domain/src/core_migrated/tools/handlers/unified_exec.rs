use crate::control_plane::{
    evaluate_vac_init_runtime_command_contract, write_vac_init_runtime_command_evidence,
};
...
use vac_otel::TOOL_CALL_UNIFIED_EXEC_METRIC;
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        let response = match tool_name.name.as_str() {
            "exec_command" => {
                ...
                let command_for_display = vac_shell_command::parse_command::shlex_join(&command);
                if let Err(reason) = enforce_vac_init_pre_command_runtime_gate(
                    &context.turn.cwd,
                    &args.cmd,
                    &command_for_display,
                ) {
                    ...
                    return Err(FunctionCallError::RespondToModel(format!(
                        "exec_command rejected by VAC pre-command gate: {reason}"
                    )));
                }
                ...
                if effective_additional_permissions
                    ...
                    && !matches!(
                        ...
                        vac_protocol::protocol::AskForApproval::OnRequest
                    )
                {
        ...
        if let Err(reason) =
            enforce_vac_init_evidence_completion_runtime_gate(&turn.cwd, response.exit_code)
        {
            return Err(FunctionCallError::RespondToModel(format!(
                "exec_command completion rejected by VAC evidence gate: {reason}"
            )));
        }
...
pub(crate) fn get_command(
    ...
    match shell_mode {
        ...
        UnifiedExecShellMode::ZshBranch(zsh_branch_config) => Ok(vec![
            zsh_branch_config
                .shell_zsh_path
                .to_string_lossy()
                .to_string(),
...
fn enforce_vac_init_pre_command_runtime_gate(
    cwd: &std::path::Path,
    raw_command: &str,
    command_for_display: &str,
) -> Result<(), String> {
    let vac_root = cwd.join(".vac");
    if !vac_root.exists() {
        return Ok(());
    }
...
fn enforce_vac_init_evidence_completion_runtime_gate(
    cwd: &std::path::Path,
