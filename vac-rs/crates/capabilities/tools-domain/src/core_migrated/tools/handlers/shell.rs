...
fn shell_payload_command(payload: &ToolPayload) -> Option<String> {
    match payload {
        ToolPayload::Function { arguments } => parse_arguments::<ShellToolCallParams>(arguments)
            .ok()
            .map(|params| vac_shell_command::parse_command::shlex_join(&params.command)),
        ToolPayload::LocalShell { params } => Some(vac_shell_command::parse_command::shlex_join(
            &params.command,
        )),
        _ => None,
    }
...
impl ShellCommandHandler {
    fn shell_runtime_backend(&self) -> ShellRuntimeBackend {
        match self.backend {
            ...
            ShellCommandBackend::ZshBranch => ShellRuntimeBackend::ShellCommandZshBranch,
        }
    }
...
impl From<ShellCommandBackendConfig> for ShellCommandHandler {
    fn from(config: ShellCommandBackendConfig) -> Self {
        let backend = match config {
            ...
            ShellCommandBackendConfig::ZshBranch => ShellCommandBackend::ZshBranch,
        };
        Self { backend }
    }
}
        ...
        match payload {
            ToolPayload::Function { arguments } => {
                ...
                Self::run_exec_like(RunExecLikeArgs {
                    ...
                    exec_params,
                    hook_command: vac_shell_command::parse_command::shlex_join(&params.command),
    ...
    async fn run_exec_like(args: RunExecLikeArgs) -> Result<FunctionToolOutput, FunctionCallError> {
        ...
        let exec_approval_requirement = session
            ...
            .create_exec_approval_requirement_for_command(ExecApprovalRequest {
                ...
                sandbox_permissions: if effective_additional_permissions.permissions_preapproved {
                    vac_protocol::models::SandboxPermissions::UseDefault
                } else {
                    effective_additional_permissions.sandbox_permissions
                },
                prefix_rule,
        ...
        let mut runtime = {
            ...
            match shell_runtime_backend {
                ...
                backend @ (ShellCommandClassic | ShellCommandZshBranch) => {
        ...
        Ok(FunctionToolOutput {
            body: vec![
                vac_protocol::models::FunctionCallOutputContentItem::InputText { text: content },
            ],
            success: Some(true),
