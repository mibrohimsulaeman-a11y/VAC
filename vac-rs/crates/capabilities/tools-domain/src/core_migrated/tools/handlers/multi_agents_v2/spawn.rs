use super::*;
use crate::agent::control::SpawnAgentBranchMode;
...
impl ToolHandler for Handler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        let branch_mode = args.branch_mode()?;
        let role_name = args
            .agent_type
            .as_deref()
            .map(str::trim)
        ...
        if matches!(branch_mode, Some(SpawnAgentBranchMode::FullHistory)) {
            reject_full_branch_spawn_overrides(
                role_name,
                args.model.as_deref(),
        ...
        let result = session
            ...
            .spawn_agent_with_metadata(
                ...
                Some(spawn_source),
                SpawnAgentOptions {
                    branch_parent_spawn_call_id: branch_mode.as_ref().map(|_| call_id.clone()),
                    branch_mode,
                    environments: Some(
                        turn.environments
                            .iter()
...
struct SpawnAgentArgs {
    ...
    branch_turns: Option<String>,
    branch_context: Option<bool>,
}

impl SpawnAgentArgs {
    fn branch_mode(&self) -> Result<Option<SpawnAgentBranchMode>, FunctionCallError> {
        if self.branch_context.is_some() {
            return Err(FunctionCallError::RespondToModel(
                "branch_context is not supported in MultiAgentV2; use branch_turns instead"
                    .to_string(),
            ));
        }
        ...
        let branch_turns = self
            ...
            .as_deref()
            .map(str::trim)
            .filter(|branch_turns| !branch_turns.is_empty())
            .unwrap_or("all");

        if branch_turns.eq_ignore_ascii_case("none") {
            return Ok(None);
        }
        if branch_turns.eq_ignore_ascii_case("all") {
            return Ok(Some(SpawnAgentBranchMode::FullHistory));
        }

        let last_n_turns = branch_turns.parse::<usize>().map_err(|_| {
            FunctionCallError::RespondToModel(
                "branch_turns must be `none`, `all`, or a positive integer string".to_string(),
            )
        })?;
        if last_n_turns == 0 {
            return Err(FunctionCallError::RespondToModel(
        ...
        Ok(Some(SpawnAgentBranchMode::LastNTurns(last_n_turns)))
    }
}
