...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    let plugins_manager = PluginsManager::new(vac_home.path().to_path_buf());

    assert!(!verified_plugin_install_completed(
        "sample@vastar-curated",
        &config,
        &plugins_manager,
    ));
...
async fn persist_disabled_install_request_writes_connector_config() {
    ...
    persist_disabled_install_request(&vac_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(vac_home.path().join(CONFIG_TOML_FILE)).expect("read config");
...
async fn persist_disabled_install_request_writes_plugin_config() {
    ...
    assert_eq!(
        ...
        Some(ToolSuggestConfig {
            discoverables: Vec::new(),
            disabled_tools: vec![ToolSuggestDisabledTool::plugin("slack@vastar-curated")],
        })
    );
}
...
[tool_suggest]
discoverables = [
  { type = "plugin", id = "sample@vastar-curated" }
]
    ...
    assert_eq!(
        ...
        Some(ToolSuggestConfig {
            ...
            disabled_tools: vec![
                ...
                ToolSuggestDisabledTool::plugin("slack@vastar-curated"),
            ],
        })
    );
}
