...
async fn invocation_for_payload(
    ...
    ToolInvocation {
        ...
        tool_name: vac_tools::ToolName::plain(tool_name),
...
#[test]
fn test_get_command_ignores_explicit_shell_in_zsh_branch_mode() -> anyhow::Result<()> {
    let json = r#"{"cmd": "echo hello", "shell": "/bin/bash"}"#;
    let args: ExecCommandArgs = parse_arguments(json)?;
    ...
    let shell_mode = UnifiedExecShellMode::ZshBranch(ZshBranchConfig {
...
async fn exec_command_pre_tool_use_payload_skips_write_stdin() {
    ...
    assert_eq!(
        handler.pre_tool_use_payload(&ToolInvocation {
            ...
            tool_name: vac_tools::ToolName::plain("write_stdin"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload,
        }),
        None
