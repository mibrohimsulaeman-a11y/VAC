...
use vac_sandboxing::policy_transforms::merge_permission_profiles;
use vac_sandboxing::policy_transforms::normalize_additional_permissions;
use vac_tools::ApplyPatchToolArgs;
...
fn format_update_chunks_for_progress(chunks: &[vac_apply_patch::UpdateFileChunk]) -> String {
...
async fn effective_patch_permissions(
    ...
    vac_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match vac_apply_patch::maybe_parse_apply_patch_verified(
            &command,
            &cwd,
            fs.as_ref(),
            sandbox.as_ref(),
        ...
        .await
        {
            vac_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
                ...
                .await
                {
                    InternalApplyPatchInvocation::Output(item) => {
                        ...
                        let (exit_code, output_text) = match &item {
                            Ok(content) => (0, content.clone()),
                            Err(FunctionCallError::RespondToModel(reason)) => (1, reason.clone()),
                            Err(e) => (1, e.to_string()),
                        };
                    ...
                    InternalApplyPatchInvocation::DelegateToRuntime(apply) => {
                        ...
                        let req = ApplyPatchRequest {
                            action: apply.action,
                            file_paths,
                            changes: changes_protocol.clone(),
                            exec_approval_requirement: apply.exec_approval_requirement,
                            additional_permissions: effective_additional_permissions
                                .additional_permissions,
                            effective_file_system_sandbox_policy: file_system_sandbox_policy,
            ...
            vac_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            vac_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            vac_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match vac_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, sandbox.as_ref())
        .await
    {
        vac_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            session
            ...
            .await
            {
                ...
                InternalApplyPatchInvocation::DelegateToRuntime(apply) => {
                    ...
                    let is_shell_tool = tool_name == "shell" || tool_name == "shell_command";
                    let opt_exec_output = if is_shell_tool {
                        out.as_ref().ok().cloned()
                    } else {
                        None
                    };
                    ...
                    let finish_res = emitter.finish(event_ctx, out).await;

                    match finish_res {
                        Ok(finished_content) => {
                            let final_content = if is_shell_tool {
        ...
        vac_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
