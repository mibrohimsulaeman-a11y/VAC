...
use vac_protocol::protocol::CollabAgentRef;
use vac_protocol::protocol::CollabAgentStatusEntry;
...
pub(crate) fn collab_spawn_error(err: VACErr) -> FunctionCallError {
    match err {
        VACErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        VACErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: VACErr) -> FunctionCallError {
    match err {
        VACErr::ThreadNotFound(id) => {
...
pub(crate) fn reject_full_branch_spawn_overrides(
    ...
    if agent_type.is_some() || model.is_some() || reasoning_effort.is_some() {
        return Err(FunctionCallError::RespondToModel(
            "Full-history branched agents inherit the parent agent type, model, and reasoning effort; omit agent_type, model, and reasoning_effort, or spawn without a full-history branch.".to_string(),
        ));
    }
    Ok(())
}
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.vac_linux_sandbox_exe = turn.vac_linux_sandbox_exe.clone();
...
fn find_spawn_agent_model_name(
    available_models: &[vac_protocol::vastar_models::ModelPreset],
    requested_model: &str,
