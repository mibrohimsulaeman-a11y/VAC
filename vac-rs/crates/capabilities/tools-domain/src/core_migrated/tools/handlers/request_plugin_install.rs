...
use vac_tools::DiscoverableToolAction;
use vac_tools::DiscoverableToolType;
use vac_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use vac_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use vac_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use vac_tools::RequestPluginInstallArgs;
use vac_tools::RequestPluginInstallResult;
use vac_tools::all_requested_connectors_picked_up;
use vac_tools::build_request_plugin_install_elicitation_request;
use vac_tools::filter_request_plugin_install_discoverable_tools_for_client;
use vac_tools::verified_connector_install_completed;
...
impl ToolHandler for RequestPluginInstallHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("vac-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in vac-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.vac_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    vac_home: &vac_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_vac_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh vac apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
