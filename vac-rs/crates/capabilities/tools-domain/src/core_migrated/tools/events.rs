...
use vac_protocol::items::FileChangeItem;
...
use vac_protocol::protocol::TurnDiffEvent;
...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::VAC(VACErr::Sandbox(SandboxErr::Timeout { output })))
            | Err(ToolError::VAC(VACErr::Sandbox(SandboxErr::Denied { output, .. }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
                let event = ToolEventStage::Failure(ToolEventFailure::Output(*output));
                let result = Err(FunctionCallError::RespondToModel(response));
                (event, result)
            }
            Err(ToolError::VAC(err)) => {
            ...
            Err(ToolError::Rejected(msg)) => {
                ...
                //
                // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): We should add a new ToolError variant for user-declined approvals.
