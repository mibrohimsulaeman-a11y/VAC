...
pub fn format_exec_output_for_model_structured(
    ...
    serde_json::to_string(&payload).unwrap_or_else(|err| {
        tracing::warn!(%err, "failed to serialize structured exec output; falling back to text payload");
        format!(
            "{formatted_output}\n[metadata serialization failed: exit_code={exit_code}, duration_seconds={duration_seconds:.1}]"
        )
    })
}
