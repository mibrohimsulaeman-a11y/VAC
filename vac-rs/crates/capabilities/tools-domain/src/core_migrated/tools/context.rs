...
use vac_protocol::models::SearchToolCallParams;
...
use vac_utils_output_truncation::formatted_truncate_text;
use vac_utils_string::take_bytes_at_char_boundary;
...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the VAC tool call id
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    vac_protocol::models::ContentItem::InputText { text }
                    | vac_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    vac_protocol::models::ContentItem::InputImage { image_url, detail } => {
