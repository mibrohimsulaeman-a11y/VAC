...
use vac_code_mode::CodeModeNestedToolCall;
use vac_code_mode::CodeModeTurnHost;
use vac_code_mode::RuntimeResponse;
...
use vac_tools::collect_code_mode_tool_definitions;
use vac_utils_output_truncation::TruncationPolicy;
use vac_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use vac_utils_output_truncation::truncate_function_output_items_with_policy;

pub(crate) use execute_handler::CodeModeExecuteHandler;
use response_adapter::into_function_call_output_content_items;
pub(crate) use wait_handler::CodeModeWaitHandler;

pub(crate) const PUBLIC_TOOL_NAME: &str = vac_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = vac_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = vac_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
pub(crate) struct CodeModeService {
    inner: vac_code_mode::CodeModeService,
}

impl CodeModeService {
    pub(crate) fn new() -> Self {
        Self {
            inner: vac_code_mode::CodeModeService::new(),
        }
    }
    ...
    pub(crate) async fn execute(
        &self,
        request: vac_code_mode::ExecuteRequest,
    ...
    pub(crate) async fn wait(
        &self,
        request: vac_code_mode::WaitRequest,
    ) -> Result<vac_code_mode::WaitOutcome, String> {
    ...
    ) -> Option<vac_code_mode::CodeModeTurnWorker> {
...
pub(super) async fn build_enabled_tools(exec: &ExecContext) -> Vec<vac_code_mode::ToolDefinition> {
...
fn tool_kind_for_spec(spec: &ToolSpec) -> vac_code_mode::CodeModeToolKind {
    if matches!(spec, ToolSpec::Freeform(_)) {
        vac_code_mode::CodeModeToolKind::Freeform
    } else {
        vac_code_mode::CodeModeToolKind::Function
    }
}
...
fn tool_kind_for_name(
    ...
    tool_name: &ToolName,
) -> Result<vac_code_mode::CodeModeToolKind, String> {
    spec.as_ref()
...
fn build_nested_tool_payload(
    ...
    match actual_kind {
        vac_code_mode::CodeModeToolKind::Function => build_function_tool_payload(tool_name, input),
        vac_code_mode::CodeModeToolKind::Freeform => build_freeform_tool_payload(tool_name, input),
    }
}
