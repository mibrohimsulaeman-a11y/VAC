...
use crate::tools::runtimes::shell::zsh_branch_backend;
...
use vac_shell_command::powershell::prefix_powershell_script_with_utf8;
...
impl<'a> ToolRuntime<UnifiedExecRequest, UnifiedExecProcess> for UnifiedExecRuntime<'a> {
    ...
    async fn run(
        ...
        if let UnifiedExecShellMode::ZshBranch(zsh_branch_config) = &self.shell_mode {
            let command =
            ...
            match zsh_branch_backend::maybe_prepare_unified_exec(
                req,
                attempt,
                ctx,
                exec_env,
            ...
            .await?
            {
                Some(prepared) => {
                    ...
                    if turn_environment.environment.is_remote() {
                        return Err(ToolError::Rejected(
                            "unified_exec zsh-branch is not supported when exec_server_url is configured".to_string(),
                        ));
                    }
                    return self
                        .manager
                        ...
                        .map_err(|err| match err {
                            UnifiedExecError::SandboxDenied { output, .. } => {
                                ToolError::VAC(VACErr::Sandbox(SandboxErr::Denied {
                ...
                None => {
                    tracing::warn!(
                        "UnifiedExec ZshBranch backend specified, but conditions for using it were not met, falling back to direct execution",
                    );
                }
            }
        }
