...
fn extract_shell_script_rejects_unsupported_shell_invocation() {
    ...
    assert_eq!(
        ...
        "unexpected shell command format for zsh-branch execution"
    );
}
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.vac_home)
        .context("recreate vac home for hook fixtures")?;
    ...
    std::fs::write(
        turn_context.config.vac_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "hooks": [{
    ...
    let expected_hook_command =
        vac_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(vac_execpolicy::Policy::empty())),
        ...
        stopwatch: vac_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = tokio::time::timeout(
        Duration::from_secs(5),
        vac_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        vac_shell_escalation::EscalationDecision::Escalate(
            vac_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
That said, if /bin/zsh is the zsh-branch, then the execve wrapper should
