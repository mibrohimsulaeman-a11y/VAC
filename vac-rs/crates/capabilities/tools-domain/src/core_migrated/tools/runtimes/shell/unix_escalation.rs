...
use uuid::Uuid;
use vac_execpolicy::Decision;
use vac_execpolicy::Evaluation;
use vac_execpolicy::MatchOptions;
use vac_execpolicy::Policy;
use vac_execpolicy::RuleMatch;
use vac_features::Feature;
use vac_hooks::PermissionRequestDecision;
use vac_protocol::config_types::WindowsSandboxLevel;
use vac_protocol::error::SandboxErr;
...
use vac_protocol::protocol::NetworkPolicyRuleAction;
use vac_protocol::protocol::ReviewDecision;
use vac_sandboxing::SandboxCommand;
use vac_sandboxing::SandboxManager;
use vac_sandboxing::SandboxTransformRequest;
use vac_sandboxing::SandboxType;
use vac_sandboxing::SandboxablePreference;
use vac_shell_command::bash::parse_shell_lc_plain_commands;
use vac_shell_command::bash::parse_shell_lc_single_command_prefix;
use vac_shell_escalation::EscalateServer;
use vac_shell_escalation::EscalationDecision;
use vac_shell_escalation::EscalationExecution;
use vac_shell_escalation::EscalationPermissions;
use vac_shell_escalation::EscalationPolicy;
use vac_shell_escalation::EscalationSession;
use vac_shell_escalation::ExecParams;
use vac_shell_escalation::ExecResult;
use vac_shell_escalation::PreparedExec;
use vac_shell_escalation::ResolvedPermissionProfile;
use vac_shell_escalation::ShellCommandExecutor;
use vac_shell_escalation::Stopwatch;
use vac_utils_absolute_path::AbsolutePathBuf;

pub(crate) struct PreparedUnifiedExecZshBranch {
...
pub(super) async fn try_run_zsh_branch(
    req: &ShellRequest,
    ...
    let Some(shell_zsh_path) = ctx.session.services.shell_zsh_path.as_ref() else {
        tracing::warn!("ZshBranch backend specified, but shell_zsh_path is not configured.");
        return Ok(None);
    };
    if !ctx.session.features().enabled(Feature::ShellZshBranch) {
        tracing::warn!("ZshBranch backend specified, but ShellZshBranch feature is not enabled.");
        return Ok(None);
    }
    if !matches!(ctx.session.user_shell().shell_type, ShellType::Zsh) {
        tracing::warn!("ZshBranch backend specified, but user shell is not Zsh.");
        return Ok(None);
    }
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::VAC(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        arg0,
        sandbox_policy_cwd,
        vac_linux_sandbox_exe: ctx.turn.vac_linux_sandbox_exe.clone(),
    ...
    let main_execve_wrapper_exe = ctx
        ...
        .ok_or_else(|| {
            ToolError::Rejected(
                "zsh branch feature enabled, but execve wrapper is not configured".to_string(),
            )
        })?;
...
pub(crate) async fn prepare_unified_exec_zsh_branch(
...
) -> Result<Option<PreparedUnifiedExecZshBranch>, ToolError> {
    let parsed = match extract_shell_script(&exec_request.command) {
        Ok(parsed) => parsed,
        Err(err) => {
            tracing::warn!("ZshBranch unified exec fallback: {err:?}");
            return Ok(None);
        }
    };
    if parsed.program != shell_zsh_path.to_string_lossy() {
        tracing::warn!(
            "ZshBranch backend specified, but unified exec command targets `{}` instead of `{}`.",
            parsed.program,
    ...
    Ok(Some(PreparedUnifiedExecZshBranch {
        exec_request,
        escalation_session,
    }))
}
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    vac_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<vac_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["VAC_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            vac_linux_sandbox_exe: self.vac_linux_sandbox_exe.as_deref(),
...
fn extract_shell_script(command: &[String]) -> Result<ParsedShellCommand, ToolError> {
    // Commands reaching zsh-branch can be wrapped by environment/sandbox helpers, so
    ...
    Err(ToolError::Rejected(
        "unexpected shell command format for zsh-branch execution".to_string(),
    ))
}
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::VAC(VACErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::VAC(VACErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
