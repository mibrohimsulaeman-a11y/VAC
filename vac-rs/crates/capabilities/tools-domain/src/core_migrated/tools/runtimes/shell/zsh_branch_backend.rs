...
/// Runs the zsh-branch shell-command backend when this request should be handled
...
/// Prepares unified exec to launch through the zsh-branch backend when the
...
pub(crate) async fn maybe_prepare_unified_exec(
    ...
    zsh_branch_config: &ZshBranchConfig,
) -> Result<Option<PreparedUnifiedExecSpawn>, ToolError> {
    imp::maybe_prepare_unified_exec(req, attempt, ctx, exec_request, zsh_branch_config).await
}
...
mod imp {
    ...
    use vac_shell_escalation::ESCALATE_SOCKET_ENV_VAR;
    use vac_shell_escalation::EscalationSession;

    #[derive(Debug)]
    struct ZshBranchSpawnLifecycle {
        escalation_session: EscalationSession,
    }

    impl SpawnLifecycle for ZshBranchSpawnLifecycle {
    ...
    pub(super) async fn maybe_run_shell_command(
        ...
        unix_escalation::try_run_zsh_branch(req, attempt, ctx, command).await
    }
    ...
    pub(super) async fn maybe_prepare_unified_exec(
        ...
        let Some(prepared) = unix_escalation::prepare_unified_exec_zsh_branch(
            req,
            attempt,
            ctx,
            exec_request,
            zsh_branch_config.shell_zsh_path.as_path(),
            zsh_branch_config.main_execve_wrapper_exe.as_path(),
        )
        .await?
        else {
            return Ok(None);
        ...
        Ok(Some(PreparedUnifiedExecSpawn {
            ...
            spawn_lifecycle: Box::new(ZshBranchSpawnLifecycle {
...
mod imp {
    ...
    pub(super) async fn maybe_prepare_unified_exec(
        ...
        let _ = (req, attempt, ctx, exec_request, zsh_branch_config);
        Ok(None)
    }
}
