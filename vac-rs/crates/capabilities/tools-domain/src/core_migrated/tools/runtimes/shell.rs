...
pub(crate) mod zsh_branch_backend;
...
pub(crate) enum ShellRuntimeBackend {
    ...
    /// zsh-branch shell-escalation adapter.
    ShellCommandClassic,
    /// zsh-branch backend for the `shell_command` tool.
    ///
    /// On Unix, attempts to run via the zsh-branch + `vac-shell-escalation`
    /// adapter, with fallback to the standard shell runtime flow if
    /// prerequisites are not met.
    ShellCommandZshBranch,
}
...
impl ToolRuntime<ShellRequest, ExecToolCallOutput> for ShellRuntime {
    ...
    async fn run(
        ...
        if self.backend == ShellRuntimeBackend::ShellCommandZshBranch {
            match zsh_branch_backend::maybe_run_shell_command(req, attempt, ctx, &command).await? {
                Some(out) => return Ok(out),
                None => {
                    tracing::warn!(
                        "ZshBranch backend specified, but conditions for using it were not met, falling back to normal execution",
                    );
                }
            }
        }
