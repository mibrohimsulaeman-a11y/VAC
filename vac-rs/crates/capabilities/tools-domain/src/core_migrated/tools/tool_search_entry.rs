...
use vac_tools::ToolSearchResultSource;
use vac_tools::ToolsConfig;
use vac_tools::dynamic_tool_to_loadable_tool_spec;
use vac_tools::tool_search_result_source_to_loadable_tool_spec;
...
pub(crate) fn build_tool_search_entries(
    ...
    for info in mcp_tools {
        match mcp_tool_search_entry(info) {
            ...
            Err(error) => {
                ...
                tracing::error!(
                    "Failed to convert deferred MCP tool `{tool_name}` to Vastar tool: {error:?}"
                );
            }
        }
    }
    ...
    for tool in dynamic_tools {
        match dynamic_tool_search_entry(tool) {
            ...
            Err(error) => {
                tracing::error!(
                    "Failed to convert deferred dynamic tool {:?} to Vastar tool: {error:?}",
                    tool.name
                );
            }
        }
