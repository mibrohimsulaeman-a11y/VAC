...
use vac_models_manager::model_info::with_config_overrides;
...
use vac_protocol::vastar_models::ConfigShellToolType;
use vac_protocol::vastar_models::ModelInfo;
use vac_tools::AdditionalProperties;
use vac_tools::ConfiguredToolSpec;
use vac_tools::DiscoverableTool;
use vac_tools::JsonSchema;
use vac_tools::LoadableToolSpec;
use vac_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use vac_tools::ResponsesApiNamespaceTool;
use vac_tools::ResponsesApiTool;
use vac_tools::ShellCommandBackendConfig;
use vac_tools::TOOL_SEARCH_TOOL_NAME;
use vac_tools::ToolName;
use vac_tools::ToolSpec;
use vac_tools::ToolsConfig;
use vac_tools::ToolsConfigParams;
use vac_tools::UnifiedExecShellMode;
use vac_tools::ZshBranchConfig;
use vac_tools::mcp_call_tool_result_output_schema;
use vac_tools::mcp_tool_to_deferred_responses_api_tool;
...
fn discoverable_connector(id: &str, name: &str, description: &str) -> DiscoverableTool {
    ...
    DiscoverableTool::Connector(Box::new(AppInfo {
        ...
        app_metadata: None,
        labels: None,
        install_url: Some(format!("vac://mcp-connectors/{slug}?id={id}")),
        is_accessible: false,
        is_enabled: true,
        plugin_display_names: Vec::new(),
    }))
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    let serialized = serde_json::to_value(ToolSpec::Function(
        mcp_tool_to_deferred_responses_api_tool(
            &ToolName::namespaced("mcp__vac_apps__", "lookup_order"),
            &tool,
        )
...
#[tokio::test]
async fn test_build_specs_gpt5_vac_default() {
...
#[tokio::test]
async fn test_build_specs_gpt51_vac_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_build_specs_gpt5_vac_unified_exec_web_search() {
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt51_vac_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
...
#[tokio::test]
async fn test_gpt_5_1_vac_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_vac_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4-mini",
        &features,
...
#[tokio::test]
async fn test_gpt_5_1_vac_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
...
#[tokio::test]
async fn shell_zsh_branch_prefers_shell_command_over_unified_exec() {
    let config = test_config().await;
    let model_info = construct_model_info_offline("o3", &config);
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    features.enable(Feature::ShellZshBranch);
    ...
    assert_eq!(
        ...
        ShellCommandBackendConfig::ZshBranch
    );
    assert_eq!(
    ...
    assert_eq!(
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\vac\vac-execve-wrapper"
                } else {
                    "/opt/vac/vac-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshBranch(ZshBranchConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/vac/zsh").unwrap(),
...
async fn search_tool_description_falls_back_to_connector_name_without_description() {
    ...
    let (tools, _) = build_specs(
        ...
        /*mcp_tools*/ None,
        Some(HashMap::from([(
            "mcp__vac_apps__calendar_create_event".to_string(),
            ToolInfo {
                server_name: VAC_APPS_MCP_SERVER_NAME.to_string(),
                callable_name: "_create_event".to_string(),
                callable_namespace: "mcp__vac_apps__calendar".to_string(),
                server_instructions: None,
                tool: mcp_tool(
...
async fn search_tool_registers_namespaced_mcp_tool_aliases() {
    ...
    let (_, registry) = build_specs(
        ...
        Some(HashMap::from([
            ...
            (
                "mcp__vac_apps__calendar_list_events".to_string(),
                ToolInfo {
                    server_name: VAC_APPS_MCP_SERVER_NAME.to_string(),
    ...
    let app_alias = ToolName::namespaced("mcp__vac_apps__calendar", "_create_event");
...
async fn unavailable_mcp_tools_are_exposed_as_dummy_function_tools() {
    ...
    let unavailable_tool = ToolName::namespaced("mcp__vac_apps__calendar", "_create_event");
    ...
    let tool = find_tool(&tools, "mcp__vac_apps__calendar_create_event");
    ...
    assert!(registry.has_handler(&ToolName::namespaced(
        "mcp__vac_apps__calendar",
        "_create_event"
    )));
    assert!(!registry.has_handler(&ToolName::plain("mcp__vac_apps__calendar_create_event")));
}
...
#[tokio::test]
async fn test_get_vastar_tools_mcp_tools_with_additional_properties_schema() {
    let config = test_config().await;
    let model_info = construct_model_info_offline("gpt-5.4", &config);
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
