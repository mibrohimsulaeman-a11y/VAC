...
use vac_runtime_protocol::McpElicitationObjectType;
use vac_runtime_protocol::McpElicitationSchema;
...
use crate::mcp_vastar_file::rewrite_mcp_tool_arguments_for_vastar_files;
...
use url::Url;
use vac_analytics::AppInvocation;
use vac_analytics::InvocationType;
use vac_analytics::build_track_events_context;
use vac_config::types::AppToolApproval;
use vac_features::Feature;
use vac_hooks::PermissionRequestDecision;
use vac_mcp::McpPermissionPromptAutoApproveContext;
use vac_mcp::SandboxState;
use vac_mcp::VAC_APPS_MCP_SERVER_NAME;
use vac_mcp::declared_vastar_file_input_param_names;
use vac_mcp::mcp_permission_prompt_is_auto_approved;
use vac_otel::sanitize_metric_tag_value;
...
use vac_protocol::request_user_input::RequestUserInputAnswer;
use vac_protocol::request_user_input::RequestUserInputArgs;
use vac_protocol::request_user_input::RequestUserInputQuestion;
use vac_protocol::request_user_input::RequestUserInputQuestionOption;
...
use vac_utils_output_truncation::truncate_text;
use vac_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;

const MCP_CALL_COUNT_METRIC: &str = "vac.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "vac.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "vac/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "vac.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str = "vac.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == VAC_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == VAC_APPS_MCP_SERVER_NAME {
    ...
    if server == VAC_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    let rewrite = rewrite_mcp_tool_arguments_for_vastar_files(
        sess,
        turn_context,
        arguments_value.clone(),
        metadata.and_then(|metadata| metadata.vastar_file_input_params.as_deref()),
    )
    .await;
    ...
    maybe_track_vac_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        vac.mcp.target.id = Empty,
        vac.mcp.server_user_flow.triggered = Empty,
    );
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        vac_linux_sandbox_exe: turn_context.vac_linux_sandbox_exe.clone(),
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                vac_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_vac_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != VAC_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    vac_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
    vastar_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_VAC_APPS_META_KEY: &str = "_vac_apps";
const MCP_TOOL_VASTAR_OUTPUT_TEMPLATE_META_KEY: &str = "vastar/outputTemplate";
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, vac_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if let Some(turn_metadata) = turn_context.turn_metadata_state.current_meta_value() {
        request_meta.insert(crate::X_VAC_TURN_METADATA_HEADER.to_string(), turn_metadata);
    }

    if server == VAC_APPS_MCP_SERVER_NAME {
        let mut vac_apps_meta = metadata
            .and_then(|metadata| metadata.vac_apps_meta.clone())
            .unwrap_or_default();
        vac_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_VAC_APPS_META_KEY.to_string(),
            serde_json::Value::Object(vac_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__vac_mcp_decline__";
const MCP_TOOL_APPROVAL_ACCEPT_AND_REMEMBER: &str = "Allow and don't ask me again";
const MCP_TOOL_APPROVAL_CANCEL: &str = "Cancel";
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "vac_approval_kind";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == VAC_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == VAC_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        vac_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_VAC_APPS_META_KEY))
            .and_then(serde_json::Value::as_object)
            .cloned(),
        // Disallow custom MCPs from uploading files via fileParams.
        vastar_file_input_params: vastar_file_input_params_for_server(
            server,
...
fn vastar_file_input_params_for_server(
    server: &str,
    meta: Option<&serde_json::Map<String, serde_json::Value>>,
) -> Option<Vec<String>> {
    (server == VAC_APPS_MCP_SERVER_NAME)
        .then_some(declared_vastar_file_input_param_names(meta))
...
fn get_mcp_app_resource_uri(
    ...
    meta.and_then(|meta| {
        meta.get("ui")
            ...
            .or_else(|| {
                meta.get(MCP_TOOL_VASTAR_OUTPUT_TEMPLATE_META_KEY)
                    .and_then(serde_json::Value::as_str)
            })
            .map(str::to_string)
    })
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == VAC_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_vac_app_tool_approval(&turn_context.config.vac_home, &connector_id, &tool_name)
            .await
    } else {
...
async fn persist_vac_app_tool_approval(
...
async fn persist_non_app_mcp_tool_approval(
    ...
    if let Some(plugin_config_name) = plugin_config_name {
        return ConfigEditsBuilder::new(&config.vac_home)
            .with_edits([ConfigEdit::SetPath {
                segments: vec![
                    "plugins".to_string(),
                    plugin_config_name,
...
async fn custom_mcp_tool_approval_config_folder(
    ...
    let servers = load_global_mcp_servers(&config.vac_home).await?;
    Ok(servers
        .contains_key(server)
        .then(|| config.vac_home.clone()))
}
