...
pub async fn build_prompt_input(
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        config.vac_self_exe.clone(),
        config.vac_linux_sandbox_exe.clone(),
    )?;
    ...
    let output = build_prompt_input_from_session(thread.thread.vac.session.as_ref(), input).await;
