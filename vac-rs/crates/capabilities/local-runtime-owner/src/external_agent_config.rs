...
use vac_core_plugins::marketplace::find_marketplace_manifest_path;
use vac_core_plugins::marketplace_add::MarketplaceAddRequest;
use vac_core_plugins::marketplace_add::add_marketplace;
use vac_core_plugins::marketplace_add::is_local_marketplace_source;
use vac_external_agent_migration::build_mcp_config_from_external;
use vac_external_agent_migration::count_missing_commands;
use vac_external_agent_migration::count_missing_subagents;
use vac_external_agent_migration::hook_migration_event_names;
use vac_external_agent_migration::import_commands;
use vac_external_agent_migration::import_hooks;
use vac_external_agent_migration::import_subagents;
use vac_external_agent_migration::missing_command_names;
use vac_external_agent_migration::missing_subagent_names;
use vac_external_agent_sessions::ExternalAgentSessionMigration;
use vac_external_agent_sessions::detect_recent_sessions;
use vac_plugin::PluginId;
use vac_protocol::protocol::Product;

const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "vac.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "vac.external_agent_config.import";
...
pub struct ExternalAgentConfigImportOutcome {
    pub plugin_import_outcomes: Vec<PluginImportOutcome>,
    pub pending_session_imports: Vec<ExternalAgentSessionMigration>,
}

impl ExternalAgentConfigImportOutcome {
    #[must_use]
    pub fn has_pending_background_work(&self) -> bool {
        !self.pending_session_imports.is_empty()
    }
}
...
impl ExternalAgentConfigService {
        ...
        for migration_item in migration_items {
            match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Plugins => {
                    ...
                    outcome
                        .plugin_import_outcomes
                        .push(self.import_plugins(cwd.as_deref(), Some(details)).await?);
                    emit_migration_metric(
                        EXTERNAL_AGENT_CONFIG_IMPORT_METRIC,
                        ExternalAgentConfigMigrationItemType::Plugins,
                        /*skills_count*/ None,
                ...
                ExternalAgentConfigMigrationItemType::Sessions => {
                    if let Some(details) = migration_item.details {
                        outcome.pending_session_imports.extend(details.sessions);
                    }
                    emit_migration_metric(
                        EXTERNAL_AGENT_CONFIG_IMPORT_METRIC,
                        ExternalAgentConfigMigrationItemType::Sessions,
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.vac_home.join("config.toml"),
            |repo_root| repo_root.join(".vac").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.vac_home.join("hooks.json"),
            |repo_root| repo_root.join(".vac").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.vac_home.join("agents"),
            |repo_root| repo_root.join(".vac").join("agents"),
        );
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                .vac_home(self.vac_home.clone())
                .fallback_cwd(Some(self.vac_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.vac_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.vac_home)?;
    ...
    pub async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.vac_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome = add_marketplace(self.vac_home.clone(), request).await;
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<()> {
        ...
        let (source_settings, target_config) = if let Some(repo_root) = repo_root.as_ref() {
            (
                ...
                repo_root.join(".vac").join("config.toml"),
            )
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".vac").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".vac").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(());
            } else {
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::VAC.matches_product_restriction(products))
            })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "VAC");
    }
    rewritten
}
...
fn emit_migration_metric(
    ...
    // Owner-native provider is intentionally telemetry-free; app-server emits
    // external-agent migration metrics at its JSON-RPC boundary.
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[tokio::test]
    async fn plugin_import_completes_synchronously_even_when_marketplace_fails() {
        let root = TempDir::new().expect("tempdir");
        let vac_home = root.path().join("vac-home");
        let external_home = root.path().join("external-home");
        fs::create_dir_all(&external_home).expect("external home");
        fs::write(
            external_home.join("settings.json"),
            r#"{
                "enabledPlugins": { "demo@remote-marketplace": true },
                "extraKnownMarketplaces": {
                    "remote-marketplace": { "source": { "repo": "missing/remote" } }
                }
            }"#,
        )
        .expect("settings");
        let service = service_for_test(vac_home, external_home);
            ...
            .import(vec![ExternalAgentConfigMigrationItem {
                ...
                details: Some(MigrationDetails {
                    plugins: vec![PluginsMigration {
                        marketplace_name: "remote-marketplace".to_string(),
                        plugin_names: vec!["demo".to_string()],
                    }],
                    ..Default::default()
                }),
            }])
            .await
            .expect("plugin import should complete without leaving pending work");

        assert!(!outcome.has_pending_background_work());
        assert_eq!(outcome.plugin_import_outcomes.len(), 1);
        assert_eq!(
            outcome.plugin_import_outcomes[0].failed_marketplaces,
            vec!["remote-marketplace".to_string()]
        );
        assert_eq!(
            outcome.plugin_import_outcomes[0].failed_plugin_ids,
            vec!["demo@remote-marketplace".to_string()]
        );
    }

    #[tokio::test]
    async fn session_imports_remain_explicit_pending_background_work() {
        let root = TempDir::new().expect("tempdir");
        let service = service_for_test(
            root.path().join("vac-home"),
            root.path().join("external-home"),
        );
        let session = ExternalAgentSessionMigration {
            path: root.path().join("session.jsonl"),
        ...
        let outcome = service
            ...
            .await
            .expect("session selection should be represented as pending background work");

        assert!(outcome.has_pending_background_work());
        assert_eq!(outcome.pending_session_imports, vec![session]);
    }

    #[tokio::test]
    async fn import_items_are_processed_in_stable_input_order() {
        let root = TempDir::new().expect("tempdir");
        let external_home = root.path().join("external-home");
        fs::create_dir_all(&external_home).expect("external home");
        fs::write(
            ...
            r#"{
                ...
                "extraKnownMarketplaces": {
                    "market-a": { "source": { "repo": "missing/a" } },
                    "market-b": { "source": { "repo": "missing/b" } }
                }
            }"#,
        )
        .expect("settings");
        let service = service_for_test(root.path().join("vac-home"), external_home);
        ...
        let outcome = service
            .import(vec![
                ExternalAgentConfigMigrationItem {
                    ...
                    details: Some(MigrationDetails {
                        plugins: vec![PluginsMigration {
                            marketplace_name: "market-a".to_string(),
                            plugin_names: vec!["alpha".to_string()],
                        }],
                        ..Default::default()
                    }),
                },
                ExternalAgentConfigMigrationItem {
                    ...
                    details: Some(MigrationDetails {
                        plugins: vec![PluginsMigration {
                            marketplace_name: "market-b".to_string(),
                            plugin_names: vec!["bravo".to_string()],
                        }],
                        ..Default::default()
                    }),
                },
            ...
            .await
            .expect("plugin import attempts complete synchronously");

        let failed_marketplaces = outcome
            .plugin_import_outcomes
            .into_iter()
            .flat_map(|outcome| outcome.failed_marketplaces)
            .collect::<Vec<_>>();
        assert_eq!(
            failed_marketplaces,
