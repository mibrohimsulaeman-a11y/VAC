...
use tracing::warn;
use vac_config::ConfigEditsBuilder;
use vac_config::McpServerConfig;
use vac_config::McpServerTransportConfig;
use vac_config::load_global_mcp_servers;
use vac_login::default_client::is_first_party_originator;
...
use vac_rmcp_client::perform_oauth_login;

use crate::SkillMetadata;
use crate::session::session::Session;
use crate::session::turn_context::TurnContext;
use crate::skills::model::SkillToolDependency;
use vac_mcp::McpOAuthLoginSupport;
use vac_mcp::McpPermissionPromptAutoApproveContext;
use vac_mcp::mcp_permission_prompt_is_auto_approved;
use vac_mcp::oauth_login_support;
use vac_mcp::resolve_oauth_scopes;
use vac_mcp::should_retry_without_scopes;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(vac_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&vac_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&vac_home)
