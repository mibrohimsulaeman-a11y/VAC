...
use vac_analytics::GuardianReviewDecision;
use vac_analytics::GuardianReviewFailureReason;
use vac_analytics::GuardianReviewTerminalStatus;
use vac_analytics::GuardianReviewTrackContext;
...
use vac_protocol::protocol::GuardianAssessmentDecisionSource;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &vac_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        vac_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
...
/// ephemeral branch from the last committed trunk rollout so parallel approvals
...
pub(super) async fn run_guardian_review_session(
    ...
    let available_models = session
        .services
        .models_manager
        .list_models(vac_models_manager::manager::RefreshStrategy::Offline)
        .await;
    let preferred_reasoning_effort = |supports_low: bool, fallback| {
        if supports_low {
            Some(vac_protocol::vastar_models::ReasoningEffort::Low)
        } else {
            fallback
        }
    };
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == vac_protocol::vastar_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == vac_protocol::vastar_models::ReasoningEffort::Low),
