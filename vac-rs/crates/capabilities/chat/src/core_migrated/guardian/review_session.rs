...
use tracing::warn;
use vac_analytics::GuardianReviewAnalyticsResult;
use vac_analytics::GuardianReviewSessionKind;
...
use crate::vac_delegate::run_vac_thread_interactive;
...
struct GuardianReviewState {
    ...
    last_committed_branch_snapshot: Option<GuardianReviewBranchSnapshot>,
}
...
#[derive(Clone)]
struct GuardianReviewBranchSnapshot {
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            vac_linux_sandbox_exe: spawn_config.vac_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.vac.shutdown_and_wait().await;
    }
    ...
    async fn branch_snapshot(&self) -> Option<GuardianReviewBranchSnapshot> {
        self.state
            .lock()
            .await
            .last_committed_branch_snapshot
            .clone()
    }

    async fn refresh_last_committed_branch_snapshot(&self) {
        match load_rollout_items_for_branch(&self.vac.session).await {
            Ok(Some(items)) if !items.is_empty() => {
                ...
                state.last_committed_branch_snapshot = Some(GuardianReviewBranchSnapshot {
                    initial_history: InitialHistory::Branched(items),
                    prior_review_count,
...
impl GuardianReviewSessionManager {
    pub(crate) async fn trunk_rollout_path(&self) -> Option<PathBuf> {
        ...
        trunk.vac.session.ensure_rollout_materialized().await;
        match trunk.vac.session.current_rollout_path().await {
            Ok(path) => path,
            Err(err) => {
    ...
    pub(super) async fn run_review(
        ...
        if keep_review_session && matches!(outcome, GuardianReviewSessionOutcome::Completed(_)) {
            trunk.refresh_last_committed_branch_snapshot().await;
        }
        drop(trunk_guard);
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, vac: VAC) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            vac.session.get_config().await.as_ref(),
        );
        self.state.lock().await.trunk = Some(Arc::new(GuardianReviewSession {
            ...
            state: Mutex::new(GuardianReviewState {
                ...
                last_committed_branch_snapshot: None,
            }),
        }));
    }

    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, vac: VAC) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            vac.session.get_config().await.as_ref(),
        );
        self.state
    ...
    #[cfg(test)]
    pub(crate) async fn committed_branch_rollout_items_for_test(&self) -> Option<Vec<RolloutItem>> {
        let trunk = self.state.lock().await.trunk.clone()?;
        let state = trunk.state.lock().await;
        let snapshot = state.last_committed_branch_snapshot.as_ref()?;
        match &snapshot.initial_history {
            InitialHistory::Branched(items) => Some(items.clone()),
    ...
    pub(crate) async fn send_trunk_event_raw_for_test(&self, event: Event) {
        ...
        trunk.vac.session.send_event_raw(event).await;
    }
    ...
    async fn run_ephemeral_review(
        ...
        branch_snapshot: Option<GuardianReviewBranchSnapshot>,
    ) -> (GuardianReviewSessionOutcome, GuardianReviewAnalyticsResult) {
        let spawn_cancel_token = CancellationToken::new();
        let mut branch_config = params.spawn_config.clone();
        branch_config.ephemeral = true;
        let review_session = match run_before_review_deadline_with_cancel(
            deadline,
            params.external_cancel.as_ref(),
            &spawn_cancel_token,
        ...
        let (outcome, _, analytics_result) = Box::pin(run_review_on_session(
            ...
            &params,
            GuardianReviewSessionKind::EphemeralBranched,
            deadline,
        ))
        .await;
...
async fn spawn_guardian_review_session(
    ...
    let (initial_history, prior_review_count, initial_transcript_cursor) = match branch_snapshot {
        Some(branch_snapshot) => (
            Some(branch_snapshot.initial_history),
            branch_snapshot.prior_review_count,
            branch_snapshot.last_reviewed_transcript_cursor,
        ),
        None => (None, 0, None),
    };
    let vac = Box::pin(run_vac_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let mut analytics_result = GuardianReviewAnalyticsResult::from_session(
        review_session.vac.session.conversation_id.to_string(),
        guardian_session_kind,
        params.model.clone(),
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.vac.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.vac.submit(Op::UserTurn {
            environments: None,
    ...
    if matches!(outcome.0, GuardianReviewSessionOutcome::Completed(_)) {
        if outcome.2
            && let Some(total_token_usage) = review_session.vac.session.total_token_usage().await
        {
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.vac.session.new_default_turn().await;
...
async fn load_rollout_items_for_branch(
    session: &Session,
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            ...
            event = review_session.vac.next_event() => {
                match event {
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<vac_network_proxy::NetworkProxyConfig>,
    active_model: &str,
    reasoning_effort: Option<vac_protocol::vastar_models::ReasoningEffort>,
...
async fn interrupt_and_drain_turn(vac: &VAC, expected_turn_id: &str) -> anyhow::Result<()> {
    let _ = vac.submit(Op::Interrupt).await;
...
mod tests {
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::VACHooks));
    }
    ...
    async fn interrupt_and_drain_turn_ignores_prior_turn_completion() {
        ...
        interrupt_and_drain_turn(&review_session.vac, "current-turn")
            .await
            .expect("drain current turn");

        assert!(review_session.vac.rx_event.try_recv().is_err());
    }
}
