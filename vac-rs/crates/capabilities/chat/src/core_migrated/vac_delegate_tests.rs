...
use vac_protocol::protocol::GuardianAssessmentAction;
...
use vac_protocol::request_user_input::RequestUserInputEvent;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let cancel = CancellationToken::new();
    let forward = tokio::spawn(forward_ops(Arc::clone(&vac), rx_ops, cancel));

    let submission = Submission {
        id: "sub-1".to_string(),
        op: Op::Interrupt,
        trace: Some(vac_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
#[tokio::test]
async fn run_vac_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    assert!(matches!(result, Err(VACErr::TurnAborted)));
}
