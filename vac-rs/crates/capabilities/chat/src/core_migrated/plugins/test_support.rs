...
pub(crate) fn write_plugins_feature_config(vac_home: &Path) {
    write_file(
...
pub(crate) async fn load_plugins_config(vac_home: &Path) -> crate::config::Config {
    ConfigBuilder::default()
        .vac_home(vac_home.to_path_buf())
        .fallback_cwd(Some(vac_home.to_path_buf()))
        .build()
        .await
