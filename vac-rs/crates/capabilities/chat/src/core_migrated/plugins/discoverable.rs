...
use vac_core_plugins::TOOL_SUGGEST_DISCOVERABLE_PLUGIN_ALLOWLIST;
use vac_core_plugins::VASTAR_BUNDLED_MARKETPLACE_NAME;
use vac_core_plugins::VASTAR_CURATED_MARKETPLACE_NAME;
use vac_features::Feature;
use vac_tools::DiscoverablePluginInfo;

const TOOL_SUGGEST_DISCOVERABLE_MARKETPLACE_ALLOWLIST: &[&str] = &[
    VASTAR_BUNDLED_MARKETPLACE_NAME,
    VASTAR_CURATED_MARKETPLACE_NAME,
];
...
pub(crate) async fn list_tool_suggest_discoverable_plugins(
    ...
    let plugins_manager = PluginsManager::new(config.vac_home.to_path_buf());
