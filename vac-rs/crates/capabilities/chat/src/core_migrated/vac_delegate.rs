...
use vac_analytics::GuardianApprovalRequestSource;
...
/// Start an interactive sub-VAC thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_vac_thread_interactive(
    config: Config,
    ...
    let VACSpawnOk { vac, .. } = Box::pin(VAC::spawn(VACSpawnArgs {
        config,
        auth_manager,
        models_manager,
        ...
        parent_rollout_thread_trace: vac_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
    ...
    let vac_for_events = Arc::clone(&vac);
    ...
    let vac_for_ops = Arc::clone(&vac);
    tokio::spawn(async move {
        forward_ops(vac_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(VAC {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: vac.agent_status.clone(),
        session: Arc::clone(&vac.session),
        session_loop_termination: vac.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_vac_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_vac_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&vac).await;
                break;
            }
            event = vac.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&vac, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(vac: &VAC) {
    let _ = vac.submit(Op::Interrupt).await;
    let _ = vac.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = vac.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = vac.submit_with_id(submission).await;
    }
}
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                vac_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                vac_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                vac_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    if routes_approval_to_guardian(parent_ctx)
        ...
        .await
    {
        let _ = vac.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(ccunningham): Support delegated MCP approval elicitations here too after
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            vac_protocol::request_user_input::RequestUserInputAnswer {
...
) -> vac_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = vac_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, vac_protocol::protocol::ReviewDecision::Abort)
                .await;
            vac_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
