...
impl Default for FileSearchOptions {
    fn default() -> Self {
        Self {
            limit: NonZero::new(20).unwrap_or(NonZero::<usize>::MIN),
            exclude: Vec::new(),
            threads: NonZero::new(2).unwrap_or(NonZero::<usize>::MIN),
...
fn recover_poisoned_lock<T>(result: std::sync::LockResult<T>) -> T {
    match result {
        Ok(guard) => guard,
...
impl SessionReporter for RunReporter {
    fn on_update(&self, snapshot: &FileSearchSnapshot) {
        let mut guard = recover_poisoned_lock(self.snapshot.write());
        *guard = snapshot.clone();
    }

    fn on_complete(&self) {
        let (cv, mutex) = &self.completed;
        let mut completed = recover_poisoned_lock(mutex.lock());
        *completed = true;
        cv.notify_all();
    }
}
...
impl RunReporter {
    fn wait_for_complete(&self) -> FileSearchSnapshot {
        ...
        let mut completed = recover_poisoned_lock(mutex.lock());
        while !*completed {
            completed = recover_poisoned_lock(cv.wait(completed));
        }
        recover_poisoned_lock(self.snapshot.read()).clone()
    }
}
