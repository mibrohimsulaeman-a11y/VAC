...
impl BlockingStdinPipe {
    fn install() -> Result<Self> {
        let mut fds = [0i32; 2];
        // SAFETY: `fds` points to two initialized i32 slots and libc::pipe fills them atomically for this test pipe.
        if unsafe { libc::pipe(fds.as_mut_ptr()) } == -1 {
            return Err(std::io::Error::last_os_error()).context("create stdin pipe");
        }

        // SAFETY: Duplicating the process stdin fd is confined to this Unix test helper and checked for -1 before use.
        let original = unsafe { libc::dup(libc::STDIN_FILENO) };
        if original == -1 {
            let err = std::io::Error::last_os_error();
            // SAFETY: Both pipe fds came from a successful libc::pipe call in this function and are closed only on this error path.
            unsafe {
                libc::close(fds[0]);
                libc::close(fds[1]);
            }
        ...
        // SAFETY: `fds[0]` is the read end of the pipe and dup2 atomically replaces stdin for this scoped test helper.
        ...
        // SAFETY: After dup2 succeeds the original read-end fd is no longer needed; closing it leaves stdin owning the duplicate.
        unsafe {
            libc::close(fds[0]);
        }
...
impl Drop for BlockingStdinPipe {
    fn drop(&mut self) {
        // SAFETY: Drop restores the saved stdin fd and closes fds owned exclusively by BlockingStdinPipe.
        unsafe {
...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command::new("/bin/bash")
        ...
        .env("VALID_NAME", "ok")
        .env("PWD", "/tmp/stale")
        .env("NEXTEST_BIN_EXE_vac-write-config-schema", "/path/to/bin")
        .env("BAD-NAME", "broken")
        .output()?;
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_vac-write-config-schema"));
...
async fn write_rollout_stub(vac_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
    let dir = vac_home.join("sessions").join("2025").join("01").join("01");
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    let dir = tempdir()?;
    let vac_home = dir.path().abs();
    let snapshot_dir = vac_home.join(SNAPSHOT_DIR);
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&vac_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&vac_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(&vac_home, active_session).await?;
...
fn set_file_mtime(path: &Path, age: Duration) -> Result<()> {
    ...
    // SAFETY: `c_path` is a valid NUL-terminated path and `times` points to two initialized timespec values for utimensat.
