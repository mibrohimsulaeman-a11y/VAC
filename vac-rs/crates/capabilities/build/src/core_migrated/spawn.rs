...
///
/// 1. The process was spawned by VAC as part of a shell tool call.
/// 2. NetworkSandboxPolicy is restricted for the tool call.
///
/// We may try to have just one environment variable for all sandboxing
/// attributes, so this may change in the future.
pub const VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "VAC_SANDBOX_NETWORK_DISABLED";
...
pub const VAC_SANDBOX_ENV_VAR: &str = "VAC_SANDBOX";
...
/// `VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }

    // If this VAC process dies (including being killed via SIGKILL), we want
    // any child processes that were spawned as part of a `"shell"` tool call
    // to also be terminated.

    #[cfg(unix)]
    // SAFETY: spawn boundary: the pre-exec closure runs after fork before exec and mutates only process-local descriptors prepared here; the multi-line unsafe block below uses inputs that remain live until the block returns.
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                vac_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
