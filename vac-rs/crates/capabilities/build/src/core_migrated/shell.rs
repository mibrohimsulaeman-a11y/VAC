...
fn get_user_shell_path() -> Option<PathBuf> {
    // SAFETY: shell runtime boundary: this launcher path checks the process/session descriptor or libc pointer before the platform call; `libc::getuid()` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: shell runtime boundary: this launcher path checks the process/session descriptor or libc pointer before the platform call; `libc::sysconf(libc::_SC_GETPW_R_SIZE_MAX)` uses inputs that remain live for the duration of this block.
    ...
    loop {
        ...
        // SAFETY: shell runtime boundary: this launcher path checks the process/session descriptor or libc pointer before the platform call; the multi-line unsafe block below uses inputs that remain live until the block returns.
        let status = unsafe {
            libc::getpwuid_r(
                uid,
                passwd.as_mut_ptr(),
        ...
        if status == 0 {
            ...
            // SAFETY: shell runtime boundary: this launcher path checks the process/session descriptor or libc pointer before the platform call; `passwd.assume_init_ref()` uses inputs that remain live for the duration of this block.
            let passwd = unsafe { passwd.assume_init_ref() };
            if passwd.pw_shell.is_null() {
                return None;
            }

            // SAFETY: shell runtime boundary: this launcher path checks the process/session descriptor or libc pointer before the platform call; `CStr::from_ptr(passwd.pw_shell)` uses inputs that remain live for the duration of this block.
