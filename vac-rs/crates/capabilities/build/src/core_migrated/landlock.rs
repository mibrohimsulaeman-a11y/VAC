...
use vac_sandboxing::landlock::allow_network_for_proxy;
use vac_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
use vac_utils_absolute_path::AbsolutePathBuf;

/// Spawn a shell tool command under the Linux sandbox helper
/// (vac-linux-sandbox), which defaults to bubblewrap for filesystem
    ...
    P: AsRef<Path>,
{
    ...
    let vac_linux_sandbox_exe = vac_linux_sandbox_exe.as_ref();
    ...
    let arg0 = if vac_linux_sandbox_exe
        .file_name()
        ...
        == Some(VAC_LINUX_SANDBOX_ARG0)
    {
        ...
        vac_linux_sandbox_exe.to_string_lossy().into_owned()
    } else {
        VAC_LINUX_SANDBOX_ARG0.to_string()
    };
    spawn_child_async(SpawnChildRequest {
        program: vac_linux_sandbox_exe.to_path_buf(),
        args,
        arg0: Some(&arg0),
        cwd: command_cwd,
