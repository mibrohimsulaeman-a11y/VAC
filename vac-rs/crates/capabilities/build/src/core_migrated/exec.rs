...
use vac_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: vac_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: vac_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let ExecParams {
        ...
        // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): Should arg0 be set on the ExecRequest that is returned?
        arg0: _,
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            vac_linux_sandbox_exe: vac_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        vac_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = vac_otel::global() {
        let _ = metrics.counter(
            "vac.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_vac_home;
    use vac_windows_sandbox::run_windows_sandbox_capture_elevated;
    use vac_windows_sandbox::run_windows_sandbox_capture_with_extra_deny_write_paths;
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(iceweasel-oai): run_windows_sandbox_capture should support all
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        VACErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let vac_home = find_vac_home().map_err(|err| {
        VACErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve vac_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_elevated(
                vac_windows_sandbox::ElevatedSandboxCaptureRequest {
                    policy_json_or_preset: policy_str.as_str(),
                    sandbox_policy_cwd: &sandbox_cwd,
                    vac_home: vac_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(VACErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, VACErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            if timed_out {
                return Err(VACErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(VACErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// VAC_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
...
pub(crate) fn resolve_windows_elevated_filesystem_overrides(
    ...
    let split_readable_roots: Vec<PathBuf> = file_system_sandbox_policy
        ...
        .into_iter()
        .map(vac_utils_absolute_path::AbsolutePathBuf::into_path_buf)
        .map(&normalize_path)
        .collect();
...
fn has_reopened_writable_descendant(
    writable_roots: &[vac_protocol::protocol::WritableRoot],
) -> bool {
