...
use vac_execpolicy::AmendError;
use vac_execpolicy::Decision;
use vac_execpolicy::Error as ExecPolicyRuleError;
use vac_execpolicy::Evaluation;
use vac_execpolicy::MatchOptions;
use vac_execpolicy::NetworkRuleProtocol;
use vac_execpolicy::Policy;
use vac_execpolicy::PolicyParser;
use vac_execpolicy::RuleMatch;
use vac_execpolicy::blocking_append_allow_prefix_rule;
use vac_execpolicy::blocking_append_network_rule;
...
use vac_shell_command::is_dangerous_command::command_might_be_dangerous;
use vac_shell_command::is_safe_command::is_known_safe_command;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(vac_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(vac_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &vac_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            vac_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
    ...
    let command_is_dangerous = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            vac_shell_command::is_dangerous_command::is_dangerous_powershell_words(command)
        }
    };
...
fn default_policy_path(vac_home: &Path) -> PathBuf {
    vac_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            vac_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and vac prompts the user to run outside the sandbox
