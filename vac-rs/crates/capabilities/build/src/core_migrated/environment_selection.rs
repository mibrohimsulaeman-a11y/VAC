...
impl ResolvedTurnEnvironments {
    ...
    pub(crate) fn primary_environment(&self) -> Option<Arc<vac_exec_server::Environment>> {
...
pub(crate) fn resolve_environment_selections(
...
) -> VACResult<ResolvedTurnEnvironments> {
    ...
    for selected_environment in environments {
        ...
        let environment = environment_manager
            ...
            .ok_or_else(|| {
                VACErr::InvalidRequest(format!("unknown turn environment id `{environment_id}`"))
            })?;
        turn_environments.push(TurnEnvironment {
            environment_id,
            environment,
            cwd: selected_environment.cwd.clone(),
            // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(starr): Resolve shell metadata per environment instead of
            // hardcoding bash.
...
mod tests {
    ...
    use vac_exec_server::REMOTE_ENVIRONMENT_ID;
    use vac_protocol::protocol::TurnEnvironmentSelection;
    use vac_utils_absolute_path::AbsolutePathBuf;
