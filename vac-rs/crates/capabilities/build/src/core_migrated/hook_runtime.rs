...
use vac_analytics::HookRunFact;
use vac_analytics::build_track_events_context;
use vac_hooks::PermissionRequestDecision;
use vac_hooks::PermissionRequestOutcome;
use vac_hooks::PermissionRequestRequest;
use vac_hooks::PostToolUseOutcome;
use vac_hooks::PostToolUseRequest;
use vac_hooks::PreToolUseOutcome;
use vac_hooks::PreToolUseRequest;
use vac_hooks::SessionStartOutcome;
use vac_hooks::UserPromptSubmitOutcome;
use vac_hooks::UserPromptSubmitRequest;
use vac_otel::HOOK_RUN_DURATION_METRIC;
use vac_otel::HOOK_RUN_METRIC;
...
use vac_protocol::protocol::HookCompletedEvent;
use vac_protocol::protocol::HookEventName;
use vac_protocol::protocol::HookRunStatus;
use vac_protocol::protocol::HookRunSummary;
use vac_protocol::protocol::HookSource;
use vac_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_pending_session_start_hooks(
    ...
    let request = vac_hooks::SessionStartRequest {
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (vac_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use vac_protocol::protocol::HookEventName;
    use vac_protocol::protocol::HookExecutionMode;
    use vac_protocol::protocol::HookHandlerType;
    use vac_protocol::protocol::HookRunStatus;
    use vac_protocol::protocol::HookScope;
    use vac_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    vac_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
