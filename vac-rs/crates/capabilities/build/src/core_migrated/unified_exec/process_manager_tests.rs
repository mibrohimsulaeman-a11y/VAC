...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("VAC_THREAD_ID".to_string(), "thread-1".to_string()),
        ("VAC_SANDBOX_NETWORK_DISABLED".to_string(), "1".to_string()),
    ]);
...
fn exec_server_params_use_env_policy_overlay_contract() {
    let cwd: vac_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let sandbox_policy = vac_protocol::protocol::SandboxPolicy::DangerFullAccess;
    let file_system_sandbox_policy =
        vac_protocol::permissions::FileSystemSandboxPolicy::from(&sandbox_policy);
    let network_sandbox_policy = vac_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile =
        vac_protocol::models::PermissionProfile::from_runtime_permissions_with_enforcement(
            vac_protocol::models::SandboxEnforcement::from_legacy_sandbox_policy(&sandbox_policy),
    ...
    let request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: vac_exec_server::ExecEnvPolicy {
                inherit: vac_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: vac_sandboxing::SandboxType::None,
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let vac_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        vac_protocol::protocol::ExecCommandStatus::Failed
    );
