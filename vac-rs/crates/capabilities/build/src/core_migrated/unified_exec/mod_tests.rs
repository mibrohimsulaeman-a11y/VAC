...
use core_test_support::test_vac::test_env as remote_test_env;
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export VAC_INTERACTIVE_SHELL_VAR=vac\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $VAC_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
        out_2.truncated_output().contains("vac"),
...
async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
    ...
    .await?;
    assert!(
        out_3.truncated_output().contains("vac"),
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export VAC_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $VAC_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
    ...
    let result = exec_command(
        &session, &turn, "echo vac", /*yield_time_ms*/ 120_000, /*workdir*/ None,
    )
    .await?;

    assert!(result.process_id.is_some());
    assert!(result.truncated_output().contains("vac"));
...
async fn completed_commands_do_not_persist_sessions() -> anyhow::Result<()> {
    let (session, turn) = test_session_and_turn().await;
    let result = exec_command(
        &session, &turn, "echo vac", /*yield_time_ms*/ 2_500, /*workdir*/ None,
    )
    .await?;
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = vac_exec_server::Environment::default_for_tests();
