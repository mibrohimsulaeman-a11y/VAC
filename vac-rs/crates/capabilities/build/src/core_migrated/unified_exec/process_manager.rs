...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the VAC sandbox network proxy.";
...
fn exec_server_env_for_request(
    ...
    Option<vac_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::VAC(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == vac_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let vac_home = crate::config::find_vac_home().map_err(|err| {
                UnifiedExecError::create_process(format!(
            ...
            let spawned = match request.windows_sandbox_level {
                vac_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    vac_windows_sandbox::spawn_windows_sandbox_session_elevated(
                        policy_json.as_str(),
                ...
                vac_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | vac_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    vac_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        policy_json.as_str(),
                        request.windows_sandbox_policy_cwd.as_path(),
                        vac_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            vac_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
            ...
            vac_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            VAC_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::VAC(VACErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;
