...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".vac-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.vac_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.vac_home).unwrap();
...
fn create_skill(vac_home: PathBuf, name: &str, description: &str) {
    let skill_dir = vac_home.join(format!("skills/{name}"));
