...
use vac_config::AppRequirementToml;
...
use vac_config::types::AppConfig;
use vac_config::types::AppToolConfig;
use vac_config::types::AppToolsConfig;
use vac_config::types::AppsDefaultConfig;
use vac_connectors::filter::filter_disallowed_connectors;
use vac_connectors::filter::filter_tool_suggest_discoverable_connectors;
use vac_connectors::merge::merge_connectors;
use vac_connectors::merge::plugin_connector_to_app_info;
use vac_connectors::metadata::connector_install_url;
use vac_connectors::metadata::connector_mention_slug;
use vac_connectors::metadata::sanitize_name;
...
fn vac_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{VAC_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| VAC_APPS_MCP_SERVER_NAME.to_string());
...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    ...
    let tools = HashMap::from([
        ...
        (
            "mcp__vac_apps__vastar_hidden".to_string(),
            vac_app_tool(
                "vastar_hidden",
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(vac_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .vac_home(vac_home.path().to_path_buf())
        .fallback_cwd(Some(vac_home.path().to_path_buf()))
        .build()
        .await
...
fn filter_disallowed_connectors_allows_non_disallowed_connectors() {
    let filtered =
        filter_disallowed_connectors(vec![app("asdk_app_hidden"), app("alpha")], "vac_cli");
    assert_eq!(filtered, vec![app("asdk_app_hidden"), app("alpha")]);
}

#[test]
fn filter_disallowed_connectors_filters_vastar_prefix() {
    let filtered = filter_disallowed_connectors(
        vec![
            app("connector_vastar_foo"),
            app("connector_vastar_bar"),
            app("gamma"),
        ],
        "vac_cli",
    );
...
#[test]
fn first_party_chat_originator_filters_target_and_vastar_prefixed_connectors() {
    let filtered = filter_disallowed_connectors(
        vec![
            app("connector_vastar_foo"),
            app("asdk_app_6938a94a61d881918ef32cb999ff937c"),
...
async fn tool_suggest_connector_ids_include_configured_tool_suggest_discoverables() {
    let vac_home = tempdir().expect("tempdir should succeed");
    std::fs::write(
        vac_home.path().join(CONFIG_TOML_FILE),
        r#"
[tool_suggest]
discoverables = [
