...
use vac_config::permissions_toml::overlay_network_domain_permissions;
...
use vac_network_proxy::NetworkProxyConstraintError;
...
use vac_network_proxy::validate_policy_against_constraints;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    let vac_home = find_vac_home().context("failed to resolve VAC_HOME")?;
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                ConfigLayerSource::Project { dot_vac_folder } => {
                    Some(dot_vac_folder.join(CONFIG_TOML_FILE))
                }
...
fn apply_exec_policy_network_rules(
    ...
    for host in allowed_domains {
        upsert_network_domain(
            config,
            host,
            vac_network_proxy::NetworkDomainPermission::Allow,
        );
    }
    for host in denied_domains {
        upsert_network_domain(
            config,
            host,
            vac_network_proxy::NetworkDomainPermission::Deny,
        );
    }
}
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: vac_network_proxy::NetworkDomainPermission,
) {
    config
        .network
