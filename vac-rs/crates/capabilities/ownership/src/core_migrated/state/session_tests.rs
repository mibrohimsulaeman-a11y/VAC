...
#[tokio::test]
async fn set_rate_limits_defaults_limit_id_to_vac_when_missing() {
    let session_configuration = make_session_configuration_for_tests().await;
    let mut state = SessionState::new(session_configuration);
...
#[tokio::test]
async fn set_rate_limits_defaults_to_vac_when_limit_id_missing_after_other_bucket() {
    let session_configuration = make_session_configuration_for_tests().await;
    let mut state = SessionState::new(session_configuration);
...
#[tokio::test]
async fn set_rate_limits_carries_credits_and_plan_type_from_vac_to_vac_other() {
    let session_configuration = make_session_configuration_for_tests().await;
    let mut state = SessionState::new(session_configuration);

    state.set_rate_limits(RateLimitSnapshot {
        limit_id: Some("vac".to_string()),
        limit_name: Some("vac".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 10.0,
            window_minutes: Some(60),
            resets_at: Some(100),
        ...
        plan_type: Some(vac_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    });
