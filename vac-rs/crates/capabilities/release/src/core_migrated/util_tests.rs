...
use vac_feedback::emit_feedback_request_tags;
...
fn emit_feedback_request_tags_records_sentry_feedback_fields() {
    ...
    assert_eq!(
        tags.get("auth_env_vastar_api_key_present")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
        tags.get("auth_env_vac_api_key_present").map(String::as_str),
        Some("false")
    );
    assert_eq!(
        tags.get("auth_env_vac_api_key_enabled").map(String::as_str),
        Some("true")
    );
    assert_eq!(
...
fn resume_command_prefers_name_over_id() {
    ...
    assert_eq!(command, Some("vac resume my-thread".to_string()));
}
...
fn resume_command_with_only_id() {
    ...
    assert_eq!(
        command,
        Some("vac resume 123e4567-e89b-12d3-a456-426614174000".to_string())
    );
}
...
fn resume_command_quotes_thread_name_when_needed() {
    ...
    assert_eq!(command, Some("vac resume -- -starts-with-dash".to_string()));

    let command = resume_command(Some("two words"), /*thread_id*/ None);
    assert_eq!(command, Some("vac resume 'two words'".to_string()));

    let command = resume_command(Some("quote'case"), /*thread_id*/ None);
    assert_eq!(command, Some("vac resume \"quote'case\"".to_string()));
}
