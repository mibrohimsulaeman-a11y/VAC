...
/// `vac_feedback::VACFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// vac_core::feedback_tags!(model = "gpt-5", cached = true);
/// vac_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
