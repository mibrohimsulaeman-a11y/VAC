...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_vac_version_mismatch: bool,
}
...
pub(crate) fn config_lockfile(config: ConfigToml) -> ConfigLockfileToml {
    ConfigLockfileToml {
        ...
        vac_version: env!("CARGO_PKG_VERSION").to_string(),
        config,
    }
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_vac_version_mismatch && expected_lock.vac_version != actual_lock.vac_version {
        return Err(config_lock_error(format!(
            "config lock VAC version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_vac_version_mismatch=true to ignore this",
            expected_lock.vac_version, actual_lock.vac_version
        )));
    }
...
fn config_lock_for_comparison(
    ...
    if options.allow_vac_version_mismatch {
        lockfile.vac_version.clear();
    }
    lockfile
}
