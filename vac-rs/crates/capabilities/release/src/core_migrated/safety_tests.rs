...
use vac_protocol::protocol::FileSystemAccessMode;
use vac_protocol::protocol::FileSystemPath;
use vac_protocol::protocol::FileSystemSandboxEntry;
use vac_protocol::protocol::FileSystemSpecialPath;
...
fn external_sandbox_auto_approves_in_on_request() {
    ...
    let policy = SandboxPolicy::ExternalSandbox {
        network_access: vac_protocol::protocol::NetworkAccess::Enabled,
    };
...
#[test]
fn missing_project_dot_vac_config_requires_approval() {
    let tmp = TempDir::new().unwrap();
    let cwd = tmp.path().abs();
    let config_path = cwd.join(".vac").join("config.toml");
