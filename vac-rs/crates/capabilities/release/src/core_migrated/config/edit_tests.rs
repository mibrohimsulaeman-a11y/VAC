...
use vac_config::types::McpServerToolConfig;
...
fn blocking_set_model_top_level() {
    ...
    let contents = std::fs::read_to_string(vac_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(vac_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn keymap_binding_edit_replaces_existing_binding_without_touching_profile() {
    ...
    let vac_home = tmp.path();
    std::fs::write(
        vac_home.join(CONFIG_TOML_FILE),
        r#"profile = "team"
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = vac_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let vac_home = tmp.path();
    let link_a = vac_home.join("a.toml");
    let link_b = vac_home.join("b.toml");
    let config_path = vac_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(vac_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(vac_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_vac_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let vac_home = tmp.path();
    std::fs::write(
        vac_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        ...
        /*profile*/ None,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-vac-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-vac-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !vac_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(vac_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(vac_home.join(CONFIG_TOML_FILE)).expect("read config");
