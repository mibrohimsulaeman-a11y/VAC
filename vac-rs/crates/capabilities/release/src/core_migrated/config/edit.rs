...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for VAC-managed fast defaults.
...
// VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(jif) move to a dedicated file
...
pub async fn apply(
    ...
    task::spawn_blocking(move || apply_blocking(&vac_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.vac_home, self.profile.as_deref(), &self.edits)
    }
