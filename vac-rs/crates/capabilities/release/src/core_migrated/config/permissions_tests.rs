...
use vac_config::permissions_toml::NetworkDomainPermissionToml;
use vac_config::permissions_toml::NetworkDomainPermissionsToml;
use vac_config::permissions_toml::NetworkToml;
use vac_config::permissions_toml::NetworkUnixSocketPermissionToml;
use vac_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let vac_home = temp_dir.path().join(".vac");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = vac_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("vac-arg0-session");
    let sibling_arg0_dir = arg0_root.join("vac-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("vac-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&vac_home)?,
    )
    .await?;
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(vac_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_enables_proxy_for_proxy_policy() {
    ...
    assert_eq!(
        ...
        Some(vac_network_proxy::NetworkDomainPermissions {
            entries: vec![vac_network_proxy::NetworkDomainPermissionEntry {
                pattern: "vastar.com".to_string(),
                permission: vac_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
