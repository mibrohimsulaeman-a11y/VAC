...
use vac_network_proxy::BlockedRequestObserver;
use vac_network_proxy::ConfigReloader;
use vac_network_proxy::ConfigState;
use vac_network_proxy::NetworkDecision;
use vac_network_proxy::NetworkPolicyDecider;
...
use vac_network_proxy::NetworkProxyHandle;
use vac_network_proxy::NetworkProxyState;
use vac_network_proxy::build_config_state;
use vac_network_proxy::host_and_port_from_network_addr;
    ...
    fn apply_requirements(
        ...
        if requirements.unix_sockets.is_some() {
            let allow_unix_sockets = requirements
                .unix_sockets
                .as_ref()
                .map(vac_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
                .unwrap_or_default();
            config
                .network
