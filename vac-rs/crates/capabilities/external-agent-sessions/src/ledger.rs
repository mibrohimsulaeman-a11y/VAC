...
pub fn has_current_session_been_imported(vac_home: &Path, source_path: &Path) -> io::Result<bool> {
    load_import_ledger(vac_home)?.contains_current_source(source_path)
}
...
pub fn record_imported_session(
    ...
    let mut ledger = load_import_ledger(vac_home)?;
    ...
    save_import_ledger(vac_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(vac_home);
...
fn import_ledger_path(vac_home: &Path) -> PathBuf {
    vac_home.join(SESSION_IMPORT_LEDGER_FILE)
}
