...
const LEGACY_CHATGPT_REMOTE_SKILLS_ENV: &str = "VAC_ENABLE_LEGACY_CHATGPT_REMOTE_SKILLS";
...
fn as_query_product_surface(product_surface: RemoteSkillProductSurface) -> &'static str {
    match product_surface {
        ...
        RemoteSkillProductSurface::VAC => "vac",
        RemoteSkillProductSurface::Api => "api",
        RemoteSkillProductSurface::Atlas => "atlas",
    }
}

fn legacy_chatgpt_remote_skills_enabled() -> bool {
    std::env::var(LEGACY_CHATGPT_REMOTE_SKILLS_ENV)
        .ok()
        .is_some_and(|value| matches!(value.as_str(), "1" | "true"))
}

fn ensure_vac_backend_auth(auth: Option<&VACAuth>) -> Result<&VACAuth> {
    if !legacy_chatgpt_remote_skills_enabled() {
        anyhow::bail!(
            "legacy ChatGPT remote skill scopes are disabled in the local coding-agent build; set VAC_ENABLE_LEGACY_CHATGPT_REMOTE_SKILLS=1 to opt in"
        );
    }
...
pub async fn list_remote_skills(
    ...
    let auth = ensure_vac_backend_auth(auth)?;
    ...
    let request = client
        ...
        .query(&query_params)
        .headers(vac_model_provider::auth_provider_from_auth(auth).to_auth_headers());
...
pub async fn export_remote_skill(
    ...
    let output_dir = vac_home.join("skills").join(skill_id);
