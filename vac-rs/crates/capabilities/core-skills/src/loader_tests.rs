...
use vac_utils_absolute_path::test_support::PathExt;

const REPO_ROOT_CONFIG_DIR_NAME: &str = ".vac";
...
async fn make_config(vac_home: &TempDir) -> TestConfig {
    make_config_for_cwd(vac_home, vac_home.path().to_path_buf()).await
}
...
fn project_layers_for_cwd(cwd: &Path) -> Vec<ConfigLayerEntry> {
    ...
    layers
        .into_iter()
        .filter_map(|dir| {
            let dot_vac = dir.join(REPO_ROOT_CONFIG_DIR_NAME);
            dot_vac.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_vac_folder: dot_vac.abs(),
                    },
...
async fn make_config_for_cwd(vac_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = vac_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = vac_home.path().join("etc/vac/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/vac");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("vac");
...
async fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    let dot_vac = project_root.join(".vac");
    fs::create_dir_all(&dot_vac)?;

    let user_file = user_folder.join("config.toml").abs();
    let project_dot_vac = dot_vac.abs();
    ...
    let layers = vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_vac_folder: project_dot_vac,
            },
            TomlValue::Table(toml::map::Map::new()),
            "marked untrusted",
        ),
    ...
    assert_eq!(
        got,
        vec![
            (SkillScope::Repo, dot_vac.join("skills")),
            (SkillScope::User, user_folder.join("skills")),
            (
                SkillScope::User,
                home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
...
fn write_skill(vac_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&vac_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(vac_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &vac_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&vac_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    let cfg = make_config(&vac_home).await;
...
async fn loads_skill_interface_metadata_from_yaml() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&vac_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&vac_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&vac_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&vac_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::VAC, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(vac_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &vac_home.path().join("skills/shared"));
...
async fn ignores_symlinked_skill_file_for_user_scope() {
    ...
    let skill_dir = vac_home.path().join("skills/demo");
    fs::create_dir_all(&skill_dir).unwrap();
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $VAC_HOME/skills/cycle/loop -> $VAC_HOME/skills/cycle
    let cycle_dir = vac_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&vac_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg).await;
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = vac_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = vac_home.path().join("skills");
    let outcome = load_skills_from_roots([SkillRoot {
...
async fn loads_valid_skill() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&vac_home, "demo", "demo-skill", "does things\ncarefully");
    let cfg = make_config(&vac_home).await;
...
async fn namespaces_plugin_skills_using_plugin_name() {
    ...
    fs::create_dir_all(plugin_root.join(".vac-plugin")).unwrap();
    fs::write(
...
async fn skips_hidden_and_invalid() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = vac_home.path().join("skills/.hidden");
    ...
    let invalid_dir = vac_home.path().join("skills/invalid");
...
async fn enforces_length_limits() {
    ...
    write_skill(&vac_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&vac_home).await;
    ...
    write_skill(&vac_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert_eq!(outcome.skills.len(), 1);
    assert_eq!(outcome.errors.len(), 1);
    assert!(
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_vac_dir() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_vac_dirs_under_project_root() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&vac_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_vac_dir_when_not_git_repo() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&vac_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&vac_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_vac_dirs() {
    let vac_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&vac_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&vac_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&vac_home, "system", "system-skill", "from system");
