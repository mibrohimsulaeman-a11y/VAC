...
use vac_protocol::config_types::WebSearchContextSize;
use vac_protocol::config_types::WebSearchFilters as ConfigWebSearchFilters;
use vac_protocol::config_types::WebSearchMode;
use vac_protocol::config_types::WebSearchUserLocation as ConfigWebSearchUserLocation;
use vac_protocol::config_types::WebSearchUserLocationType;
use vac_protocol::vastar_models::WebSearchToolType;

const WEB_SEARCH_TEXT_AND_IMAGE_CONTENT_TYPES: [&str; 2] = ["text", "image"];

/// When serialized as JSON, this produces a valid "Tool" in the Vastar
/// Responses API.
...
pub enum ToolSpec {
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): Understand why we get an error on web_search although the API docs
    // say it's supported.
    // https://platform.vastar.com/docs/guides/tools-web-search?api-mode=responses#:~:text=%7B%20type%3A%20%22web_search%22%20%7D%2C
    // The `external_web_access` field determines whether the web search is over
    // cached or live content.
    // https://platform.vastar.com/docs/guides/tools-web-search#live-internet-access
...
/// Responses API:
/// https://platform.vastar.com/docs/guides/function-calling?api-mode=responses
