...
use vac_code_mode::CodeModeToolKind;
use vac_code_mode::ToolDefinition as CodeModeToolDefinition;
            ...
            for tool in &mut namespace.tools {
                match tool {
                    ResponsesApiNamespaceTool::Function(tool) => {
                        ...
                        tool.description =
                            vac_code_mode::augment_tool_definition(definition).description;
                    }
                }
            }
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    vac_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| vac_code_mode::augment_tool_definition(definition))
}
...
pub fn collect_code_mode_tool_definitions<'a>(
    ...
    let mut tool_definitions = specs
        ...
        .filter(|definition| vac_code_mode::is_code_mode_nested_tool(&definition.name))
        .map(vac_code_mode::augment_tool_definition)
        .collect::<Vec<_>>();
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        name: vac_code_mode::PUBLIC_TOOL_NAME.to_string(),
        description: vac_code_mode::build_exec_tool_description(
            enabled_tools,
