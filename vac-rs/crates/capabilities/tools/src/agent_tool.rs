...
fn spawn_agent_common_properties_v1(agent_type_description: &str) -> BTreeMap<String, JsonSchema> {
    BTreeMap::from([
        ...
        (
            "branch_context".to_string(),
            JsonSchema::boolean(Some(
                "When true, branch the current thread history into the new agent before sending the initial prompt. This must be used when you want the new agent to have exactly the same context as you."
                    .to_string(),
            )),
        ),
        (
...
fn spawn_agent_common_properties_v2(agent_type_description: &str) -> BTreeMap<String, JsonSchema> {
    BTreeMap::from([
        ...
        (
            "branch_turns".to_string(),
            JsonSchema::string(Some(
                "Optional number of turns to branch. Defaults to `all`. Use `none`, `all`, or a positive integer string such as `3` to branch only the most recent turns."
                    .to_string(),
            )),
        ),
        (
...
- When delegating coding work, instruct the submodel to edit files directly in its branched workspace and list the file paths it changed in the final answer.
