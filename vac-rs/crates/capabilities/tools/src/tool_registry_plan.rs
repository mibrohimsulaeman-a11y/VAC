...
use vac_protocol::vastar_models::ApplyPatchToolType;
...
pub fn build_tool_registry_plan(
    ...
    if config.code_mode_enabled {
        let namespace_descriptions = params
            ...
            .map(|(namespace, detail)| {
                (
                    namespace.clone(),
                    vac_code_mode::ToolNamespaceDescription {
        ...
        plan.register_handler(
            vac_code_mode::PUBLIC_TOOL_NAME,
        ...
        plan.register_handler(vac_code_mode::WAIT_TOOL_NAME, ToolHandlerKind::CodeModeWait);
    }
        ...
        && (deferred_mcp_tools_for_search.is_some() || !deferred_dynamic_tools.is_empty())
    {
        ...
        if !deferred_dynamic_tools.is_empty() {
            search_source_infos.push(ToolSearchSourceInfo {
                ...
                description: Some("Tools provided by the current VAC thread.".to_string()),
            });
        }
            ...
            for tool in entries {
                match mcp_tool_to_responses_api_tool(&tool.name, tool.tool) {
                    ...
                    Err(error) => {
                        let tool_name = &tool.name;
                        tracing::error!(
                            "Failed to convert `{tool_name}` MCP tool to Vastar tool: {error:?}"
                        );
                    }
                }
            }
    ...
    for tool in params.dynamic_tools {
        match dynamic_tool_to_loadable_tool_spec(tool) {
            ...
            Err(error) => {
                tracing::error!(
                    "Failed to convert dynamic tool {:?} to Vastar tool: {error:?}",
                    tool.name
                );
            }
        }
...
fn compare_code_mode_tools(
    left: &vac_code_mode::ToolDefinition,
    right: &vac_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, vac_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &vac_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, vac_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
