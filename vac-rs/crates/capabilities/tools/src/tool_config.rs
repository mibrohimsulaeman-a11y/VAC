...
pub enum UnifiedExecShellMode {
    Direct,
    ZshBranch(ZshBranchConfig),
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub struct ZshBranchConfig {
...
impl UnifiedExecShellMode {
    pub fn for_session(
        ...
        if cfg!(unix)
            && shell_command_backend == ShellCommandBackendConfig::ZshBranch
        ...
        {
            Self::ZshBranch(ZshBranchConfig {
                shell_zsh_path,
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams<'_>) -> Self {
        ...
        // API-key auth bypasses VAC backend entitlement/tool normalization, so
        ...
        let shell_command_backend =
            if features.enabled(Feature::ShellTool) && features.enabled(Feature::ShellZshBranch) {
        ...
        } else if features.enabled(Feature::ShellZshBranch) {
            ConfigShellToolType::ShellCommand
        } else if unified_exec_enabled {
            if vac_utils_pty::conpty_supported() {
