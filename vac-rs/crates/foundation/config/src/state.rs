...
use vac_runtime_protocol::ConfigLayer;
use vac_runtime_protocol::ConfigLayerMetadata;
...
pub struct LoaderOverrides {
    ...
    //VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(gt): Add a macos_ prefix to this field and remove the target_os check.
    #[cfg(target_os = "macos")]
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("vac-config-tests");
        Self {
...
impl ConfigLayerEntry {
    ...
    // Get the `.vac/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ConfigLayerSource::Mdm { .. } => None,
            ConfigLayerSource::System { file } => file.parent(),
            ConfigLayerSource::User { file } => file.parent(),
            ConfigLayerSource::Project { dot_vac_folder } => Some(dot_vac_folder.clone()),
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_vac_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_vac_folder: current_project_dot_vac_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_vac_folder {
                ...
                if previous == current_project_dot_vac_folder
                    || !current_project_dot_vac_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_vac_folder = Some(current_project_dot_vac_folder);
        }
    }
