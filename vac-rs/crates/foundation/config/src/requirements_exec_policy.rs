...
use vac_execpolicy::RuleRef;
use vac_execpolicy::rule::PatternToken;
use vac_execpolicy::rule::PrefixPattern;
use vac_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: VAC merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `vac-execpolicy`.
