...
async fn replace_mcp_servers_serializes_per_tool_approval_overrides() -> anyhow::Result<()> {
    ...
    let vac_home = std::env::temp_dir().join(format!(
        "vac-config-mcp-edit-test-{}-{unique_suffix}",
        std::process::id()
    ));
...
"#
    ...
    let loaded = load_global_mcp_servers(&vac_home).await?;
    assert_eq!(loaded, servers);

    std::fs::remove_dir_all(&vac_home)?;
