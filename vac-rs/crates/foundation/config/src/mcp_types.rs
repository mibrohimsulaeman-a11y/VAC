...
pub struct McpServerConfig {
    ...
    /// Experimental environment selector for where VAC should start this MCP server.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub experimental_environment: Option<String>,

    /// When `false`, VAC skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `vac exec` exits with an error if this MCP server fails to initialize.
