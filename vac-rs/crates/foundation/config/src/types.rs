//! Types used to define loaded and effective VAC configuration values.
...
use std::fmt;
pub use vac_protocol::config_types::AltScreenMode;
pub use vac_protocol::config_types::ApprovalsReviewer;
use vac_protocol::config_types::EnvironmentVariablePattern;
pub use vac_protocol::config_types::ModeKind;
pub use vac_protocol::config_types::Personality;
pub use vac_protocol::config_types::ServiceTier;
use vac_protocol::config_types::ShellEnvironmentPolicy;
use vac_protocol::config_types::ShellEnvironmentPolicyInherit;
pub use vac_protocol::config_types::WebSearchMode;
...
/// Determine where VAC should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in VAC_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in VAC_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where VAC should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across VAC product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across VAC product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in VAC rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, VAC does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when VAC cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$VAC_HOME/themes` for custom themes.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use VAC's terminal-specific default. Set to `0` to
...
/// (primarily the VAC IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of VAC-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Tracks whether the user has seen the gpt-5.1-vac-max migration prompt
    #[serde(rename = "hide_gpt-5.1-vac-max_migration_prompt")]
    pub hide_gpt_5_1_vac_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, VAC skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time VAC successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision VAC last successfully activated for this marketplace.
    #[serde(default)]
...
impl From<SandboxWorkspaceWrite> for vac_runtime_protocol::SandboxSettings {
