...
use vac_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == vac_features::Feature::Artifact {
            continue;
        }
        if feature.id == vac_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<vac_features::FeatureToml<
                    vac_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == vac_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<vac_features::FeatureToml<
                    vac_features::AppsMcpPathOverrideConfigToml,
                >>(),
            );
            continue;
        }
