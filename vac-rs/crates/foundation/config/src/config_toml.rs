//! Schema-heavy configuration TOML types used by VAC.
...
/// Base config deserialized from ~/.vac/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by VAC will likely degrade model performance.
    ...
    /// file: Use a file in the VAC home directory.
    ...
    /// Defaults to `$VAC_SQLITE_HOME` when set. Otherwise uses `$VAC_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where VAC writes log files, for example `vac-tui.log`.
    /// Defaults to `$VAC_HOME/log`.
    ...
    /// Base URL override for the built-in `vastar` model provider.
    pub vastar_base_url: Option<String>,
    ...
    /// directories for `.vac` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different VAC version.
    pub allow_vac_version_mismatch: Option<bool>,
...
pub use vac_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use vac_protocol::protocol::RealtimeVoice;
...
Built-in providers cannot be overridden. Rename your custom provider (for example, `vastar-custom`).",
            conflicts.join(", ")
        ))
    }
}
