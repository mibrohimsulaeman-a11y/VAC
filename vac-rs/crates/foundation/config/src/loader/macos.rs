...
const MANAGED_PREFERENCES_APPLICATION_ID: &str = "com.vastar.vac";
...
fn load_managed_preference(key_name: &str) -> io::Result<Option<String>> {
    ...
    // SAFETY: macOS loader boundary: the CoreFoundation pointer comes from a checked API result and is copied or released on this path; extern block declares platform ABI symbols and all calls remain behind checked wrappers.
    unsafe extern "C" {
        fn CFPreferencesCopyAppValue(key: CFStringRef, application_id: CFStringRef) -> *mut c_void;
    }

    // SAFETY: macOS loader boundary: the CoreFoundation pointer comes from a checked API result and is copied or released on this path; the multi-line unsafe block below uses inputs that remain live until the block returns.
    ...
    // SAFETY: macOS loader boundary: the CoreFoundation pointer comes from a checked API result and is copied or released on this path; `CFString::wrap_under_create_rule(value_ref as _)` uses inputs that remain live for the duration of this block.
