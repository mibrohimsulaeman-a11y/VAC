...
#[cfg(unix)]
const VAC_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/vac/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/vac/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(vac_home)),
    )?;
...
pub(super) fn managed_config_default_path(vac_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = vac_home;
        PathBuf::from(VAC_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        vac_home.join("managed_config.toml")
    }
}
