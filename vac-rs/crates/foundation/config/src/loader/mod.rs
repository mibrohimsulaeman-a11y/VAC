...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/vac/config.toml";
...
/// - system    `/etc/vac/requirements.toml` (Unix) or
///   `%ProgramData%\Vastar\VAC\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/vac/config.toml` (Unix) or
///   `%ProgramData%\Vastar\VAC\config.toml` (Windows)
/// - user      `${VAC_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.vac/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.vac/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.vastar.com/vac/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, vac_home, overrides.clone()).await?;
    if !ignore_managed_requirements {
    ...
    // Add a layer for $VAC_HOME/config.toml so folder-derived resources such
    // as rules/ can still be discovered. When user config is ignored, preserve
    // the layer metadata without reading config.toml.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, vac_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so vac_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, vac_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/vac/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_vac_system_dir() -> PathBuf {
...
fn windows_program_data_dir_from_known_folder() -> io::Result<PathBuf> {
    ...
    // SAFETY: Config loader boundary: raw environment or pointer access is confined here and copied into owned Rust data; the multi-line unsafe block below uses inputs that remain live until the block returns.
    let hr = unsafe {
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_vac_folder: dot_vac_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_vac_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let vac_home_abs = AbsolutePathBuf::from_absolute_path(vac_home)?;
    let vac_home_normalized =
        normalize_path(vac_home_abs.as_path()).unwrap_or_else(|_| vac_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_vac_abs = dir.join(".vac");
        if !fs
            .get_metadata(&dot_vac_abs, /*sandbox*/ None)
            .await
        ...
        let dot_vac_normalized =
            normalize_path(dot_vac_abs.as_path()).unwrap_or_else(|_| dot_vac_abs.to_path_buf());
        if dot_vac_abs == vac_home_abs || dot_vac_normalized == vac_home_normalized {
            continue;
        }
        let config_file = dot_vac_abs.join(CONFIG_TOML_FILE);
        match fs.read_file_text(&config_file, /*sandbox*/ None).await {
            Ok(contents) => {
                ...
                let config = resolve_relative_paths_in_config_toml(config, dot_vac_abs.as_path())?;
                ...
                let entry = project_layer_entry(&dot_vac_abs, config, disabled_reason.clone());
                layers.push(entry);
            }
            Err(err) => {
...
/// from a file like `/etc/vac/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for VAC to function correctly.
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("Vastar").join("VAC").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("Vastar").join("VAC").join("config.toml"))
        );
    }
}
