...
This module is the canonical place to **load and describe VAC configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `vac_config::loader`:

- `load_config_layers_state(fs, vac_home, cwd_opt, cli_overrides, overrides, cloud_requirements, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
...
use vac_config::CloudRequirementsLoader;
