...
fn external_migration_is_experimental_and_disabled_by_default() {
    ...
    assert_eq!(
        ...
        Some(
            "Show a startup prompt when VAC detects migratable external agent config for this machine or project."
        )
    );
...
#[test]
fn vac_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::VACHooks));
    assert_eq!(feature_for_key("vac_hooks"), Some(Feature::VACHooks));
}
