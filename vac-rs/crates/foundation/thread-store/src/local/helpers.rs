...
pub(super) fn rollout_path_is_archived(vac_home: &Path, path: &Path) -> bool {
    path.starts_with(vac_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
