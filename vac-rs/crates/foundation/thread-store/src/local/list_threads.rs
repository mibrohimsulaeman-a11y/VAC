...
use vac_rollout::find_thread_names_by_ids;
use vac_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => vac_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => vac_rollout::ThreadSortKey::UpdatedAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => vac_rollout::SortDirection::Asc,
        SortDirection::Desc => vac_rollout::SortDirection::Desc,
    };
    let rollout_config = RolloutConfig {
        vac_home: store.config.vac_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.vac_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.vac_home.as_path(), &thread_ids).await
    {
...
async fn list_rollout_threads(
    ...
    params: &ListThreadsParams,
    cursor: Option<&vac_rollout::Cursor>,
    sort_key: vac_rollout::ThreadSortKey,
    sort_direction: vac_rollout::SortDirection,
) -> ThreadStoreResult<vac_rollout::ThreadsPage> {
