...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl vac_rollout::RolloutConfigView) -> Self {
        Self {
    ...
    pub async fn state_db(&self) -> Option<StateDbHandle> {
        self.state_db
            .get_or_try_init(|| async {
                vac_rollout::state_db::init_with_roots(
                    self.config.vac_home.clone(),
