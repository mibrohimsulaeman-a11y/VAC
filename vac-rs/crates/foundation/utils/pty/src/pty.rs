...
fn open_unix_pty(size: TerminalSize) -> Result<(File, File)> {
    ...
    // SAFETY: PTY process boundary: the surrounding pty setup owns the fd/handle through this child/session handoff; `(File::from_raw_fd(master), File::from_raw_fd(slave))` uses inputs that remain live for the duration of this block.
    Ok(unsafe { (File::from_raw_fd(master), File::from_raw_fd(slave)) })
}

#[cfg(unix)]
fn set_cloexec(fd: RawFd) -> std::io::Result<()> {
    // SAFETY: PTY process boundary: the surrounding pty setup owns the fd/handle through this child/session handoff; `libc::fcntl(fd, libc::F_GETFD)` uses inputs that remain live for the duration of this block.
    let flags = unsafe { libc::fcntl(fd, libc::F_GETFD) };
    if flags == -1 {
        return Err(std::io::Error::last_os_error());
    }
    // SAFETY: PTY process boundary: the surrounding pty setup owns the fd/handle through this child/session handoff; `libc::fcntl(fd, libc::F_SETFD, flags | libc::FD_CLOEXEC)` uses inputs that remain live for the duration of this block.
    ...
    if let Ok(dir) = std::fs::read_dir("/dev/fd") {
        ...
        for entry in dir {
            ...
            if let Some(num) = num {
                ...
                // SAFETY: PTY process boundary: the surrounding pty setup owns the fd/handle through this child/session handoff; `libc::fcntl(num, libc::F_GETFD)` uses inputs that remain live for the duration of this block.
