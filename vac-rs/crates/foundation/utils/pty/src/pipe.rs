...
async fn spawn_process_with_stdin_mode(
    ...
    #[cfg(target_os = "linux")]
    // SAFETY: PTY process boundary: the surrounding pty setup owns the fd/handle through this child/session handoff; `libc::getpid()` uses inputs that remain live for the duration of this block.
