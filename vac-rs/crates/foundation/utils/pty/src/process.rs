...
fn resize_raw_pty(raw_fd: RawFd, size: TerminalSize) -> anyhow::Result<()> {
    ...
    // SAFETY: PTY process boundary: the surrounding pty setup owns the fd/handle through this child/session handoff; `libc::ioctl(raw_fd, libc::TIOCSWINSZ, &mut winsize)` uses inputs that remain live for the duration of this block.
