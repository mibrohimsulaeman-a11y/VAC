...
const ASSERTION_REASON: &str = "VAC is running an active turn";
...
impl PowerRequest {
    fn new_system_required(reason: &str) -> Result<Self, String> {
        ...
        // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; `PowerCreateRequest(&context)` uses inputs that remain live for the duration of this block.
        ...
        // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; `PowerSetRequest(handle, request_type)` uses inputs that remain live for the duration of this block.
        if unsafe { PowerSetRequest(handle, request_type) } == 0 {
            ...
            // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; `CloseHandle(handle)` uses inputs that remain live for the duration of this block.
...
impl Drop for PowerRequest {
    fn drop(&mut self) {
        ...
        // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; `PowerClearRequest(self.handle, self.request_type)` uses inputs that remain live for the duration of this block.
        ...
        // in `Drop`.
        // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; `CloseHandle(self.handle)` uses inputs that remain live for the duration of this block.
