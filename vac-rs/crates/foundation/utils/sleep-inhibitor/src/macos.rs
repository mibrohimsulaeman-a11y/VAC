...
mod iokit {
    ...
    // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; extern block declares platform ABI symbols and all calls remain behind checked wrappers.
    unsafe extern "C" {}
...
impl MacSleepAssertion {
    fn create(name: &str) -> Result<Self, IOReturn> {
        ...
        // SAFETY: sleep inhibitor boundary: the OS handle comes from a checked platform API and is paired with cleanup on this path; the multi-line unsafe block below uses inputs that remain live until the block returns.
        let result = unsafe {
