//! Mapping from VAC protocol events into raw rollout-trace events.
//!
...
//! `vac-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `vac-rollout-trace`.
//!
...
use vac_protocol::protocol::PatchApplyStatus;
...
pub(crate) struct VACTurnTraceEvent {
    pub context_turn_id: VACTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn vac_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<VACTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let vac_turn_id = event.turn_id.clone();
            Some(VACTurnTraceEvent {
                context_turn_id: vac_turn_id.clone(),
                payload: RawTraceEventPayload::VACTurnStarted {
                    vac_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(VACTurnTraceEvent {
                context_turn_id: vac_turn_id.clone(),
                payload: RawTraceEventPayload::VACTurnEnded {
                    vac_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a vac_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a vac_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a vac_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a vac_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a vac_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a vac_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a vac_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a vac_protocol::protocol::CollabCloseEndEvent),
}
