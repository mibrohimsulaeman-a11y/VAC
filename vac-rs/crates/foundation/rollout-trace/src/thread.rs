...
use crate::protocol_event::vac_turn_trace_event;
...
pub const VAC_ROLLOUT_TRACE_ROOT_ENV: &str = "VAC_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `VAC_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the VAC
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(VAC_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed VAC turn lifecycle events from protocol lifecycle events.
    pub fn record_vac_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            vac_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what VAC did while executing that boundary.
    pub fn record_tool_call_event(&self, vac_turn_id: impl Into<VACTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(vac_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_vac_turn_id: impl Into<VACTurnId>,
    ...
    pub fn record_vac_turn_started(&self, vac_turn_id: impl Into<VACTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let vac_turn_id = vac_turn_id.into();
        context.append_with_context_best_effort(
            vac_turn_id.clone(),
            RawTraceEventPayload::VACTurnStarted {
                vac_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one VAC turn.
    ///
