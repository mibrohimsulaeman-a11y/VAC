//! Trace bundle format, writer, and reducer for VAC rollouts.
//!
//! This crate owns the trace schema. Hot-path VAC code should depend on the
...
/// Shared recorder context for inference attempts within one VAC turn.
...
pub use thread::VAC_ROLLOUT_TRACE_ROOT_ENV;
/// Request data for the canonical VAC tool boundary.
...
/// Append-only writer used by hot-path VAC instrumentation.
