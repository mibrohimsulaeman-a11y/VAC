...
impl TraceReducer {
    ...
    /// Starts a VAC turn inside an existing thread.
    pub(super) fn start_vac_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.vac_turns.contains_key(&vac_turn_id) {
            bail!("duplicate vac turn start for {vac_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.vac_turns.insert(
            vac_turn_id.clone(),
            VACTurn {
                vac_turn_id,
                thread_id,
    ...
    /// Marks a VAC turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_vac_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.vac_turns.get(&vac_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "vac turn end for {vac_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.vac_turns.get_mut(&vac_turn_id) else {
            bail!("vac turn end referenced unknown turn {vac_turn_id}");
        };
