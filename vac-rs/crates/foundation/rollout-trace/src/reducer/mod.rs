    ...
    fn apply_event(&mut self, event: RawTraceEvent) -> Result<()> {
        ...
        match event.payload {
            ...
            RawTraceEventPayload::VACTurnStarted {
                ...
                self.start_vac_turn(event.seq, event.wall_time_unix_ms, vac_turn_id, thread_id)?;
            }
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(vac_turn_id) = event.vac_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a vac turn id"
                    );
                };
