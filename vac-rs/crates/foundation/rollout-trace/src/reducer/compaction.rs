...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.vac_turns.get(&started.vac_turn_id) else {
            bail!(
                "compaction request {} referenced unknown vac turn {}",
                started.compaction_request_id,
                started.vac_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but vac turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.vac_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                vac_turn_id: started.vac_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.vac_turns.get(&vac_turn_id) else {
            bail!("compaction install {compaction_id} referenced unknown vac turn {vac_turn_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but vac turn {vac_turn_id} belongs to {}",
                turn.thread_id
            );
        }
