...
impl StateRuntime {
    /// Initialize the state runtime using the provided VAC home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `vac_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(vac_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        tokio::fs::create_dir_all(&vac_home).await?;
        ...
        .await;
        let state_path = state_db_path(vac_home.as_path());
    ...
    /// Return the configured VAC home directory for this runtime.
    pub fn vac_home(&self) -> &Path {
...
pub fn state_db_path(vac_home: &Path) -> PathBuf {
    vac_home.join(state_db_filename())
}
...
pub fn logs_db_path(vac_home: &Path) -> PathBuf {
    vac_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(vac_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read vac_home for {db_label} db cleanup {}: {err}",
                vac_home.display(),
            );
            return;
        }
