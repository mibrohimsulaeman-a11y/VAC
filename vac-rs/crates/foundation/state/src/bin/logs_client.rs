...
use vac_state::LogQuery;
use vac_state::LogRow;
use vac_state::StateRuntime;

#[derive(Debug, Parser)]
#[command(name = "vac-state-logs")]
#[command(about = "Tail VAC logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to VAC_HOME. Defaults to $VAC_HOME or ~/.vac.
    #[arg(long, env = "VAC_HOME")]
    vac_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --vac-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let vac_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(vac_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let vac_home = args.vac_home.clone().unwrap_or_else(default_vac_home);
    Ok(vac_state::logs_db_path(vac_home.as_path()))
}

fn default_vac_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".vac");
    }
    PathBuf::from(".vac")
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["vac-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["vac-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["vac-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["vac-state-logs", "--level", "WARN"])
