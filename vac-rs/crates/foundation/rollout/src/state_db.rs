...
pub type StateDbHandle = Arc<vac_state::StateRuntime>;
...
pub async fn init_with_roots(
    ...
    let runtime =
        match vac_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
        {
            Ok(runtime) => runtime,
            Err(err) => {
    ...
    if backfill_state.status != vac_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = vac_state::state_db_path(config.sqlite_home());
...
pub async fn open_if_present(vac_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = vac_state::state_db_path(vac_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        vac_state::StateRuntime::init(vac_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
    require_backfill_complete(runtime, vac_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == vac_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<vac_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&vac_state::StateRuntime>,
    vac_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.vac_home() != vac_home {
        warn!(
            "state db vac_home mismatch: expected {}, got {}",
            ctx.vac_home().display(),
            vac_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => vac_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => vac_state::SortKey::UpdatedAt,
            },
        ...
        .list_threads(
            ...
            vac_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => vac_state::SortDirection::Asc,
                    SortDirection::Desc => vac_state::SortDirection::Desc,
                },
                search_term,
            },
        )
