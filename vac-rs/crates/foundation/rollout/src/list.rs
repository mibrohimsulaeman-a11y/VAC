...
use crate::state_db;
use vac_file_search as file_search;
...
use vac_protocol::protocol::USER_MESSAGE_BEGIN;
...
impl From<vac_state::Anchor> for Cursor {
    fn from(anchor: vac_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = vac_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.vac/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(jif): sqlite migration phase 1
    ...
    let state_db_ctx = state_db::open_if_present(vac_home, "").await;
    ...
    let mut root = vac_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(vac_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(vac_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
