...
async fn extract_metadata_from_rollout_uses_session_meta() {
    ...
    let session_meta = SessionMeta {
        ...
        agent_nickname: None,
        agent_role: None,
        model_provider: Some("vastar".to_string()),
    ...
    let outcome = extract_metadata_from_rollout(&path, "vastar")
        .await
        .expect("extract");

    let builder = builder_from_session_meta(&session_meta_line, path.as_path()).expect("builder");
    let mut expected = builder.build("vastar");
    apply_rollout_item(&mut expected, &rollout_line.item, "vastar");
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let vac_home = dir.path().to_path_buf();
    ...
    let runtime = vac_state::StateRuntime::init(vac_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(vac_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), vac_home.as_path(), "test-provider").await;
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = vac_home.join("sessions");
