...
use vac_state::BackfillState;
use vac_state::BackfillStats;
use vac_state::BackfillStatus;
use vac_state::DB_ERROR_METRIC;
use vac_state::DB_METRIC_BACKFILL;
use vac_state::DB_METRIC_BACKFILL_DURATION_MS;
use vac_state::ExtractionOutcome;
use vac_state::ThreadMetadataBuilder;
use vac_state::apply_rollout_item;
...
pub(crate) async fn backfill_sessions(
    runtime: &vac_state::StateRuntime,
    vac_home: &Path,
    default_provider: &str,
) {
    let metric_client = vac_otel::global();
    ...
    let sessions_root = vac_home.join(SESSIONS_SUBDIR);
    let archived_root = vac_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(vac_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(vac_home: &Path, path: &Path) -> String {
    path.strip_prefix(vac_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
