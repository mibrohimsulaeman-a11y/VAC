...
async fn insert_state_db_thread(
    ...
    let runtime = vac_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn find_thread_path_repairs_missing_db_row_after_filesystem_fallback() {
    ...
    let _runtime = vac_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
        .expect("state db should initialize");
    _runtime
        .mark_backfill_complete(/*last_watermark*/ None)
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let vastar_id = Uuid::from_u128(1);
    ...
    let vastar_id_str = vastar_id.to_string();
    let none_id_str = none_id.to_string();
    let vastar_filter = provider_vec(&["vastar"]);
    let vastar_sessions = get_threads(
        home,
        /*page_size*/ 10,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        NO_SOURCE_FILTER,
        Some(vastar_filter.as_slice()),
        /*cwd_filters*/ None,
        "vastar",
    )
    .await?;
    assert_eq!(vastar_sessions.items.len(), 2);
    let vastar_ids: Vec<_> = vastar_sessions
        .items
        .iter()
        .filter_map(|item| item.thread_id.as_ref().map(ToString::to_string))
        .collect();
    assert!(vastar_ids.contains(&vastar_id_str));
    assert!(vastar_ids.contains(&none_id_str));
