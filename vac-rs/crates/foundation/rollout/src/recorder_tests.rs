...
fn test_config(vac_home: &Path) -> RolloutConfig {
    RolloutConfig {
...
async fn list_threads_db_enabled_drops_missing_rollout_paths() -> std::io::Result<()> {
    ...
    let runtime =
        vac_state::StateRuntime::init(home.path().to_path_buf(), config.model_provider_id.clone())
            .await
            .expect("state db should initialize");
    runtime
        .mark_backfill_complete(/*last_watermark*/ None)
...
async fn list_threads_default_filter_returns_filesystem_scan_results() -> std::io::Result<()> {
    ...
    let created_at = chrono::Utc
        ...
        .expect("valid datetime");
    let mut builder =
        vac_state::ThreadMetadataBuilder::new(thread_id, real_path, created_at, SessionSource::Cli);
    builder.model_provider = Some(config.model_provider_id.clone());
