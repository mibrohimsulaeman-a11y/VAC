//! Persist VAC session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use vac_protocol::protocol::GitInfo as ProtocolGitInfo;
...
use vac_utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.vac/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.vac/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
pub enum RolloutRecorderParams {
    Create {
        ...
        branched_from_id: Option<ThreadId>,
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided VAC home directory.
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(vac_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = vac_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(vac_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(vac_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.vac/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.vac_home().to_path_buf();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(vac_state::rollout_item_affects_thread_metadata)
    {
...
impl From<vac_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: vac_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
...
fn thread_item_from_state_metadata(item: vac_state::ThreadMetadata) -> ThreadItem {
    ThreadItem {
