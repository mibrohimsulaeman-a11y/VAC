...
static REDACTED_SECRET_MARKER_RE: LazyLock<Result<Regex, String>> = LazyLock::new(|| {
    Regex::new(r"\[REDACTED_SECRET:[^:\]]+:[^:\]]+\]")
        .map_err(|error| format!("invalid redacted-secret marker regex: {error}"))
});

fn find_protected_spans(content: &str) -> Vec<(usize, usize)> {
    match REDACTED_SECRET_MARKER_RE.as_ref() {
        Ok(regex) => regex
            .find_iter(content)
...
pub fn redact_secrets(
    ...
    sorted_by_start.sort_by_key(|item| item.start_pos);
    ...
    deduplicated_secrets.sort_by_key(|item| std::cmp::Reverse(item.start_pos));
...
pub fn restore_secrets(redacted_string: &str, redaction_map: &HashMap<String, String>) -> String {
    ...
    let Ok(marker_re) = REDACTED_SECRET_MARKER_RE.as_ref() else {
        return redacted_string.to_string();
    };

    for marker_match in marker_re.find_iter(redacted_string) {
