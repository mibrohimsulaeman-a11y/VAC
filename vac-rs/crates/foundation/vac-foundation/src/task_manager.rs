...
fn terminate_process_group(process_id: u32) {
    #[cfg(unix)]
    {
        ...
        // Only terminate if the process actually exists.
        if check_result
            .map(|output| output.status.success())
            .unwrap_or(false)
        {
            // Use a negative-PID process-group signal only when that group is
            // observable. In constrained test/container environments the child
            // may fail to become a group leader; in that case fall back to the
            // individual PID rather than signaling an unrelated group.
...
#[derive(Debug, Clone)]
struct StructuredTaskCommand {
    runner: String,
    args: Vec<String>,
}

fn parse_structured_task_command(command: &str) -> Result<StructuredTaskCommand, TaskCompletion> {
    let error = |detail: String| TaskCompletion {
        output: format!(
            "VAC_STRUCTURED_COMMAND_REQUIRED: background tasks require a VAC-bound runner+args command; free-form shell is blocked: {detail}"
        ),
        error: Some("VAC structured command gate blocked background task".to_string()),
        final_status: TaskStatus::Failed,
    };
    ...
    if command.contains('\n') || command.contains('\r') {
        return Err(error("multi-line command".to_string()));
    }
    ...
    for raw in command.split_whitespace() {
        ...
        {
            return Err(error(format!("shell token rejected: {raw}")));
        }
        parts.push(raw.to_string());
    }
    let Some(runner) = parts.first().cloned() else {
        return Err(error("missing runner".to_string()));
    };
    if runner.contains('/')
        || runner.contains('\\')
        || matches!(
    ...
    {
        return Err(error(
            "runner must be registry executable name, not a shell/path".to_string(),
        ));
    }
    Ok(StructuredTaskCommand {
        runner,
...
impl TaskManager {
        ...
        match msg {
            ...
            TaskMessage::GetAllTasks { response_tx } => {
                let mut tasks: Vec<TaskInfo> = self
                    ...
                    .collect();
                tasks.sort_by_key(|task| std::cmp::Reverse(task.start_time));
    ...
    async fn execute_local_task(
        ...
        let structured = match parse_structured_task_command(&command) {
            Ok(command) => command,
...
mod tests {
    ...
    async fn test_task_manager_start_and_complete_task() {
        ...
        // Start a simple task
        let task_info = handle
            .start_task("echo Hello".to_string(), StartTaskOptions::default())
            .await
            .expect("Failed to start task");
    ...
    async fn task_manager_local_task_receives_child_env_defaults() {
        ...
        child_env.insert("VAC_PROFILE".to_string(), "ops".to_string());

        let task_info = handle
            .start_task(
                "printenv VAC_PROFILE".to_string(),
                StartTaskOptions {
                    child_env,
    ...
    async fn resumed_local_task_reuses_child_env_defaults() {
        ...
        let pause_runner = temp_dir.path().join("vac_test_pause");
        std::fs::write(&pause_runner, "#!/bin/sh\nexit 10\n").expect("write pause runner");
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = std::fs::metadata(&pause_runner)
                .expect("pause runner metadata")
                .permissions();
            perms.set_mode(0o755);
            std::fs::set_permissions(&pause_runner, perms).expect("chmod pause runner");
        }
        let path = format!(
            "{}:{}",
        ...
        let mut child_env = HashMap::new();
        child_env.insert("VAC_PROFILE".to_string(), "ops".to_string());
        child_env.insert("PATH".to_string(), path);

        let task_info = handle
            .start_task(
                "vac_test_pause".to_string(),
                StartTaskOptions {
                    child_env,
                    ..StartTaskOptions::default()
                },
        ...
        handle
            .resume_task(task_info.id.clone(), "printenv VAC_PROFILE".to_string())
            .await
    ...
    async fn test_task_manager_handle_drop_kills_child_processes() {
        ...
        // Start a task that writes a marker file while running. The task manager
        // only accepts structured runner+args commands, so use a PATH-scoped
        // helper instead of a shell pipeline.
        let marker = format!("/tmp/vac_test_drop_{}", std::process::id());
        let temp_dir = tempfile::TempDir::new().expect("temp dir");
        let marker_runner = temp_dir.path().join("vac_test_marker_sleep");
        std::fs::write(
            &marker_runner,
            format!("#!/bin/sh\ntouch {}\nexec sleep 30\n", marker),
        )
        .expect("write marker runner");
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = std::fs::metadata(&marker_runner)
                .expect("marker runner metadata")
                .permissions();
            perms.set_mode(0o755);
            std::fs::set_permissions(&marker_runner, perms).expect("chmod marker runner");
        }
        let mut child_env = HashMap::new();
        child_env.insert(
            "PATH".to_string(),
        ...
        let task_info = handle
            .start_task(
                "vac_test_marker_sleep".to_string(),
                StartTaskOptions {
                    child_env,
                    ..StartTaskOptions::default()
                },
    ...
    async fn test_task_manager_detects_immediate_exit_code_failure() {
        ...
        let result = handle
            .start_task("false".to_string(), StartTaskOptions::default())
            .await;
