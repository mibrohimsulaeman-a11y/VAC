...
impl SSHClient {
    ...
    pub fn new(hostname: impl Into<String>, port: u16) -> Self {
        Self {
            ...
            port,
            known_hosts: KnownHostsVerifier::load_default(),
        }
    }
}
...
impl KnownHostsVerifier {
    ...
    pub fn load_default() -> Self {
        let allow_unknown_l1_degraded = std::env::var("VAC_REMOTE_ALLOW_UNKNOWN_HOSTS_L1_DEGRADED")
            .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        let mut entries = Vec::new();
        for path in known_hosts_candidates() {
            if let Ok(text) = fs::read_to_string(path) {
                entries.extend(parse_known_hosts_entries(&text));
            }
        }
        Self {
            entries,
            allow_unknown_l1_degraded,
        }
    }
...
#[must_use]
pub fn known_hosts_candidates() -> Vec<PathBuf> {
    let mut paths = Vec::new();
    if let Ok(path) = std::env::var("VAC_KNOWN_HOSTS") {
        paths.push(PathBuf::from(path));
    }
    if let Some(home) = dirs::home_dir() {
        paths.push(home.join(".ssh/known_hosts"));
        paths.push(home.join(".ssh/known_hosts2"));
    }
    paths
}

#[must_use]
pub fn parse_known_hosts_entries(text: &str) -> Vec<KnownHostEntry> {
    text.lines()
        .filter_map(|line| {
...
impl RemoteConnection {
    ...
    async fn create_authenticated_session_static(
        ...
        let mut session = client::connect(
            ...
            SSHClient::new(parsed.hostname.clone(), parsed.port),
        )
        .await
