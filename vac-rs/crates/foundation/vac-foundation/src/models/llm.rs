...
pub enum ProviderConfig {
    ...
    /// VAC provider configuration
    ///
    /// Routes inference through VAC's unified API, which provides:
    ...
    /// # Example TOML
    /// ```toml
    /// [profiles.myprofile.providers.vac]
    /// type = "vac"
    /// api_endpoint = "https://api.vastar.ai/vac"  # optional, this is the default
    ///
    /// [profiles.myprofile.providers.vac.auth]
    /// type = "api"
    /// key = "your-vac-remote-service-key"
    ///
    /// # Then use models as:
    /// model = "vac/anthropic/claude-sonnet-4-5-20250929"
    /// ```
    VAC {
        /// Legacy API key field (prefer `auth` field)
...
impl ProviderConfig {
    ...
    pub fn api_key(&self) -> Option<&str> {
        ...
        match self {
            ...
            ProviderConfig::VAC { api_key, .. } => api_key.as_deref(),
    ...
    fn get_auth_ref(&self) -> Option<&ProviderAuth> {
        match self {
            ...
            ProviderConfig::VAC { auth, .. } => auth.as_ref(),
    ...
    pub fn api_endpoint(&self) -> Option<&str> {
        match self {
            ...
            ProviderConfig::VAC { api_endpoint, .. } => api_endpoint.as_deref(),
    ...
    /// Create a VAC provider config (legacy, uses api_key field)
    pub fn vac(api_key: String, api_endpoint: Option<String>) -> Self {
        ProviderConfig::VAC {
    ...
    /// Create a VAC provider config with auth
    pub fn vac_with_auth(auth: ProviderAuth, api_endpoint: Option<String>) -> Self {
        ProviderConfig::VAC {
            api_key: None,
            api_endpoint,
            auth: Some(auth),
