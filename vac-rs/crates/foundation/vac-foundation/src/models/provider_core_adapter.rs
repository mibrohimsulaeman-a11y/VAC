//! Adapter layer between CLI LLM types and VAC Provider Core SDK
//!
//! This module provides conversion functions and a wrapper client to use the VAC Provider Core SDK
...
use vac_provider_core::{
    ...
    providers::anthropic::AnthropicConfig as VacAnthropicConfig,
    providers::openai::OpenAIConfig as VacOpenAIConfig, providers::openrouter::OpenRouterConfig,
    registry::ProviderRegistry,
};

/// Convert CLI LLMMessage to VAC Provider Core Message
pub fn to_provider_core_message(msg: &LLMMessage) -> Message {
    ...
    let content: MessageContent = match &msg.content {
        ...
        LLMMessageContent::List(parts) => {
            let content_parts: Vec<ContentPart> =
                parts.iter().map(to_provider_core_content_part).collect();
            content_parts.into()
        }
    };
...
/// Convert CLI LLMMessageTypedContent to VAC Provider Core ContentPart
fn to_provider_core_content_part(part: &LLMMessageTypedContent) -> ContentPart {
    match part {
...
/// Convert VAC Provider Core Message to CLI LLMMessage
pub fn from_provider_core_message(msg: &Message) -> LLMMessage {
    ...
    let content = match &msg.content {
        ...
        MessageContent::Parts(parts) => {
            LLMMessageContent::List(parts.iter().map(from_provider_core_content_part).collect())
        }
    };
...
/// Convert VAC Provider Core ContentPart to CLI LLMMessageTypedContent
fn from_provider_core_content_part(part: &ContentPart) -> LLMMessageTypedContent {
    match part {
...
/// Convert CLI LLMTool to VAC Provider Core Tool
pub fn to_provider_core_tool(tool: &LLMTool) -> Tool {
    Tool {
...
/// Convert VAC Provider Core Tool to CLI LLMTool
pub fn from_provider_core_tool(tool: &Tool) -> LLMTool {
    LLMTool {
...
/// Convert VAC Provider Core StreamEvent to CLI GenerationDelta
pub fn from_provider_core_stream_event(event: &StreamEvent) -> Option<GenerationDelta> {
    match event {
        ...
        StreamEvent::Finish { usage, .. } => {
            let llm_usage = from_provider_core_usage(usage);
...
/// Convert VAC Provider Core Usage to CLI LLMTokenUsage
pub fn from_provider_core_usage(usage: &Usage) -> LLMTokenUsage {
    use crate::models::llm::PromptTokensDetails;

    // Convert VAC Provider Core input_token_details to CLI PromptTokensDetails
...
/// Convert VAC Provider Core FinishReason to string
pub fn finish_reason_to_string(reason: &FinishReason) -> String {
    ...
    if let Some(raw) = &reason.raw {
        ...
        match reason.unified {
            vac_provider_core::FinishReasonKind::Stop => "stop".to_string(),
            vac_provider_core::FinishReasonKind::Length => "length".to_string(),
            vac_provider_core::FinishReasonKind::ContentFilter => "content_filter".to_string(),
            vac_provider_core::FinishReasonKind::ToolCalls => "tool_calls".to_string(),
            vac_provider_core::FinishReasonKind::Error => "error".to_string(),
            vac_provider_core::FinishReasonKind::Other => "other".to_string(),
        }
    }
}

/// Convert CLI LLMProviderOptions to VAC Provider Core ProviderOptions
pub fn to_provider_core_provider_options(
...
/// Convert VAC Provider Core GenerateResponse to CLI LLMCompletionResponse
pub fn from_provider_core_response(
    ...
    for content in &response.content {
        match content {
            vac_provider_core::ResponseContent::Text { text } => {
                content_parts.push(LLMMessageTypedContent::Text { text: text.clone() });
            }
            vac_provider_core::ResponseContent::Reasoning { reasoning } => {
            ...
            vac_provider_core::ResponseContent::ToolCall(tool_call) => {
    ...
    LLMCompletionResponse {
        ...
        usage: Some(from_provider_core_usage(&response.usage)),
    }
}

fn resolve_provider_core_openai_config(
    provider_config: &ProviderConfig,
) -> Result<Option<VacOpenAIConfig>, String> {
    ...
    Ok(resolved.map(|config| config.to_provider_core_config()))
}

/// Build VAC Provider Core InferenceConfig from CLI LLMProviderConfig
    ...
    for (name, provider_config) in &config.providers {
        match provider_config {
            ProviderConfig::OpenAI { .. } => {
                if let Some(openai_config) = resolve_provider_core_openai_config(provider_config)? {
            ...
            ProviderConfig::Anthropic { api_endpoint, .. } => {
                ...
                let provider_core_config = if let Some(token) = provider_config.access_token() {
                    // OAuth authentication - uses Bearer token header
                    let mut cfg = VacAnthropicConfig::with_oauth(token);
                    ...
                    let mut cfg = VacAnthropicConfig::new(key);
                    if let Some(endpoint) = api_endpoint {
                        cfg = cfg.with_base_url(endpoint);
                    }
                    Some(cfg)
                ...
                if let Some(cfg) = provider_core_config {
            ...
            ProviderConfig::VAC { api_endpoint, .. } => {
                // Skip if no api_key - vac is optional
                if let Some(api_key) = provider_config.api_key() {
                    inference_config =
                        inference_config.vac(api_key.to_string(), api_endpoint.clone());
                }
            }
            ProviderConfig::OpenRouter { api_endpoint, .. } => {
                if let Some(api_key) = provider_config.api_key() {
                    let mut openrouter_config = OpenRouterConfig::new(api_key.to_string());
                    openrouter_config = openrouter_config.with_http_referer("https://vastar.ai/");
                    openrouter_config = openrouter_config.with_site_title("VAC");
                    if let Some(endpoint) = api_endpoint {
            ...
            ProviderConfig::Bedrock {
                ...
                #[cfg(feature = "bedrock")]
                {
                    use vac_provider_core::providers::bedrock::BedrockConfig;
...
fn build_provider_registry_direct(config: &LLMProviderConfig) -> Result<ProviderRegistry, String> {
    use vac_provider_core::providers::anthropic::{
        AnthropicConfig as VacAnthropicConfig, AnthropicProvider,
    };
    use vac_provider_core::providers::copilot::{CopilotConfig, CopilotProvider};
    use vac_provider_core::providers::gemini::{GeminiConfig as VacGeminiConfig, GeminiProvider};
    use vac_provider_core::providers::openai::{OpenAIConfig as VacOpenAIConfig, OpenAIProvider};
    use vac_provider_core::providers::openrouter::{OpenRouterConfig, OpenRouterProvider};
    use vac_provider_core::providers::vac::{VacProvider, VacProviderConfig};
    ...
    for (name, provider_config) in &config.providers {
        match provider_config {
            ...
            ProviderConfig::Gemini { api_endpoint, .. } => {
                if let Some(api_key) = provider_config.api_key() {
                    let mut gemini_config = VacGeminiConfig::new(api_key.to_string());
                    if let Some(endpoint) = api_endpoint {
            ...
            ProviderConfig::VAC { api_endpoint, .. } => {
                ...
                let mut vac_config = VacProviderConfig::new(api_key.to_string())
                    .with_user_agent(format!("VAC/{}", env!("CARGO_PKG_VERSION")));
                if let Some(endpoint) = api_endpoint {
                    vac_config = vac_config.with_base_url(endpoint.clone());
                }
                let provider = VacProvider::new(vac_config)
                    .map_err(|e| format!("Failed to create VAC provider: {}", e))?;
                registry = registry.register("vac", provider);
            }
            ProviderConfig::OpenRouter { api_endpoint, .. } => {
                if let Some(api_key) = provider_config.api_key() {
                    let mut openrouter_config = OpenRouterConfig::new(api_key.to_string());
                    ...
                    let provider = OpenRouterProvider::new(openrouter_config).map_err(
                        |e: vac_provider_core::prelude::Error| {
            ...
            ProviderConfig::Custom { api_endpoint, .. } => {
                ...
                let openai_config = VacOpenAIConfig::new(key).with_base_url(api_endpoint.clone());
            ...
            ProviderConfig::Bedrock {
                ...
                #[cfg(feature = "bedrock")]
                {
                    use vac_provider_core::providers::bedrock::{BedrockConfig, BedrockProvider};
                    let mut bedrock_config = BedrockConfig::new(region.clone());
                    if let Some(profile) = profile_name {
                        bedrock_config = bedrock_config.with_profile_name(profile.clone());
                    }
...
/// Get model string for VAC Provider Core
pub fn get_provider_core_model_string(model: &Model) -> String {
    model.id.clone()
}

/// Wrapper around VAC Provider Core Inference for CLI usage
#[derive(Clone)]
pub struct VacProviderClient {
    inference: Inference,
}

impl VacProviderClient {
    /// Create a new VAC Provider Core client from CLI provider config
    ...
    /// Create a new VAC Provider Core client with custom provider registry
    ...
    pub async fn chat(&self, input: LLMInput) -> Result<LLMCompletionResponse, AgentError> {
        ...
        if let Some(tools) = &input.tools {
            for tool in tools {
                options = options.add_tool(to_provider_core_tool(tool));
            }
        }

        // Add custom headers if present
        if let Some(headers) = &input.headers {
            let mut provider_core_headers = Headers::new();
            for (key, value) in headers {
                provider_core_headers.insert(key, value);
            }
            options = options.headers(provider_core_headers);
        }
        ...
        let provider_options = input
            .provider_options
            .as_ref()
            .and_then(|opts| to_provider_core_provider_options(opts, &input.model));
        ...
        Ok(from_provider_core_response(response, &input.model.id))
    }
        ...
        while let Some(event_result) = stream.next().await {
            match event_result {
                Ok(event) => {
                    ...
                    if let Some(delta) = from_provider_core_stream_event(&event) {
                    ...
                    if let StreamEvent::Finish { reason, usage } = event {
                        ...
                        final_usage = from_provider_core_usage(&usage);
                    }
                }
                Err(e) => {
...
mod tests {
    ...
    fn test_role_conversion_user() {
        ...
        let provider_core_msg = to_provider_core_message(&msg);
        assert!(matches!(provider_core_msg.role, Role::User));

        let back = from_provider_core_message(&provider_core_msg);
    ...
    fn test_role_conversion_assistant() {
        ...
        let provider_core_msg = to_provider_core_message(&msg);
        assert!(matches!(provider_core_msg.role, Role::Assistant));
    ...
    fn test_role_conversion_system() {
        ...
        let provider_core_msg = to_provider_core_message(&msg);
        assert!(matches!(provider_core_msg.role, Role::System));
    ...
    fn test_role_conversion_tool() {
        ...
        let provider_core_msg = to_provider_core_message(&msg);
        assert!(matches!(provider_core_msg.role, Role::Tool));
    ...
    fn test_string_content_conversion() {
        ...
        let provider_core_msg = to_provider_core_message(&msg);
        assert_eq!(
            provider_core_msg.text(),
            Some("Simple text message".to_string())
        );
    ...
    fn test_list_content_with_text() {
        ...
        let provider_core_msg = to_provider_core_message(&msg);
        let parts = provider_core_msg.parts();
    ...
    fn test_tool_conversion_basic() {
        ...
        let provider_core_tool = to_provider_core_tool(&tool);
        assert_eq!(provider_core_tool.tool_type, "function");
        assert_eq!(provider_core_tool.function.name, "get_weather");
        assert_eq!(
            provider_core_tool.function.description,
            "Get weather for a location"
        );
        assert_eq!(provider_core_tool.function.parameters["type"], "object");

        let back = from_provider_core_tool(&provider_core_tool);
    ...
    fn test_stream_event_text_delta() {
        ...
        let delta = from_provider_core_stream_event(&event);
    ...
    fn test_finish_reason_conversion() {
        ...
        use vac_provider_core::FinishReasonKind;
    ...
    fn test_model_string_anthropic() {
        ...
        let model_str = get_provider_core_model_string(&model);
    ...
    fn test_response_conversion_text_only() {
        let response = GenerateResponse {
            content: vec![vac_provider_core::ResponseContent::Text {
        ...
        let llm_response = from_provider_core_response(response, "gpt-4");
    ...
    fn test_response_conversion_with_tool_calls() {
        let response = GenerateResponse {
            content: vec![
                vac_provider_core::ResponseContent::Text {
                    text: "I'll check the weather for you.".to_string(),
                },
                vac_provider_core::ResponseContent::ToolCall(vac_provider_core::ToolCall {
        ...
        let llm_response = from_provider_core_response(response, "claude-3");
    ...
    fn test_provider_options_anthropic_thinking() {
        ...
        let model = Model::custom("claude-sonnet-4-5-20250929", "anthropic");
        let result = to_provider_core_provider_options(&opts, &model);
    ...
    fn test_message_roundtrip_simple() {
        ...
        let provider_core_msg = to_provider_core_message(&original);
        let back = from_provider_core_message(&provider_core_msg);
    ...
    fn test_tool_roundtrip() {
        ...
        let provider_core_tool = to_provider_core_tool(&original);
        let back = from_provider_core_tool(&provider_core_tool);
