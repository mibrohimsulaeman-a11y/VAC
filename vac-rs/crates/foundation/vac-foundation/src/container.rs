...
///   mount entry from [`vac_agent_default_mounts`].
...
/// caller then relies on the default mount in [`vac_agent_default_mounts`].
...
/// Pre-create any Docker named volumes found in [`vac_agent_default_mounts`].
///
...
pub fn ensure_named_volumes_exist() {
    for vol in vac_agent_default_mounts() {
...
mod tests {
    ...
    fn knowledge_store_mount_present_and_rw() {
        ...
        assert!(
            entry.starts_with("~/.vac/knowledge:"),
            "host side should be ~/.vac/knowledge: {entry}"
        );
        assert!(
            !entry.ends_with(":ro"),
