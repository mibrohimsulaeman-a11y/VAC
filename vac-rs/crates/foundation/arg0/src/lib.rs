...
#[cfg(all(unix, feature = "arg0-helpers"))]
use std::os::unix::fs::symlink;
use tempfile::TempDir;
#[cfg(feature = "arg0-helpers")]
use vac_apply_patch::VAC_CORE_APPLY_PATCH_ARG1;
#[cfg(feature = "arg0-helpers")]
use vac_exec_server::VAC_FS_HELPER_ARG1;
#[cfg(all(target_os = "linux", feature = "arg0-helpers"))]
use vac_sandboxing::landlock::VAC_LINUX_SANDBOX_ARG0;
use vac_utils_home_dir::find_vac_home;
...
#[cfg(all(unix, feature = "arg0-helpers"))]
const EXECVE_WRAPPER_ARG0: &str = "vac-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current VAC executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    let _exe_name = Path::new(&argv0)
        .file_name()
        .and_then(|s| s.to_str())
        .unwrap_or("");

    #[cfg(all(unix, feature = "arg0-helpers"))]
    if _exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            vac_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    #[cfg(all(target_os = "linux", feature = "arg0-helpers"))]
    if _exe_name == VAC_LINUX_SANDBOX_ARG0 {
    ...
    #[cfg(feature = "arg0-helpers")]
    {
        ...
        if argv1 == VAC_FS_HELPER_ARG1 {
            vac_exec_server::run_fs_helper_main();
        }
        if argv1 == VAC_CORE_APPLY_PATCH_ARG1 {
            let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
            let exit_code = match patch_arg {
                Some(patch_arg) => {
                    let mut stdout = std::io::stdout();
                    let mut stderr = std::io::stderr();
                    let cwd = match vac_utils_absolute_path::AbsolutePathBuf::current_dir() {
                        Ok(cwd) => cwd,
                        Err(_) => std::process::exit(1),
                    };
                    let runtime = match tokio::runtime::Builder::new_current_thread()
                    ...
                    match runtime.block_on(vac_apply_patch::apply_patch(
                        &patch_arg,
                        &cwd,
                        &mut stdout,
                        &mut stderr,
                        vac_exec_server::LOCAL_FS.as_ref(),
                        /*sandbox*/ None,
                    )) {
                        Ok(()) => 0,
                        Err(_) => 1,
    ...
    match prepend_path_entry_for_vac_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that VAC will proceed successfully even if
...
/// While we want to deploy the VAC CLI as a single executable for simplicity,
...
/// `vac-linux-sandbox` we *directly* execute
/// [`vac_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.vac/.env` before creating any threads.
/// 2.  Construct a Tokio multi-thread runtime.
/// 3.  Capture the current executable path and derive the
///     `vac-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        vac_self_exe: current_exe.clone(),
        vac_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `vac-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().vac_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "VAC_";

/// Load env vars from ~/.vac/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `VAC_`.
fn load_dotenv() {
    if let Ok(vac_home) = find_vac_home()
        && let Ok(iter) = dotenvy::from_path_iter(vac_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `VAC_` keys.
    ...
    I: IntoIterator<Item = Result<(String, String), dotenvy::Error>>,
{
    for (key, value) in iter.into_iter().flatten() {
        if !key.to_ascii_uppercase().starts_with(ILLEGAL_ENV_VAR_PREFIX) {
            ...
            // SAFETY: process argv boundary: this argv reconstruction path owns the C-string pointer during the read or replacement; `std::env::set_var(&key, &value)` uses inputs that remain live for the duration of this block.
...
///   with the hidden `--vac-run-as-apply-patch` flag.
///
...
pub fn prepend_path_entry_for_vac_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let vac_home = find_vac_home()?;
    #[cfg(not(debug_assertions))]
    {
        // Guard against placing helpers in system temp directories outside debug builds.
        let temp_root = std::env::temp_dir();
        if vac_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (vac_home: {vac_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a VAC_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = vac_home.join("tmp").join("arg0");
...
"#,
    ...
    // SAFETY: process argv boundary: this argv reconstruction path owns the C-string pointer during the read or replacement; the multi-line unsafe block below uses inputs that remain live until the block returns.
    unsafe {
        std::env::set_var("PATH", updated_path_env_var);
    }

    let paths = Arg0DispatchPaths {
        vac_self_exe: std::env::current_exe().ok(),
        vac_linux_sandbox_exe: {
            #[cfg(all(target_os = "linux", feature = "arg0-helpers"))]
            {
                Some(path.join(VAC_LINUX_SANDBOX_ARG0))
            }
            #[cfg(not(all(target_os = "linux", feature = "arg0-helpers")))]
            {
                None
            }
        },
        main_execve_wrapper_exe: {
            ...
            #[cfg(not(all(unix, feature = "arg0-helpers")))]
            {
                None
            }
        },
...
mod tests {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_vac_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("vac-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                vac_self_exe: Some(PathBuf::from("/usr/bin/vac")),
                vac_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/vac"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("vac-helper-alias");
