...
mod platform {
    ...
    impl AsSocket for WindowsUnixListener {
        fn as_socket(&self) -> BorrowedSocket<'_> {
            // SAFETY: UDS boundary: sockaddr/fd pointers reference buffers owned by this function for the immediate socket call; `BorrowedSocket::borrow_raw(self.as_raw_socket())` uses inputs that remain live for the duration of this block.
    ...
    // SAFETY: UDS boundary: sockaddr/fd pointers reference buffers owned by this function for the immediate socket call; unsafe impl is constrained to the invariants documented on this wrapper type.
