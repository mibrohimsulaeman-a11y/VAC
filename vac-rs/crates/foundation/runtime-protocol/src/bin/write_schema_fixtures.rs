...
#[derive(Parser, Debug)]
#[command(about = "Regenerate vendored runtime protocol schema fixtures")]
struct Args {
...
fn main() -> Result<()> {
    ...
    vac_runtime_protocol::write_schema_fixtures_with_options(
        &schema_root,
        args.prettier.as_deref(),
        vac_runtime_protocol::SchemaFixtureOptions {
            experimental_api: args.experimental,
        },
    )
    .with_context(|| {
        format!(
            "failed to regenerate runtime protocol schema fixtures under {}",
            schema_root.display()
        )
    })
}
