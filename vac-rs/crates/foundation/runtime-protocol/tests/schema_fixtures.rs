...
use vac_runtime_protocol::generate_json_with_experimental;
use vac_runtime_protocol::generate_typescript_schema_fixture_subtree_for_tests;
use vac_runtime_protocol::read_schema_fixture_subtree;
...
fn schema_root() -> Result<PathBuf> {
    ...
    let typescript_index = vac_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle =
        vac_utils_cargo_bin::find_resource!("schema/json/vac_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/vac_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
