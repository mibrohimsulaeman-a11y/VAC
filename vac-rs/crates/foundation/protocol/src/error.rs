...
use crate::protocol::VACErrorInfo;
use chrono::DateTime;
use chrono::Datelike;
use chrono::Local;
use chrono::Utc;
...
use vac_async_utils::CancelErr;
use vac_utils_string::truncate_middle_chars;
use vac_utils_string::truncate_middle_with_token_budget;

pub type Result<T> = std::result::Result<T, VACErr>;
...
pub enum VACErr {
    ...
    #[error(
        "VAC ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `VACError::Spawn` variant.
    ...
    #[error("vac-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for VACErr {
...
impl VACErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | VACErr::EnvVar(_)
            | VACErr::Fatal(_)
            | VACErr::UsageNotIncluded
            | VACErr::QuotaExceeded
            | VACErr::InvalidImageRequest()
            | VACErr::InvalidRequest(_)
            | VACErr::RefreshTokenFailed(_)
            | VACErr::UnsupportedOperation(_)
            | VACErr::Sandbox(_)
            | VACErr::LandlockSandboxExecutableNotProvided
            | VACErr::RetryLimit(_)
            | VACErr::ContextWindowExceeded
            | VACErr::ThreadNotFound(_)
            | VACErr::AgentLimitReached { .. }
            | VACErr::Spawn
            | VACErr::SessionConfiguredNotFirstEvent
            | VACErr::UsageLimitReached(_)
            | VACErr::ServerOverloaded
            | VACErr::CyberPolicy { .. } => false,
            VACErr::Stream(..)
            | VACErr::Timeout
            | VACErr::UnexpectedStatus(_)
            | VACErr::ResponseStreamFailed(_)
            | VACErr::ConnectionFailed(_)
            | VACErr::InternalServerError
            | VACErr::InternalAgentDied
            | VACErr::Io(_)
            | VACErr::Json(_)
            | VACErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            VACErr::LandlockRuleset(_) | VACErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<VACErr>()` checks continue to compile
    ...
    pub fn to_vac_protocol_error(&self) -> VACErrorInfo {
        match self {
            VACErr::ContextWindowExceeded => VACErrorInfo::ContextWindowExceeded,
            VACErr::UsageLimitReached(_) | VACErr::QuotaExceeded | VACErr::UsageNotIncluded => {
                VACErrorInfo::UsageLimitExceeded
            }
            VACErr::ServerOverloaded => VACErrorInfo::ServerOverloaded,
            VACErr::CyberPolicy { .. } => VACErrorInfo::CyberPolicy,
            VACErr::RetryLimit(_) => VACErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            VACErr::ConnectionFailed(_) => VACErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            VACErr::ResponseStreamFailed(_) => VACErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            VACErr::RefreshTokenFailed(_) => VACErrorInfo::Unauthorized,
            VACErr::SessionConfiguredNotFirstEvent
            | VACErr::InternalServerError
            | VACErr::InternalAgentDied => VACErrorInfo::InternalServerError,
            VACErr::UnsupportedOperation(_)
            | VACErr::ThreadNotFound(_)
            | VACErr::AgentLimitReached { .. } => VACErrorInfo::BadRequest,
            VACErr::Sandbox(_) => VACErrorInfo::SandboxError,
            _ => VACErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            vac_error_info: Some(self.to_vac_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            VACErr::RetryLimit(err) => Some(err.status),
            VACErr::UnexpectedStatus(err) => Some(err.status),
            VACErr::ConnectionFailed(err) => err.source.status(),
            VACErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("vac")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit for the configured provider. Check your provider quota or switch models{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit for the configured provider. Switch models or configure another provider{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit for the configured provider. Check your provider quota{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &VACErr) -> String {
    let message = match e {
        ...
        VACErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
