...
fn enable_hooks_and_rmcp_server(
    ...
    if let Err(err) = config.features.enable(Feature::VACHooks) {
...
async fn pre_tool_use_blocks_mcp_tool_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_hook_inputs(test.vac_home_path(), "pre_tool_use_hook_log.jsonl")?;
...
async fn post_tool_use_records_mcp_tool_payload_and_context() -> Result<()> {
    ...
    let hook_inputs = read_hook_inputs(test.vac_home_path(), "post_tool_use_hook_log.jsonl")?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(
        json!({
            "hook_event_name": hook_inputs[0]["hook_event_name"],
