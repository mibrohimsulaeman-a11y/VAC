...
use core_test_support::test_vac::test_env;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/vac-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/vac-remote-dotdot-{}", std::process::id()));
...
async fn remote_test_env_copy_preserves_symlink_source() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/vac-remote-copy-link-{}", std::process::id()));
    let allowed_dir = root.join("allowed");
...
fn remote_test_file_path() -> PathBuf {
    ...
    PathBuf::from(format!(
        "/tmp/vac-remote-test-env-{}-{nanos}.txt",
        std::process::id()
    ))
}
