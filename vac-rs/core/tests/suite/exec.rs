...
use vac_core::spawn::VAC_SANDBOX_ENV_VAR;
...
fn skip_test() -> bool {
    if std::env::var(VAC_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{VAC_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
