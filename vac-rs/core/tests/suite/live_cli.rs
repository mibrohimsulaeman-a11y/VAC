...
//! Optional smoke tests that hit the real Vastar /v1/responses endpoint. They are `#[ignore]` by
//! default so CI stays deterministic and free. Developers can run them locally with
//! `cargo test --test live_cli -- --ignored` provided they set a valid `VASTAR_API_KEY`.
...
fn require_api_key() -> String {
    std::env::var("VASTAR_API_KEY")
        .expect("VASTAR_API_KEY env var not set — skip running live tests")
}
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let mut cmd = Command::new(vac_utils_cargo_bin::cargo_bin("vac-rs").unwrap());
    cmd.current_dir(dir.path());
    cmd.env("VASTAR_API_KEY", require_api_key());
    ...
    let mut child = cmd.spawn().expect("failed to spawn vac-rs");
...
fn live_create_file_hello_txt() {
    if std::env::var("VASTAR_API_KEY").is_err() {
        eprintln!("skipping live_create_file_hello_txt – VASTAR_API_KEY not set");
        return;
    }
...
fn live_print_working_directory() {
    if std::env::var("VASTAR_API_KEY").is_err() {
        eprintln!("skipping live_print_working_directory – VASTAR_API_KEY not set");
        return;
    }
