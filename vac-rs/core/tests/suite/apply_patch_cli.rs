...
use core_test_support::test_vac::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestVACHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestVACBuilder) -> TestVACBuilder,
) -> Result<TestVACHarness> {
    let builder = configure(test_vac()).with_config(|config| {
    ...
    Box::pin(TestVACHarness::with_remote_aware_builder(builder)).await
}

async fn submit_without_wait(harness: &TestVACHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_vac_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let vac_linux_sandbox_exe = harness
        .test()
        .config
        .vac_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include vac-linux-sandbox helper");
    assert_eq!(
        vac_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(VAC_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&vac_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
