...
fn disabled_user_turn(test: &TestVAC, items: Vec<UserInput>, model: String) -> Op {
...
async fn create_workspace_directory(test: &TestVAC, rel_path: &str) -> anyhow::Result<PathBuf> {
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, vac_protocol::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        vac_protocol::items::TurnItem::ImageView(item) => {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_can_preserve_original_resolution_when_requested_on_gpt5_3_vac()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn view_image_tool_resizes_when_model_lacks_original_detail_support() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_vac().with_model("gpt-5.2");
    let test = builder.build_remote_aware(&server).await?;
    let TestVAC {
        vac,
        session_configured,
