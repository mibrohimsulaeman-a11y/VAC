...
//! Integration tests that cover compacting, resuming, and branching conversations.
//!
...
//! request payload that VAC would send to the model and assert that the
...
use vac_core::spawn::VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
fn network_disabled() -> bool {
    std::env::var(VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
/// Scenario: compact an initial conversation, resume it, branch one turn back, and
/// ensure the model-visible history matches expectations at each request.
async fn compact_resume_and_branch_preserve_model_history_view() {
    if network_disabled() {
        println!("Skipping test because network is disabled in this sandbox");
        return;
    }

    // 1. Arrange mocked SSE responses for the initial compact/resume/branch flow.
    let server = MockServer::start().await;
    let request_log = mount_initial_flow(&server).await;
    let expected_model = "gpt-5.4";
    // 2. Start a new conversation and drive it through the compact/resume/branch steps.
    ...
    let branched = branch_thread(&manager, &config, resumed_path, /*nth_user_message*/ 2).await;
    user_turn(&branched, "AFTER_BRANCH").await;

    // 3. Capture the requests to the model and validate the history slices.
    let mut requests = gather_request_bodies(&request_log);
    normalize_compact_prompts(&mut requests);

    // input after compact is a prefix of input after resume/branch
    let input_after_compact = json!(requests[requests.len() - 3]["input"]);
    let input_after_resume = json!(requests[requests.len() - 2]["input"]);
    let input_after_branch = json!(requests[requests.len() - 1]["input"]);
    ...
    let branch_arr = input_after_branch
        .as_array()
        .expect("input after branch should be an array");
    ...
    assert!(
        compact_arr.len() <= branch_arr.len(),
        "after-branch input should have at least as many items as after-compact",
    );
    assert_eq!(
        &compact_arr.as_slice()[..compact_arr.len()],
        &branch_arr[..compact_arr.len()]
    );
    ...
    let summary_after_branch = extract_summary_user_text(&requests[4], SUMMARY_TEXT);
    ...
    let after_branch_user_texts = json_message_input_texts(&requests[4], "user");
    let mut expected_after_branch_history_prefix =
        vec!["hello world".to_string(), summary_after_branch];
    expected_after_branch_history_prefix.extend_from_slice(seeded_user_prefix);
    expected_after_branch_history_prefix.push("AFTER_COMPACT".to_string());
    let (after_branch_last, after_branch_prefix) = after_branch_user_texts
        .split_last()
        .unwrap_or_else(|| panic!("after-branch request missing user messages"));
    assert_eq!(after_branch_last, "AFTER_BRANCH");
    assert!(
        after_branch_prefix.starts_with(&expected_after_branch_history_prefix),
        "after-branch user texts should preserve compacted user history prefix"
    );
    let after_branch_seeded_suffix =
        &after_branch_prefix[expected_after_branch_history_prefix.len()..];
    if seeded_user_prefix.is_empty() {
        assert!(
            after_branch_seeded_suffix.is_empty(),
            "after-branch request should not append unexpected user prefix items"
        );
    } else {
        let mut chunks = after_branch_seeded_suffix.chunks_exact(seeded_user_prefix.len());
        assert!(
            chunks.remainder().is_empty(),
            "after-branch suffix should be whole seeded-prefix repeats"
        );
        for chunk in &mut chunks {
            assert_eq!(chunk, seeded_user_prefix);
        }
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
/// Scenario: after the branched branch is compacted, resuming again should reuse
...
async fn compact_resume_after_second_compaction_preserves_history() -> Result<()> {
    ...
    // 2. Drive the conversation through compact -> resume -> branch -> compact -> resume.
    ...
    let branched = branch_thread(&manager, &config, resumed_path, /*nth_user_message*/ 3).await;
    user_turn(&branched, "AFTER_BRANCH").await;

    compact_conversation(&branched).await;
    user_turn(&branched, "AFTER_COMPACT_2").await;
    let branched_path = fetch_conversation_path(&branched);
    assert!(
        branched_path.exists(),
        "second compact test expects branched path {branched_path:?} to exist",
    );

    shutdown_conversation(&branched).await;
    let resumed_again = resume_conversation(&manager, &config, branched_path).await;
    ...
    let mut expected_branch_local_user_texts =
        vec!["AFTER_BRANCH".to_string(), summary_after_second_compact];
    expected_branch_local_user_texts.extend_from_slice(seeded_user_prefix);
    expected_branch_local_user_texts.push("AFTER_COMPACT_2".to_string());
        ...
        .position(|window| window == expected_after_second_compact_user_texts)
    {
        ...
        .windows(expected_branch_local_user_texts.len())
        .position(|window| window == expected_branch_local_user_texts)
    {
        start + expected_branch_local_user_texts.len()
    } else {
...
async fn mount_initial_flow(server: &MockServer) -> Vec<ResponseMock> {
    ...
    let match_first = |req: &wiremock::Request| {
        ...
        body.contains("\"text\":\"hello world\"")
            ...
            && !body.contains("\"text\":\"AFTER_BRANCH\"")
    };
    ...
    let match_after_branch = |req: &wiremock::Request| {
        let body = std::str::from_utf8(&req.body).unwrap_or("");
        body.contains("\"text\":\"AFTER_BRANCH\"")
    };
    let after_branch = mount_sse_once_match(server, match_after_branch, sse5).await;

    vec![first, compact, after_compact, after_resume, after_branch]
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<VACThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.vac)
}

async fn user_turn(conversation: &Arc<VACThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<VACThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<VACThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<VACThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager =
        vac_core::test_support::auth_manager_from_auth(vac_login::VACAuth::from_api_key("dummy"));
