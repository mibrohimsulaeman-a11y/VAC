...
use uuid::Uuid;
use vac_core::REVIEW_PROMPT;
use vac_core::VACThread;
use vac_core::config::Config;
use vac_core::review_format::render_review_output_text;
use vac_protocol::models::ContentItem;
use vac_protocol::models::ResponseItem;
use vac_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use vac_protocol::protocol::EventMsg;
use vac_protocol::protocol::ExitedReviewModeEvent;
use vac_protocol::protocol::Op;
use vac_protocol::protocol::ReviewCodeLocation;
use vac_protocol::protocol::ReviewFinding;
use vac_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under VAC sandbox network restrictions.
    skip_if_no_network!();
    ...
    let vac_home = Arc::new(TempDir::new().unwrap());
    let vac = new_conversation_for_server(&server, vac_home.clone(), |_| {}).await;

    // Submit review request.
    vac.submit(Op::Review {
    ...
    let _entered = wait_for_event(&vac, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&vac, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&vac, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = vac.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&vac, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let vac = new_conversation_for_server(&server, vac_home.clone(), |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&vac, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&vac, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = vac_home.path().join("resume.jsonl");
    {
        ...
        let user = vac_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
            content: vec![vac_protocol::models::ContentItem::InputText {
        ...
        let assistant = vac_protocol::models::ResponseItem::Message {
            id: None,
            role: "assistant".to_string(),
            content: vec![vac_protocol::models::ContentItem::OutputText {
    ...
    let vac =
        resume_conversation_for_server(&server, vac_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let vac = new_conversation_for_server(&server, vac_home.clone(), move |config| {
        config.cwd = initial_cwd_path.abs();
    })
    .await;

    vac.submit(Op::OverrideTurnContext {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    let mut builder = test_vac().with_home(vac_home).with_config(move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    let mut builder = test_vac()
        .with_home(vac_home.clone())
        .with_config(move |config| {
            config.model_provider.base_url = Some(base_url.clone());
            mutator(config);
        });
    builder
        .resume(server, vac_home, resume_path)
        .await
