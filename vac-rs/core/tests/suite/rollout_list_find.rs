...
use uuid::Uuid;
use vac_core::EventPersistenceMode;
use vac_core::RolloutRecorder;
use vac_core::RolloutRecorderParams;
use vac_core::config::ConfigBuilder;
use vac_core::find_archived_thread_path_by_id_str;
use vac_core::find_thread_meta_by_name_str;
use vac_core::find_thread_path_by_id_str;
...
fn write_minimal_rollout_with_id_in_subdir(vac_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = vac_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(vac_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(vac_home, "sessions", id)
}

async fn upsert_thread_metadata(vac_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(vac_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = vac_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_vac_home_directory() {
    let repo = TempDir::new().unwrap();
    let vac_home = repo.path().join(".vac");
    std::fs::create_dir_all(&vac_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".vac/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&vac_home, id);

    let found = find_thread_path_by_id_str(&vac_home, &id.to_string())
        .await
        .unwrap();
...
async fn find_locates_rollout_file_written_by_recorder() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::default()
        .vac_home(home.path().to_path_buf())
        .build()
        .await?;
