...
use vac_core::ARCHIVED_SESSIONS_SUBDIR;
use vac_core::SESSIONS_SUBDIR;
use vac_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use vac_core::personality_migration::PersonalityMigrationStatus;
use vac_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(vac_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(vac_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(vac_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(vac_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = vac_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(vac_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = vac_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
