...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    wait_for_event(&vac, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    vac.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&vac, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_vac()
        .with_model("gpt-5.4")
        .build(&server)
        .await
        .unwrap();
    let vac = Arc::clone(&fixture.vac);
