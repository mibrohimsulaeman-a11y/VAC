...
async fn tool_call_output_configured_limit_chars_type() -> Result<()> {
    ...
    let mut builder = test_vac().with_model("gpt-5.2").with_config(|config| {
...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_vac().with_config(move |config| {
        ...
        servers.insert(
            ...
            vac_config::types::McpServerConfig {
                transport: vac_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    // 1x1 PNG data URL
    let vastar_png = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMB/ee9bQAAAABJRU5ErkJggg==";
    ...
    wait_for_event(&fixture.vac, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    assert_eq!(
        arr[1],
        json!({"type": "input_image", "image_url": vastar_png, "detail": "high"})
    );
