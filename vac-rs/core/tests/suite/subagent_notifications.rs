...
use vac_core::ThreadConfigSnapshot;
use vac_core::config::AgentRoleConfig;
...
const TURN_0_BRANCH_PROMPT: &str = "seed branch context";
const TURN_1_PROMPT: &str = "spawn a child and continue";
const TURN_2_NO_WAIT_PROMPT: &str = "follow up without wait";
const CHILD_PROMPT: &str = "child: do work";
const INHERITED_MODEL: &str = "gpt-5.3-vac";
...
async fn wait_for_spawned_thread_id(test: &TestVAC) -> Result<String> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_vac::TestVACBuilder,
    ) -> core_test_support::test_vac::TestVACBuilder,
) -> Result<(TestVAC, String)> {
    ...
    let mut builder = configure_test(test_vac().with_config(|config| {
        config
            .features
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn spawned_child_receives_branched_parent_context() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let seed_turn = mount_sse_once_match(
        &server,
        |req: &wiremock::Request| body_contains(req, TURN_0_BRANCH_PROMPT),
        sse(vec![
    ...
    test.submit_turn(TURN_0_BRANCH_PROMPT).await?;
    ...
    let child_request = loop {
        ...
        if Instant::now() >= deadline {
            anyhow::bail!("timed out waiting for branched child request");
        }
        sleep(Duration::from_millis(10)).await;
    };
    assert!(body_contains(&child_request, TURN_0_BRANCH_PROMPT));
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.vac_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
