...
use vac_core::context::ContextualUserFragment;
use vac_core::context::PermissionsInstructions;
use vac_core::load_exec_policy;
...
async fn resume_replays_permissions_messages() -> Result<()> {
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.vac, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.vac, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn resume_and_branch_append_permissions_messages() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut branch_config = initial.config.clone();
    branch_config.permissions.approval_policy =
        Constrained::allow_any(AskForApproval::UnlessTrusted);
    let branched = initial
        .thread_manager
        .branch_thread(
    ...
    branched
        ...
        .await?;
    wait_for_event(&branched.thread, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;

    let permissions_branch = permissions_texts(&req4.single_request());
    assert_eq!(permissions_branch.len(), permissions_base.len() + 1);
    assert_eq!(
        &permissions_branch[..permissions_base.len()],
        permissions_base.as_slice()
    );
    let new_permissions = &permissions_branch[permissions_base.len()..];
    assert_eq!(new_permissions.len(), 1);
    assert_eq!(permissions_branch, permissions_resume);
