...
async fn wait_for_completion(test: &TestVAC) {
...
async fn expect_exec_approval(test: &TestVAC, expected_command: &str) -> ExecApprovalRequestEvent {
    let event = wait_for_event(&test.vac, |event| {
        matches!(
            event,
...
async fn wait_for_exec_approval_or_completion(test: &TestVAC) -> Option<ExecApprovalRequestEvent> {
    let event = wait_for_event(&test.vac, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn request_permissions_session_grants_carry_across_turns() -> Result<()> {
    ...
    let completion_event = wait_for_event(&test.vac, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
