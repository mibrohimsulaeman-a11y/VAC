...
use vac_protocol::vastar_models::ModelVisibility;
...
const REMOTE_MODEL_SLUG: &str = "vac-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 VAC".to_string(),
    ...
    let config = load_default_config_for_test(&vac_home).await;

    let auth = VACAuth::create_dummy_chatgpt_auth_for_testing();
    let provider = ModelProviderInfo {
        base_url: Some(format!("{}/v1", server.uri())),
        ..built_in_model_providers(/* vastar_base_url */ /*vastar_base_url*/ None)["vastar"].clone()
    };
    let manager = vac_core::test_support::models_manager_with_provider(
        vac_home.path().to_path_buf(),
        vac_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-vac-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-vac-test");
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let turn_started_event = wait_for_event(&vac, |event| {
        matches!(
            event,
...
async fn remote_models_long_model_slug_is_sent_with_high_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-vac-test";
    let prefix_model = "gpt-5.3-vac";
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "custom/gpt-5.2-vac";
    let response_mock = mount_sse_once(
        &server,
        sse(vec![ev_response_created("resp-1"), ev_completed("resp-1")]),
    )
    ...
    loop {
        let event = wait_for_event(&vac, |_| true).await;
        match event {
...
async fn remote_models_remote_model_uses_unified_exec() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&vac, |msg| match msg {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "vac-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "vac-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "vac-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    vac_core::test_support::all_model_presets()
        .iter()
