...
use vac_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use vac_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use vac_core::ModelClient;
use vac_core::ModelClientSession;
use vac_core::Prompt;
use vac_core::ResponseEvent;
use vac_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
use vac_otel::MetricsClient;
use vac_otel::MetricsConfig;
...
const MODEL: &str = "gpt-5.3-vac";
const VASTAR_BETA_HEADER: &str = "Vastar-Beta";
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(VASTAR_BETA_HEADER),
    ...
    assert_eq!(
        ...
        Some(vac_login::default_client::get_vac_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-vac-installation-id"].as_str(),
...
async fn responses_websocket_preconnect_is_reused_even_with_header_changes() {
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            /*turn_metadata_header*/ None,
            &vac_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn responses_websocket_prewarm_uses_v2_when_provider_supports_websockets() {
    ...
    let handshake = server.single_handshake();
    let vastar_beta_header = handshake
        .header(VASTAR_BETA_HEADER)
        .expect("missing Vastar-Beta header");
    assert!(
        vastar_beta_header
            .split(',')
            .map(str::trim)
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-vac-primary-used-percent": "100.0",
            "x-vac-secondary-used-percent": "87.5",
            "x-vac-primary-over-secondary-limit-percent": "95.0",
            "x-vac-primary-window-minutes": "15",
            "x-vac-secondary-window-minutes": "60"
        }
    });
    ...
    let token_event = wait_for_event(&test.vac, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.vac, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let enriched_turn_metadata = r#"{"turn_id":"turn-123","thread_source":"user","sandbox":"workspace-write","workspaces":[{"root_path":"/tmp/repo","latest_git_commit_hash":"abc123","associated_remote_urls":["git@github.com:vastar/vac.git"],"has_changes":true}]}"#;
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![
        message_item("hello"),
        assistant_message_item("msg-1", "assistant output"),
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-vac-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-vac-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn responses_websocket_preserves_custom_turn_metadata_fields() {
    ...
    assert_eq!(body["type"].as_str(), Some("response.create"));
    assert_eq!(
        body["client_metadata"]["x-vac-turn-metadata"]
            .as_str()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_websocket_v2_sets_vastar_beta_header() {
    skip_if_no_network!();
...
async fn websocket_harness_with_provider_options(
    ...
    let mut config = load_default_config_for_test(&vac_home).await;
    ...
    let model_info = vac_core::test_support::construct_model_info_offline(MODEL, &config);
    let conversation_id = ThreadId::new();
    let auth_manager =
        vac_core::test_support::auth_manager_from_auth(VACAuth::from_api_key("Test API Key"));
    let exporter = InMemoryMetricExporter::default();
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "vac-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
