...
use core_test_support::test_vac::turn_permission_fields;
...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    vac.submit(Op::RunUserShellCommand { command: list_cmd })
        .await
        .unwrap();
    let msg = wait_for_event(&vac, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
    ...
    vac.submit(Op::RunUserShellCommand { command: cat_cmd })
        .await
        .unwrap();
    let msg = wait_for_event(&vac, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
    let EventMsg::ExecCommandEnd(ExecCommandEndEvent {
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    vac.submit(Op::RunUserShellCommand { command: sleep_cmd })
        .await
        .unwrap();

    // Wait until it has started (ExecCommandBegin), then interrupt.
    let _begin = wait_for_event_match(vac, |ev| match ev {
    ...
    .await;
    vac.submit(Op::Interrupt).await.unwrap();
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    let server = start_mock_server().await;
    let mut builder = test_vac().with_model("gpt-5.4");
    ...
    let _ = wait_for_event_match(&fixture.vac, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.vac.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_vac::test_vac().with_config(move |config| {
        config
            .features
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:VAC_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${VAC_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.vac, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.vac, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.vac, |ev| match ev {
    ...
    let _ = wait_for_event(&test.vac, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_vac::test_vac().with_config(|config| {
        let file_system_sandbox_policy = config.permissions.file_system_sandbox_policy();
        config.permissions.permission_profile =
            vac_config::Constrained::allow_any(PermissionProfile::from_runtime_permissions(
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:VAC_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${VAC_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();

    test.vac.submit(Op::RunUserShellCommand { command }).await?;
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.vac, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(not(target_os = "windows"))] // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening): unignore on windows
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_vac::test_vac();
    let test = builder
        .with_config(|config| {
...
async fn user_shell_command_is_truncated_only_once() -> anyhow::Result<()> {
    ...
    let mut builder = test_vac().with_model("gpt-5.4").with_config(|config| {
        config.tool_output_token_limit = Some(100);
    });
    let fixture = builder.build(&server).await?;
