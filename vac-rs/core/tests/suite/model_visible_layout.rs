...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
// VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(ccunningham): Diff `user_instructions` and emit updates when AGENTS.md content changes
...
async fn snapshot_model_visible_layout_resume_with_personality_change() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut initial_builder = test_vac().with_config(|config| {
        config.model = Some("gpt-5.2".to_string());
    });
    let initial = initial_builder.build(&server).await?;
    let vac = Arc::clone(&initial.vac);
    ...
    let mut resume_builder = test_vac().with_config(|config| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.vac, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
