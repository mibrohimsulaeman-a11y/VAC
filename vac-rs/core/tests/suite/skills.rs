...
fn write_home_skill(vac_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = vac_home.join("skills").join(dir);
...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_vac().with_workspace_setup(move |cwd, fs| async move {
    ...
    core_test_support::wait_for_event(test.vac.as_ref(), |event| {
        matches!(event, vac_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn list_skills_includes_repo_and_home_skills_remote_aware() -> Result<()> {
    ...
    test.vac
        ...
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.vac.as_ref(), |event| match event {
            vac_protocol::protocol::EventMsg::ListSkillsResponse(response) => {
                Some(response.clone())
            }
            _ => None,
        })
    ...
    assert_eq!(repo_skill.scope, vac_protocol::protocol::SkillScope::Repo);
    ...
    assert_eq!(home_skill.scope, vac_protocol::protocol::SkillScope::User);
...
async fn list_skills_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let thread_manager = ThreadManager::new(
        &config,
        vac_core::test_support::auth_manager_from_auth(VACAuth::from_api_key("dummy")),
        SessionSource::Exec,
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_vac().with_pre_build_hook(|home| {
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.vac_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, vac_protocol::protocol::SkillScope::System);
