...
async fn request_body_is_zstd_compressed_for_vac_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/provider-api/vac/v1", server.uri());
    ...
    let vac = builder.build(&server).await?.vac;
