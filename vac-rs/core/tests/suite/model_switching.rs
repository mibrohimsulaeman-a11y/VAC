...
use vac_config::types::Personality;
...
fn read_only_user_turn(test: &TestVAC, items: Vec<UserInput>, model: String) -> Op {
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_vac().with_model("gpt-5.3-vac");
    ...
    test.vac
        ...
        .await?;
    wait_for_event(&test.vac, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let mut builder = test_vac().with_model("gpt-5.3-vac").with_config(|config| {
        config
            .features
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-vac-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_vac().build(&server).await?;
...
async fn model_change_from_image_to_text_strips_prior_image_content() -> Result<()> {
    ...
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &vac_protocol::models::image_open_tag_text()),
        "second request should preserve the image open tag text"
    );
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &vac_protocol::models::image_close_tag_text()),
...
async fn thread_rollback_after_generated_image_drops_entire_image_turn_history() -> Result<()> {
    ...
    test.vac.submit(Op::ThreadRollback { num_turns: 1 }).await?;
    wait_for_event(&test.vac, |ev| matches!(ev, EventMsg::ThreadRolledBack(_))).await;
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.vac, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.vac, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.vac, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
