...
async fn build_vac(server: &StreamingSseServer) -> Arc<VACThread> {
    test_vac()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming VAC test session: {err}"))
        .vac
}

async fn submit_user_input(vac: &VACThread, text: &str) {
...
async fn submit_danger_full_access_user_turn(test: &TestVAC, text: &str) {
...
async fn steer_user_input(vac: &VACThread, text: &str) {
    vac.steer_input(
        vec![UserInput::Text {
            text: text.to_string(),
            text_elements: Vec::new(),
...
async fn submit_queue_only_agent_mail(vac: &VACThread, text: &str) {
    vac.submit(Op::InterAgentCommunication {
    ...
    vac.submit(Op::ListMcpTools)
        .await
        .unwrap_or_else(|err| panic!("submit list-mcp-tools barrier: {err}"));
    wait_for_event(vac, |event| {
        matches!(event, EventMsg::McpListToolsResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(vac: &VACThread) {
    wait_for_event(vac, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(vac: &VACThread, text: &str) {
...
async fn wait_for_turn_complete(vac: &VACThread) {
...
#[ignore = "VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(aibrahim): flaky"]
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let vac = build_vac(&server).await;

    submit_user_input(&vac, "first prompt").await;

    wait_for_reasoning_item_started(&vac).await;

    submit_queue_only_agent_mail(&vac, "queued child update").await;

    let _ = gate_reasoning_done_tx.send(());

    wait_for_turn_complete(&vac).await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&vac, "first answer").await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&vac, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    let vac = test_vac()
        .with_model("gpt-5.4")
        .with_config(|config| {
            config.model_provider.name = "Vastar (test)".to_string();
    ...
    submit_user_input(&vac, "first prompt").await;
    submit_user_input(&vac, "second prompt").await;

    wait_for_agent_message(&vac, "resumed old task").await;
    wait_for_turn_complete(&vac).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&vac, "processed steered prompt").await;
    wait_for_turn_complete(&vac).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&vac, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&vac, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
