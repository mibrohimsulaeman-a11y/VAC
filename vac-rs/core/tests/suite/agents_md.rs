...
async fn agents_instructions(mut builder: TestVACBuilder) -> Result<String> {
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions = agents_instructions(test_vac().with_workspace_setup(|cwd, fs| async move {
