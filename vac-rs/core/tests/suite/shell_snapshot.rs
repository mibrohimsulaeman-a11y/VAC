...
use core_test_support::test_vac::TestVACHarness;
...
const POLICY_PATH_FOR_TEST: &str = "/vac/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/vac/snapshot/path";
const SNAPSHOT_MARKER_VAR: &str = "VAC_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(vac_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = vac_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_vac().with_config(move |config| {
    ...
    let harness = TestVACHarness::with_builder(builder).await?;
    let args = json!({
        "cmd": command,
        "yield_time_ms": 1000,
    });
    ...
    let vac_home = test.home.path().to_path_buf();
    let session_model = test.session_configured.model.clone();
    let cwd = test.cwd_path().to_path_buf();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, cwd.as_path());

    vac.submit(Op::UserTurn {
        environments: None,
    ...
    let begin = wait_for_event_match(&vac, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    let snapshot_path = wait_for_snapshot(&vac_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&vac, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&vac, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.vac_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    let builder = test_vac().with_config(|config| {
        config
            .features
            .enable(Feature::ShellSnapshot)
            .expect("test config should allow feature update");
    ...
    let harness = TestVACHarness::with_builder(builder).await?;
    let vac_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(vac_home));
...
async fn shell_command_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    wait_for_event(&vac, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let vac_home = home.path().to_path_buf();
    let vac = harness.test().vac.clone();

    let snapshot_path = wait_for_snapshot(&vac_home).await?;
    assert!(snapshot_path.exists());

    vac.submit(Op::Shutdown {}).await?;
    wait_for_event(&vac, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
#[ignore = "windows shell snapshot smoke; run in ignored-test lane on Windows"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
