...
use vac_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "vac-test-ttl";
const VERSIONED_MODEL: &str = "vac-test-versioned";
const MISSING_VERSION_MODEL: &str = "vac-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "vac-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let mut builder = test_vac().with_auth(VACAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let cache_path = config.vac_home.join(CACHE_FILE);
    ...
    vac.submit(Op::UserTurn {
        ...
        approval_policy: vac_protocol::protocol::AskForApproval::Never,
    ...
    let _ = wait_for_event(&vac, |event| matches!(event, EventMsg::TurnComplete(_))).await;
