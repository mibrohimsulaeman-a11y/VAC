...
use core_test_support::zsh_branch::build_zsh_branch_test;
use core_test_support::zsh_branch::restrictive_workspace_write_profile;
use core_test_support::zsh_branch::zsh_branch_runtime;
use std::fs;
use std::path::Path;
...
fn write_skill_metadata(home: &Path, name: &str, contents: &str) -> Result<()> {
    ...
    fs::write(metadata_dir.join("vastar.yaml"), contents)?;
    Ok(())
}
...
fn skill_script_command(test: &TestVAC, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestVAC) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.vac.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestVAC) {
    wait_for_event(test.vac.as_ref(), |event| {
...
async fn shell_zsh_branch_skill_scripts_ignore_declared_permissions() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let Some(runtime) = zsh_branch_runtime("zsh-branch skill script ignores permissions test")?
    else {
        return Ok(());
    };
    ...
    let test = build_zsh_branch_test(
        &server,
        runtime,
        approval_policy,
    ...
    let call_id = "zsh-branch-skill-script-ignores-permissions";
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_zsh_branch_still_enforces_workspace_write_sandbox() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let Some(runtime) = zsh_branch_runtime("zsh-branch workspace sandbox test")? else {
        return Ok(());
    };

    let server = start_mock_server().await;
    let tool_call_id = "zsh-branch-workspace-write-deny";
    let outside_path = "/tmp/vac-zsh-branch-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();
    ...
    submit_turn_with_policies(
        &test,
        "write outside workspace with zsh branch",
        AskForApproval::Never,
        workspace_write_profile,
    )
    .await?;
