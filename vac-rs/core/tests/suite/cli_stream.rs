...
use vac_utils_cargo_bin::find_resource;
use wiremock::MockServer;

fn repo_root() -> std::path::PathBuf {
    #[expect(clippy::expect_used)]
    vac_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = vac_utils_cargo_bin::cargo_bin("vac").unwrap();
    ...
    // VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening)(jif) fix
...
/// Ensures `vastar_base_url` config override routes built-in vastar provider requests.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_mode_stream_cli_supports_vastar_base_url_config_override() {
    ...
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("vastar_base_url=\"{}/v1\"", server.uri()))
        .arg("-C")
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("VAC_HOME", home.path())
...
/// received by a mock Vastar Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_cli_applies_model_instructions_file() {
    ...
    // VAC to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `vac exec --profile ...` preserves the active profile when it
...
/// 2. Configures vac to read from this fixture via VAC_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg("vastar_base_url=\"http://unused.local\"")
        .arg("-C")
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("VAC_HOME", home.path())
        .env("VASTAR_API_KEY", "dummy")
        .env("VAC_RS_SSE_FIXTURE", fixture);
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the vac CLI and invoke `exec`, which is what records a session.
    let bin = vac_utils_cargo_bin::cargo_bin("vac").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = vac_utils_cargo_bin::cargo_bin("vac").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("VAC_HOME", home.path())
        .env("VASTAR_API_KEY", "dummy")
        .env("VAC_RS_SSE_FIXTURE", &fixture);

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume vac-cli run failed");
