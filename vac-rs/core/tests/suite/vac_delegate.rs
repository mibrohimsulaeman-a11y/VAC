...
#[ignore = "VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening) once we have a delegate that can ask for approvals"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn vac_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    let test = builder.build(&server).await.expect("build test vac");
    ...
    wait_for_event(&test.vac, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;

    // Expect parent-side approval request (forwarded by delegate).
    let approval_event = wait_for_event(&test.vac, |ev| {
    ...
    wait_for_event(&test.vac, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
...
#[ignore = "VAC-DEBT(owner=vac-o5o6-zero-residual,target=post-cargo-hardening) once we have a delegate that can ask for approvals"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn vac_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn vac_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.vac, |_| true).await;
        match ev {
