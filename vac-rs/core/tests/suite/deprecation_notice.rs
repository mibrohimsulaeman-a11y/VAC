...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestVAC { vac, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&vac, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://developers.vastar.com/vac/config-basic#feature-flags for details."
        ),
    );
...
async fn emits_deprecation_notice_for_notify() -> anyhow::Result<()> {
    ...
    let mut builder = test_vac().with_config(|config| {
        config.notify = Some(vec!["notify-send".to_string(), "VAC".to_string()]);
    });
