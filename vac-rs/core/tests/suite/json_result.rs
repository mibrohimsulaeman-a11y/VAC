...
async fn vac_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    vac_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn vac_returns_json_result_for_gpt5_vac() -> anyhow::Result<()> {
    vac_returns_json_result("gpt-5.4".to_string()).await
}

async fn vac_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("vac_output_schema".into()))
    ...
    let TestVAC { vac, cwd, .. } = test_vac().build(&server).await?;
    ...
    let message = wait_for_event(&vac, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
