...
async fn run_turn(test: &TestVAC, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestVAC, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_vac_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestVAC> {
    let mut builder = test_vac().with_model("test-gpt-5.1-vac");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_vac_with_test_tool(&server).await?;
