...
async fn responses_api_parent_and_subagent_requests_include_identity_headers() -> Result<()> {
    ...
    let parent_mock = mount_sse_once_match(
        ...
        |req: &wiremock::Request| {
            request_body_contains(req, PARENT_PROMPT)
                && request_header(req, "x-vastar-subagent").is_none()
        },
        sse(vec![
    ...
    let child_mock = mount_sse_once_match(
        ...
        |req: &wiremock::Request| {
            request_body_contains(req, CHILD_PROMPT)
                ...
                && request_header(req, "x-vastar-subagent") == Some("collab_spawn")
        },
        sse(vec![
    ...
    let parent = wait_for_matching_request(&parent_mock, "parent request", |request| {
        request.body_contains_text(PARENT_PROMPT) && request.header("x-vastar-subagent").is_none()
    })
    .await?;
    let child = wait_for_matching_request(&child_mock, "child request", |request| {
        request.body_contains_text(CHILD_PROMPT)
            ...
            && request.header("x-vastar-subagent").as_deref() == Some("collab_spawn")
    })
    .await?;

    let parent_window_id = parent
        .header("x-vac-window-id")
        .ok_or_else(|| anyhow!("parent request missing x-vac-window-id"))?;
    let child_window_id = child
        .header("x-vac-window-id")
        .ok_or_else(|| anyhow!("child request missing x-vac-window-id"))?;
    ...
    assert_eq!(parent.header("x-vastar-subagent"), None);
    assert_eq!(
        child.header("x-vastar-subagent").as_deref(),
        Some("collab_spawn")
    );
    assert_eq!(
        child.header("x-vac-parent-thread-id").as_deref(),
...
async fn submit_turn_with_timeout(test: &TestVAC, prompt: &str) -> Result<()> {
...
async fn wait_for_event_result<F>(test: &TestVAC, stage: &str, mut predicate: F) -> Result<EventMsg>
where
    F: FnMut(&EventMsg) -> bool,
{
    ...
    tokio::time::timeout(TURN_TIMEOUT, async {
        loop {
            let event = test.vac.next_event().await?;
