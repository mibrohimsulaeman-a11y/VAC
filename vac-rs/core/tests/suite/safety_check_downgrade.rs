...
const REQUESTED_MODEL: &str = "gpt-5.3-vac";
const TRUSTED_ACCESS_FOR_CYBER_VERIFICATION: &str = "trusted_access_for_cyber";

const CYBER_POLICY_MESSAGE: &str =
    "This request has been flagged for potentially high-risk cyber activity.";

fn disabled_text_turn(test: &TestVAC, text: &str) -> Op {
...
async fn vastar_model_header_mismatch_emits_warning_event_and_warning_item() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let response =
        sse_response(sse_completed("resp-1")).insert_header("Vastar-Model", SERVER_MODEL);
    let _mock = mount_response_once(&server, response).await;

    let mut builder = test_vac().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.vac, |event| {
    ...
    let warning = wait_for_event(&test.vac, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let warning_item = wait_for_event(&test.vac, |event| {
        matches!(
            event,
    ...
    let _ = wait_for_event(&test.vac, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.vac, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.vac_error_info, Some(VACErrorInfo::CyberPolicy));
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let response = sse_response(sse(vec![
        serde_json::json!({
            ...
            "response": {
                "id": "resp-1",
                "headers": {
                    "Vastar-Model": SERVER_MODEL
                }
            }
        }),
        core_test_support::responses::ev_completed("resp-1"),
    ]))
    .insert_header("Vastar-Model", REQUESTED_MODEL);
    let _mock = mount_response_once(&server, response).await;
    ...
    let warning = wait_for_event(&test.vac, |event| {
        matches!(
            event,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn vastar_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    .insert_header("Vastar-Model", SERVER_MODEL);
    ...
    loop {
        let event = wait_for_event(&test.vac, |_| true).await;
        match event {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn vastar_model_header_casing_only_mismatch_does_not_warn() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let requested_header = REQUESTED_MODEL.to_ascii_uppercase();
    let response = sse_response(sse_completed("resp-1"))
        .insert_header("Vastar-Model", requested_header.as_str());
    let _mock = mount_response_once(&server, response).await;
