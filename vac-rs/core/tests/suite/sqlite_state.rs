...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let rollout_path = test.vac.rollout_path().expect("rollout path");
    let db_path = vac_state::state_db_path(test.config.sqlite_home.as_path());
    ...
    let db = test.vac.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_vac()
        .with_pre_build_hook(move |vac_home| {
            let rollout_path = vac_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = vac_state::state_db_path(test.config.sqlite_home.as_path());
    let rollout_path = test.config.vac_home.join(&rollout_rel_path);
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    .await;
    wait_for_event_match(&test.vac, |event| match event {
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = vac_state::log_db::start(db.clone());
