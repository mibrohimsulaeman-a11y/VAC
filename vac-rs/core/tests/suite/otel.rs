...
fn extract_log_field_handles_empty_bare_values() {
    let line = "event.name=\"vac.tool_result\" mcp_server= mcp_server_origin=";
...
fn extract_log_field_does_not_confuse_similar_keys() {
    let line = "event.name=\"vac.tool_result\" mcp_server_origin=stdio";
...
async fn responses_api_emits_api_request_event() {
    ...
    let TestVAC { vac, .. } = test_vac().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("vac.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected vac.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("vac.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected vac.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing vac.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("vac.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing vac.sse_event".to_string()))
    });
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("handle_responses{otel.name=\"completed\"")
                ...
                && line.contains("vac.usage.reasoning_output_tokens=2")
                && line.contains("vac.usage.total_tokens=9")
        }),
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("turn{otel.name=\"session_task.turn\"")
                && line.contains("vac.turn.token_usage.input_tokens=3")
                && line.contains("vac.turn.token_usage.cached_input_tokens=1")
                && line.contains("vac.turn.token_usage.non_cached_input_tokens=2")
                && line.contains("vac.turn.token_usage.output_tokens=5")
                && line.contains("vac.turn.token_usage.reasoning_output_tokens=2")
                && line.contains("vac.turn.token_usage.total_tokens=9")
        }),
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    wait_for_event(&vac, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("vac.tool_result") && line.contains("call_id=custom-tool-call")
            })
            .ok_or_else(|| "missing vac.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| line.contains("vac.tool_result") && line.contains("call_id=function-call"))
            .ok_or_else(|| "missing vac.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_local_shell_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| line.contains("vac.tool_result") && line.contains("call_id=shell-call"))
            .ok_or_else(|| "missing vac.tool_result event".to_string())?;
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("vac.tool_decision") && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing vac.tool_decision event for {call_id}"))?;
    ...
    mount_sse_once(
        ...
        sse(vec![
            ev_local_shell_call(
                "user_approved_call",
                "completed",
                vec!["/usr/bin/touch", "vac-otel-approval-test"],
            ),
            ev_completed("done"),
        ]),
    )
