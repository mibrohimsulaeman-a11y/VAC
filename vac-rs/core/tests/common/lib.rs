...
use vac_utils_cargo_bin::CargoBinError;
...
pub use vac_utils_absolute_path::test_support::PathBufExt;
pub use vac_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    vac_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    vac_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(vac_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = vac_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("vac-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\vac\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.vac`.
pub async fn load_default_config_for_test(vac_home: &TempDir) -> Config {
    vac_core::config::ConfigBuilder::default()
        .vac_home(vac_home.path().to_path_buf())
        .harness_overrides(default_test_overrides())
        .build()
        .await
        .expect("load default config for test should succeed")
}

#[cfg(target_os = "linux")]
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        vac_linux_sandbox_exe: Some(
            find_vac_linux_sandbox_exe().expect("should find binary for vac-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_vac_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    vac_utils_cargo_bin::cargo_bin("vac-linux-sandbox")
}
...
/// makes it trivial to extend the fixtures as Vastar adds new event kinds or
/// fields.
...
pub async fn wait_for_event<F>(vac: &VACThread, predicate: F) -> vac_protocol::protocol::EventMsg
where
    F: FnMut(&vac_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(vac, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(vac: &VACThread, matcher: F) -> T
where
    F: Fn(&vac_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(vac, |ev| matcher(ev).is_some()).await;
...
) -> vac_protocol::protocol::EventMsg
where
    F: FnMut(&vac_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), vac.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    vac_core::spawn::VAC_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    vac_core::spawn::VAC_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "VAC_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    vac_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    vac_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    vac_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a VAC sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! vac_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_vac_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("vac-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
