...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-vastar-subagent", "review"),
        response_body,
    )
    .await;
    ...
    let vac_home = TempDir::new().expect("failed to create TempDir");
    let mut config = load_default_config_for_test(&vac_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort;
    let summary = config.model_reasoning_summary;
    let model = vac_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);

    let conversation_id = ThreadId::new();
    let auth_mode = TelemetryAuthMode::ProviderCredential;
    let session_source = SessionSource::SubAgent(SubAgentSource::Review);
    let model_info = vac_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    assert_eq!(
        request.header("x-vastar-subagent").as_deref(),
        Some("review")
    );
    assert_eq!(
        request.header("x-vac-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-vac-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-vac-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(request.header("x-vac-sandbox"), None);
}
...
async fn responses_stream_includes_subagent_header_on_other() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-vastar-subagent", "my-task"),
        response_body,
    )
    .await;
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_vac().build(&server).await.expect("build test vac");
    ...
    let initial_header = first_request
        ...
        .expect("x-vac-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-vac-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-vac-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/vastar/vac.git",
    ]);
