# Hourly Repo Sentinel Audit — 2026-05-27 16:04
Previous run: `docs/scheduled-audits/2026-05-27/1504-repo-sentinel.md`
...
|---|---|---|---|---|---|
| CRITICAL | Policy / Registry | Validasi internal `vac doctor` gagal karena aturan `network_access` dengan ID `block-network-for-local-gates` di `.vac/policies/runtime-owner-replacement.yaml` tidak memiliki blok `network` yang valid. | `./vac-rs/target/debug/vac doctor registry .` → `error: rules[2].match.network: network policy matches require a network scope` (Exit Code 1) | Ganti `path: any` dengan struktur blok `network` yang berisi properti `host` dan `protocol` sesuai skema validasi, atau hapus matcher bermasalah tersebut jika tidak relevan. | `.vac/policies/runtime-owner-replacement.yaml` |
...
## Plan Candidates
- Title: Perbaikan Aturan Skema Aksi Jaringan pada runtime-owner-replacement.yaml
