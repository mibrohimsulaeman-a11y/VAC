...
Status: **confirmed_intent_domain_coverage=SV-Pass**, **confirmed_intent_traceability_gate=SV-Pass**, **confirmed_intent_negative_fixtures=SV-Pass**, **confirmed_intent_executable_fixtures=SV-Pass**, **all_negative_cases_rejected=true**, **crate_without_intent_or_rationale=0**, **external_provider_remote_process_io_e2e=TV-Pending**.
...
| vac-messaging-gateway | `vac-rs/crates/integrations/vac-messaging-gateway` | `vac.integrations.messaging_gateway (.vac/capabilities/vac-integrations-messaging_gateway.yaml)` | `.vac/specs/confirmed/vac-messaging-gateway-intent.yaml` | SV-Pass | Medium: notification boundary | `delivery_failure_is_explicit` now has an executable SV fixture; token redaction remains manual-review. |
...
| vac-autopilot | `vac-rs/crates/runtime/vac-autopilot`<br>`vac-rs/crates/surfaces/vac-cli/src/commands/autopilot` | `vac.runtime.agent_loop (.vac/capabilities/vac-runtime-agent_loop.yaml)`<br>`vac.surfaces.cli (.vac/capabilities/vac-surfaces-cli.yaml)` | `.vac/specs/confirmed/vac-autopilot-intent.yaml` | SV-Pass | Medium-high: scheduled/continuous orchestration boundary | CLI release-readiness non-overclaim now has an executable SV fixture; config reload/process expansion remain manual-review. |
...
## Executable invariant fixtures

`confirmed_intent_executable_fixtures=SV-Pass` is provided by `scripts/check-confirmed-intent-executable-fixtures.py` over `tests/fixtures/confirmed-intent/executable-invariants.json`. Current executable SV cases cover `broker_does_not_upgrade_l1_to_l2_claim`, `autopilot_cli_output_does_not_overclaim_release_readiness`, and `delivery_failure_is_explicit`. Each case also mutates in a forbidden overclaim token and verifies that the fixture would reject it.
